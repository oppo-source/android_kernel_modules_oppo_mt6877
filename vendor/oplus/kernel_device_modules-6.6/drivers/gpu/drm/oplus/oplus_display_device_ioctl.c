...
int oplus_display_get_brightness_time(void *data) {
	...
	crtc = list_first_entry(&(ddev)->mode_config.crtc_list,
			typeof(*crtc), head);
	if (!crtc) {
		OPLUS_DSI_ERR("find crtc fail, p_time=%p\n", p_time);
		return -EINVAL;
	}
	...
	OPLUS_DSI_INFO("get brightness time: brightness_time[%s]\n", p_time->brightness_time);
	return 0;
}
...
int oplus_display_panel_set_white_point_status(void *data)
{
	...
	/* set white point compensation */
	if (comp && comp->funcs && comp->funcs->io_cmd)
		comp->funcs->io_cmd(comp, cmdq_handle, DSI_WHITE_POINT_COMPENSATION, &status);
