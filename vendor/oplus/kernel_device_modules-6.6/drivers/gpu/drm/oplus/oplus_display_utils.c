...
bool disable_scaling_mode_mapping = false;
EXPORT_SYMBOL(disable_scaling_mode_mapping);
...
int oplus_panel_ext_init(struct drm_crtc *crtc)
{
	...
	if (!strcmp("panel_ae031_p_3_a0026_dsi_vdo", ctx->panel_info.panel_name)) {
		disable_scaling_mode_mapping = true;
	}
#endif /* OPLUS_FEATURE_DISPLAY_MAINLINE*/
	return rc;
}
...
int mode_switch_wait_te(struct mtk_dsi *dsi)
{
...
EXPORT_SYMBOL(mode_switch_wait_te);
