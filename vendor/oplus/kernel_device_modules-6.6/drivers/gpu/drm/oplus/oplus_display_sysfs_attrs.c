...
********************************************************/
extern void set_brightness2_for_show(unsigned int level);
#ifdef OPLUS_FEATURE_DISPLAY
extern int oplus_i2c_setbl(unsigned int level);
#endif
...
static ssize_t oplus_display_get_brightness2(struct kobject *obj,
		struct kobj_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "%d\n", oplus_display_brightness);
}

static ssize_t oplus_display_set_brightness2(struct kobject *obj,
		struct kobj_attribute *attr, const char *buf, size_t num)
{
    ...
    if (ret) {
        OPLUS_DSI_ERR("kstrtoint error %d\n", ret);
        return -EINVAL;
       }
    ...
    np = of_find_compatible_node(NULL, NULL, "mediatek,mtk-leds");
    if (np) {
        ret = of_property_read_u32(np, "brightness2-i2c", &state);
        if (ret < 0) {
            mtk_drm_setbacklight(crtc, oplus_set_brightness, 0, 0x1 << SET_BACKLIGHT_LEVEL, 1);
        } else {
            if (state) {
                oplus_i2c_setbl(oplus_set_brightness);
            } else {
                mtk_drm_setbacklight(crtc, oplus_set_brightness, 0, 0x1 << SET_BACKLIGHT_LEVEL, 1);
            }
        }
        ...
        mtk_drm_setbacklight(crtc, oplus_set_brightness, 0, 0x1 << SET_BACKLIGHT_LEVEL, 1);
        OPLUS_DSI_ERR("find mtk-leds node failed\n");
    }
    set_brightness2_for_show(oplus_set_brightness);
    return num;
}
...
int oplus_panel_serial_number_read(struct drm_crtc *crtc)
{
	...
	while ((count--) > 0) {
		...
		if (!strcmp(panel_name, "ac168_p_B_a0012_cmd_panel")
			...
			oplus_ddic_dsi_read_cmd(0x01, 5, para);
			panel_serial_info.day       = para[0] & 0x1F;
			oplus_ddic_dsi_read_cmd(0x02, 5, para);
			panel_serial_info.hour       = para[0] & 0x1F;
			oplus_ddic_dsi_read_cmd(0x03, 5, para);
			panel_serial_info.minute       = para[0] & 0x3F;
			oplus_ddic_dsi_read_cmd(0x04, 5, para);
			panel_serial_info.second       = para[0] & 0x3F;
			oplus_ddic_dsi_read_cmd(0x05, 5, para);
			panel_serial_info.reserved[0]    = para[0] & 0xFF;
			oplus_ddic_dsi_read_cmd(0x06, 5, para);
...
EXPORT_SYMBOL(oplus_dc_threshold);
...
static OPLUS_ATTR(oplus_brightness2, S_IRUGO | S_IWUSR, oplus_display_get_brightness2, oplus_display_set_brightness2);
...
static struct attribute *oplus_display_attrs[] = {
	...
	&oplus_attr_oplus_brightness2.attr,
