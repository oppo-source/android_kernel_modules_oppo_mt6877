...
//#include "ac138_p_3_a0004_dsi_cmd.h"
#ifdef OPLUS_FEATURE_DISPLAY_ADFR
#include "oplus_adfr.h"
#endif /* OPLUS_FEATURE_DISPLAY_ADFR */

#include "ac138_data_hw_roundpattern.h"

#define REGFLAG_CMD				0xFFFA
#define REGFLAG_DELAY			0xFFFC
#define REGFLAG_UDELAY			0xFFFB
#define REGFLAG_END_OF_TABLE	0xFFFD
#define PHYSICAL_WIDTH_AC138 (69690)
#define PHYSICAL_HEIGHT_AC138 (155330)
...
static int get_mode_enum(struct drm_display_mode *m)
{
	...
	if (m == NULL) {
		...
		pr_info("get_mode_enum : m_vrefresh=%d\n", m_vrefresh);
		if (m_vrefresh == 60) {
			ret = 60;
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	//pr_debug(" call lpm_smc_spm_dbg keep vcore off for display off!\n");
...
static struct mtk_panel_params ext_params_60Hz = {
	...
	.physical_width_um = PHYSICAL_WIDTH_AC138,
	.physical_height_um = PHYSICAL_HEIGHT_AC138,
};
...
static int oplus_panel_set_backlight_cmdq(void *dsi, dcs_write_gce_pack cb, void *handle, unsigned int level)
{
	...
	pr_info("bl_level[0].para_list[1]=%x, bl_level[0].para_list[2]=%x, level:%d\n", bl_level[1].para_list[1] , bl_level[1].para_list[2],level);
	return 0;
}
		...
		void *handle, unsigned int hbm_mode)
{
	...
	pr_info("oplus_display_brightness= %u, hbm_mode=%u\n", oplus_display_brightness, hbm_mode);
		...
		dcs_write_gce_pack cb, void *handle, bool en)
{
    ...
    pr_info("oplus_display_brightness= %u, en=%u\n", oplus_display_brightness, en);
...
/* Switch back to VDO mode */
	...
	//pr_info(" call lpm_smc_spm_dbg keep vcore off for exit AOD!\n");
    //pr_info("%s:success\n", __func__);
...
static int panel_doze_enable(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	...
	//pr_info("call lpm_smc_spm_dbg keep vcore on for enter AOD then Succ!\n");
...
static int vddr6_buck4_regulator_enable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(mt6315_6_vbuck4)) {
		...
		if (ret < 0)
			pr_err("set voltage mt6315_6_vbuck4 fail, ret = %d\n", ret);
		retval |= ret;
	}
	...
	if (!IS_ERR_OR_NULL(mt6315_6_vbuck4)) {
		...
		if (ret < 0)
			pr_err("enable regulator mt6315_6_vbuck4 fail, ret = %d\n", ret);
		retval |= ret;
	}
	pr_info("get vddr6_buck4_regulator_enable\n");
	return retval;
}
static int vddr6_buck4_regulator_disable(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(mt6315_6_vbuck4)) {
		...
		if (ret < 0)
			pr_err("disable regulator mt6315_6_vbuck4 fail, ret = %d\n", ret);
		retval |= ret;
	}
	return retval;
}
...
static int vddr6_buck4_regulator_get_voltage(struct device *dev)
{
	...
	if (!IS_ERR_OR_NULL(mt6315_6_vbuck4)) {
		...
		if (ret < 0)
			pr_err("get voltage mt6315_6_vbuck4 fail, ret = %d\n", ret);
		retval |= ret;
	}

	pr_info("get vddr6_buck4_regulator_get_voltage = %d\n", retval);
	return retval;
}
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	pr_debug("p_3 a0004 lcm ctx->prepared %d\n", ctx->prepared);
	...
	if (ret < 0) {
		pr_err("debug for pmic_oled ldo3 set 3000 fail\n");
	} else {
		pr_err("debug for pmic_oled ldo3 set 3000 success\n");
	}
...
void oplus_lcm_read_info(struct drm_panel *panel, int read_ic)
{
	...
	if (ret < 0) {
		pr_err("vddr6_buck4_regulator_get_voltage get lV2 fail!\n");
	} else {
...
static int lcm_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	pr_info("drm_mode 1 : clock=%d,htotal=%d,vtotal=%d,hskew=%d,vrefresh=%d\n", mode[0]->clock, mode[0]->htotal,
	...
	pr_info("drm_mode 2 clock=%d,htotal=%d,vtotal=%d,hskew=%d,vrefresh=%d\n", mode[1]->clock, mode[1]->htotal,
	...
	pr_info("drm_mode 3 clock=%d,htotal=%d,vtotal=%d,hskew=%d,vrefresh=%d\n", mode[2]->clock, mode[2]->htotal,
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("[LCM] %s+ ac138_p_3_a0004_cmd_panel Start\n", __func__);
...
//#endif /* OPLUS_FEATURE_DISPLAY */
	...
	pr_info("ac138_p_3_a0004_cmd_panel End.\n");
	return ret;
}
