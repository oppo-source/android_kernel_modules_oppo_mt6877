...
static int lcm_unprepare(struct drm_panel *panel)
{
...
#else
	pr_info("%s: tp_gesture_enable_flag = %d \n", __func__, tp_gesture_enable_notifier(0));
	if (1 == shutdown_flag) {
	pr_info("%s:  \n", __func__);
	ctx->reset_gpio =
	...
	} else if (/*esd_flag == 1 ||*/ 0 == tp_gesture_enable_notifier(0)) {
		...
		void *handle, unsigned int level)
{
	...
	if(level == 0 && lcm_dimming_flag == 0) {
		...
		pr_info("dimming on 53-2c-68-01-05, level = %d\n", level);
		lcm_dimming_flag = 1;
	} else if (level > 0 && lcm_dimming_flag == 1) {
		lcm_dimming_off_23053(dsi, cb, handle);
		pr_info("dimming off 53-24, level = %d\n", level);
		lcm_dimming_flag = 0;
	}
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("lcm_probe 23253_ili9883c_boe Begin\n");
	...
	pr_info("lcm_probe 23253_ili9883c_boe Successful\n");
