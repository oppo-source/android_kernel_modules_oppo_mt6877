...
#include "oplus22277data_hw_roundedpattern_r_boe.h"
#include "oplus22277data_hw_roundedpattern_l_boe.h"
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	OPLUS_DISP_DEBUG(TAG,"boe_nt37705 lcm ctx->prepared %d\n", ctx->prepared);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	OPLUS_DISP_INFO(TAG,"[LCM] %s+ oplus22277_boe_nt37705_fhd_dsi_cmd_lcm Start\n", __func__);
	...
	if (IS_ERR(ctx->vddr1p2_enable_gpio)) {
		OPLUS_DISP_ERR(TAG," cannot get vddr1p2_enable_gpio %ld\n",PTR_ERR(ctx->vddr1p2_enable_gpio));
	...
	ctx->vci_enable_gpio = devm_gpiod_get(ctx->dev, "3p0-enable", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->vci_enable_gpio)) {
		OPLUS_DISP_ERR(TAG," cannot get vci_enable_gpio %ld\n",PTR_ERR(ctx->vci_enable_gpio));
...
//#endif /* OPLUS_FEATURE_DISPLAY */
	...
	OPLUS_DISP_INFO(TAG,"[LCM] %s+ oplus22277_boe_nt37705_fhd_dsi_cmd_lcm End\n", __func__);
	return ret;
}
