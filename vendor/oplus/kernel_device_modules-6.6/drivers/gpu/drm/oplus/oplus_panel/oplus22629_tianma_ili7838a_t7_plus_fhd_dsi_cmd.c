...
extern int oplus_ofp_init(void *node);
...
static struct LCM_setting_table lcm_setbrightness_normal_mode[] = {
	{REGFLAG_CMD,3, {0x51, 0x07, 0xFF}},
};

static struct LCM_setting_table lcm_setbrightness_aod[] = {
...
static int jdi_setbacklight_cmdq(void *dsi, dcs_write_gce_pack cb, void *handle,
				 unsigned int level)
{
	...
	BL_LSB = level >> 8;
	BL_MSB = level & 0xFF;
	lcm_setbrightness_normal_mode[0].para_list[1] = BL_LSB;
  	lcm_setbrightness_normal_mode[0].para_list[2] = BL_MSB;
	if (!cb)
		return -1;

	panel_init = false;
	if (level ==1) {
		panel_send_pack_hs_cmd(dsi, lcm_setbrightness_aod, sizeof(lcm_setbrightness_aod)/sizeof(struct LCM_setting_table), cb, handle);
	...
	panel_send_pack_hs_cmd(dsi, lcm_setbrightness_normal_mode, sizeof(lcm_setbrightness_normal_mode)/sizeof(struct LCM_setting_table), cb, handle);
		...
		void *handle, unsigned int hbm_mode)
{
	...
	if(hbm_mode == 1) {
		...
		msleep(10);
		panel_send_pack_hs_cmd(dsi, lcm_set_HBM_again, sizeof(lcm_set_HBM_again)/sizeof(struct LCM_setting_table), cb, handle);
	} else if (hbm_mode == 0) {
		//level = oplus_lcm_dc_backlight(dsi,cb,handle, oplus_display_brightness, 0);
		jdi_setbacklight_cmdq(dsi, cb, handle, oplus_display_brightness); //level
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.oplus_set_backlight_cmdq = jdi_setbacklight_cmdq,
