				 ...
				 unsigned int level)
{
	...
	unsigned int mode;
	char bl_diming_off[] = {0x53, 0x24}; //bl_diming_off
	...
	if ((bl_level <= MAX_NORMAL_BRIGHTNESS) && (bl_level != 0)) {
		bl_level = map_exp[bl_level];
	}
	if ((bl_level < MIN_NORMAL_BRIGHTNESS) && (bl_level != 0)) {
		bl_level = MIN_NORMAL_BRIGHTNESS;
	}
