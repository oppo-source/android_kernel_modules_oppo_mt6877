...
extern bool oplus_ofp_local_hbm_unlocking_acceleration_is_enabled(void);
extern void oplus_display_get_panel_brightness_time(void);
extern int mode_switch_wait_te(struct mtk_dsi *dsi);
...
static int get_mode_enum(struct drm_display_mode *m)
{
	...
	if (m_vrefresh == 60 && (m->hskew == STANDARD_MFR || m->hskew == STANDARD_ADFR)) {
		...
		ret = FHD_SDC30;
	} else if (m_vrefresh == 165 && m->hskew == STANDARD_ADFR) {
		ret = FHD_SDC165;
	}
...
static void lcm_panel_init(struct dsi_panel_lcm *ctx)
{
	...
	if (m_db < 3 && m_db >= 0 && lcm_all_cmd_table[DSI_CMD_SET_ON_T0].cmd_lines > 0)
		oplus_dsi_panel_send_cmd(dsi, DSI_CMD_SET_ON_T0, NULL, DSI_CMD_FUNC_DEFAULT);
	else
...
static int panel_set_seed(void *dsi, dcs_write_gce_pack cb, void *handle, unsigned int mode)
{
	...
	if (apuir_enable) {
		...
		OPLUS_DSI_INFO("panel_set_seed mode=%d apuir_enable %d apuir_state %d oplus_seed_mode = %d\n", mode, apuir_enable, apuir_state, oplus_seed_mode);
	}
	if (apuir_enable && apuir_state) {
		return 0;
	}
#endif
	if (cb) {
...
static int oplus_ofp_set_lhbm_pressed_icon(struct drm_panel *panel, void *dsi_drv, dcs_write_gce_pack cb, void *handle, uint64_t lhbm_pressed_icon_on)
{
	...
	if (lhbm_pressed_icon_on) {
		...
		} else if (strcmp("ac332_p_3_a0036_dsi_cmd_panel", panel_name) == 0) {
			if (oplus_display_brightness > 0x047B) {
				cmd_set_id = DSI_CMD_LHBM_PRESSED_ICON_ON;
			} else {
				cmd_set_id = DSI_CMD_LHBM_PRESSED_ICON_ON_PWM;
			}
...
static int panel_doze_disable(struct drm_panel *panel, void *dsi, dcs_write_gce_pack cb, void *handle)
{
	...
	if (oplus_ofp_need_to_do_aod_off_compensation()
	...
	} else if (oplus_display0_params->need_aod_off_insert_black_frame && oplus_ofp_get_aod_unlocking()) {
		if (handle) {
		...
		OFP_INFO("send aod off cmd with insert back frame\n");
	} else if (oplus_ofp_local_hbm_unlocking_acceleration_is_enabled() && !oplus_ofp_get_aod_unlocking()) {
		if (handle) {
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_AOD_OFF_INSERT_BLACK, handle, DSI_CMD_FUNC_DEFAULT);
		} else {
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_AOD_OFF_INSERT_BLACK, handle, DSI_CMD_FUNC_GCE2);
		}
		OFP_INFO("send aod off cmd without 2C\n");
	} else {
		if (handle) {
...
static int panel_doze_disable_ramless(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	struct mtk_dsi *mtk_dsi = dsi;
	struct drm_crtc *crtc = NULL;
	struct mtk_crtc_state *mtk_state = NULL;
	...
	if (handle) {
		oplus_dsi_panel_send_cmd(dsi, DSI_CMD_SET_NOLP, handle, DSI_CMD_FUNC_DYN);
	} else {
		oplus_dsi_panel_send_cmd(dsi, DSI_CMD_SET_NOLP, handle, DSI_CMD_FUNC_GCE2);
	}
	OFP_INFO("send aod off cmd\n");
...
static int panel_doze_enable_ramless(struct drm_panel *panel, void *dsi, dcs_write_gce cb, void *handle)
{
	struct mtk_dsi *mtk_dsi = dsi;
	struct drm_crtc *crtc = NULL;
	struct mtk_crtc_state *mtk_state = NULL;
	...
	if(oplus_ofp_video_mode_30hz_aod_accelerate_is_enabled() && oplus_ofp_get_hbm_state()) {
		if (handle)
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_LHBM_PRESSED_ICON_OFF, handle, DSI_CMD_FUNC_DYN);
		else
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_LHBM_PRESSED_ICON_OFF, handle, DSI_CMD_FUNC_GCE2);
		OFP_INFO("should off hbm in aod\n");
	}
	...
	OFP_INFO("crtc_active:%d, doze_active:%llu\n", crtc->state->active, mtk_state->prop_val[CRTC_PROP_DOZE_ACTIVE]);
	if (handle)
		oplus_dsi_panel_send_cmd(dsi, DSI_CMD_SET_LP1, handle, DSI_CMD_FUNC_DYN);
	else
		oplus_dsi_panel_send_cmd(dsi, DSI_CMD_SET_LP1, handle, DSI_CMD_FUNC_GCE2);
...
static int panel_set_aod_light_mode_ramless(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	if (level == 0) {
		if (handle)
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_AOD_HIGH_LIGHT_MODE, handle, DSI_CMD_FUNC_DYN);
		else
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_AOD_HIGH_LIGHT_MODE, handle, DSI_CMD_FUNC_GCE2);
	} else {
		if (handle)
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_AOD_LOW_LIGHT_MODE, handle, DSI_CMD_FUNC_DYN);
		else
			oplus_dsi_panel_send_cmd(dsi, DSI_CMD_AOD_LOW_LIGHT_MODE, handle, DSI_CMD_FUNC_GCE2);
	}
	OFP_INFO("level = %d\n", level);
		...
		unsigned int id)
{
	...
	if (!strcmp(panel_name, "panel_ac390_p_7_a0034_dsi_vdo")) {
		if (m_vrefresh == 60 && (m->hskew == STANDARD_MFR || m->hskew == STANDARD_ADFR)) {
			...
			*ext_param = &(ctx->ext_params_all[4]);
                        ...
                        *ext_param = &(ctx->ext_params_all[5]);
                } else {
			*ext_param = &(ctx->ext_params_all[0]);
		}
		OPLUS_DSI_INFO("panel_name[%s] fps=%d\n", panel_name, m_vrefresh);
	} else {
		if (m_vrefresh == 60 && (m->hskew == STANDARD_MFR || m->hskew == STANDARD_ADFR)) {
			*ext_param = &(ctx->ext_params_all)[1];
		} else if (m_vrefresh == 120 && m->hskew == STANDARD_ADFR) {
	...
	if (!strcmp(panel_name, "panel_ac390_p_7_a0034_dsi_vdo")) {
		if (m_vrefresh == 60 && (m->hskew == STANDARD_MFR || m->hskew == STANDARD_ADFR)) {
			...
			ext->params = &(ctx->ext_params_all[4]);
                        ...
                        ext->params = &(ctx->ext_params_all[5]);
                } else {
			ext->params = &(ctx->ext_params_all[0]);
		}
		OPLUS_DSI_INFO("panel_name[%s] fps=%d\n", panel_name, m_vrefresh);
			...
			enum MTK_PANEL_MODE_SWITCH_STAGE stage, dcs_write_gce_pack cb)
{
	...
	int m_vrefresh = 0;
	int src_vrefresh = 0;
	unsigned int src_data_rate = 0;
	unsigned int dst_data_rate = 0;
	...
	struct dsi_panel_lcm *ctx = panel_to_lcm(panel);
	struct mtk_panel_params *src_panel_params = NULL;
	struct mtk_panel_params *dst_panel_params = NULL;
#ifdef OPLUS_FEATURE_DISPLAY_ADFR
	struct mtk_dsi *dsi = dsi_drv;
	struct drm_crtc *crtc = NULL;
	...
	if (!ext->funcs->ext_param_get(panel, connector, &src_panel_params, cur_mode) && src_panel_params) {
		src_data_rate = src_panel_params->data_rate;
	}
	if (!ext->funcs->ext_param_get(panel, connector, &dst_panel_params, dst_mode) && dst_panel_params) {
		dst_data_rate = dst_panel_params->data_rate;
	}

	if (!oplus_display0_params) {
		OPLUS_DSI_ERR("oplus_display0_params is NULL\n");
		return -EINVAL;
	}

	OPLUS_DSI_INFO("dsi_panel_mode_id:%d->%d, hdisplay:%d->%d, hskew:%d->%d, vrefresh:%d->%d, datarate:%d->%d, mode_switch_h2l_delay=%d\n",
			get_mode_enum(src_m), dsi_panel_mode_id, src_m->hdisplay, m->hdisplay, src_m->hskew,
			m->hskew, src_vrefresh, m_vrefresh, src_data_rate, dst_data_rate, oplus_display0_params->mode_switch_h2l_delay);

	if (stage == BEFORE_DSI_POWERDOWN) {
		if (m->hskew == STANDARD_MFR || m->hskew == STANDARD_ADFR) {
			if (src_vrefresh > m_vrefresh) {
				mode_switch_hs_send(m_vrefresh, dsi_drv);
				OPLUS_DSI_INFO("%s: switch fps BEFORE_DSI_POWERDOWN mode_switch_h2l_delay=%d\n", __func__, oplus_display0_params->mode_switch_h2l_delay);
				if (oplus_display0_params->mode_switch_h2l_delay) {
					if (src_data_rate > dst_data_rate) {
						mode_switch_wait_te(dsi);
						OPLUS_DSI_INFO("%s: wait TE  BEFORE_DSI_POWERDOWN\n", __func__);
					}
				}
			}
		}
...
#endif /* OPLUS_FEATURE_DISPLAY_ADFR */
			mode_switch_hs_send(m_vrefresh, dsi_drv);
			OPLUS_DSI_INFO("%s: switch fps AFTER_DSI_POWERON mode_switch_h2l_delay=%d\n", __func__, oplus_display0_params->mode_switch_h2l_delay);
			if (oplus_display0_params->mode_switch_h2l_delay) {
				if (src_data_rate > dst_data_rate) {
					mode_switch_wait_te(dsi);
					OPLUS_DSI_INFO("%s: wait TE  AFTER_DSI_POWERON\n", __func__);
				}
			}
		}
	}
...
static int oplus_display_panel_set_white_point_compensation(void *dsi, dcs_write_gce_pack cb, void *handle, unsigned int enable)
{
	OPLUS_DSI_INFO("enable=%d\n", enable);
	if (!dsi) {
		OPLUS_DSI_ERR("Invalid params\n");
		return -EINVAL;
	}
	...
	if (enable) {
		OPLUS_DSI_INFO("Set white point compensation enable");
		oplus_dsi_panel_send_cmd(dsi, DSI_CMD_WHITE_POINT_COMPENSATION_ON, handle, DSI_CMD_FUNC_DEFAULT);
	} else {
		OPLUS_DSI_INFO("Set white point compensation disable");
		oplus_dsi_panel_send_cmd(dsi, DSI_CMD_WHITE_POINT_COMPENSATION_OFF, handle, DSI_CMD_FUNC_DEFAULT);
	}
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.oplus_doze_disable_ramless = panel_doze_disable_ramless,
	.oplus_doze_enable = panel_doze_enable,
	.oplus_doze_enable_ramless = panel_doze_enable_ramless,
	.oplus_set_aod_light_mode = panel_set_aod_light_mode,
	.oplus_set_aod_light_mode_ramless = panel_set_aod_light_mode_ramless,
	...
	.lcm_set_white_point_compensation = oplus_display_panel_set_white_point_compensation,
#endif
};
#endif
...
static int __init lcm_drv_init(void)
{
	...
	if (!check_cmdline_valid() || ((strcmp("panel_x25n16_p_7_a0028_dsi_vdo", lcm_driver.driver.name) == 0))
		|| ((strcmp("panel_x25n16_p_7_2_a0028_dsi_vdo", lcm_driver.driver.name) == 0))
		|| ((strcmp("panel_xn011_p_7_a0028_dsi_vdo", lcm_driver.driver.name) == 0))
		|| ((strcmp("panel_xn011_p_7_2_a0028_dsi_vdo", lcm_driver.driver.name) == 0))) {
		OPLUS_DSI_WARN("Invalid display cmdline or not support new driver,, will use the old driver architecture\n");
		return 0;
	}
