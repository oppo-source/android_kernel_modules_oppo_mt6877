...
#include "AC102_P_3_A0010_fhdp_dsi_vdo.h"
#endif
...
#include "AC102_P_3_A0010_data_hw_roundedpattern.h"
#endif
...
static void lcm_dcs_write(struct lcm *ctx, const void *data, size_t len)
{
	...
	if (ret < 0) {
		pr_err("[error]error %zd writing seq: %ph\n", ret, data);
		ctx->error = ret;
	}
}
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	if (IS_ERR(ctx->reset_gpio)) {
		pr_err("[error]: cannot get reset_gpio %ld\n",
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	pr_info(" tp_gesture_enable_flag = %d  shutdown_flag = %d, esd_flag = %lu\n", tp_gesture_enable_notifier(0), shutdown_flag, esd_flag);
	if (1 == shutdown_flag) {
		ctx->reset_gpio =
	...
	if (0 == tp_gesture_enable_notifier(0) || (esd_flag == 1) || (1 == shutdown_flag)) {
		...
		if (IS_ERR(ctx->bias_ldo)) {
			pr_err("[error]: cannot get bias_ldo %ld\n",
		...
		if (IS_ERR(ctx->bias_pos)) {
			pr_err("[error]: cannot get bias_pos %ld\n",
		...
		if (IS_ERR(ctx->bias_neg)) {
			pr_err("[error]: cannot get bias_neg %ld\n",
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	if (ret < 0) {
		pr_debug("fail to lcm gate driver IC read 0x03\n");
	}else {
		pr_debug("_20015_lcm_i2c_read_bytes 0x03 = 0x%x   ret = %d\n", reg_value, ret);
	}
	usleep_range(1000, 1100);
	ret = _20015_lcm_i2c_write_bytes(0x00, 0x0f); // set VSP voltage etc. 4.0+ (parameter 2)* 0.10
	if (ret < 0) {
		pr_debug("fail to lcm gate driver IC write 0x00\n");
	}else {
		pr_debug("_20015_lcm_i2c_write_bytes 0x00 return value = %d\n", ret);
	}
	usleep_range(1000, 1100);
	ret = _20015_lcm_i2c_write_bytes(0x01, 0x0f); //set VSN voltage etc. 4.0+ (parameter 2)* 0.10
	if (ret < 0) {
		pr_debug("fail to lcm gate driver IC write 0x01\n");
	}else {
		pr_debug("_20015_lcm_i2c_write_bytes 0x01 return value = %d\n", ret);
	}
	usleep_range(1000, 1100);
	ret = _20015_lcm_i2c_write_bytes(0x03, 0x43); //Applications configure register,default:0x43
	if (ret < 0) {
		pr_debug("fail to lcm gate driver IC write 0x03\n");
	}else {
	...
	void *handle, unsigned int level)
{
    ...
    if(level < 14 && level > 0 && g_GammaFlag == 0){
        g_GammaFlag = 1;
        pr_debug(" backlight < 14 enter gamma!\n");
        lcm_gamma_enter(dsi, cb, handle);
    }else if(level > 13 && g_GammaFlag == 1){
        g_GammaFlag = 0;
        pr_debug(" backlight > 13 exit gamma!\n");
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (IS_ERR(ctx->bias_neg)) {
		pr_err("[error]%s: cannot get bias-neg 2 %ld\n",
