...
const char *dsi_cmd_map[DSI_CMD_ID_MAX] = {
	...
	"oplus,dsi-panel-white-point-compensation-off-command",
	"oplus,dsi-panel-white-point-compensation-on-command",
...
const char *dsi_cmd_state_map[DSI_CMD_ID_MAX] = {
	...
	"oplus,dsi-panel-white-point-compensation-off-command-state",
	"oplus,dsi-panel-white-point-compensation-on-command-state",
#ifdef OPLUS_FEATURE_AP_UIR_DIMMING
	...
	struct mtk_ddic_cmd *ddic_cmd, void *handle, int line, enum dsi_cmd_func cmd_func)
{
	...
	} else if (cmd_func == DSI_CMD_FUNC_DYN) {
			oplus_dsi_display_get_dcs_write_gce_dyn()(dsi, handle, ddic_cmd->para_list, ddic_cmd->cmd_num);
	}  else {
		...
		void *handle, enum dsi_cmd_func cmd_func)
{
	...
	if (cmd_func) {
		...
		case DSI_CMD_FUNC_GCE2:
			...
			break;
		case DSI_CMD_FUNC_DYN:
			cmd_state = DSI_CMD_SET_STATE_GCE_DYN;
		default:
			break;
		}
	}
	...
	*/
	...
	for (i = 0; i < count; i++) {
		...
		} else if (cmd_state == DSI_CMD_SET_STATE_LP_GCE || cmd_state == DSI_CMD_SET_STATE_GCE_DYN) {
...
int oplus_panel_cmd_reg_read_specific_row(enum dsi_cmd_id cmd_set_id, u8 *read_reg, size_t read_reg_len, u32 row)
{
	unsigned int i;
	struct dsi_cmd_table *tb = NULL;
	unsigned int lcm_cmd_count = 0;
	...
	u8 *tx_buf = NULL;
	if(!read_reg) {
		OPLUS_DSI_ERR("apuirdriver0 invalid cmd reg\n");
		return -ENODEV;
	}

	if (cmd_set_id >= DSI_CMD_ID_MAX) {
		OPLUS_DSI_ERR("apuirdriver0 cmd_id is error\n");
		return false;
	}
	...
	if (row > lcm_cmd_count) {
		OPLUS_DSI_ERR("apuirdriver0 change the number of rows of the command is wrong row = %d lcm_cmd_count = %d\n", row, lcm_cmd_count);
		return -EFAULT;
	}

	index = row - 1;
	tx_len = tb[index].count;
	tx_buf = (u8 *)tb[index].para_list;

	if ((tx_len - 1) != read_reg_len) {
		OPLUS_DSI_ERR("apuirdriver0 the number of rows of the command is wrong tx_len = %zu read_reg_len = %zu\n", tx_len, read_reg_len);
		return -EFAULT;
	}
...
int oplus_panel_cmd_reg_replace_specific_row(enum dsi_cmd_id cmd_set_id, u8 *replace_reg, size_t replace_reg_len, u32 row)
{
	unsigned int i;
	struct dsi_cmd_table *tb = NULL;
	unsigned int lcm_cmd_count = 0;
	...
	if (row > lcm_cmd_count) {
		OPLUS_DSI_ERR("change the number of rows of the command is wrong row = %d lcm_cmd_count = %d\n", row, lcm_cmd_count);
		return -EFAULT;
	}
	...
	if ((tx_len - 1) != replace_reg_len) {
		OPLUS_DSI_ERR("the number (tx_len - 1) != replace_reg_len tx_len = %zu replace_reg_len = %zu\n", tx_len, replace_reg_len);
		return -EFAULT;
	}
	tx_buf++;
