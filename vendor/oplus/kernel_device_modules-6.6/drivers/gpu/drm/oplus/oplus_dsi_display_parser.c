...
#endif
extern bool disable_scaling_mode_mapping;
...
int oplus_dsi_panel_parser_base(struct device_node *node, void *ext_param_dev, void *ctx_dev)
{
	...
	rc = utils->get_bool(node, "oplus,disable-scaling-mode-mapping", &disable_scaling_mode_mapping);
	...
	rc = utils->get_bool(node, "oplus,config-urgent", &ext_param->config_urgent);
	OPLUS_DSI_INFO("oplus,config-urgent = %d\n", ext_param->config_urgent);
	return rc;
}
...
int oplus_dsi_panel_parser_dsc(struct device_node *node, void *ext_param_dev, void *ctx_dev)
{
	...
	if (child) {
		rc = utils->get_int(child, "enable", &ext_param->dsc_params.ext_pps_cfg.enable);
		pps_cfg_count = of_property_count_elems_of_size(child, "oplus,rc-buf-thresh", sizeof(int));
		if (pps_cfg_count > 0) {
			pps_cfg_data = kzalloc(pps_cfg_count * sizeof(int), GFP_KERNEL);
			if (pps_cfg_data) {
				ext_param->dsc_params.ext_pps_cfg.rc_buf_thresh = pps_cfg_data;
				rc = utils->get_intarr(child, "oplus,rc-buf-thresh", pps_cfg_data, pps_cfg_count);
				if (rc < 0) {
					kfree(pps_cfg_data);
					pps_cfg_data = NULL;
				}
		...
		pps_cfg_count = of_property_count_elems_of_size(child, "oplus,range-min-qp", sizeof(int));
		if (pps_cfg_count > 0) {
			pps_cfg_data = kzalloc(pps_cfg_count * sizeof(int), GFP_KERNEL);
			if (pps_cfg_data) {
				ext_param->dsc_params.ext_pps_cfg.range_min_qp = pps_cfg_data;
				rc = utils->get_intarr(child, "oplus,range-min-qp", pps_cfg_data, pps_cfg_count);
				if (rc < 0) {
					kfree(pps_cfg_data);
					pps_cfg_data = NULL;
				}
		...
		pps_cfg_count = of_property_count_elems_of_size(child, "oplus,range-max-qp", sizeof(int));
		if (pps_cfg_count > 0) {
			pps_cfg_data = kzalloc(pps_cfg_count * sizeof(int), GFP_KERNEL);
			if (pps_cfg_data) {
				ext_param->dsc_params.ext_pps_cfg.range_max_qp = pps_cfg_data;
				rc = utils->get_intarr(child, "oplus,range-max-qp", pps_cfg_data, pps_cfg_count);
				if (rc < 0) {
					kfree(pps_cfg_data);
					pps_cfg_data = NULL;
				}
		...
		pps_cfg_count = of_property_count_elems_of_size(child, "oplus,range-bpg-ofs", sizeof(int));
		if (pps_cfg_count > 0) {
			pps_cfg_data = kzalloc(pps_cfg_count * sizeof(int), GFP_KERNEL);
			if (pps_cfg_data) {
				ext_param->dsc_params.ext_pps_cfg.range_bpg_ofs = pps_cfg_data;
				rc = utils->get_intarr(child, "oplus,range-bpg-ofs", pps_cfg_data, pps_cfg_count);
				if (rc < 0) {
					kfree(pps_cfg_data);
					pps_cfg_data = NULL;
				}
...
int oplus_dsi_panel_parser_dyn_fps(struct device_node *node, void *ext_param_dev, void *ctx_dev)
{
	...
	rc = utils->get_int(node, "switch_en", &ext_param->dyn_fps.switch_en);
        rc = utils->get_int(node, "dfps_cmd_num", &ext_param->dyn_fps.dfps_cmd_num);
...
#ifdef OPLUS_FEATURE_AP_UIR_DIMMING
int oplus_panel_parse_apuir_ds_list(struct device_node *node, void *ctx_dev) {
	int rc = -EINVAL, i = 0, j = 0;
	u32 test_len, check_value_count;
	u8 *lenp;
	struct dsi_panel_lcm *ctx = ctx_dev;
	...
	struct dsi_parser_utils *utils = ctx->parser_utils;
	int ic_type = 0;
	int upnit_ds_count = 3;
	u32 *upnit_ds_list = NULL;
	int lessnit_ds_count = 2;
	u32 *lessnit_ds_list = NULL;
	int nit_index_count = 4;
	u32 *nit_index_list = NULL;
	int upnit_index_count = 4;
	u32 *upnit_index_list = NULL;
	int lessnit_index_count = 4;
	u32 *lessnit_index_list = NULL;
	int modeset_count = 3;
	u32 *modeset_list = NULL;
	int apl_lhset_count = 6;
	u32 *apl_lhset_list = NULL;

	rc = utils->get_int(node, "oplus,apuir-ic-type", &ic_type);
	if ((rc < 0) || !ic_type) {
		OPLUS_DSI_WARN("parse oplus,apuir-ic-type=%d, rc=%d\n", ic_type, rc);
		oplus_set_apuir_ictype(0);
		return rc;
	}
	...
	rc = utils->get_intarr(node, "oplus,apuir-upnit-ds-list", upnit_ds_list, upnit_ds_count);
	if (rc < 0) {
	...
	rc = utils->get_intarr(node, "oplus,apuir-lessnit-ds-list", lessnit_ds_list, lessnit_ds_count);
	if (rc < 0) {
		OPLUS_DSI_ERR("apuir upnit_ds_list parse failed!\n");
		kfree(lessnit_ds_list);
		return 0;
	...
	if (ic_type == 1) {
		/* get nit_index_count */
		nit_index_list = kcalloc(nit_index_count, sizeof(u32), GFP_KERNEL);
		if (!nit_index_list) {
			kfree(nit_index_list);
			OPLUS_DSI_ERR("apuir oplus,apuir-nit-index-list alloc failed!\n");
			return 0;
		}

		rc = utils->get_intarr(node, "oplus,apuir-nit-index-list", nit_index_list, nit_index_count);
		if (rc < 0) {
			OPLUS_DSI_ERR("apuir nit_index_count parse failed!\n");
			kfree(nit_index_list);
			return 0;
		} else {
			nit_index_count = rc;
		}
		apuir_set_ili_nit_index_list(nit_index_count, nit_index_list);
		kfree(nit_index_list);
	} else if (ic_type == 2) {
		...
		rc = utils->get_intarr(node, "oplus,apuir-upnit-index-list", upnit_index_list, upnit_index_count);
		if (rc < 0) {
			OPLUS_DSI_ERR("apuir upnit_index_count parse failed!\n");
			kfree(upnit_index_list);
			return 0;
		} else {
			upnit_index_count = rc;
		}
		...
		rc = utils->get_intarr(node, "oplus,apuir-lessnit-index-list", lessnit_index_list, lessnit_index_count);
		if (rc < 0) {
			OPLUS_DSI_ERR("apuir lessnit_index_count parse failed!\n");
			kfree(lessnit_index_list);
			return 0;
		} else {
			lessnit_index_count = rc;
		}
		...
		rc = utils->get_intarr(node, "oplus,apuir-modeset-list", modeset_list, modeset_count);
		if (rc < 0) {
		...
		rc = utils->get_intarr(node, "oplus,apuir-apl_lhset-list", apl_lhset_list, apl_lhset_count);
		if (rc < 0) {
