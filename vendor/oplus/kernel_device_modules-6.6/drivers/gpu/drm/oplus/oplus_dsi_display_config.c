...
int oplus_dsi_panel_parse_param(struct device_node *dst_node, struct dsi_panel_lcm *ctx)
{
	...
	oplus_panel_parse_apuir_ds_list(dst_node, ctx);
#endif
...
int oplus_dsi_display_init(void *dsi_device, void *ctx_dev)
{
	...
	if (ctx->parser_utils->get_int(dst_node, "oplus,dsi-mode-flags", (int *)&dsi->mode_flags) < 0) {
	...
	OPLUS_DSI_INFO("[LCM] %s config dsi mode_flags=0x%x\n", __func__, dsi->mode_flags);
	of_node_put(dst_node);
	of_node_put(dsi_node);
	return ret;
}
...
static int oplus_dsi_display_feature_deinit(void *dsi_device, void *ctx_dev)
{
	...
	ret = oplus_dsi_display_adfr_deinit(ctx_dev, dsi_device);
	if (ret < 0) {
