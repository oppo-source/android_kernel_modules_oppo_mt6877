...
static int oplus_ofp_cmdq_pkt_wait(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *cmdq_handle, int te_count, int delay_us, bool hbm_en, bool before_hbm)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base)) {
		...
		if(panel_ext->oplus_ofp_video_hbm_delay) {
			cmdq_pkt_wfe(cmdq_handle, mtk_crtc->gce_obj.event[EVENT_VDO_EOF]);
			while (wait_te_count > 0) {
				OFP_INFO("start to wait EVENT_TE, remain %d te count\n", wait_te_count);
				cmdq_pkt_wfe(cmdq_handle, mtk_crtc->gce_obj.event[EVENT_VDO_EOF]);
				OFP_INFO("complete the EVENT_TE waiting\n");
				wait_te_count--;
			}
		} else {
			while (wait_te_count) {
...
static int oplus_ofp_hbm_wait_handle(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle, bool before_hbm, bool hbm_en)
{
	...
	if(oplus_ofp_video_mode_aod_fod_is_enabled()&&!panel_ext->oplus_ofp_video_hbm_delay) {
	...
	if (before_hbm) {
		if (hbm_en) {
			if (p_oplus_ofp_params->aod_unlocking == true) {
				...
				if(panel_ext->oplus_ofp_video_hbm_delay) {
					OFP_INFO("Video mode set te_count = 0");
					te_count = 0;
					delay_us = 0;
				}
			}
		...
		if (hbm_en) {
			...
			} else if (panel_ext->oplus_ofp_video_hbm_delay)
				te_count = 1;
		} else {
		...
		if (oplus_ofp_is_supported()) {
			...
			if (oplus_ofp_get_hbm_state() == true) {
				if ((mtk_crtc_is_frame_trigger_mode(crtc) && !oplus_ofp_full_screen_aod_mode_is_enabled())
						|| (oplus_ofp_video_mode_aod_fod_is_enabled() && !oplus_ofp_full_screen_aod_mode_is_enabled())) {
...
int oplus_ofp_aod_off_set_cmdq(struct drm_crtc *crtc)
{
	...
	if (output_comp->funcs && output_comp->funcs->io_cmd) {
		...
		if (!doze_en && oplus_dc_is_support()) {
			output_comp->funcs->io_cmd(output_comp,
				cmdq_handle, LCM_SEED, &seed_mode);
		}
		if(oplus_project_public_brightness_support() && !doze_en
				&& !oplus_ofp_backlight_filter(crtc, cmdq_handle, oplus_display_brightness)
				&& mtk_crtc->enabled) {
			output_comp->funcs->io_cmd(output_comp,
				cmdq_handle, DSI_SET_BL, &oplus_display_brightness);
		}
	}
	OFP_INFO("DSI_SET_DOZE %d\n", doze_en);
	OPLUS_OFP_TRACE_END("DSI_SET_DOZE");
...
int oplus_ofp_video_mode_aod_handle(void *drm_crtc, void *mtk_panel_ext, void *drm_panel, void *mtk_dsi, void *dcs_write_gce, void *cmdq_handle)
{
	...
	if (!oplus_ofp_get_aod_state() && (refresh_rate == 30)) {
		...
		/* aod on */
		if (ext && ext->funcs && ext->funcs->oplus_doze_enable_ramless)
			ext->funcs->oplus_doze_enable_ramless(drm_panel, mtk_dsi, dcs_write_gce, handle);
		OFP_INFO("debug for oplus_doze_enable\n");

		if (p_oplus_ofp_params->aod_light_mode) {
			if (ext && ext->funcs && ext->funcs->oplus_set_aod_light_mode_ramless)
				ext->funcs->oplus_set_aod_light_mode_ramless(mtk_dsi, dcs_write_gce, handle, p_oplus_ofp_params->aod_light_mode);
			OFP_INFO("set_aod_light_mode:%u\n", p_oplus_ofp_params->aod_light_mode);
		}
	} else if (oplus_ofp_get_aod_state() && (refresh_rate != 30)) {
		if (ext && ext->funcs && ext->funcs->oplus_doze_disable_ramless)
			ext->funcs->oplus_doze_disable_ramless(drm_panel, mtk_dsi, dcs_write_gce, handle);
