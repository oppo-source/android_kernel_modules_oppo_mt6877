...
void aw882xx_dev_mute(struct aw_device *aw_dev, bool mute)
{
	...
	aw_dev_info(aw_dev->dev, "mute: %d, cali_result: %d (%d: mute!!!)",
				mute, aw_dev->cali_desc.cali_result, CALI_RESULT_ERROR);
...
int aw882xx_dev_get_cali_f0(struct aw_device *aw_dev, int32_t *cali_f0)
{

	return aw882xx_dsp_read_cali_f0(aw_dev, cali_f0);
}

int aw882xx_dev_set_air_pressure(struct aw_device *aw_dev, int32_t air_pressure)
{

	return aw882xx_dsp_write_air_pressure(aw_dev, air_pressure);
}


int aw882xx_dev_get_air_pressure(struct aw_device *aw_dev, int32_t *air_pressure)
{

	return aw882xx_dsp_read_air_pressure(aw_dev, air_pressure);
}

int aw882xx_dev_get_t25_f0(struct aw_device *aw_dev, int32_t *t25_f0)
{

	return aw882xx_dsp_read_t25_f0(aw_dev, t25_f0);
}

int aw882xx_dev_get_real_time_f0(struct aw_device *aw_dev, int32_t *real_time_f0)
{

	return aw882xx_dsp_real_time_f0(aw_dev, real_time_f0);
}
...
int aw882xx_dev_init_cali_re(struct aw_device *aw_dev)
{
	...
	if (cali_desc->mode) {
		if (cali_desc->cali_re == AW_ERRO_CALI_VALUE) {
			...
			if (cali_desc->cali_re < aw_dev->re_min ||
				...
				if (aw_dev->cali_desc.cali_check_st) {
					...
					aw_dev_err(aw_dev->dev, "aw cali_check_st failed , mute!! \n");
				}
				return -EINVAL;
			}
		...
		if (cali_desc->cali_f0 == AW_ERRO_CALI_VALUE) {
			ret = aw882xx_cali_read_f0_from_nvram(&cali_desc->cali_f0, aw_dev->channel);
			if (ret) {
				aw_dev_info(aw_dev->dev, "read nvram cali failed, use default f0");
				cali_desc->cali_f0 = AW_ERRO_CALI_VALUE;
				cali_desc->cali_result = CALI_RESULT_NONE;
				return 0;
			}

			aw_dev_info(aw_dev->dev, "read f0 value: %d", cali_desc->cali_f0);
...
static void aw_dev_cali_re_update(struct aw_device *aw_dev)
{
	...
	if (desc->mode && (desc->cali_re != AW_ERRO_CALI_VALUE)) {
		if ((desc->cali_re >= aw_dev->re_min) && (desc->cali_re <= aw_dev->re_max)) {
			aw882xx_dsp_write_cali_re(aw_dev, desc->cali_re);
			aw_dev_info(aw_dev->dev, "start play update re: %d", desc->cali_re);
		} else {
			aw_dev_err(aw_dev->dev, "aw_dev_cali_re_update failed %d ", desc->cali_re);
		}
	}
}

static void aw_dev_cali_f0_update(struct aw_device *aw_dev)
{
	struct aw_cali_desc *desc = &aw_dev->cali_desc;

	if (desc->mode && (desc->cali_f0 != AW_ERRO_CALI_VALUE)) {
		aw882xx_dsp_write_cali_f0(aw_dev, desc->cali_f0);
		aw_dev_info(aw_dev->dev, "start play update f0: %d", desc->cali_f0);
	}
}
