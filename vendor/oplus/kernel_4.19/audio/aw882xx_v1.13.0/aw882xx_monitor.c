...
static void aw_monitor_set_gain(struct aw_device *aw_dev, uint16_t gain)
{
	...
	aw_dev_info(aw_dev->dev, "compared_vol set reg val = 0x%x, gain = 0x%x",
				compared_vol, gain);
}
						...
						uint32_t vmax)
{
	...
	if (aw_dev->monitor_desc.monitor_mode == AW_MON_KERNEL_MODE) {
		...
		if (ret) {
			aw_dev_err(aw_dev->dev, "dsp set vmax failed %d", ret);
			return;
        }
...
int aw882xx_monitor_stop(struct aw_monitor_desc *monitor_desc)
{
	...
	if ((monitor_desc->monitor_mode == AW_MON_KERNEL_MODE) && delayed_work_pending(&monitor_desc->delay_work))
