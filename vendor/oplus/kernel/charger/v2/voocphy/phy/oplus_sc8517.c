...
static int sc8517_cp_enable(struct oplus_chg_ic_dev *ic_dev, bool en)
{
	...
	if (en)
		ret = sc8517_write_byte(chip->client, SC8517_REG_04, 0x36); /* WD:1000ms */
	else
		ret = sc8517_write_byte(chip->client, SC8517_REG_04, 0x06); /* dsiable wdt */

	if (ret < 0) {
		chg_err("failed to set sc8517_cp_enable (%d)(%d)\n", en, ret);
		return ret;
	}
	return 0;
}
