...
static const uint32_t def_value[8] =
{
	0x6a09e667,
	0xbb67ae85,
	0x3c6ef372,
...
static void sha256_transform(struct sha256_ctx_t *s, const uint8_t *buf)
{
	uint32_t t1, t2, a, b, c, d, e, f, g, h, m[64];
	uint32_t i, j;

	for (i = 0, j = 0; i < 16; i++, j += 4) {
		m[i] = (uint32_t)buf[j] << 24 | (uint32_t)buf[j + 1] << 16 | (uint32_t)buf[j + 2] <<
		8 | (uint32_t)buf[j + 3];
	}
	for (; i < 64; i++)
...
void sha256_update(struct sha256_ctx_t *s, const uint8_t *m, uint32_t len)
{
	...
	if (r) {
		if (len + r < SHA256_BLOCKLEN) {
			memmove(s->buf + r, p, len);
			return;
		}
		memmove(s->buf + r, p, SHA256_BLOCKLEN - r);
		len -= SHA256_BLOCKLEN - r;
		p += SHA256_BLOCKLEN - r;
		sha256_transform(s, s->buf);
	}
	for (; len >= SHA256_BLOCKLEN;
		 len -= SHA256_BLOCKLEN, p += SHA256_BLOCKLEN)
...
{
	struct sha256_ctx_t sha256;
	size_t i;
	sha256_init(&sha256);
	sha256_update(&sha256, message, len);
	sha256_final(&sha256, hash);

#if U_ECC_VLI_NATIVE_LITTLE_ENDIAN == 1
	/* Convert hash to little endian */
	for (i = 0; i < SHA256_DIGESTLEN; i += 4) {
		uint32_t data = (hash[i + 3] << 24) | (hash[i + 2] << 16) | (hash[i + 1] << 8) | hash[i];
		hash[i + 3] = data & 0xFF;
		hash[i + 2] = (data >> 8) & 0xFF;
		hash[i + 1] = (data >> 16) & 0xFF;
		hash[i + 0] = (data >> 24) & 0xFF;
	}
#endif
} /* calculate_sha256 */
