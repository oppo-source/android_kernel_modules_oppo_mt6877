...
/* Values defined as in U */
#define POWER_SUPPLY_TYPE_USB_FLOAT	23
#define POWER_SUPPLY_PD_INACTIVE	0
#define POWER_SUPPLY_PD_ACTIVE		1
#define POWER_SUPPLY_PD_PPS_ACTIVE	2
...
static void usb_dwork_handler(struct work_struct *work)
{
	...
	case DR_DEVICE:
		...
		if (ret < 0 || val.intval == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			break;
	/* Handle fall through logic to avoid compile errors */
		stop_usb_host(rpmd);
