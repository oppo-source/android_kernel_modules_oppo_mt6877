				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	static int pr_fast_chg_type = CHARGER_SUBTYPE_DEFAULT;
	union oplus_chg_mod_propval pval = {
		0,
	};
	...
	if (vooc_online) {
		...
		pr_fast_chg_type = fast_chg_type;
	} else if (oplus_quirks_keep_connect_status() == 1) {
		fast_chg_type = pr_fast_chg_type;
	} else {
		fast_chg_type = subtype;
	}

	chg_err("fast_chg_type: %d, pr_fast_chg_type: %d, subtype: %d\n", fast_chg_type, pr_fast_chg_type, subtype);
                                      ...
                                      struct device_attribute *attr, char *buf)
{
	...
	if (oplus_quirks_keep_connect_status() == 1)
