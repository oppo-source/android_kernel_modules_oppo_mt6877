...
void oplus_chg_variables_reset(struct oplus_chg_chip *chip, bool in)
{
	...
	chip->vooc_check_first_time = true;
...
static void oplus_chg_variables_init(struct oplus_chg_chip *chip)
{
	...
	chip->notify_code = 0;
	chip->check_time_sec = 0;
	chip->nonStandardChargerSwitch = 0;
	chip->vooc_check_first_time = true;
	chip->notify_flag = 0;
	chip->cool_down = 0;
	chip->tbatt_pre_shake = TBATT_PRE_SHAKE_INVALID;
...
#define NOTIFY_VOOC_CHECK_TIME  30
static void battery_notify_vooc_check(struct oplus_chg_chip *chip) {
	...
	if (chip->nonStandardChargerSwitch <= 0) {
		chg_debug("%s: RUS switch control %d\n", __func__, chip->nonStandardChargerSwitch);
		return;
	}

	if (chip->wireless_support && (oplus_chg_is_wls_present() || oplus_wpc_get_wireless_charge_start())) {
		chg_debug("%s: in wireless charging\n", __func__);
		return;
	}

	if (oplus_vooc_get_fastchg_dummy_started() || !chip->vooc_check_first_time) {
		chg_debug("%s: dummy started or not first time\n", __func__);
		return;
	}

	if (chip->chg_ops->check_pdphy_ready && chip->chg_ops->check_pdphy_ready() == false) {
		chg_debug("%s: PD phy not ready\n", __func__);
		return;
	}
	...
	if (oplus_pps_get_chg_status() != PPS_NOT_SUPPORT && chip->pps_force_svooc == false) {
		if (chip->chg_ops->oplus_chg_get_pd_type) {
			if (chip->chg_ops->oplus_chg_get_pd_type() == PD_PPS_ACTIVE) {
				chip->vooc_check_first_time = false;
				return;
			}
		}
	}
	...
	if (chip->check_time_sec == 0) {
		...
		if (time_now.tv_sec - chip->check_time_sec >= NOTIFY_VOOC_CHECK_TIME) {
			chip->notify_code |= 1 << NOTIFY_VOOC_CHECK_FAIL;
			chip->vooc_check_first_time = false;
		}
	}
}
...
static void oplus_chg_battery_notify_check(struct oplus_chg_chip *chip)
{
	...
	battery_notify_vooc_check(chip);
