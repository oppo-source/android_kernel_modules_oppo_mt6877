...
// 		}
// 	}
// 	pr_err("%s: debuglog1 ledon:%d\n", __func__, g_charger_chip->led_on);
// 	return 0;
// }
...
static int chg_mtk_drm_notifier_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	chg_err("mtk gki notifier event:%lu, blank:%d", event, *blank);
	switch (event) {
	...
	pr_err("%s: debuglog3 ledon:%d\n", __func__, g_charger_chip->led_on);
	chg_err("debuglog event end!\n");
	return 0;
}
// #else
// void oplus_chg_set_led_status(bool val)
