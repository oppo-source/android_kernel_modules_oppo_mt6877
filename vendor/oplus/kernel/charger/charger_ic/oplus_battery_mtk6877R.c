				...
				struct device *dev)
{
	...
	info->subboard_ntc_unsupport_millivolt_accuracy =
		of_property_read_bool(np, "qcom,subboard_ntc_unsupport_millivolt_accuracy");
	/* PE */
...
static int oplus_get_temp_volt(struct ntc_temp *ntc_param)
{
	...
	if (sub_temp_volt > ntc_param->i_rap_pull_up_voltage ||
		pinfo->subboard_ntc_unsupport_millivolt_accuracy) {
