...
void ecc_mod(u32 *b, u32 *c, u32 c_digits, u32 *p, u32 p_digits)
{
	bignum_mod(b, c, c_digits, p, p_digits);
}

#ifdef P192
/* default modular reduction */
/* not efficient for special NIST primes, as not using their structure */
void ecc_mod192r1(u32 *b, u32 *c, u32 c_digits, u32 *p, u32 p_digits)
{
	bignum_mod(b, c, c_digits, p, p_digits);
}
#endif /* P192 */
...
/* default modular reduction */
/* not efficient for special NIST primes, as not using their structure */
void ecc_mod256r1(u32 *b, u32 *c, u32 c_digits, u32 *p, u32 p_digits)
{
	bignum_mod(b, c, c_digits, p, p_digits);
}
#endif /* P256 */

int point_less_than_psquare(u32 *c, u32 c_digits, u32 *psquare, u32 p_digits)
{
	if (c_digits < p_digits) {
		return (UCL_TRUE);
	} else {
		if (c_digits > p_digits)
			return (UCL_FALSE);
		else
			if (bignum_cmp(c, psquare, c_digits) >= ZERO_VALUE)
				return (UCL_FALSE);
			else
				return (UCL_TRUE);
	}
...
void ecc_modcurve(u32 *b, u32 *c, u32 c_digits, ucl_type_curve *curve_params)
{
	switch(curve_params->curve) {
#ifdef P192
	case SECP192R1:
		ecc_mod192r1(b, c, c_digits, (u32*)(curve_params->p), curve_params->curve_wsize);
	break;
#endif /* P192 */
#ifdef P256
	case SECP256R1:
		ecc_mod256r1(b, c, c_digits, (u32*)(curve_params->p), curve_params->curve_wsize);
	break;
#endif /* P256 */
	default:
		ecc_mod(b, c, c_digits, (u32*)(curve_params->p), curve_params->curve_wsize);
	break;
	}
}
...
int ecc_double_jacobian(ucl_type_ecc_jacobian_point q3, ucl_type_ecc_jacobian_point q1,
...
int ecc_add_jacobian_affine(ucl_type_ecc_jacobian_point q3, ucl_type_ecc_jacobian_point q1,
...
int ecc_convert_affine_to_jacobian(ucl_type_ecc_jacobian_point q, ucl_type_ecc_digit_affine_point x1,
			ucl_type_curve *curve_params)
{
...
int ecc_mult_jacobian(ucl_type_ecc_digit_affine_point q, u32 *m, ucl_type_ecc_digit_affine_point x1,
		ucl_type_curve *curve_params)
{
	int i;
	int j;
