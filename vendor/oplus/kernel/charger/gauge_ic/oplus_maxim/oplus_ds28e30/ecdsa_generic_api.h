...
* @param[out] signature: pointer to a ucl_type_ecdsa_signature structure, containing the
						 signature (r,s) values
* @param[in] *d: input, the secret key
* @param[in] *ucl_hash: input, the pointer to the hash function (see hash
						functions documentation for already available ones)
* @param[in] *input: input, the message or the hash digest to be signed,
* @param[in] inputlength: input, the input length, in bytes
* @param[in] *curve_params: the pointer to a ucl_type_curve structure, containing the curve
					domain parameters (already existing ones are described in
					the documentation)
...
- UCL_R_PRECOMP<<UCL_PRECOMP_SHIFT+ UCL_MSG_INPUT<<UCL_INPUT_SHIFT:
	signing a message, using pre-computed r
 * @return Error code
 * @retval #UCL_OK in case of correct computation
 * @retval #UCL_INVALID_INPUT or #UCL_INVALID OUTPUT in
 * 			case of wrong parameters configuration (e.g. NULL pointers)
 * @ingroup UCL_ECDSA */
int ucl_ecdsa_signature(ucl_type_ecdsa_signature signature, u8 *d,
...
* @param[in] public key: pointer to a ucl_type_ecc_u8_affine_point structure,
					containing the ECC public key, used for signature verification
* @param[in] signature: pointer to a ucl_type_ecdsa_signature structure,
						containing the signature (r,s) values
...
* @param[in] *curve_params: the pointer to a ucl_type_curve structure,
					containing the curve domain parameters (already existing
					ones are described in the documentation)
* @param[in] configuration (combination of any of these lines)
* @param[in]
...
* @ingroup UCL_ECDSA */
int ucl_ecdsa_verification(ucl_type_ecc_u8_affine_point Q,
			ucl_type_ecdsa_signature signature, int(*ucl_hash)(u8*, u8*, u32),
			u8 *input, u32 inputlength, ucl_type_curve *curve_params, u32 configuration);

/* <b>ECC multiplication</b>.
* multiply a scalar by a ECC point, using affine parameters,
  computation performed in Jacobian parameters
* @param[out] Q: pointer to a ucl_ecc_digit_affine_point structure,
				 containing the ECC result point,
* @param[in] m: pointer to a word array containing the scalar,
* @param[in] X1: pointer to a ucl_ecc_digit_affine_point structure, containing
				 the ECC point, to be multiplied by the scalar,
* @param[in] *curve_params: the pointer to a ucl_type_curve structure,
...
* @retval #UCL_INVALID_INPUT in case of wrong parameters
* @ingroup UCL_ECDSA */
int ecc_mult_jacobian(ucl_type_ecc_digit_affine_point Q, u32 *m,
...
* @retval #UCL_INVALID_INPUT in case of wrong parameters
* @ingroup UCL_ECDSA */
int ecc_add(ucl_type_ecc_digit_affine_point q3, ucl_type_ecc_digit_affine_point q1,
...
* @retval #UCL_INVALID_INPUT in case of wrong parameters
* @ingroup UCL_ECDSA */
int ecc_double(ucl_type_ecc_digit_affine_point q3, ucl_type_ecc_digit_affine_point q1,
