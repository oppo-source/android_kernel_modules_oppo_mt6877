...
void bignum_d2us(u8 *a, u32 len, u32 *b, u32 digits)
{
	...
	for (i = 0, j = (int)len - 1; (i < (int)digits) && (j >= 0); i++) {
		t = b[i];
		for (u = 0; j >= 0 && u < (int)DIGIT_BITS; j--, u += BYTE_LENGTH_8)
			a[j] = (u8)(t >> u);
	}
	for (; j >= 0; j--)
		a[j] = 0;
...
{
	u32 t[2 * MAX_DIGITS], tmp[BYTE_LENGTH_2], carry, carrynext, carryover;
	u32 bDigits, c0, c1;
	int i, j;
...
void bignum_divide(u32 *a, u32 *b, u32 *c, u32 c_digits, u32 *d, u32 d_digits)
{
	u32 ai, cc[2*MAX_DIGITS + 1], dd[MAX_DIGITS], t;
	int i;
	u32 dd_digits, shift;
	dd_digits = bignum_digits(d, d_digits);
	if (dd_digits == 0)
		return;
	shift = DIGIT_BITS - bignum_digitbits(d[dd_digits - 1]);
	bignum_copyzero(cc, dd_digits);
	cc[c_digits] = bignum_leftshift(cc, c, shift, c_digits);
	bignum_leftshift(dd, d, shift, dd_digits);
	t = dd[dd_digits - 1];
	if (NULL != a)
		bignum_copyzero(a, c_digits);
	for (i  = (int)(c_digits - dd_digits); i >= 0; i--) {
		if (t == MAX_DIGIT) {
			ai = cc[i + (int)dd_digits];
		} else {
			bignum_scalardiv(&ai, &cc[i + (int)dd_digits - 1], t + 1);
		}
		cc[i + (int)dd_digits] -= bignum_subscalarmult(&cc[i], &cc[i], ai, dd, dd_digits);
		while (cc[i + (int)dd_digits] ||(bignum_cmp(&cc[i], dd, dd_digits) >= 0)) {
			ai++;
			cc[i + (int)dd_digits] -= bignum_sub(&cc[i], &cc[i], dd, dd_digits);
		}
		if (NULL != a)
			a[i] = ai;
	}
	if (b != NULL) {
		bignum_copyzero(b, d_digits);
		bignum_rightshift(b, cc, shift, dd_digits);
	}
}
...
void bignum_mod(u32 *b, u32 *c, u32 c_digits, u32 *d, u32 d_digits)
{
	u32 copy_a[2 * MAX_DIGITS], dd_digits;
	dd_digits = bignum_digits(d, d_digits);
	bignum_divide(NULL, copy_a, c, c_digits, d, dd_digits);
	bignum_copyzero(b, d_digits);
	bignum_copy(b, copy_a, dd_digits);
}
...
void bignum_div(u32 *quot, u32 *b, u32 *c, u32 c_digits, u32 *d, u32 d_digits)
{
	u32 copy_a[2*MAX_DIGITS], dd_digits;
	int i;
	dd_digits = bignum_digits(d, d_digits);
	if (dd_digits == 0)
		return;
	bignum_divide(quot, copy_a, c, c_digits, d, dd_digits);
	if (b != NULL) {
		bignum_copy(b, copy_a, dd_digits);
		for (i = (int)dd_digits;i < (int)d_digits; i++)
			b[i] = 0;
	}
}
