...
struct oplus_maxim_gauge_chip {
	...
	unsigned char sn_num[BATT_SN_NUM_LEN];
...
static int oplus_maxim_parse_dt(struct oplus_maxim_gauge_chip *chip)
{
	...
	rc = of_property_read_u8_array(node, "oplus,batt_info", chip->sn_num, len);
	if (rc) {
		chg_err("maxim get oplus,batt_info failed %d\n", rc);
		return -1;
	}
	for (i = 0; i < len; i++) {
		chg_info("parse oplus,batt_info, sn_num[%d] = %d\n", i, chip->sn_num[i]);
	}
...
static void oplus_maxim_auth_work(struct work_struct *work)
{
	...
	ret = authenticate_ds28e30(g_maxim_chip->sn_num, 0);
	if (ret == false) {
...
static __init oplus_maxim_driver_init(void)
{
	int ret;
	chg_info("%s: start\n", __func__);
