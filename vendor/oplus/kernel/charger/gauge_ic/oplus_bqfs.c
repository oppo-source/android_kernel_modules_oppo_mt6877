...
static char bqfs_info[BQFS_INFO_LEN] = { 0 };
...
static void oplus_bqfs_track_update_work(struct work_struct *work)
{
	...
	oplus_bqfs_track_upload_upgrade_info(chip, bqfs_info);
}
...
#define PUSH_DELAY_MS 5000
	...
	int i, buflen, len;
	int rc = -1, rec_cnt = 0, retry_times = 0, index;
	int batt_id = BAT_TYPE_UNKNOWN, type = BAT_TYPE_UNKNOWN;
	char dm_name[128] = {0};
	...
	if (rc < 0) {
		pr_err(" %s batt_id_chan get failed, rc = %d\n", __func__, rc);
	...
	sprintf(dm_name, "bqfs_dm_%d", type);
	sprintf(data_name, "sinofs_bqfs_data_%d", type);

	rc = of_property_read_u32(bqfs_node, "bqfs_unfilt", &bqfs_unfilt);
	if (rc) {
		bqfs_unfilt = BQ27426_BQFS_FILT;
	}
	chip->bqfs_ship = of_property_read_bool(bqfs_node, "oplus,bqfs_ship");
	...
	rc = of_property_read_u32(bqfs_node, dm_name, &bqfs_dm);
	if (rc) {
		bqfs_dm = 0;
	}
	...
	buflen = of_property_count_u8_elems(bqfs_node, data_name);
	...
	rc = of_property_read_u8_array(bqfs_node, data_name, pBuf, buflen);
	if (rc) {
...
BQFS_END:
	if (!chip->bqfs_init) {
		index = snprintf(bqfs_info, BQFS_INFO_LEN, "$$bqfs_result@@%d$$bqfs_init@@%d$$value_dm@@0x%x$$bqfs_dm@@0x%x$$bqfs_flag@@0x%x$$batt_id@@%d$$bqfs_type@@%d",
			rc, chip->bqfs_init, value_dm, bqfs_dm, read_buf, batt_id, type);
		schedule_delayed_work(&chip->bqfs_track_update_work, msecs_to_jiffies(PUSH_DELAY_MS));
	}
	chg_err(" end[%d %d 0x%x 0x%x %d %d]\n", rc, chip->bqfs_init, value_dm, read_buf, chip->bqfs_ship, type);
