...
static void bq28z610_bcc_set_buffer(int *buffer)
{
	...
	if ((true == gauge_ic->batt_zy0603) || (DEVICE_ZY0602 == gauge_ic->device_type)) {
		buffer[17] = SW_GAUGE;
		   ...
		   (true == gauge_ic->batt_bq28z610) || gauge_ic->batt_bq27z561) {
		buffer[17] = TI_GAUGE;
	} else {
	...
	struct chip_bq27541 *chip, int *dod_time, int *qmax_time)
{
	...
	static int dod_calib_time = 0;
	static int qmax_calib_time = 0;
	static int check_args_pre[CALIB_TIME_CHECK_ARGS] = {0};
...
try:
		mutex_lock(&chip->bq28z610_alt_manufacturer_access);
		ret = bq27541_i2c_txsubcmd(chip, BQ27Z561_DATAFLASHBLOCK, extend[i].addr);
		if (ret < 0) {
			mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
			return -1;
		}
		...
		if (sizeof(extend_data) >= extend[i].len + 2) {
			usleep_range(1000, 1000);
			ret = bq27541_read_i2c_block(chip, BQ27Z561_DATAFLASHBLOCK, (extend[i].len + 2), extend_data);
			mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
			if (ret < 0)
				return -1;
	...
	if (!init_flag || check_args[0] != check_args_pre[0])
		dod_calib_time = 0;
	else
		dod_calib_time += 1;

	if (!init_flag || check_args[1] != check_args_pre[1] || check_args[2] != check_args_pre[2])
		qmax_calib_time = 0;
	else
		qmax_calib_time += 1;

	init_flag = true;
	memcpy(check_args_pre, check_args, sizeof(check_args));
	*dod_time = dod_calib_time;
	*qmax_time = qmax_calib_time;
	pr_info("bq27z561_get_calib_time:calib_time[%d, %d], check_args[%d, %d, %d, %d, %d, %d]\n",
		dod_calib_time, qmax_calib_time, check_args[0], check_args[1],
		check_args[2], check_args[3], check_args[4], check_args[5]);
...
try:
	mutex_lock(&chip->bq28z610_alt_manufacturer_access);
	ret = bq27541_i2c_txsubcmd(chip, BQ27Z561_DATAFLASHBLOCK, extend.addr);
	if (ret < 0) {
		mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
		return ret;
	}
	usleep_range(1000, 1000);
	ret = bq27541_read_i2c_block(chip, BQ27Z561_DATAFLASHBLOCK, (extend.len + 2), extend_data);
	if (ret < 0) {
		mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
		return ret;
	}
...
device_type_set:
	...
	if (device_type == DEVICE_TYPE_BQ28Z610 ||
	    device_type == DEVICE_TYPE_ZY0603 || chip->batt_bq27z561) {
...
static bool bq27541_sha256_hmac_authenticate(struct chip_bq27541 *chip)
{
	int i;
	int ret;
	int count = 0;
	...
	ret = bq27541_write_i2c_block(chip, BQ27Z561_DATAFLASHBLOCK, sizeof(data_buf), data_buf);
	if (ret < 0)
		goto error;

	ret = bq27541_write_i2c_block(chip, BQ27Z561_AUTHENDATA_1ST, half_len, p_temp);
	if (ret < 0)
		goto error;

	ret = bq27541_write_i2c_block(chip, BQ27Z561_AUTHENDATA_2ND, half_len, p_temp + half_len);
	if (ret < 0)
		goto error;

	ret = bq27541_i2c_txsubcmd_onebyte(chip, BQ27Z561_AUTHENCHECKSUM, checksum);
	if (ret < 0)
		goto error;

	ret = bq27541_i2c_txsubcmd_onebyte(chip, BQ27Z561_AUTHENLEN, 0x24);
	if (ret < 0)
		goto error;
...
try:
	msleep(60);
	ret = bq27541_read_i2c_block(chip, BQ27Z561_AUTHENDATA_1ST, half_len, p_temp);
	...
	ret = bq27541_read_i2c_block(chip, BQ27Z561_AUTHENDATA_2ND, half_len, p_temp + half_len);
	if (ret < 0)
		goto error;
	mutex_unlock(&chip->bq28z610_alt_manufacturer_access);
	return true;
...
static bool init_sha256_gauge_auth(struct chip_bq27541 *chip)
{
	...
	ret = bq27541_sha256_hmac_authenticate(chip);
	if (!ret)
		return ret;
...
static void bq27541_reset(struct i2c_client *client_chip)
{
	...
	if (bq27541_chip->batt_zy0603 || bq27541_chip->batt_bq27z561)
		return;
