...
static void aol_push_message(struct aol_core_rb *rb, unsigned int type, unsigned int *buf)
{
    ...
    if (rb_data) {
        ...
        } else if (type == AOL_MSG_ID_LOG) {
            memcpy(rb_data->raw_data.custom_log, buf, strnlen((const char *)buf, MAX_LOG_SIZE));
            rb_data->raw_data.custom_log[strnlen((const char *)buf, MAX_LOG_SIZE)] = '\0';
            pr_info("[%s] Log message: %s\n", __func__, rb_data->raw_data.custom_log);
...
static int opfunc_recv_msg(struct msg_op_data *op)
{
    ...
    if (type == AOL_MSG_ID_WIFI_FENCE) {
        ...
        pr_info("[%s] Log message: %s\n", __func__, rb_data->raw_data.custom_log);
        ptr = (char *)&(rb_data->raw_data.custom_log);
        sz = sizeof(rb_data->raw_data.custom_log);
    } else if (type == AOL_MSG_ID_CRITICAL_EVENT) {
...
void aol_core_msg_notify(unsigned int msg_id, unsigned int *buf, unsigned int size)
{
    ...
    }else if (msg_id == AOL_MSG_ID_LOG) {
        char *ptr = (char *) buf;
        if (ptr != NULL) {
            pr_info("[%s] log message: %s\n", __func__, ptr);
        } else {
            pr_info("[%s] log message: NULL\n", __func__);
        }
        expect_size = size;
...
static int aol_core_handler(int cmd, void *data)
{
    ...
    if (ret) {
        ...
        spin_lock_irqsave(&(g_rb1.lock), flags);
        rb_data = aol_core_rb_pop_active(&g_rb1);
        pr_info("[%s] Send to msg thread fail, pop active rb_data\n", __func__);

        if (rb_data != NULL) {
            aol_core_rb_push_free(&g_rb1, rb_data);
            pr_info("[%s] Send to msg thread fail, push free rb_data\n", __func__);
        }
        spin_unlock_irqrestore(&(g_rb1.lock), flags);
    }
