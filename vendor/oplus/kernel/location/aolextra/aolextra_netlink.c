...
static int aol_nl_bind(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (g_already_bind) {
        pr_info("[%s] Already bind before, save the pid, and change port=[%d]", __func__, port);
        last_pid = g_ctx2->bind_pid;
                ...
                unsigned int msg_id, char *buf, unsigned int length)
{
    ...
    if (ret != 0) {
        ...
        if (ret == -ECONNREFUSED) {
            ret = aol_netlink_msg_send(tag, msg_id, buf, length, last_pid, g_ctx2->seqnum);
            pr_info("[%s] genlmsg_unicast fail with ECONNREFUSED, retry ret=[%d].\n", __func__, ret);
            if (ret == 0) {
                pr_info("[%s] genlmsg_unicast success when using pid=[%d], will update the pid\n", __func__, last_pid);
                g_ctx2->bind_pid = last_pid;
            }
        }
    }
