...
#ifdef CONFIG_LOCKING_ENTITY_ELIGIBLE
...
void locking_state_systrace_c(unsigned int cpu, struct task_struct *p)
{
	...
	if (IS_ERR_OR_NULL(ots)) {
		...
		if (task_inlock(ots)) {
			locking_state =  LK_STATE_LOCK;
#ifdef CONFIG_LOCKING_ENTITY_ELIGIBLE
			if (ots->ineligible_flag) {
				locking_state |= LOCKING_INELIGIBLE_MASK;
				ots->ineligible_flag = 0;
			}
#endif
		} else {
			locking_state =  LK_STATE_UNLOCK;
		}
...
#ifdef CONFIG_LOCKING_ENTITY_ELIGIBLE
		if (!vruntime_eligible(cfs_rq_of(key_se), key_se->vruntime)) {
			key_ots->ineligible_flag = 1;
			continue;
		}
#endif
		/*
...
#else
	if (task_inlock(ots) && !test_task_ux(p)) {
		if (vruntime_eligible(cfs_rq_of(&curr->se), curr->se.vruntime)) {
			*nopreempt = true;
			*preempt = false;
		}
	}
