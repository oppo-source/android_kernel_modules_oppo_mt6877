...
do {								\
	int ret = register_trace_##vendor_hook(handler, data);				\
	if (ret) {						\
...
void mutex_lock_handler(void * unused, struct mutex * lock, unsigned long jiffies)
{
	...
	if (jiffies) {
		lock_handler(current, (u64)lock, MUTEX);
	}
	else {
		unlock_handler(current, (u64)lock, MUTEX);
	}
...
void rwsem_lock_handler(void * unused, struct rw_semaphore * sem, unsigned long jiffies)
{
	...
	if (jiffies) {	
		lock_handler(current, (u64)sem, RWSEM);
	}
	else {
		unlock_handler(current, (u64)sem, RWSEM);
	}
}
...
void rtmutex_lock_handler(void * unused, struct rt_mutex * lock, unsigned long jiffies)
{
	...
	if (jiffies)
		lock_handler(current, (u64)lock, RTMUTEX);
	else
		unlock_handler(current, (u64)lock, RTMUTEX);
}
...
/*
	if (jiffies)
		lock_handler(current, (u64)sem, PCP_RWSEM);
	else
		unlock_handler(current, (u64)sem, PCP_RWSEM);
*/
}
...
void rwsem_write_wait_handler(void * unused, struct rw_semaphore *sem)
{
	/* Contend stats. */
	atomic64_inc(&wait_cnt[RWSEM]);
...
void __exit krn_reliab_exit(void)
{
	exit_clear_lock_hold_timer();
}
