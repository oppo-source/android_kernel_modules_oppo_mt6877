				...
				unsigned long callstack2)
{
	...
	if (type == FREQ_QOS_MIN)
		snprintf(buf, sizeof(buf), "C|8888|min_freq_%ps<-%ps|%d\n", callstack1, callstack2, new_value);
	else if (type == FREQ_QOS_MAX)
		snprintf(buf, sizeof(buf), "C|8888|max_freq_%ps<-%ps|%d\n", callstack1, callstack2, new_value);
...
static void monitor_req_update(struct freq_qos_request *req, int new_value)
{
	...
	list_for_each_entry(monitor, &(monitors.node), node) {
		if (monitor->req == req) {
			...
			pr_info("%s req_type=%d req_min_value=%d req_max_value=%d req_from_pid=%d last_update_time=%llu callstack:(%ps<-%ps<-%ps<-%ps)\n",
				__func__, req->type, req->qos->min_freq.target_value, req->qos->max_freq.target_value, monitor->pid, monitor->last_update_time,
				monitor->callstack[0], monitor->callstack[1], monitor->callstack[2], monitor->callstack[3]);
...
void freq_qos_update_request_handler(void *unused, struct freq_qos_request *req, int new_value)
{
	...
	pr_info("%s req_type=%d new_value=%d\n", __func__, req->type, new_value);
}
