...
/*
 * Record execution time of ux or rt tasks.
 *
 * In order to record the running time of the task, this function
 * should be called in scheduler_tick and check_preempt_wakeup.
 * In particular, it is necessary to call this function before
 * obtaining the latest exec_time.
 */
void record_exec_time(struct rq *rq, struct task_struct *curr)
{
	...
	delta = curr->se.sum_exec_runtime - ots->exec_baseline;
	...
	ots->exec_baseline += delta;
	ots->exec_time += delta;

#ifdef DEBUG_LB_EXEC_TIME
	record_exec_time_systrace(task_cpu(curr), curr, ots->exec_time);
#endif
...
/*
 * WARNING:
 * The execution time of the task should be obtained through
 * the get_exec_time interface instead of using ots->exec_time
 * directly.
 */
u64 get_exec_time(struct rq *rq, struct task_struct *curr)
{
	struct oplus_task_struct *ots;
	...
	/*
	 * Note:
	 * Before obtaining exec_time, it needs to be updated
	 * through the record_exec_time function.
	 */
	record_exec_time(rq, curr);

	return ots->exec_time;
}
...
bool ux_need_up_migration(struct task_struct *p, struct rq *rq)
{
	...
	/*
	 * Note:
	 * You need to hold the rq_lock lock when obtaining the
	 * execution time of a task during the tick balance,
	 * otherwise it may trigger a crash due to the lockdep
	 * check.
	 */
	rq_lock_irqsave(rq, &rf);
	exectime = get_exec_time(rq, p);
	rq_unlock_irqrestore(rq, &rf);
	threshold_time = get_threshold_time(migr_running_ux);

#ifdef DEBUG_LB_TICK
	trace_printk("OPLUS_LB_TICK[%d]: task=%s$%d, mask=[%*pbl], "
		"cpu=%d, cls=%d, exec_time=%llu, threshold_time=%llu, true=%d\n",
		__LINE__, p->comm, p->pid, cpumask_pr_args(p->cpus_ptr),
		cpu_of(rq), topology_physical_package_id(cpu_of(rq)),
		exectime, threshold_time, exectime >= threshold_time);
#endif

	return exectime >= threshold_time;
}
...
int find_cpu_in_migration(struct task_struct *p,
			int prev_cpu, enum migr_type type)
{
	struct rq *rq = NULL;
			...
			struct task_struct *task)
{
	...
	new_cpu = find_cpu_in_migration(task, prev_cpu, down_migr);
	...
	new_cpu = find_cpu_in_migration(task, prev_cpu, up_migr);
	...
	new_cpu = find_cpu_in_migration(task, prev_cpu, normal_migr);
...
#ifdef DEBUG_LB_RT_TICK
	trace_printk("DEBUG_LB_RT_TICK[%d]: src_cpu=%d, dst_cpu=%d, "
		"active=%d-%d, cpu=%d, ab=%d, nr=%d, pid=%d\n",
		__LINE__, busiest_cpu, target_cpu,
		cpu_active(busiest_cpu), cpu_active(target_cpu), smp_processor_id(),
		busiest_rq->active_balance, busiest_rq->nr_running, orq->lb.pid);
...
static bool oplus_migrate_running_ux(void *data, struct rq *rq)
{
	 ...
	 */
	new_cpu = find_cpu_in_migration(curr, prev_cpu, up_migr);
...
void oplus_record_runnable_time(void *unused,
			struct task_struct *p, u64 delta)
{
	...
	ots->runnable_time += delta;
#ifdef DEBUG_LB_TICK_RUNNABLE_TIME
	record_runnable_time_systrace(task_cpu(p), p, ots->runnable_time);
#endif
}
...
u64 oplus_get_runnable_time_for_rt(int cpu, struct task_struct *p)
{
	...
	now = rq->clock;
	enqueue_time = oplus_get_enqueue_time(p);

	/*
	 * Note:
	 * a) If this rt_task has just been enqueued into the runqueue of
	 *    this cpu, but it has not yet been executed, then the value
	 *    of enqueue_time is 0.
	 * b) If this rt_task was run on another cpu last time, there will
	 *    be a situation where 'now' is less than 'enqueue_time'
	 *    because rq->clock comes from a different cpu.
	 */
	if (!enqueue_time || now <= enqueue_time) {
#ifdef DEBUG_LB_RT_RUNNABLE_TIME
		trace_printk("DEBUG_LB_RT_RUNNABLE_TIME[%d]: curr=%s$%d, state=%d, "
			"task_cpu=%d, cpu=%d, true1=%d, "
			"now=%llu, ""enqueue_time=%llu, true2=%d\n",
			__LINE__, p->comm, p->pid, READ_ONCE(p->state),
			task_cpu(p), cpu, task_cpu(p) == cpu,
			now, enqueue_time, now <= enqueue_time);
#endif
		return 0;
	}

	exectime = get_exec_time(cpu_rq(cpu), p);
	runnable_time = now - enqueue_time;
	if (runnable_time <= exectime) {
#ifdef DEBUG_LB_RT_RUNNABLE_TIME
		trace_printk("DEBUG_LB_RT_RUNNABLE_TIME[%d]: curr=%s$%d, state=%d, "
			"task_cpu=%d, cpu=%d, true=%d, "
			"now=%llu, enqueue_time=%llu, exectime=%llu\n",
			__LINE__, p->comm, p->pid, READ_ONCE(p->state),
			task_cpu(p), cpu, task_cpu(p) == cpu,
			now, enqueue_time, exectime);
#endif
		return 0;
	}

	runnable_time = runnable_time - exectime;

#ifdef DEBUG_LB_RT_RUNNABLE_TIME
	trace_printk("DEBUG_LB_RT_RUNNABLE_TIME[%d]: curr=%s$%d, state=%d, "
		"task_cpu=%d, cpu=%d, true=%d, "
		"now=%llu, enqueue_time=%llu, exectime=%llu, rb=%llu\n",
		__LINE__, p->comm, p->pid, READ_ONCE(p->state),
		task_cpu(p), cpu, task_cpu(p) == cpu,
		now, enqueue_time, exectime, runnable_time);

	runnable_time_rt_systrace(cpu, p, runnable_time);
#endif
...
u64 oplus_get_runnable_time(int cpu, struct task_struct *p)
{
	...
	runnable_time = ots->runnable_time;

	rq = cpu_rq(cpu);
	if (!(rq->clock_update_flags & RQCF_UPDATED))
		update_rq_clock(rq);

	/* The task is in runnable state */
	if (p->on_rq && !p->on_cpu) {
		se = &p->se;
		delta = cpu_rq(cpu)->clock - schedstat_val(se->statistics.wait_start);
		runnable_time += delta;
	}
#ifdef DEBUG_LB_TICK_RUNNABLE_TIME
	runnable_time_systrace(cpu, p, runnable_time);
#endif
	return runnable_time;
}
...
static bool oplus_migrate_runnable_ux(void *data, struct rq *rq)
{
	 ...
	 */
	new_cpu = find_cpu_in_migration(ux_task, this_cpu, normal_migr);
	if (new_cpu < 0)
		return false;
...
{
	struct rq_flags rf;
	u64 runtime, threshold_time = ULLONG_MAX;
	 ...
	 */
	rq_lock_irqsave(rq, &rf);
	runtime = get_exec_time(rq, p);
	rq_unlock_irqrestore(rq, &rf);
	threshold_time = get_threshold_time(migr_running_rt_boost);

#ifdef DEBUG_LB_RT_TICK
	oplus_loadbalance_systrace_print(OPLUS_LB_SYSTRACE_PID,
				"rt_exectime", cpu_of(rq), runtime);

	trace_printk("DEBUG_LB_RT_TICK[%d]: task=%s$%d, mask=[%*pbl], "
		"cpu=%d, cls=%d, runtime=%llu, threshold_time=%llu, true=%d\n",
		__LINE__, p->comm, p->pid, cpumask_pr_args(p->cpus_ptr),
		cpu_of(rq), topology_physical_package_id(cpu_of(rq)),
		runtime, threshold_time, runtime >= threshold_time);
#endif

	return runtime >= threshold_time;
}
...
#ifdef DEBUG_LB_RT_TICK
	trace_printk("DEBUG_LB_RT_TICK[%d]: curr=%s$%d, rt_boost=%d, ret=%d\n",
		__LINE__, curr->comm, curr->pid, task_is_rt_boost(curr), ret);
#endif
	return ret;
}
...
void oplus_loadbalance_init(void)
{
	...
	/* Used to count the time the cfs task is in the runnable state. */
	register_trace_sched_stat_wait(oplus_record_runnable_time, NULL);
...
void oplus_loadbalance_deinit(void)
{
	 ...
	 */
	unregister_trace_sched_stat_wait(oplus_record_runnable_time, NULL);
}
