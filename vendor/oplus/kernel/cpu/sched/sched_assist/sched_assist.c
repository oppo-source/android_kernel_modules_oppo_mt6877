...
/*static int cpu_stop_handler(void *data)
{
	...
	do_each_thread(g, p) {
		...
		if (IS_ERR_OR_NULL(ots))
			continue;
		//if possible stop_machine set cpu_mask to be sliver cluster, and restart machine？
		//cpus_requested will be init to be sliver cluster?
...
#ifndef CONFIG_OPLUS_SYSTEM_KERNEL_QCOM
	/* no need to listen stop_machine, keeping "cpus_requested" field empty is fine*/
