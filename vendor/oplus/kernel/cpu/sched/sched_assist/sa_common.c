...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_LOADBALANCE)
			if (!ots->exec_time)
				ots->exec_baseline = t->se.sum_exec_runtime;
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_LOADBALANCE)
	/*
	 * We need to record the execution time of all tasks.
	 */
	if (!ots->exec_time)
		ots->exec_baseline = p->se.sum_exec_runtime;
#endif
