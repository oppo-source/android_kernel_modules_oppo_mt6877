...
static inline void  oplus_set_stune_idx(struct task_struct *t, int stune_idx)
{
	...
	if (!ots || stune_idx < 0 || stune_idx >= BOOSTGROUPS_COUNT)
		return;
...
static void schedtune_enqueue_task(struct task_struct *p, int cpu)
{
	...
	if (idx < 0 || idx >= BOOSTGROUPS_COUNT)
		goto done;
...
void android_vh_sched_move_task_handler(void *unused, struct task_struct *task)
{
	...
	if (src_idx < 0 || src_idx >= BOOSTGROUPS_COUNT
			|| unlikely(dst_idx == src_idx)) {
		goto done;
	}
...
static void schedtune_handle_pre_init(void)
{
	struct cgroup_subsys_state *root_css = &root_task_group.css, *css;

	android_vh_cpu_cgroup_css_alloc_early_handler(NULL, NULL);

	rcu_read_lock();
	css_for_each_child(css, root_css)
		android_vh_cpu_cgroup_css_alloc_handler(NULL, css_tg(css), css->parent);
	rcu_read_unlock();
}
