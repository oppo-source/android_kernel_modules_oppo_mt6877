...
static void oplus_kevent_send_to_user(void *data, struct kernel_packet_info *userinfo, int *p_retval)
{
	...
	*p_retval = 0;
	...
	/*
	* Reference linux kernel code, there no more need free skb, genlmsg_unicast can do this,
	* user free will lead no message at the other end of socker,free may cause crash
	*/
	*p_retval = 0;
#endif
}

void kevent_send_to_user(void *data, async_cookie_t c)
{
	int ret = 0;
	struct kernel_packet_info *userinfo = data;
...
void report_security_event(const char *event_name, unsigned int event_type, const char *more)
{
	...
	if (dcs_event_h == NULL) {
		pr_err(SG_KEVENT "%s, kmalloc error.\n", __func__);
		return;
	}
