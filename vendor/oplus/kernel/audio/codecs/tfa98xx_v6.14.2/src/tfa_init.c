...
static enum Tfa98xx_Error tfa9873_specific(struct tfa_device* tfa)
{
	...
	irqmask = (TFA_BF_MSK(TFA9878_BF_IEVDDS) |
...
static enum Tfa98xx_Error tfa986x_specific(struct tfa_device *tfa)
{
	...
	/*OTP shadow registers updated after power on*/
	if (((tfa->rev & 0xff) == 0x66) && tfa_get_bf(tfa, 0xf3b1)) {
		rev = rev - 1;	/* To > 0x65 */
	...
	case 0x3a64:/**TFA9864 N1A3**/
		...
		tfa_reg_write(tfa, 0x62, 0x05c4); //POR=0x0582
