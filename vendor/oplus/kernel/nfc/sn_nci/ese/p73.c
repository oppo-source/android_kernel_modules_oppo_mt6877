...
static int p61_spi_clk_enable(struct p61_dev *p61_dev)
{
    int ret = 0;
    struct spi_device *spi = p61_dev->spi;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 2)
    struct mtk_spi *spi_ms = spi_controller_get_devdata(spi->controller);
#else
			  ...
			  unsigned long arg)
{
    ...
    case P61_SPI_IOC_SPI_CLOCK_SET:
        P61_DBG_MSG(KERN_ALERT "P61_SPI_IOC_SPI_CLOCK_SET set: %lu\n", arg);
        if(arg == 1) {
            ret = p61_spi_clk_enable(p61_dev);
        } else if (arg == 0) {
...
#if DRAGON_P61 && (defined P61_IRQ_ENABLE)
...
static int p61_probe(struct spi_device *spi)
{
	...
	//tCSV must be larger than 58ns according to sn220 data sheet,
	//set the cs_setup to 100ns to meet the data sheet's requirement.
	spi->cs_setup.value = 100;
