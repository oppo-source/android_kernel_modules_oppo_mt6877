...
{
    int ret = SUCCESS;
    struct mtk_spi *mt_spi;
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 9, 0)
    mt_spi = spi_master_get_devdata(ese->client->master);
#else
    mt_spi = spi_controller_get_devdata(ese->client->controller);
#endif
    TMS_DEBUG("arg = %lu\n", arg);
...
#else
    TMS_DEBUG("chip select = %d , bus number = %d \n",
              client->chip_select[0], client->controller->bus_num);
#endif
