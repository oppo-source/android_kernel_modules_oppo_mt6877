...
#define JD5516W_NAME				"jd5516w_24071"
...
static int jd5516w_set_position(struct jd5516w_device *jd5516w, u16 val)
{
    struct i2c_client *client = v4l2_get_subdevdata(&jd5516w->sd);
    if (!jd5516w->active) {
        pr_info("jd5516w set position is %d, active is %d", val, jd5516w->active);
        return 0;
    }
...
static int jd5516w_release(struct jd5516w_device *jd5516w)
{
    ...
    // last step to origin
    ret = jd5516w_set_position(jd5516w, JD5516W_MIDDLE_CODE);
    if (ret) {
...
static int jd5516w_vcm_resume(struct jd5516w_device *jd5516w)
{
    ...
    ret = jd5516w_set_position(jd5516w, jd5516w->focus->val);
    if (ret) {
        pr_info("%s I2C failure: %d", __func__, ret);
        return ret;
    }
    pr_info("%s exit stand by mode, active:%d\n", __func__, jd5516w->active);
    return ret;
}
...
static const struct of_device_id jd5516w_of_table[] = {
	{ .compatible = "oplus,jd5516w_24071" },
	{ },
};
