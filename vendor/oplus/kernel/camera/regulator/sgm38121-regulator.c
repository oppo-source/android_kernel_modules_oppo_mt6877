...
//#define SGM38121_DISCR_MASK BIT(7)
...
} multi_reg_sgm38121_registers_t;

#define MULTI_REG_MAX_REG (SGM38121_REG_MAX)
...
#define SGM38121_DLDO(_num, _supply, _min_val, _min_sel, _max_sel, _step)                \
    [SGM38121_LDO ## _num] = {                    \
        .name =           "ONLDO"#_num,            \
        ...
        .linear_ranges =   (struct linear_range[]) {        \
              REGULATOR_LINEAR_RANGE(_min_val, _min_sel, _max_sel, _step),    \
        },                            \
        .n_linear_ranges = 1,                    \
        .vsel_reg =       SGM38121_LDO ## _num ## VOUT,    \
        .vsel_mask =       0xff,                \
        .enable_reg =       SGM38121_ENABLE,            \
...
#define SGM38121_ALDO(_num, _supply, _min_val, _min_sel, _max_sel, _step)                \
    [SGM38121_LDO ## _num] = {                    \
        .name =           "ONLDO"#_num,                \
        .of_match =       of_match_ptr("ONLDO"#_num),        \
        .regulators_node = of_match_ptr("regulators"),        \
...
static struct regulator_desc multi_reg_sgm38121_regulators[] = {
    SGM38121_DLDO(1, "vin1", 528000, 0x03, 0x7D, 8000),
    SGM38121_DLDO(2, "vin1", 528000, 0x03, 0x7D, 8000),
    SGM38121_ALDO(3, "vin2", 1504000, 0x0F, 0xFF, 8000),
    SGM38121_ALDO(4, "vin2", 1504000, 0x0F, 0xFF, 8000),
};
typedef struct {
...
static mult_reg_dev_info_t dev_info[] = {
    ...
    {.mult_reg_slave_id = 0x28,
     .product_id_reg = SGM38121_PRODUCT_ID,
     .product_id = 0x80,
     .reg_desc = &multi_reg_sgm38121_regulators[0],
     .reg_desc_size = ARRAY_SIZE(multi_reg_sgm38121_regulators),
    },
    /*Ivalid*/
    {0xff},
};
...
static int multi_reg_i2c_probe(struct i2c_client *i2c)
{
    ...
    /*process the fan53870 wl2868 sgm38121 IC*/
    ...
    while(dev_info[index].mult_reg_slave_id != 0xFF){
        ...
        if (data == PRODUCT_ID2){
            ret = regmap_write(pdata->regmap, SGM38121_DISCR, 0x0F);
            ret = regmap_read(pdata->regmap, SGM38121_DISCR, &RegData);
            pr_err("%s WL2866 discharge enable, addr 0x%08x = 0x%08x ret = %d\n",
                __func__,
                SGM38121_DISCR, RegData, ret);
        }
...
MODULE_DESCRIPTION("SGM38121 PMIC voltage regulator driver");
MODULE_AUTHOR("XXX");
MODULE_LICENSE("GPL");
