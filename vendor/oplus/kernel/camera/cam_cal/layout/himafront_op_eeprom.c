...
struct himalayanfront_txd_front_otp_struct {
	...
	unsigned char module_info[HIMALAYANFRONT_TXD_FRONT_OTP_MODULE_LENS];
	unsigned char wb_data[HIMALAYANFRONT_TXD_FRONT_OTP_AWB_LENS];
	unsigned char lrc_data[HIMALAYANFRONT_TXD_FRONT_OTP_LRC_LENS];
	unsigned char sn_data[HIMALAYANFRONT_TXD_FRONT_OTP_SN_LENS];
	unsigned char lsc_data[HIMALAYANFRONT_TXD_FRONT_OTP_LSC_LENS];
};

struct himalayanfront_txd_front_otp_struct himalayanfront_txd_front_otp = {
	.ModuleFlag = 0
};
EXPORT_SYMBOL(himalayanfront_txd_front_otp);

int himafront_i2c_rd_u8(struct i2c_client *i2c_client,
...
int himafront_i2c_wr_u8(struct i2c_client *i2c_client,
		u16 addr, u16 reg, u8 val)
{
	int ret;
	u8 buf[3];
...
({ \
	u8 __val = 0xff; \
	himafront_i2c_rd_u8(i2c_client, \
		i2c_client->addr >> 1, reg, &__val); \
	__val; \
})

#define subdrv_i2c_wr_u8(i2c_client, reg, val) \
	himafront_i2c_wr_u8(i2c_client, \
		i2c_client->addr >> 1, reg, val)

static unsigned int do_part_number_himafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_single_lsc_himafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_himafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_himafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int layout_check_himafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int sensorID, unsigned int *_cfg);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x30440320, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number_himafront},
		{0x00000001, 0x00000530, 0x0000074C, do_single_lsc_himafront},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_himafront}, //Start address, block size is useless
		...
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_himafront}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT himafront_op_eeprom = {
	.name = "himafront_op_eeprom",
	.check_layout_function = layout_check_himafront,
	.read_function = Common_read_region,
	.layout = &cal_layout_table,
	.sensor_id = HIMALAYANFRONT_SENSOR_ID,
	.i2c_write_id = OTP_I2C_ADDR,
	.max_size = 0x1FFF,
	.enable_preload = 1,
	.preload_size = 0x1FFF,
};
static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &himafront_op_eeprom;
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	memcpy(&pCamCalData->PartNumber[0], &himalayanfront_txd_front_otp.module_info[0], block_size);
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (table_size > 0) {
		...
		memcpy(&pCamCalData->SingleLsc.LscTable.MtkLcsData.SlimLscType, &himalayanfront_txd_front_otp.lsc_data[0], block_size);
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		memcpy(&CalValue, &himalayanfront_txd_front_otp.lrc_data[0], 4);
		// awb_offset = 0x60;
		...
		/* AWB Unit Gain (5000K) */
		debug_log("5000K AWB\n");
		memcpy(&CalGain, &himalayanfront_txd_front_otp.wb_data[0], 8);
		// awb_offset = 0x10;
		// read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		memcpy(&FacGain, &himalayanfront_txd_front_otp.wb_data[8], 8);
		// awb_offset = 0x18;
		// read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
		...
		rgCalValue  = CalValue & 0xFFFF;
		bgCalValue = (CalValue >> 16) & 0xFFFF;
		debug_log("Light source calibration value 4000K R/G:%d, B/G:%d",rgCalValue, bgCalValue);
		err = CAM_CAL_ERR_NO_ERR;
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	memcpy(&pCamCalData->LensDrvId[0], &himalayanfront_txd_front_otp.module_info[8], 2);
	return CAM_CAL_ERR_NO_ERR;
}

static int himalayanfront_checksum(struct i2c_client *client, u16 checkAddr, unsigned char *checkArray, int length)
{
	...
	if (sum % 255 != checksum)
		return HIMALAYANFRONT_OTP_RET_FAIL;
	return HIMALAYANFRONT_OTP_RET_SUCCESS;
}

static void himalayanfront_read(struct i2c_client *client, unsigned int start, unsigned int end, unsigned char *pinputdata)
{
	u16 addr = start;
...
int himalayanfront_set_threshold(struct i2c_client *client, u8 threshold) //set thereshold
{
	...
	if (threshold < 3 && threshold >= 0) {
		...
		pr_info("himalayanfront_otp set_threshold %d\n", threshold);
	} else {
		pr_err("himalayanfront_otp set invalid threshold %d\n", threshold);
...
int himalayanfront_set_page_and_load_data(struct i2c_client *client, int page) //set page
{
	u16 Startaddress = 0;
	u16 EndAddress = 0;
	int delay = 0;
	...
	while ((subdrv_i2c_rd_u8(client, 0x4420) & 0x01) == 0x01) {
		delay++;
		pr_info("himalayanfront_otp set_page waitting, OTP is still busy for loading %d times\n", delay);
		if (delay == 10) {
			pr_err("himalayanfront_otp set_page fail, load timeout!!!\n");

			return HIMALAYANFRONT_OTP_RET_FAIL;
		}
		mdelay(10);
	}
	pr_info("himalayanfront_otp set_page success\n");

	return HIMALAYANFRONT_OTP_RET_SUCCESS;
}

static int himalayanfront_sensor_otp_read_data(struct i2c_client *client, u16 ui4_offset,
...
static int himalayanfront_sensor_otp_read_module_info(struct i2c_client *client, unsigned char *pinputdata)
{
	int ret = HIMALAYANFRONT_OTP_RET_FAIL;

	himalayanfront_txd_front_otp.ModuleFlag = subdrv_i2c_rd_u8(client, HIMALAYANFRONT_OTP_MODULE_FLAGADDR);
	pr_info("himalayanfront_otp Read ModuleFlag addr :0x%x, data:0x%x\n",
			HIMALAYANFRONT_OTP_MODULE_FLAGADDR,
			himalayanfront_txd_front_otp.ModuleFlag);

	if (himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP1_FLAG) {
		ret = himalayanfront_sensor_otp_read_data(client,
			HIMALAYANFRONT_OTP_MODULE_GROUP1_STARTADDR,
			HIMALAYANFRONT_TXD_FRONT_OTP_MODULE_LENS, pinputdata);
		pr_info("himalayanfront_otp group1 ret = %d!\n", ret);
	} else if (himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP2_FLAG) {
		ret = himalayanfront_sensor_otp_read_data(client,
			HIMALAYANFRONT_OTP_MODULE_GROUP2_STARTADDR,
			HIMALAYANFRONT_TXD_FRONT_OTP_MODULE_LENS, pinputdata);
		pr_info("himalayanfront_otp group2 ret = %d!\n", ret);
	} else {
		pr_err("himalayanfront_otp invalid flag :0x%x\n",
				himalayanfront_txd_front_otp.ModuleFlag);
	}
...
static int himalayanfront_sensor_otp_read_awb_info(struct i2c_client *client, unsigned char *pinputdata)
{
	int ret = HIMALAYANFRONT_OTP_RET_FAIL;

	if(himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP1_FLAG){
		ret = himalayanfront_sensor_otp_read_data(client,
			HIMALAYANFRONT_OTP_AWB_GROUP1_STARTADDR,
			HIMALAYANFRONT_TXD_FRONT_OTP_AWB_LENS, pinputdata);
		pr_info("himalayanfront_otp group1 ret = %d!\n", ret);
	} else if(himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP2_FLAG) {
		ret = himalayanfront_sensor_otp_read_data(client,
			HIMALAYANFRONT_OTP_AWB_GROUP2_STARTADDR,
			HIMALAYANFRONT_TXD_FRONT_OTP_AWB_LENS, pinputdata);
		pr_info("himalayanfront_otp group2 ret = %d!\n", ret);
	} else
		pr_err("himalayanfront_otp invalid!\n");

	return ret;
}

static int himalayanfront_sensor_otp_read_lrc_info(struct i2c_client *client, unsigned char *pinputdata)
{
	int ret = HIMALAYANFRONT_OTP_RET_FAIL;

	if(himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP1_FLAG){
		ret = himalayanfront_sensor_otp_read_data(client,
			HIMALAYANFRONT_OTP_LRC_GROUP1_STARTADDR,
			HIMALAYANFRONT_TXD_FRONT_OTP_LRC_LENS, pinputdata);
		pr_info("himalayanfront_otp group1 ret = %d!\n", ret);
	} else if(himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP2_FLAG) {
		ret = himalayanfront_sensor_otp_read_data(client,
			HIMALAYANFRONT_OTP_LRC_GROUP2_STARTADDR,
			HIMALAYANFRONT_TXD_FRONT_OTP_LRC_LENS, pinputdata);
		pr_info("himalayanfront_otp group2 ret = %d!\n", ret);
	} else
		pr_err("himalayanfront_otp invalid!\n");
...
static int himalayanfront_sensor_otp_read_lsc_info(struct i2c_client *client, unsigned char *pinputdata)
{
	int ret = HIMALAYANFRONT_OTP_RET_FAIL;

	if (himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP1_FLAG) {
		// lsc 1:
		himalayanfront_set_page_and_load_data(client, page_2);
		himalayanfront_read(client, 0x8303, 0x83FF, &himalayanfront_txd_front_otp.lsc_data[0]);

		// lsc 2:
		himalayanfront_set_page_and_load_data(client, page_3);
		himalayanfront_read(client, 0x847A, 0x85FF, &himalayanfront_txd_front_otp.lsc_data[0xFD]);

		// lsc 3:
		himalayanfront_set_page_and_load_data(client, page_4);
		himalayanfront_read(client, 0x867A, 0x87FF, &himalayanfront_txd_front_otp.lsc_data[0x283]);

		// lsc 4:
		himalayanfront_set_page_and_load_data(client, page_5);
		himalayanfront_read(client, 0x887A, 0x89FF, &himalayanfront_txd_front_otp.lsc_data[0x409]);

		// lsc 5:
		himalayanfront_set_page_and_load_data(client, page_6);
		himalayanfront_read(client, 0x8A7A, 0x8BFF, &himalayanfront_txd_front_otp.lsc_data[0x58F]);

		// lsc 6:
		himalayanfront_set_page_and_load_data(client, page_7);
		himalayanfront_read(client, 0x8C7A, 0x8CB0, &himalayanfront_txd_front_otp.lsc_data[0x715]);

		// checksum
		ret = himalayanfront_checksum(client, 0x8CB2, &himalayanfront_txd_front_otp.lsc_data[0], 1868);
	} else if (himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP2_FLAG) {
		// lsc 1:
		himalayanfront_set_page_and_load_data(client, page_7);
		himalayanfront_read(client, 0x8CB3, 0x8DFF, &himalayanfront_txd_front_otp.lsc_data[0]);

		// lsc 2:
		himalayanfront_set_page_and_load_data(client, page_8);
		himalayanfront_read(client, 0x8E7A, 0x8FFF, &himalayanfront_txd_front_otp.lsc_data[0x14D]);

		// lsc 3:
		himalayanfront_set_page_and_load_data(client, page_9);
		himalayanfront_read(client, 0x907A, 0x91FF, &himalayanfront_txd_front_otp.lsc_data[0x2D3]);

		// lsc 4:
		himalayanfront_set_page_and_load_data(client, page_10);
		himalayanfront_read(client, 0x927A, 0x93FF, &himalayanfront_txd_front_otp.lsc_data[0x459]);

		// lsc 5:
		himalayanfront_set_page_and_load_data(client, page_11);
		himalayanfront_read(client, 0x947A, 0x95E8, &himalayanfront_txd_front_otp.lsc_data[0x5DF]);

		// checksum
		ret = himalayanfront_checksum(client, 0x95E8, &himalayanfront_txd_front_otp.lsc_data[0], 1868);
	} else {
...
static int himalayanfront_sensor_otp_read_sn_info(struct i2c_client *client, unsigned char *pinputdata)
{
	int ret = HIMALAYANFRONT_OTP_RET_FAIL;

	if (himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP1_FLAG) {
		ret = himalayanfront_sensor_otp_read_data(client,
			HIMALAYANFRONT_OTP_SN_GROUP1_STARTADDR,
			HIMALAYANFRONT_TXD_FRONT_OTP_SN_LENS, pinputdata);
		pr_info("himalayanfront_otp group1 ret = %d!\n", ret);
	} else if (himalayanfront_txd_front_otp.ModuleFlag == HIMALAYANFRONT_GROUP2_FLAG) {
		ret = himalayanfront_sensor_otp_read_data(client,
			HIMALAYANFRONT_OTP_SN_GROUP2_STARTADDR,
			HIMALAYANFRONT_TXD_FRONT_OTP_SN_LENS, pinputdata);
		pr_info("himalayanfront_otp group2 ret = %d!\n", ret);
	} else
		pr_err("himalayanfront_otp sn_info invalid!\n");

	return ret;
}

static int himalayanfront_sensor_otp_read_by_group(struct i2c_client *client)
{
	int threshold = 0;
	int ret = HIMALAYANFRONT_OTP_RET_FAIL;

	for (threshold = 0; threshold < 3; threshold++) {
		himalayanfront_set_threshold(client, threshold);
		himalayanfront_set_page_and_load_data(client, page_2);
		ret = himalayanfront_sensor_otp_read_module_info(client,
			himalayanfront_txd_front_otp.module_info);
		if (ret == HIMALAYANFRONT_OTP_RET_FAIL) {
			himalayanfront_txd_front_otp.ModuleFlag = HIMALAYANFRONT_INVALID_FLAG;
			pr_err("himalayanfront_otp read module info in threshold R%d fail\n", threshold);
			continue;
		}

		ret = himalayanfront_sensor_otp_read_awb_info(client,
			himalayanfront_txd_front_otp.wb_data);
		if (ret == HIMALAYANFRONT_OTP_RET_FAIL) {
			pr_err("himalayanfront_otp read awb info in threshold R%d fail\n", threshold);
			continue;
		}

		ret = himalayanfront_sensor_otp_read_lrc_info(client,
			himalayanfront_txd_front_otp.lrc_data);
		if (ret == HIMALAYANFRONT_OTP_RET_FAIL) {
			pr_err("himalayanfront_otp read light source in threshold R%d fail\n", threshold);
			continue;
		}

		ret = himalayanfront_sensor_otp_read_sn_info(client,
			himalayanfront_txd_front_otp.sn_data);
		if (ret == HIMALAYANFRONT_OTP_RET_FAIL) {
			pr_err("himalayanfront_otp read sn_data in threshold R%d fail\n", threshold);
			continue;
		}

		ret = himalayanfront_sensor_otp_read_lsc_info(client,
			himalayanfront_txd_front_otp.lsc_data);
		if (ret == HIMALAYANFRONT_OTP_RET_FAIL) {
			pr_err("himalayanfront_otp read lsc info in threshold R%d fail\n", threshold);
			continue;
		}

		pr_info("himalayanfront_otp read all otp data in threshold R%d success\n", threshold);
		break;
	}
	if (ret == HIMALAYANFRONT_OTP_RET_FAIL) {
		pr_err("himalayanfront_otp read otp data  in threshold R1 R2 R3 all failed!\n");
	}

	return ret;
}

int himalayanfront_sensor_otp_read_all_data(struct i2c_client *client)
{
	int ret = HIMALAYANFRONT_OTP_RET_FAIL;
	int delay = 0;
	...
	ret = himalayanfront_sensor_otp_read_by_group(client);
	if (ret == HIMALAYANFRONT_OTP_RET_FAIL) {
		pr_err("himalayanfront_otp read lsc info in threshold R1 R2 R3 all failed!!!\n");

		return ret;
	}
	pr_info("himalayanfront_otp read otp data success\n");
	...
	while ((subdrv_i2c_rd_u8(client, 0x4420) & 0x01) == 0x01) {
		delay++;
		pr_info("himalayanfront_otp read otp is waitting, OTP is still busy for loading %d times\n", delay);
		if (delay == 10) {
			pr_err("himalayanfront_otp read otp data fail, load timeout!\n");
...
EXPORT_SYMBOL(himalayanfront_sensor_otp_read_all_data);

static unsigned int layout_check_himafront(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID, unsigned int *_cfg)
{
	...
	if (himalayanfront_txd_front_otp.ModuleFlag == 0) {
		pr_info("read sensor otp!\n");
		himalayanfront_sensor_otp_read_all_data(client);
	}

	memcpy(&check_id, &himalayanfront_txd_front_otp.module_info[header_offset], 4);
