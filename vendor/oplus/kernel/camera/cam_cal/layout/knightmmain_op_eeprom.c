...
#define PFX "CAM_CAL_KNIGHTMMAIN"
...
static unsigned int do_single_lsc_knightmmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_knightmmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_knightmmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_knightmmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01A30115, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00000C00, 0x0000074C, do_single_lsc_knightmmain},
		{0x00000001, 0x00000020, 0x0000000E, do_2a_gain_knightmmain},  //Start address, block size is useless
		{0x00000001, 0x00001400, 0x00000A3A, do_pdaf_knightmmain},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000001, 0x00000000, 0x00004000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_knightmmain}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT knightmmain_op_eeprom = {
	.name = "knightmmain_op_eeprom",
...
#define KNIGHTMMAIN_HVBIN_PDAF_PROC1_SIZE  (496)
#define KNIGHTMMAIN_HVBIN_PDAF_PROC2_SIZE  (1004)
#define KNIGHTMMAIN_HVBIN_PDAF_PROC1_ADDR  (0x1400)
#define KNIGHTMMAIN_HVBIN_PDAF_PROC2_ADDR  (0x1600)

unsigned int do_pdaf_knightmmain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = KNIGHTMMAIN_HVBIN_PDAF_PROC1_SIZE + KNIGHTMMAIN_HVBIN_PDAF_PROC2_SIZE;
...
//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			KNIGHTMMAIN_HVBIN_PDAF_PROC1_ADDR + KNIGHTMMAIN_HVBIN_PDAF_PROC1_SIZE,
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			KNIGHTMMAIN_HVBIN_PDAF_PROC2_ADDR + KNIGHTMMAIN_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		if (read_data_size > 0)	{
			...
			rgCalValue  = CalValue & 0xFFFF;
			bgCalValue = (CalValue >> 16) & 0xFFFF;
			debug_log("Light source calibration value 2850 R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read CalGain Failed\n");
		...
		pr_debug_if(dump_enable, "2850K AWB\n");
		awb_offset = 0x44;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 8, (unsigned char *)&CalGain);
		if (read_data_size > 0)	{
		...
		#ifdef DEBUG_CALIBRATION_LOAD
		pr_debug("AWB Calibration @2850K\n");
		pr_debug("[CalGain] = 0x%llx\n", CalGain);
		pr_debug("[FacGain] = 0x%llx\n", FacGain);
		pr_debug("[rCalGain.u4R] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4R_low);
		pr_debug("[rCalGain.u4G] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4G_low);
