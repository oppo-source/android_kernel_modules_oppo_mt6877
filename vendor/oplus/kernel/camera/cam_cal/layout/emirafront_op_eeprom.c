...
static unsigned int do_single_lsc_emirafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_emirafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_emirafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_emirafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

//static unsigned int layout_check_emirafront(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID, unsigned int *_cfg);
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01c40055, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, 0x00000530, 0x0000074C, do_single_lsc_emirafront},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_emirafront}, //Start address, block size is useless
		{0x00000001, 0x00000000, 0x00000000, do_pdaf_emirafront},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00002000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_emirafront}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT emirafront_op_eeprom = {
	.name = "emirafront_op_eeprom",
//	.check_layout_function = layout_check_emirafront,
...
//static struct STRUCT_CAM_CAL_CONFIG_STRUCT *cam_cal_config = &emirafront_op_eeprom;

/*
unsigned int do_pdaf_emirafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
...
#if 0
#define EMIRAFRONT_HVBIN_PDAF_PROC1_SIZE  (0x1BA0-0x17D0) //976      0x3D0
#define EMIRAFRONT_HVBIN_PDAF_PROC2_SIZE  (0x2208-0x1BB0) //1624     2600  0xA28
#define EMIRAFRONT_HVBIN_PDAF_PROC1_ADDR  (0x17D0)
#define EMIRAFRONT_HVBIN_PDAF_PROC2_ADDR  (0x1BB0)

//partial PD
#define EMIRAFRONT_PARTIAL_PD_PROC1_SIZE  (0x3590-0x33A0) //496       0X1F0
#define EMIRAFRONT_PARTIAL_PD_PROC2_SIZE  (0x398C-0x35A0) //1004      1500   0x5DC
#define EMIRAFRONT_PARTIAL_PD_PROC1_ADDR  (0x33A0)
#define EMIRAFRONT_PARTIAL_PD_PROC2_ADDR  (0x35A0)
unsigned int do_pdaf_emirafront(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	return err;
	...
	debug_log("======================PDAF Data==================\n");
	debug_log("[%s] QPD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, EMIRAFRONT_HVBIN_PDAF_PROC1_ADDR, EMIRAFRONT_HVBIN_PDAF_PROC1_SIZE);
	debug_log("[%s] QPD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, EMIRAFRONT_HVBIN_PDAF_PROC2_ADDR, EMIRAFRONT_HVBIN_PDAF_PROC2_SIZE);
	debug_log("[%s] partial PD proc1 start_addr =%x table_size=%d\n", __FUNCTION__, EMIRAFRONT_PARTIAL_PD_PROC1_ADDR, EMIRAFRONT_PARTIAL_PD_PROC1_SIZE);
	debug_log("[%s] partial PD proc2 start_addr =%x table_size=%d\n", __FUNCTION__, EMIRAFRONT_PARTIAL_PD_PROC2_ADDR, EMIRAFRONT_PARTIAL_PD_PROC2_SIZE);
...
// partial PD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			EMIRAFRONT_PARTIAL_PD_PROC1_ADDR + EMIRAFRONT_PARTIAL_PD_PROC1_SIZE,
	...
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			...
			bios += EMIRAFRONT_PARTIAL_PD_PROC1_SIZE;
	...
	// partial PD proc2
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				EMIRAFRONT_PARTIAL_PD_PROC2_ADDR + EMIRAFRONT_PARTIAL_PD_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
			...
			if (read_data_size > 0) {
				debug_log("[%s] partial PD proc2 First five %x, %x, %x, %x, %x\n",
					...
					pCamCalData->PDAF.Data[bios + 3],
					pCamCalData->PDAF.Data[bios + 4]);
				bios += EMIRAFRONT_PARTIAL_PD_PROC2_SIZE;
...
// QPD proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			EMIRAFRONT_HVBIN_PDAF_PROC1_ADDR + EMIRAFRONT_HVBIN_PDAF_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc1 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += EMIRAFRONT_HVBIN_PDAF_PROC1_SIZE;
			qpd_proc1_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
...
// QPD proc2
	if(qpd_proc1_flag) {
		isValid = 0;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				EMIRAFRONT_HVBIN_PDAF_PROC2_ADDR + EMIRAFRONT_HVBIN_PDAF_PROC2_SIZE,
				1, (unsigned char *)&isValid);
		if(isValid != 1) {
		...
		if (read_data_size > 0) {
			debug_log("[%s] QDP proc2 First five %x, %x, %x, %x, %x\n",
				...
				pCamCalData->PDAF.Data[bios + 3],
				pCamCalData->PDAF.Data[bios + 4]);
			bios += EMIRAFRONT_HVBIN_PDAF_PROC2_SIZE;
			qpd_proc2_flag = true;
			debug_log("[%s] bios = %d  0x%x\n",__FUNCTION__, bios, bios);
		}
	}
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		if (read_data_size > 0)	{
			...
			printk("emiraLight source calibration 5100K value R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
		...
		if (read_data_size > 0)	{
			...
			printk("emira5000k UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d",
					CalR, CalG, CalB, tempMax);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read CalGain Failed\n");
			printk("emira5000k fail UnitR:%d, UnitG:%d, UnitB:%d, New Unit Max=%d",
					CalR, CalG, CalB, tempMax);
			show_cmd_error_log(pCamCalData->Command);
		}
		...
		if (read_data_size > 0)	{
			...
			printk("emiraGoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d",
					FacR, FacG, FacB, tempMax);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read FacGain Failed\n");
			printk("emirafail GoldenR:%d, GoldenG:%d, GoldenB:%d, New Golden Max=%d",
					FacR, FacG, FacB, tempMax);
			show_cmd_error_log(pCamCalData->Command);
		}
		...
		if (read_data_size > 0)	{
			...
			printk("emiraLight source calibration value 3100 R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read CalGain Failed\n");
			printk("emirafail Light source calibration value 3100 R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			show_cmd_error_log(pCamCalData->Command);
		}
		/* AWB Unit Gain (3100K) */
		awb_offset = 0x0034;
...
static unsigned int layout_check_emirafront(struct EEPROM_DRV_FD_DATA *pdata, unsigned int sensorID, unsigned int *_cfg)
{
	...
	if (check_id == 0x01c40055 || check_id == 0x01C3012B || check_id == 0x00510055) {	// hearder id on OTP guide
		...
		printk("emiraheader_id matched 0x%08x\n", check_id);
		result = CAM_CAL_ERR_NO_ERR;
	} else{
		debug_log("header_id not matched 0x%08x\n", check_id);
		printk("emiraheader_id not matched 0x%08x\n", check_id);
	}
	return result;
}*/
