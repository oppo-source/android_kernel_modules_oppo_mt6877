...
static unsigned int do_single_lsc_kkthtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_kkthtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_kkthtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_pdaf_kkthtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static int do_ois_kkthtele(struct EEPROM_DRV_FD_DATA *pdata, struct ois_cal_data *data);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000006, 0x01ae010d, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00000B00, 0x0000074C, do_single_lsc_kkthtele},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_kkthtele}, //Start address, block size is useless
		{0x00000001, 0x00001300, 0x00000A3A, do_pdaf_kkthtele},
		{0x00000000, 0x00000000, 0x00000000, do_stereo_data},
		{0x00000001, 0x00000000, 0x00008000, do_dump_all},
		{0x00000001, 0x00000008, 0x00000002, do_lens_id_kkthtele},
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT kkthtele_op_eeprom = {
	.name = "kkthtele_op_eeprom",
	...
	.preload_size = 0x8000,
	.has_stored_data = 1,
	.parsing_ois_cal_data_from_preload = do_ois_kkthtele,
};


#define KKTHTELE_HVBIN_PDAF_PROC1_SIZE  (496)
#define KKTHTELE_HVBIN_PDAF_PROC2_SIZE  (1004)
#define KKTHTELE_HVBIN_PDAF_PROC1_ADDR  (0x1300)
#define KKTHTELE_HVBIN_PDAF_PROC2_ADDR  (0x1500)

unsigned int do_pdaf_kkthtele(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	pCamCalData->PDAF.Size_of_PDAF = KKTHTELE_HVBIN_PDAF_PROC1_SIZE + KKTHTELE_HVBIN_PDAF_PROC2_SIZE;
	error_log("PDAF start_addr =%x table_size=%d\n", start_addr, block_size);

//proc1
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			KKTHTELE_HVBIN_PDAF_PROC1_ADDR + KKTHTELE_HVBIN_PDAF_PROC1_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		error_log("[%s]  proc1 unvalid\n", __FUNCTION__);
...
//proc2
	isValid = 0;
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			KKTHTELE_HVBIN_PDAF_PROC2_ADDR + KKTHTELE_HVBIN_PDAF_PROC2_SIZE,
			1, (unsigned char *)&isValid);
	if(isValid != 1) {
		error_log("[%s]  proc2 unvalid\n", __FUNCTION__);
		err =  CamCalReturnErr[pCamCalData->Command];
		return err;
	}
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			KKTHTELE_HVBIN_PDAF_PROC2_ADDR, KKTHTELE_HVBIN_PDAF_PROC2_SIZE,
			(unsigned char *)&pCamCalData->PDAF.Data[KKTHTELE_HVBIN_PDAF_PROC1_SIZE]);
	if (read_data_size > 0)
		err = CAM_CAL_ERR_NO_ERR;
...
*/
	...
	if (0x1 & AWBAFConfig) {
		...
		if (read_data_size > 0)	{
			...
			error_log("Light source calibration 5100K value R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
		...
		#ifdef DEBUG_CALIBRATION_LOAD
		error_log("AWB Calibration @4000K\n");
		error_log("[CalGain] = 0x%llx\n", CalGain);
		error_log("[FacGain] = 0x%llx\n", FacGain);
		error_log("[rCalGain.u4R] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4R_mid);
		error_log("[rCalGain.u4G] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4G_mid);
		...
		if (read_data_size > 0)	{
			...
			rgCalValue  = CalValue & 0xFFFF;
			bgCalValue = (CalValue >> 16) & 0xFFFF;
			error_log("Light source calibration value 3100 R/G:%d, B/G:%d",rgCalValue, bgCalValue);
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
			error_log("Read CalGain Failed\n");
		...
		#ifdef DEBUG_CALIBRATION_LOAD
		error_log("AWB Calibration @3100K\n");
		error_log("[CalGain] = 0x%llx\n", CalGain);
		error_log("[FacGain] = 0x%llx\n", FacGain);
		error_log("[rCalGain.u4R] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4R_low);
		error_log("[rCalGain.u4G] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4G_low);
	...
	if (0x2 & AWBAFConfig) {
		...
		////Only AF Gathering <////
		#ifdef DEBUG_CALIBRATION_LOAD
		printk("======================#kkthtele#AF CAM_CAL==================\n");
		printk("#kkthtele#[AFInf] = %d\n", AFInf);
		printk("#kkthtele#[AFMacro] = %d\n", AFMacro);
		printk("#kkthtele#[AF_50cm] = %d\n", AF_50cm);
		printk("======================#kkthtele#AF CAM_CAL==================\n");
		#endif
	}
	return err;
...
static int do_ois_kkthtele(struct EEPROM_DRV_FD_DATA *pdata, struct ois_cal_data *data)
{
	...
	offset = 0x1960;
	length = 2341;
	memcpy(data, pdata->pdrv->config_info.mp_eeprom_preload + offset, length);
	return 0;
}
