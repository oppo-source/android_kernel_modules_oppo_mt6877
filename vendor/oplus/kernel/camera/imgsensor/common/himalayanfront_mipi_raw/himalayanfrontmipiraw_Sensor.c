 ...
 * Filename:
 * ---------
 *	 himalayanfrontmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
 ...
 *============================================================================
 ****************************************************************************/
#include "himalayanfrontmipiraw_Sensor.h"

#define PFX "himalayanfront_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)
#define MODULEINFO_SIZE	17
#define QRCODE_SIZE		24
#define HIMALAYANFRONT_EEPROM_READ_ID       0xA1
#define HIMALAYANFRONT_EEPROM_WRITE_ID      0xA0
#define OPLUS_CAMERA_COMMON_DATA_LENGTH     40
#define HIMALAYANFRONT_SENSOR_GAIN_BASE                1024
#define HIMALAYANFRONT_SENSOR_GAIN_MAX                 32768 /* (32 * HIMALAYANFRONT_SENSOR_GAIN_BASE) */
#define HIMALAYANFRONT_SENSOR_GAIN_MAX_VALID_INDEX     6

static u16 get_gain2reg(u32 gain);
static void himalayanfront_sensor_init(struct subdrv_ctx *ctx);
static int  himalayanfront_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int himalayanfront_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int  himalayanfront_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int himalayanfront_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int himalayanfront_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int vsync_notify(struct subdrv_ctx *ctx,	unsigned int sof_cnt);
static int himalayanfront_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int himalayanfront_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);

/* STRUCT */

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, himalayanfront_set_test_pattern},
	{SENSOR_FEATURE_SET_ESHUTTER, himalayanfront_set_shutter},
	{SENSOR_FEATURE_SET_GAIN, himalayanfront_set_gain},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, himalayanfront_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, himalayanfront_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, himalayanfront_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, himalayanfront_streaming_resume},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = himalayanfront_preview_setting,
		.mode_setting_len = ARRAY_SIZE(himalayanfront_preview_setting),
	...
	},
	{
		...
		.mode_setting_table = himalayanfront_capture_setting,
		.mode_setting_len = ARRAY_SIZE(himalayanfront_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = himalayanfront_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(himalayanfront_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = himalayanfront_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(himalayanfront_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = himalayanfront_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(himalayanfront_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = himalayanfront_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(himalayanfront_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = himalayanfront_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(himalayanfront_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = himalayanfront_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(himalayanfront_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	...
	.ana_gain_step = 32,
	.ana_gain_table = himalayanfront_ana_gain_table,
	.ana_gain_table_size = sizeof(himalayanfront_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	.init_setting_table = himalayanfront_init_setting,
	.init_setting_len = ARRAY_SIZE(himalayanfront_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry himalayanfront_mipi_raw_entry = {
	.name = "himalayanfront_mipi_raw",
	.id = HIMALAYANFRONT_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void himalayanfront_read_eeprom_data(struct subdrv_ctx *ctx)
{
	...
	memcpy(&oplus_eeprom_info.moduleInfo[0], &himalayanfront_txd_front_otp.module_info[0], MODULEINFO_SIZE);
	memcpy(&oplus_eeprom_info.qrcodeInfo[0], &himalayanfront_txd_front_otp.sn_data[0], QRCODE_SIZE);
}

static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	if (himalayanfront_txd_front_otp.ModuleFlag == 0) {
		himalayanfront_sensor_otp_read_all_data(ctx->i2c_client);
	}
	LOG_INF("ModuleFlag = 0x%02x\n", himalayanfront_txd_front_otp.ModuleFlag);
	himalayanfront_read_eeprom_data(ctx);
...
static int himalayanfront_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	memcpy(para, &himalayanfront_txd_front_otp, sizeof(himalayanfront_txd_front_otp));
	*len = sizeof(himalayanfront_txd_front_otp);
	return 0;
}
...
static int open(struct subdrv_ctx *ctx)
{
	...
	/* initail setting */
	himalayanfront_sensor_init(ctx);
...
static void himalayanfront_set_dummy(struct subdrv_ctx *ctx)
{
...
static void himalayanfront_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate, kal_bool min_framelength_en)
{
...
static int himalayanfront_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	uint32_t HIMALAYANFRONT_AGC_Param[HIMALAYANFRONT_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0x00 },
		{  2048,  0x08 },
		{  4096,  0x09 },
		{  8192,  0x0b },
	...
	rg_gain = gain;
	if (rg_gain < HIMALAYANFRONT_SENSOR_GAIN_BASE) {
        rg_gain = HIMALAYANFRONT_SENSOR_GAIN_BASE;
	} else if (rg_gain > HIMALAYANFRONT_SENSOR_GAIN_MAX) {
        rg_gain = HIMALAYANFRONT_SENSOR_GAIN_MAX;
	}
	...
	for (gain_index = HIMALAYANFRONT_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (rg_gain >= HIMALAYANFRONT_AGC_Param[gain_index][0])
			break;
	/* write gain */
	subdrv_i2c_wr_u8(ctx, 0x3e08, HIMALAYANFRONT_AGC_Param[gain_index][1]);
	temp_gain = rg_gain * HIMALAYANFRONT_SENSOR_GAIN_BASE / HIMALAYANFRONT_AGC_Param[gain_index][0];
	subdrv_i2c_wr_u8(ctx, 0x3e07, (temp_gain >> 3) & 0xff);
	DRV_LOG(ctx, "gain[0x%x], %x, %x\n", rg_gain, HIMALAYANFRONT_AGC_Param[gain_index][1], (temp_gain >> 3) & 0xff);
...
static int himalayanfront_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (ctx->autoflicker_en && ctx->line_length && ctx->frame_length) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305) {
			himalayanfront_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps >= 147 && realtime_fps <= 150) {
			himalayanfront_set_max_framerate(ctx, 146, 0);
		} else {
...
static int himalayanfront_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
		...
		if (*(u32 *)para)
			himalayanfront_set_shutter(ctx, para, len);
