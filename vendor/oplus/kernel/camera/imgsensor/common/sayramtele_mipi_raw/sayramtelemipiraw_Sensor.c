...
static void sayramtele_update_mode_info(struct subdrv_ctx *ctx, enum SENSOR_SCENARIO_ID_ENUM scenario_id)
{
	if (scenario_id >= ctx->s_ctx.sensor_mode_num) {
	...
	ctx->pclk = ctx->s_ctx.mode[scenario_id].pclk;
	ctx->line_length = ctx->s_ctx.mode[scenario_id].linelength;
	if (ctx->line_length == 0 || ctx->max_fps == 0) {
		ctx->frame_length = ctx->s_ctx.mode[scenario_id].framelength;
		DRV_LOGE(ctx, "line_length or max_fps error!!\n");
	} else {
		ctx->frame_length = ctx->pclk / ctx->line_length * 10 / ctx->max_fps;
	}
...
static int sayramtele_seamless_switch(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	sayramtele_update_mode_info(ctx, scenario_id);
