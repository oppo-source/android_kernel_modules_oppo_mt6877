 ...
 *============================================================================
 ****************************************************************************/
#include "emirafrontmipiraw_Sensor.h"
#include "emirafront_Sensor_setting.h"

#define PFX "emirafront_camera_sensor"
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)

#define EMIRAFRONT_EEPROM_READ_ID			0xA9
#define EMIRAFRONT_EEPROM_WRITE_ID			0xA8
#define EMIRAFRONT_MAX_OFFSET				0x4000
...
#define EMIRAFRONT_STEREO_START_ADDR 0
#define EMIRAFRONT_AESYNC_START_ADDR 0

static kal_uint8 otp_data_checksum[OTP_SIZE] = {0};
static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
static int emirafront_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_check_sensor_id(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_eeprom_common_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_set_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_get_otp_checksum_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_streaming_suspend(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int emirafront_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id);
static int open(struct subdrv_ctx *ctx);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void get_sensor_cali(void* arg);
static void set_sensor_cali(void *arg);
static int emirafront_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static int emirafront_get_eeprom_4cell_info(struct subdrv_ctx *ctx, u8 *para, u32 *len);

/* STRUCT */

static struct eeprom_map_info emirafront_eeprom_info[] = {
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, emirafront_set_test_pattern},
	{SENSOR_FEATURE_CHECK_SENSOR_ID, emirafront_check_sensor_id},
	{SENSOR_FEATURE_GET_EEPROM_COMDATA, get_eeprom_common_data},
	{SENSOR_FEATURE_SET_SENSOR_OTP, emirafront_set_eeprom_calibration},
	{SENSOR_FEATURE_GET_EEPROM_STEREODATA, emirafront_get_eeprom_calibration},
	{SENSOR_FEATURE_GET_SENSOR_OTP_ALL, emirafront_get_otp_checksum_data},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, emirafront_streaming_suspend},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, emirafront_streaming_resume},
	{SENSOR_FEATURE_SET_ESHUTTER, emirafront_set_shutter},
	{SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME, emirafront_set_shutter_frame_length},
	{SENSOR_FEATURE_SET_MULTI_SHUTTER_FRAME_TIME, emirafront_set_multi_shutter_frame_length_ctrl},
	{SENSOR_FEATURE_SET_GAIN, emirafront_set_gain},
	{SENSOR_FEATURE_GET_4CELL_DATA, emirafront_get_eeprom_4cell_info},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	{ // B1
		...
		.mode_setting_table = emirafront_preview_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_preview_setting),
	...
	{ // B1
		...
		.mode_setting_table = emirafront_capture_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_capture_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ // B2-3
		...
		.mode_setting_table = emirafront_normal_video_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_normal_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	{ // B3-4
		...
		.mode_setting_table = emirafront_hs_video_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_hs_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = emirafront_slim_video_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_slim_video_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = emirafront_custom1_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_custom1_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = emirafront_custom2_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_custom2_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = emirafront_custom3_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_custom3_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
	...
	},
	{
		...
		.mode_setting_table = emirafront_custom4_setting,
		.mode_setting_len = ARRAY_SIZE(emirafront_custom4_setting),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = EMIRAFRONT_SENSOR_ID,
	...
	.ana_gain_step = 1,
	.ana_gain_table = emirafront_ana_gain_table,
	.ana_gain_table_size = sizeof(emirafront_ana_gain_table),
	.tuning_iso_base = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 4,
	...
	.init_setting_table = emirafront_init_setting,
	.init_setting_len =  ARRAY_SIZE(emirafront_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry emirafront_mipi_raw_entry = {
	.name = "emirafront_mipi_raw",
	.id = EMIRAFRONT_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void emirafront_set_dummy(struct subdrv_ctx *ctx)
{
...
static void emirafront_set_max_framerate(struct subdrv_ctx *ctx, UINT16 framerate,
			kal_bool min_framelength_en)
{
	...
	emirafront_set_dummy(ctx);
}

static void emirafront_set_shutter_frame_length_convert(struct subdrv_ctx *ctx, u32 *shutter, u32 frame_length, bool auto_extend_en)
{
	...
	if (ctx->autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305) {
			emirafront_set_max_framerate(ctx, 296, 0);
		} else if (realtime_fps >= 147 && realtime_fps <= 150) {
			emirafront_set_max_framerate(ctx, 146, 0);
		} else {
			// Extend frame length
			subdrv_i2c_wr_u8(ctx, 0x0104, 0x01);
			subdrv_i2c_wr_u8(ctx, 0x0340, ctx->frame_length >> 8);
...
static int emirafront_set_shutter_frame_length(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	emirafront_set_shutter_frame_length_convert(ctx, (u32 *)para, (u32) (*(para + 1)), (u16) (*(para + 2)));
	return 0;
}

static void emirafront_write_shutter(struct subdrv_ctx *ctx)
{
	kal_uint16 realtime_fps = 0;
...
static void emirafront_set_shutter_convert(struct subdrv_ctx *ctx, u32 *shutter)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *shutter);
	ctx->exposure[0] = *shutter;

	emirafront_write_shutter(ctx);
}

static int emirafront_set_shutter(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	DRV_LOG(ctx, "set_shutter shutter =%d\n", *para);
	emirafront_set_shutter_convert(ctx, (u32 *)para);
	return 0;
}

static void emirafront_set_multi_shutter_frame_length(struct subdrv_ctx *ctx,
...
static int emirafront_set_multi_shutter_frame_length_ctrl(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	emirafront_set_multi_shutter_frame_length(ctx, (u32 *)(*feature_data),
...
static unsigned int read_emirafront_eeprom_info(struct subdrv_ctx *ctx, kal_uint16 meta_id,
	BYTE *data, int size)
{
	kal_uint16 addr;
	int readsize;

	if (meta_id != emirafront_eeprom_info[meta_id].meta)
		return -1;

	if (size != emirafront_eeprom_info[meta_id].size)
		return -1;

	addr = emirafront_eeprom_info[meta_id].start;
	readsize = emirafront_eeprom_info[meta_id].size;
...
static kal_uint16 read_cmos_eeprom_8(struct subdrv_ctx *ctx, kal_uint16 addr)
{
    ...
    adaptor_i2c_rd_u8(ctx->i2c_client, EMIRAFRONT_EEPROM_READ_ID >> 1, addr, (u8 *)&get_byte);
    return get_byte;
}
        ...
        kal_uint16 addr, kal_uint8 *para, kal_uint32 len)
{
    ...
    ret = adaptor_i2c_wr_p8(ctx->i2c_client, EMIRAFRONT_EEPROM_WRITE_ID >> 1,
            addr, para, len);
...
static kal_int32 write_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, EMIRAFRONT_EEPROM_WRITE_ID >> 1, reg, 0xA1);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, EMIRAFRONT_EEPROM_WRITE_ID >> 1, reg, 0xA0);
    }
    ...
    ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == EMIRAFRONT_SENSOR_ID) && (data_length == CALI_DATA_SLAVE_LENGTH)
            && (data_base == EMIRAFRONT_STEREO_START_ADDR)) {
        ...
        } else if ((pStereodata->uSensorId == EMIRAFRONT_SENSOR_ID) && (data_length < AESYNC_DATA_LENGTH_TOTAL)
            && (data_base == EMIRAFRONT_AESYNC_START_ADDR)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                read_cmos_eeprom_8(ctx, EMIRAFRONT_AESYNC_START_ADDR),
                read_cmos_eeprom_8(ctx, EMIRAFRONT_AESYNC_START_ADDR+1),
                read_cmos_eeprom_8(ctx, EMIRAFRONT_AESYNC_START_ADDR+2),
                read_cmos_eeprom_8(ctx, EMIRAFRONT_AESYNC_START_ADDR+3),
                read_cmos_eeprom_8(ctx, EMIRAFRONT_AESYNC_START_ADDR+4),
                read_cmos_eeprom_8(ctx, EMIRAFRONT_AESYNC_START_ADDR+5),
                read_cmos_eeprom_8(ctx, EMIRAFRONT_AESYNC_START_ADDR+6),
                read_cmos_eeprom_8(ctx, EMIRAFRONT_AESYNC_START_ADDR+7));
        ...
        LOG_INF("emirafront write_Module_data pStereodata is null\n");
        return -1;
    }
    return ret;
}
...
static int emirafront_get_eeprom_calibration(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	read_emirafront_eeprom_info(ctx, EEPROM_META_STEREO_DATA,
			(BYTE *)para, *len);
	return 0;
}
...
static bool read_cmos_eeprom_p8(struct subdrv_ctx *ctx, kal_uint16 addr,
                    BYTE *data, int size)
{
	if (adaptor_i2c_rd_p8(ctx->i2c_client, EMIRAFRONT_EEPROM_READ_ID >> 1,
			addr, data, size) < 0) {
		return false;
	}
	return true;
...
{
	DRV_LOGE(ctx, "emirafront read_otp_info begin\n");
	read_cmos_eeprom_p8(ctx, 0, otp_data_checksum, OTP_SIZE);
	DRV_LOGE(ctx, "emirafront read_otp_info end\n");
}
...
static int emirafront_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	...
	if (mode) {
		if (mode == 5) {
			subdrv_i2c_wr_u8(ctx, 0x0601, 0x0001); /*black*/
		} else {
...
int emirafront_set_gain(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u64* feature_data = (u64*)para;
