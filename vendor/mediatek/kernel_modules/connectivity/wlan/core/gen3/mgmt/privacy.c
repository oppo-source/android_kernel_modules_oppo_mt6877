...
#if CFG_SUPPORT_802_11W
UINT_8 secIsRobustMgmtFrame(IN P_ADAPTER_T prAdapter, IN void *prPacket)
{
...
UINT_8 secIsRobustActionFrame(IN P_ADAPTER_T prAdapter, IN void *prPacket)
{
	P_WLAN_MAC_HEADER_T prWlanHeader = NULL;
	...
	prActFrame = (P_WLAN_ACTION_FRAME)prWlanHeader;
	...
	return ucCategory == CATEGORY_SPEC_MGT ||
	       ...
	       ucCategory == CATEGORY_VENDOR_SPECIFIC_ACTION_PROTECTED
	       ? TRUE : FALSE;
}
#endif
...
{
	if (prMsdu->ucBssIndex > HW_BSSID_NUM)
		return FALSE;
...
BOOL secPrivacySeekForEntry(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prSta)
{
...
#if 1 /* Currently only using fix start index and max index */
	...
	if (ucMaxIDX != 0) {
#if 0 /* Currently only using fix start index and max index */
