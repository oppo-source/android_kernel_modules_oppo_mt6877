		      ...
		      OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	UINT_32 i, u4BssidListExLen, u4SavedApCount = 0;
	P_PARAM_BSSID_LIST_EX_T prList;
	P_PARAM_BSSID_EX_T prBssidEx;
	PUINT_8 cp;
	WLAN_INFO_T *prWlanInfo;
	PARAM_BSSID_EX_T *prScanResult;
	...
	*pu4QueryInfoLen = u4BssidListExLen;
	...
	if (prAdapter->fgIsRadioOff == FALSE && prWlanInfo->u4ScanResultNum > 0) {
		...
		for (i = 0; i < u4SavedApCount; i++) {
			...
			kalMemCopy(prBssidEx,
				   &(prScanResult[i]), OFFSET_OF(PARAM_BSSID_EX_T, pucIE));
	...
	*pu4SetInfoLen = u4SetBufferLen;
...

	{
		...
		if (prCmdKey->ucAlgorithmId == CIPHER_SUITE_BIP) {
			...
			DBGLOG_LIMITED(RSN, TRACE, "BIP BC wtbl index:%d\n",
				prCmdKey->ucWlanIndex);
		} else
#endif
		if (prCmdKey->ucTxKey) {
	...
	DBGLOG(RSN, TRACE, "Add key to wlanIdx %d,BSS=%d," MACSTR
