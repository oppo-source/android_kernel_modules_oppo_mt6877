		...
		bool no_cck, bool dont_wait_for_ack, int link_id, u64 *cookie)
{
	...
	do {
		...
		if (p2pFuncIsForceDevByFrameType(prGlueInfo->prAdapter,
					(struct WLAN_ACTION_FRAME *) buf)) {
			/* Device Interface. */
			DBGLOG(P2P, INFO, "Force to use dev\n");
			ucBssIdx = prGlueInfo->prAdapter->ucP2PDevBssIdx;
		} else if (mtk_Netdev_To_RoleIdx(prGlueInfo, dev,
						 &ucRoleIdx) < 0) {
			/* Device Interface. */
			ucBssIdx = prGlueInfo->prAdapter->ucP2PDevBssIdx;
		} else {
