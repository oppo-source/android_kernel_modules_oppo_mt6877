			    ...
			    struct MSDU_INFO *prMsduInfo)
{
	...
	if ((prBssDesc->eBand == BAND_5G) &&
		...
		if (IS_BSS_AIS(prBssInfo)) {
			...
			aisGetAisFsmInfo(prAdapter, prMsduInfo->ucBssIndex);

			if (prAisFsmInfo->ucAisIndex != AIS_DEFAULT_INDEX &&
			    prAisFsmInfo->fgIsSameBssConnected) {
				DBGLOG(RLM, INFO,
					"Don't add VHT IE in 2.4G when " MACSTR
					" is already connected by primary.\n",
					MAC2STR(prBssDesc->aucBSSID));
				return;
			}
		}
		DBGLOG(RLM, INFO, "Add VHT IE in 2.4G\n");
		rlmFillVhtCapIE(prAdapter, prBssInfo, prMsduInfo);
	}
#endif
}
