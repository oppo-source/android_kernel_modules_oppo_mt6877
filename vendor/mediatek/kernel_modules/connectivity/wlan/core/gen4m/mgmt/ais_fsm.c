...
static uint32_t aisCreateStaRecFromBssDesc(struct ADAPTER *prAdapter,
	struct AIS_FSM_INFO *prAisFsmInfo);
		...
		uint8_t ucAisIndex)
{
	...
	prAisFsmInfo->fgIsSameBssConnected = FALSE;
	...
	uint8_t ucLinkIndex)
{
	...
	prStaRec = aisGetLinkStaRec(prAisFsmInfo, ucLinkIndex);
	if (!prStaRec) {
		DBGLOG(AIS, ERROR,
...
uint8_t aisFillBssInfoFromBssDesc(struct ADAPTER *prAdapter,
	struct AIS_FSM_INFO *prAisFsmInfo,
...
uint32_t aisCreateStaRecFromBssDesc(struct ADAPTER *prAdapter,
	struct AIS_FSM_INFO *prAisFsmInfo)
{
	uint8_t i;
	struct BSS_INFO *prAisBssInfo;
	...
	for (i = 0; i < MLD_LINK_MAX; i++) {
		...
		aisSetLinkStaRec(prAisFsmInfo, prStaRec, i);
	}
	...
	enum ENUM_AIS_STATE eNextState, uint8_t ucBssIndex)
{
	...
	do {
		...
		case AIS_STATE_SEARCH: {
			...
			if (eNewState == AIS_STATE_REQ_CHANNEL_JOIN) {
				u4State = aisCreateStaRecFromBssDesc(
			 ...
			 struct MSG_HDR *prMsgHdr)
{
	...
	/* disallow same bssid connection when primary STA is CSA  */
	if (prAisFsmInfo->ucAisIndex != AIS_DEFAULT_INDEX) {
		uint8_t mainBssIdx = aisGetDefaultLinkBssIndex(prAdapter);
		struct AIS_FSM_INFO *mainAis;
		struct CONNECTION_SETTINGS *mainConn;
		struct PARAM_BSSID_EX *mainCurrBssid;

		mainAis = aisGetAisFsmInfo(prAdapter, mainBssIdx);
		mainConn = aisGetConnSettings(prAdapter, mainBssIdx);
		mainCurrBssid = aisGetCurrBssId(prAdapter, ucBssIndex);

		if (EQUAL_SSID(mainCurrBssid->rSsid.aucSsid,
				mainCurrBssid->rSsid.u4SsidLen,
				prConnSettings->aucSSID,
				prConnSettings->ucSSIDLen) &&
		    EQUAL_MAC_ADDR(mainCurrBssid->arMacAddress,
				prConnSettings->aucBSSID) &&
		    aisFsmIsSwitchChannel(prAdapter, mainAis)) {
			DBGLOG(AIS, INFO,
				"Disallow same bssid connection when primary STA is CSA\n");
			aisFsmSteps(prAdapter,
				AIS_STATE_JOIN_FAILURE, ucBssIndex);
			return;
		}
	}
