		...
		enum ENUM_MBMC_BN *eHwBandIdx)
{
	...
	if (!prAdapter || !prP2pRoleFsmInfo) {
		DBGLOG(P2P, ERROR,
		       "Null ptr prAdapter=%u, prP2pRoleFsmInfo=%u\n",
	...
	prChnlReqInfo = &(prP2pRoleFsmInfo->rChnlReqInfo[0]);
	/* ucChannelNum = 0 means fgIsOffChannel = 0, which indicates op ch */
	if (ucChannelNum &&
	    (!prChnlReqInfo->fgIsChannelRequested ||
	     prP2pGoBssInfo->ucPrimaryChannel != ucChannelNum)) {
		DBGLOG(P2P, LOUD,
		       "fgIsChannelRequested=%u, ucChannelNum=%u, ucPrimaryChannel=%u\n",
		       prChnlReqInfo->fgIsChannelRequested,
		       ucChannelNum,
		       prP2pGoBssInfo->ucPrimaryChannel);
		return FALSE;
	}
		...
		struct P2P_ROLE_FSM_INFO *prP2pRoleFsmInfo)
{
	...
	if (timerPendingTimer(
		...
		&prP2pRoleFsmInfo->rP2pRoleFsmCacTimer)) {
		DBGLOG(P2P, INFO,
				"p2pRoleFsmRunEventDfsShutDown: timer stop.\n");
	...
	if (ucTokenID == prChnlReqInfo->ucSeqNumOfChReq) {
		...
		case P2P_ROLE_STATE_DFS_CAC:
			...
			u4CacTimeMs = prP2pRoleFsmInfo->rChnlReqInfo[0]
				.u4MaxInterval;
#else
			u4CacTimeMs = SAP_CHNL_HOLD_TIME_MS;
#endif
