		...
		enum ENUM_P2P_DEV_STATE *peNextState)
{
	...
	do {
		...
		if (!prP2pDevFsmInfo) {
			DBGLOG(P2P, ERROR, "Null prP2pDevFsmInfo\n");
			break;
		}

		prP2pMgmtTxInfo = &(prP2pDevFsmInfo->rMgmtTxInfo);
		...
		if (prP2pDevFsmInfo->ucGoStartedBitmap == 0)
			/* Stay in IDLE state. */
			nicDeactivateNetwork(prAdapter,
				prAdapter->ucP2PDevBssIdx);
		else
			DBGLOG(P2P, INFO,
				"Keep P2P Device alive. GoStartedBitmap[0x%x]\n",
