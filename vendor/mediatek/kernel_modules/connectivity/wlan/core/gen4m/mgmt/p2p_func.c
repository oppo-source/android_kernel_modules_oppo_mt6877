		...
		u_int8_t fgNonCckRate)
{
	...
	do {
		...
		case MAC_FRAME_ACTION: {
			...
			if (p2pFuncIsDevUseDirectTx(prAdapter, prMgmtTxMsdu)) {
				prNewMgmtTxMsdu =
...
p2pFuncNeedForceSleep(struct ADAPTER *prAdapter)
{
	...
	if (!prP2pRoleFsmInfo)
		...
		(prP2pRoleFsmInfo->eCurrentState == P2P_ROLE_STATE_DFS_CAC))) {
...
u_int8_t p2pFuncIsForceDevByFrameType(struct ADAPTER *prAdapter,
				      struct WLAN_ACTION_FRAME *prActFrame)
{
	/* dynamic mode decide by supplicant */
	if (prAdapter->rWifiVar.ucRegP2pMode == RUNNING_P2P_DEV_MODE)
		return FALSE;

	if ((prActFrame->u2FrameCtrl & MASK_FRAME_TYPE) != MAC_FRAME_ACTION ||
	    prActFrame->ucCategory != WLAN_ACTION_PUBLIC)
		return FALSE;

	if (prActFrame->ucAction == WLAN_PA_GAS_INITIAL_REQ ||
	    prActFrame->ucAction == WLAN_PA_GAS_INITIAL_RESP)
		return TRUE;

	/* P2P Action Frame */
	if (*((uint8_t *)prActFrame + 26 + 3) == 0x09)
		return TRUE;

	return FALSE;
}

u_int8_t p2pFuncIsDevUseDirectTx(struct ADAPTER *prAdapter,
				 struct MSDU_INFO *prMgmtTxMsdu)
{
	struct WLAN_ACTION_FRAME *prActFrame;
	u_int8_t fgIsDirectTx = FALSE;

	prActFrame = (struct WLAN_ACTION_FRAME *)
		((uintptr_t) prMgmtTxMsdu->prPacket +
		MAC_TX_RESERVED_FIELD);
	...
	if (prActFrame->ucAction == WLAN_PA_GAS_INITIAL_REQ ||
		...
		fgIsDirectTx = TRUE;
	/* PROV */
	else if (*((uint8_t *)prActFrame + 26 + 3) == 0x09 &&
	    *((uint8_t *)prActFrame + 26 + 4) == P2P_PROV_DISC_RESP)
		fgIsDirectTx = TRUE;
