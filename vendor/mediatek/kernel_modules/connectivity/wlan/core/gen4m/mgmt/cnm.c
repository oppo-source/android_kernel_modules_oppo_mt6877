		       ...
		       uint8_t ucBssIndex)
{
	...
	if (IS_BSS_AIS(prBssInfo) && prBssDesc) {
		...
		prStaRec = aisGetMainLinkStaRec(prAisFsmInfo);

		if (prStaRec &&
			!(prStaRec->ucDesiredPhyTypeSet & PHY_TYPE_SET_80211ACAXBE)
			&& ucMaxBandwidth > MAX_BW_40MHZ) {
			DBGLOG(CNM, INFO,
				"Downgrade Bss[%d] bw from %u to %u, due to PhyType=0x%x\n",
				ucBssIndex, ucMaxBandwidth,
				MAX_BW_40MHZ, prStaRec->ucDesiredPhyTypeSet);
			ucMaxBandwidth = MAX_BW_40MHZ;
		}
	}
