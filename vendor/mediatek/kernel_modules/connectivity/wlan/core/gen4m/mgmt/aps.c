...
skip_rcpi_check:
	...
	scanExpiredTime = wlanWfdEnabled(prAdapter) ?
		SCN_BSS_DESC_STALE_SEC_WFD : prAdapter->rWifiVar.ucScanExpiredTime;

	if (CHECK_FOR_TIMEOUT(kalGetTimeTick(), prBssDesc->rUpdateTime,
		SEC_TO_SYSTIME(scanExpiredTime))) {
