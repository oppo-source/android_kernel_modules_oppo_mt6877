...
#define QUEUE_ENTRY_INITIALIZE(prEntry) \
	do { \
		if (!prEntry) { \
			DBGLOG(QM, ERROR, "NULL Queue Entry\n"); \
			break; \
		} \
		((struct QUE_ENTRY *)(prEntry))->prNext = NULL; \
		((struct QUE_ENTRY *)(prEntry))->prPrev = NULL; \
	} while (0)
...
#define QUEUE_IS_VALID_ENTRY(prEntry) \
	(prEntry && \
	 (((struct QUE_ENTRY *)(prEntry))->prNext || \
	  ((struct QUE_ENTRY *)(prEntry))->prPrev))
...
/*
 * Caller MUST ensure prEntry is valid and is in prQueue, or prEntry not valid.
 */
#define QUEUE_TRY_REMOVE_ENTRY(prQueue, prEntry) \
	do { \
		struct QUE *_prQueue = (struct QUE *)(prQueue); \
		struct QUE_ENTRY *_prEntry = (struct QUE_ENTRY *)(prEntry); \
		\
		if (!_prQueue) { \
			DBGLOG(QM, ERROR, "NULL Queue\n"); \
			break; \
		} \
		if (!_prEntry) { \
			DBGLOG(QM, ERROR, "NULL Queue Entry\n"); \
		...
		if (_prQueue->u4NumElem == 0) \
			break; \
		if (!QUEUE_IS_VALID_ENTRY(_prEntry)) \
			break; \
		if (_prEntry->prPrev) \
			_prEntry->prPrev->prNext = _prEntry->prNext; \
		if (_prEntry->prNext) \
			_prEntry->prNext->prPrev = _prEntry->prPrev; \
		QUEUE_ENTRY_INITIALIZE(_prEntry); \
		_prQueue->u4NumElem--; \
	} while (0)
