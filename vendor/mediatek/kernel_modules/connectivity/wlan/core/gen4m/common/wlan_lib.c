...
#undef __STR_MIN_RESERVED
	...
	INIT_UINT(prWifiVar->ucRelaxBtmLimit, "RelaxBtmLimit", 0,
                  FEATURE_TO_CUSTOMER);
...
#endif
	INIT_UINT(prWifiVar->ucScanExpiredTime,
		"ScanExpiredTime", SCN_BSS_DESC_STALE_SEC,
		FEATURE_TO_CUSTOMER);
...
#define LOW_LATENCY_LOG_TEMPLATE \
	...
	wlanSetForceRTS(prAdapter, fgEnMode, fgEnScan);
...
uint32_t wlanSetForceRTS(
	struct ADAPTER *prAdapter,
	u_int8_t fgEnForceRTS, u_int8_t fgEnableScpScan)
{
	struct CMD_SET_FORCE_RTS rForceRts = {0};

	rForceRts.ucForceRtsEn = fgEnForceRTS;
	rForceRts.ucEnableScpScan = fgEnableScpScan;
	rForceRts.ucRtsPktNum = 0;
	DBGLOG(REQ, INFO, "fgEnForceRTS=%d, fgEnableScpScan=%d\n",
			fgEnForceRTS, fgEnableScpScan);
