...
int btmtk_uart_send_set_uart_cmd(struct hci_dev *hdev, struct UART_CONFIG *uart_cfg)
{
	...
	/* setting config */
	{
		...
		/* control by fw */
		cif_dev->ap_cfg.detail.picus_fc_en = (ap_picus_fc_en & cif_dev->fw_cfg.detail.picus_fc_en);
	}
...
static int btmtk_query_fw_uart_settings_cmd(struct btmtk_dev *bdev)
{
	...
	BTMTK_INFO("%s: inband_en[%d], wakeup_enh_en[%d], hub_mode_en[%d]",
			__func__, cif_dev->fw_cfg.detail.inband_en,
	...
	while (1) {
		...
		if (sent_len > 0) {
			...
			if (bdev && bdev->interface_state != BTMTK_STATE_DISCONNECT) {
				...
				if(ret<= 0){
					BTMTK_DBG("%s: tty write fail, ret = %d,zero_pkt_cnt= %d",__func__, ret, zero_pkt_cnt);
					usleep_range(1000, 1100);
				}
				log_time = jiffies_to_msecs(jiffies) - jiffies_to_msecs(log_time);
				if (log_time > TIME_DUMP_OF_FW_PKG_DL)
		...
		const int cmd_len, int retry, int delay)
{
	...
	if (cif_dev->tty) {
		...
		BTMTK_DBG_RAW(cmd, cmd_len, "%s, len[%d] write_retry[%d] flush_retry[%d] CMD :", __func__, cmd_len,
							count, flush_retry);
	}
