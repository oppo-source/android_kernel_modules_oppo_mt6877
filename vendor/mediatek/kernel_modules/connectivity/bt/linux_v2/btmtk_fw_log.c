...
ssize_t btmtk_fops_writefwlog(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (strncmp(i_fwlog_buf, "PowerOffFinderMode=", strlen("PowerOffFinderMode=")) == 0) {
		u8 val = *(i_fwlog_buf + strlen("PowerOffFinderMode=")) - '0';

		val = !!val;
		atomic_set(&pp_bdev[hci_idx]->poweroff_finder_mode, val);

		BTMTK_INFO("%s: set PowerOffFinderMode[%u]", __func__, atomic_read(&pp_bdev[hci_idx]->poweroff_finder_mode));
		ret = count;
		goto exit;
	}
...
coredump_fail_unlock:
		...
		//#ifdef OPLUS_BT_STACK_UNIFY
		// Do not send fw schedule evt to host,since we are not using mbarin.
		return 1;
		//#endif /* OPLUS_BT_STACK_UNIFY */
	} else if ((bt_cb(skb)->pkt_type == HCI_EVENT_PKT) &&
				skb->len > 7 && skb->data[0] == 0xFF &&
