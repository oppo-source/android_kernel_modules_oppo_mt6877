...
/*CONNECTIVITY.WIFI.HARDWARE.CUSTOMER_FEM.7974813, 2024/9/23, i/epa co-existence*/
#ifdef OPLUS_FEATURE_CUSTOMER_FEM
#include <soc/oplus/system/oplus_project.h>
#endif /*OPLUS_FEATURE_CUSTOMER_FEM*/
...
/*CONNECTIVITY.WIFI.HARDWARE.CUSTOMER_FEM.7974813, 2024/9/23, i/epa co-existence*/
#ifdef OPLUS_FEATURE_CUSTOMER_FEM
static int cfm_dt_epaelna_hwid_pcb_parse(struct device_node *np,
		struct platform_device *pdev,
		unsigned int *hwid_out);
#endif /*OPLUS_FEATURE_CUSTOMER_FEM*/
...
#ifdef OPLUS_FEATURE_CUSTOMER_FEM
	hwid_tmp = 0;
	ret  = cfm_dt_epaelna_hwid_pcb_parse(np_hwid, pdev, &hwid_tmp);
	if (ret < 0 && ret != -ENOENT) {
		return -EINVAL;
	} else if (ret != -ENOENT) {
		hwid = hwid_tmp;
...
static int cfm_dt_epaelna_hwid_pcb_parse(struct device_node *np,
		...
		unsigned int *hwid_out)
{
	struct device_node *pcb_node = NULL;
	unsigned int pcb_num = 0;
	unsigned int oplus_pcb = 0;
	unsigned int pcb_id = 0;
	unsigned int hwid = 0;
	/*format pair<pcb_num parts_index>*/
	int pcb_item = 2;
	int i, ret;

	/* Get node of pcb if it is exists */
	pcb_node = of_get_child_by_name(np, CFM_DT_NODE_PCB);
	if (!pcb_node)
		return -ENOENT;

	oplus_pcb = get_PCB_Version();

	ret = of_property_read_u32(pcb_node, CFM_DT_PROP_PCB_NUM, &pcb_num);
	if (ret) {
		pr_info("Fail to parse prop pcb num");
		return -EINVAL;
	}

	for (i = 0; i < pcb_num; i++) {
		int index = pcb_item * i;
		ret =  of_property_read_u32_index(pcb_node, CFM_DT_PROP_PCB_MAP, index, &pcb_id);
		if (ret) {
			pr_info("Fail to parse prop pcb map index");
			return -EINVAL;
		}

		if (pcb_id == oplus_pcb) {
			ret = of_property_read_u32_index(pcb_node, CFM_DT_PROP_PCB_MAP, index + 1, &hwid);
			if (ret) {
				pr_info("Fail to parse prop pcb map value");
				return -EINVAL;
			}
	...
	pr_info("pcb num %u,pcbVersion %u,index[%d],pcb_id[%u],hwid[%u]", pcb_num, oplus_pcb, i, pcb_id, hwid);
	return 0;
}
#endif /*OPLUS_FEATURE_CUSTOMER_FEM*/
