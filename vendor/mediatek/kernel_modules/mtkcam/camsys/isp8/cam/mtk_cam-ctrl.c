...
static int mtk_cam_watchdog_monitor_vsync(struct mtk_cam_watchdog *wd)
{
	...
	for (i = 0; i < ARRAY_SIZE(ctx->hw_raw); i++) {
		if (ctx->hw_raw[i]) {
			...
			if (!raw_dev)
				continue;
			if (atomic_read(&raw_dev->enable_irq_failed)) {
				mtk_cam_event_error(ctrl, MSG_ENABLE_IRQ_ERROR);
				atomic_set(&raw_dev->enable_irq_failed, 0);
			}
		}
	}
#endif
			      ...
			      int inner_cookie, const char *desc)
{
	...
	bool completed;
	if (ctx_id >= cam->max_stream_num) {
		dev_info(cam->dev, "unexpected ctx_id, engine_type:0x%x engine_id:0x%x inner_cookie:0x%x\n",
			engine_type, engine_id, inner_cookie);
		goto SKIP_SCHEDULE_WORK;
	}
	ctrl = &cam->ctxs[ctx_id].cam_ctrl;
	wd = &ctrl->watchdog;
#else
	struct mtk_cam_ctrl *ctrl = &cam->ctxs[ctx_id].cam_ctrl;
