...
static void dump_job_info(struct mtk_cam_job *job, const char *desc)
{
	...
	dma_addr_t vas[MTK_RAW_TOTAL_NODES];
	int node_id[MTK_RAW_TOTAL_NODES];
	int buf_idx = 0;
	int i;

	fp = (struct mtkcam_ipi_frame_param *)job->ipi.vaddr;
#ifdef OPLUS_FEATURE_CAMERA_COMMON
	dev_info(dev, "%s: ctx-%d pipe %x job type %d req-%d-0x%x eng %x master %x\n",
		 __func__,
	...
	list_for_each_entry(buf, &req->buf_list, list) {
		if (buf->vbb.vb2_buf.vb2_queue &&
			...
			if (node->uid.pipe_id != get_raw_subdev_idx(ctx->used_pipe))
				continue;
			if (buf->daddr && buf_idx < MTK_RAW_TOTAL_NODES) {
				vas[buf_idx] = buf->daddr;
				node_id[buf_idx] = node->uid.id;
				buf_idx++;
			}
		}
	}
	...
	/* print */
	for (i = 0; i < buf_idx; i++)
		dev_info(dev, "%s:node_id:%d iova:0x%llx", __func__, node_id[i], vas[i]);
}
