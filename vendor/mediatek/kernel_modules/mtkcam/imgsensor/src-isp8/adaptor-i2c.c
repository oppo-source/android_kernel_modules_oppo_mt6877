		...
		u16 addr, u16 *list, u32 len)
{
	...
	while (sent < total) {
		...
		if (ret < 0){
			...
			#if defined(OPLUS_FEATURE_CAMERA_COMMON) && defined(CONFIG_OPLUS_CAM_EVENT_REPORT_MODULE)
			if (ctx)
				cam_olc_raise_exception(EXCEP_I2C, payLoadConvert(ctx, payload, "ixc_wr_regs_u8", addr, list[0], list[1]));
			#endif /* OPLUS_FEATURE_CAMERA_COMMON */
			kfree(pmem);
			return -EIO;
		}
		...
		u16 addr, u16 *list, u32 len)
{
	...
	while (sent < total) {
		...
		if (ret < 0){
			...
			#if defined(OPLUS_FEATURE_CAMERA_COMMON) && defined(CONFIG_OPLUS_CAM_EVENT_REPORT_MODULE)
			if (ctx)
				cam_olc_raise_exception(EXCEP_I2C, payLoadConvert(ctx, payload, "adaptor_ixc_wr_regs_u8_max", addr, list[0], list[1]));
			#endif /* OPLUS_FEATURE_CAMERA_COMMON */
			kfree(pmem);
			return -EIO;
		}
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
int adaptor_i2c_rd_u8_u8(struct i2c_client *i2c_client, 	 u16 addr, u16 reg, u8 *val)
{
    int ret;
    u8 buf[1];
    struct i2c_msg msg[2];
  ...
  int adaptor_i2c_wr_u8_u8(struct i2c_client *i2c_client,		u16 addr, u16 reg, u8 val)
  {
	int ret;
	u8 buf[2];
	struct i2c_msg msg;
  ...
  int adaptor_i2c_wr_regs_u8_u8(struct i2c_client *i2c_client,	u16 addr, u16 *list, u32 len)
  {
    struct cache_wr_regs_u8 *pmem;
	struct i2c_msg *pmsg;
	u8 *pbuf;
