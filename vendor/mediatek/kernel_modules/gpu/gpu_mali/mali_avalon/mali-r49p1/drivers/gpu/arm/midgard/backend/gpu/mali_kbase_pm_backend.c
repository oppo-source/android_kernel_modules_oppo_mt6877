...
int kbase_hwaccess_pm_init(struct kbase_device *kbdev)
{
	...
	if ((kbdev->gpu_props.gpu_id.arch_major > 11) ||
		...
		// CONFIG_MALI_MTK_SOI=n : clear bit
		// MTK need clear the bit
...
#if IS_ENABLED(CONFIG_MALI_MTK_POWEROFF_KTHREAD_WORKER) && IS_ENABLED(CONFIG_MALI_MTK_KBASE_THREAD_DEBUG)
	MALI_TRACE_BEGIN("kbase_pm_gpu_poweroff_wait_wq");
#endif /* CONFIG_MALI_MTK_POWEROFF_KTHREAD_WORKER */
...
int kbase_pm_handle_runtime_suspend(struct kbase_device *kbdev)
{
	...
	{
		dev_dbg(kbdev->dev, "scheduler.lock trylock ,runtime suspend retry");
		kbase_reset_gpu_allow(kbdev);
		ret = -EBUSY;
		goto out;
	}
#if !MALI_USE_CSF
	if (!mutex_trylock(&kbdev->js_data.runpool_mutex))
	{
		kbase_csf_scheduler_unlock(kbdev);
		kbase_reset_gpu_allow(kbdev);
		dev_dbg(kbdev->dev, "runpool_mutex lock trylock ,runtime suspend retry");
		ret = -EBUSY;
		goto out;
	}
#else
	if (!mutex_trylock(&kbdev->pm.lock))
	{
		kbase_csf_scheduler_unlock(kbdev);
		kbase_reset_gpu_allow(kbdev);
		dev_dbg(kbdev->dev, "pm.lock trylock, runtime suspend retry");
		ret = -EBUSY;
		goto out;
	}
#endif /* !MALI_USE_CSF */
