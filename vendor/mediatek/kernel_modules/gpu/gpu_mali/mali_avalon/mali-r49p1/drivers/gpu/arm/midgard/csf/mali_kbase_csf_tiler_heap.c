					 ...
					 u64 *new_chunk_ptr)
{
	...
	mutex_lock(&kctx->csf.tiler_heaps.lock);
#if IS_ENABLED(CONFIG_MALI_MTK_WORKER_TOO_LONG_DEBUG)
	// if lock execute too long, trigger debug message
	execute_time = ktime_to_ms(ktime_sub(ktime_get(), begin_timestamp));
	if (execute_time >= KBASE_FUNCTION_EXECUTE_DEBUG_TIMEOUT) {
		dev_err(kctx->kbdev->dev, "ctx:%d_%d %s tiler_heaps.lock 1 too long! (%llums)", kctx->tgid, kctx->id, __func__, execute_time);
	}
#endif /* CONFIG_MALI_MTK_WORKER_TOO_LONG_DEBUG */
	...
	execute_time = ktime_to_ms(ktime_sub(ktime_get(), begin_timestamp));
	if (execute_time >= KBASE_FUNCTION_EXECUTE_DEBUG_TIMEOUT) {
		dev_err(kctx->kbdev->dev, "ctx:%d_%d %s tiler_heaps.lock 2 too long! (%llums)", kctx->tgid, kctx->id, __func__, execute_time);
	}
#endif /* CONFIG_MALI_MTK_WORKER_TOO_LONG_DEBUG */
	heap = find_tiler_heap(kctx, gpu_heap_va);
