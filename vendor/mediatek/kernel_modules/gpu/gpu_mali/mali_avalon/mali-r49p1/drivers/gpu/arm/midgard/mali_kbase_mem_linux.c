		     ...
		     void __user *phandle, u32 padding, u64 *gpu_va, u64 *va_pages, u64 *flags)
{
	...
	if (!reg)
	{
		dev_warn(kctx->kbdev->dev, "%s reg is NULL", __func__);
		goto no_reg;
	}
	...
	if (*flags & (BASE_MEM_SAME_VA | BASE_MEM_NEED_MMAP)) {
		...
		if (bitmap_empty(kctx->cookies, BITS_PER_LONG))
		{
			dev_warn(kctx->kbdev->dev, "%s no cookie", __func__);
			goto no_cookie;
		}
		/* return a cookie */
		*gpu_va = find_first_bit(kctx->cookies, BITS_PER_LONG);
		...
		*gpu_va <<= PAGE_SHIFT;
		...
		{
			dev_warn(kctx->kbdev->dev, "%s no gpu va", __func__);
			goto no_gpu_va;
		}
		/* return real GPU VA */
		*gpu_va = reg->start_pfn << PAGE_SHIFT;
		...
		{
			dev_warn(kctx->kbdev->dev, "%s 2nd no gpu va", __func__);
			goto no_gpu_va;
		}
		/* return real GPU VA */
		*gpu_va = reg->start_pfn << PAGE_SHIFT;
