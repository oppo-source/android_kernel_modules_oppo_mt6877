...
page_fault_retry:
	...
	if (execute_time >= KBASE_FUNCTION_EXECUTE_DEBUG_TIMEOUT) {
		dev_err(kbdev->dev, "ctx:%d_%d %s page_fault_try_alloc too long! (%llums)", kctx->tgid, kctx->id, __func__, execute_time);
	}
#endif /* CONFIG_MALI_MTK_WORKER_TOO_LONG_DEBUG */
	...
	if (grown) {
		...
		if (err) {
			...
			execute_time = ktime_to_ms(ktime_sub(ktime_get(), section_timestamp));
			if (execute_time >= KBASE_FUNCTION_EXECUTE_DEBUG_TIMEOUT) {
				dev_err(kbdev->dev, "ctx:%d_%d %s grown err too long! (%llums)", kctx->tgid, kctx->id, __func__, execute_time);
			}
#endif /* CONFIG_MALI_MTK_WORKER_TOO_LONG_DEBUG */
			goto fault_done;
		}
		...
		execute_time = ktime_to_ms(ktime_sub(ktime_get(), section_timestamp));
		if (execute_time >= KBASE_FUNCTION_EXECUTE_DEBUG_TIMEOUT) {
			dev_err(kbdev->dev, "ctx:%d_%d %s grown too long! (%llums)", kctx->tgid, kctx->id, __func__, execute_time);
		}
#endif /* CONFIG_MALI_MTK_WORKER_TOO_LONG_DEBUG */
	} else {
		int ret = -ENOMEM;
		...
		if (pages_to_grow > 0) {
			if (kbase_is_large_pages_enabled() && grow_2mb_pool) {
				...
				execute_time = ktime_to_ms(ktime_sub(ktime_get(), section_timestamp));
				if (execute_time >= KBASE_FUNCTION_EXECUTE_DEBUG_TIMEOUT) {
					dev_err(kbdev->dev, "ctx:%d_%d %s mem_pool_grow too long! (%llums)", kctx->tgid, kctx->id, __func__, execute_time);
				}
#endif /* CONFIG_MALI_MTK_WORKER_TOO_LONG_DEBUG */
...
fault_done:
	...
	if (execute_time >= KBASE_FUNCTION_EXECUTE_DEBUG_TIMEOUT) {
		dev_err(kbdev->dev, "ctx:%d_%d %s too long! (%llums), request size: %zu pages", kctx->tgid, kctx->id, __func__, execute_time, new_pages);
	}
#endif /* CONFIG_MALI_MTK_WORKER_TOO_LONG_DEBUG */
}
