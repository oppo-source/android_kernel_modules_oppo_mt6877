...
static void inline bt_dump_bgfsys_misc_pm_info(void)
{
	BTMTK_INFO("%s: BGF_SW_IRQ_STATUS = 0x%08x", __func__, g_value);
}
...
static void bgfsys_dump_conn_wt_slp_ctrl_reg(void)
{
	...
	base = ioremap_nocache(0x180050A8, 0x10);
	if (base) {
...
static inline int32_t bgfsys_power_off(void)
{
	...
	/* ack sw irq */
	g_value = bgfsys_get_sw_irq_status();
	BTMTK_INFO("BGF_SW_IRQ_STATUS = 0x%08x", g_value);
