...
extern uint8_t wlanGetChannelIndex(uint8_t channel);
...
static struct ADAPTER * get_adapter() {
    ...
    prGlueInfo = (struct GLUE_INFO *) wiphy_priv(wdev_sta->wiphy);
    if (prGlueInfo == NULL || !prGlueInfo->fgIsRegistered) {
        debug("prGlueInfo is null or netdevice not register, exit, register=%d", prGlueInfo->fgIsRegistered);
        goto EXIT;
    }
