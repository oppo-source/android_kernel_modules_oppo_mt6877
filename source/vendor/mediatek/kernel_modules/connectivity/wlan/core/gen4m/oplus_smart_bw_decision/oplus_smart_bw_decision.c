...
VOID connectLogActiveBSSRSSI(IN P_BSS_DESC_T prSPBssDesc)
{
        ...
        if (prSPBssDesc) {
                ...
                snprintf(buf, sizeof(buf), "[SCN]: "OPLUS_SSIDSTRTYPE", SPRSSI= %d, ",
                        OPLUS_SSIDSTR(prSPBssDesc->aucSSID, prSPBssDesc->ucSSIDLen), rRssi);
        }
		...
		if (prBssDesc->ucChannelNum > 214)
	        ...
	        if ((!CHECK_FOR_TIMEOUT(kalGetTimeTick(), prBssDesc->rUpdateTime, SEC_TO_SYSTIME(SCAN_RESULT_EXPIRE))) &&
	                (prBssDesc->eBand == BAND_2G4)) {
                                ...
                                snprintf(buf + bufStrLen, sizeof(buf) - bufStrLen, OPLUS_SSIDSTRTYPE" RSSI = %d ",
                                        OPLUS_SSIDSTR(prBssDesc->aucSSID, prBssDesc->ucSSIDLen) , rRssi);
                                bufStrLen = strlen(buf);
...
VOID connectLogSummary(IN P_BSS_DESC_T prBssDesc, uint8_t ucBssIndex)
{
        ...
        if (isBSS24GHT40(prBssDesc->eBand, prBssDesc->eChannelWidth, prBssDesc->eSco)) {
                ...
                if (prBssDesc->ucSSIDLen) {
                        DBGLOG(AIS, TRACE, "2.4G HT40,"OPLUS_SSIDSTRTYPE" Channel = %d, Ratio: %d/100\n",
                                OPLUS_SSIDSTR(prBssDesc->aucSSID, prBssDesc->ucSSIDLen), prBssDesc->ucChannelNum,
                ...
                if (prBssDesc->ucSSIDLen) {
                        DBGLOG(AIS, TRACE, "5G VHT160,"OPLUS_SSIDSTRTYPE" Channel = %d, Ratio: %d/100\n",
                                OPLUS_SSIDSTR(prBssDesc->aucSSID, prBssDesc->ucSSIDLen), prBssDesc->ucChannelNum,
        ...
        PARAM_RSSI rRssi, OS_SYSTIME rTimeSec, unsigned char * aucBSSID)
{
        if (aucSSID && ucSSIDLen) {
                DBGLOG(SCN, TRACE, OPLUS_SSIDSTRTYPE"\n", OPLUS_SSIDSTR(aucSSID, ucSSIDLen));
        }
        ...
        if (aucBSSID) {
                DBGLOG(SCN, TRACE, "BSSID: %x:%x:**:**:%x:%x\n",
                        aucBSSID[0], aucBSSID[1], aucBSSID[4], aucBSSID[5]);
        }
}
...
BOOLEAN extraCheck2GSupportBW(IN P_BSS_DESC_T prSPBssDesc)
{
        ...
        snprintf(buf, sizeof(buf),
                "[SMART_BW]: [HT40_SELECT]: eBand = %d, eChannelWidth = %d, eSco = %d, Active2GNum = %d, RSSI: %d,"OPLUS_SSIDSTRTYPE
                ", CH: %d, 2.4G HT40 status: %d, fgIsNeedMonitorLink: %d, fgIsNeedMonitorAPIOT: %d, isIn2G40MBlacklist: %d",
                prSPBssDesc->eBand, prSPBssDesc->eChannelWidth, prSPBssDesc->eSco,
                prSmartBW->u4ScanResultActive2GNum, RCPI_TO_dBm(prSPBssDesc->ucRCPI), OPLUS_SSIDSTR(prSPBssDesc->aucSSID, prSPBssDesc->ucSSIDLen),
                prSPBssDesc->ucChannelNum, ret, prSmartBW->fgIsNeedMonitorLink, prSmartBW->fgIsNeedMonitorAPIOT, isIn2G40MBlacklist(prSPBssDesc));
