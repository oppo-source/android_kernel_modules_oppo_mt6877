...
static int dump_slab_info(bool verbose)
{
	...
	if (likely(!verbose)) {
		...
		if (max_cachep || prev_max_cachep)
			osvelte_info("name            <active_objs> <num_objs> <objsize> <objperslab> <pagesperslab> : tunables <limit> <batchcount> <sharedfactor> : slabdata <active_slabs> <num_slabs> <sharedavail>");
...
static void __lowmem_dbg_dump(struct lowmem_dbg_cfg *cfg)
{
	...
	unsigned long vmalloc, pgtbl, kernel_stack;
	unsigned long dmabuf, dmabuf_pool, gpu;
	struct sysinfo si;
	...
	osvelte_info("vmalloc: %lu page_tables: %lu kernel_stack: %lu\n",
		     K(vmalloc), K(pgtbl), K(kernel_stack));
	osvelte_info("dmabuf: %lu dmabuf_pool: %lu gpu: %lu",
		     K(dmabuf), K(dmabuf_pool), K(gpu));
