...
static int tfa98xx_check_status_reg(void )
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		num++;
		err = tfa98xx_read_register16(tfa98xx->tfa, 0x10, &reg_val);
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			tfa98xx->pstream = 1;
#ifdef OPLUS_ARCH_EXTENDS
			if ((tfa98xx->tfa->is_probus_device) && (s_receiver_judgement != 1)) {
				tfa98xx_adsp_send_calib_values();
			} else {
				dev_info(&tfa98xx->i2c->dev, "%s: not send calib values for receiver mode\n", __func__);
			}
#else
			if (tfa98xx->tfa->is_probus_device)
				tfa98xx_adsp_send_calib_values();
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	if (init_times == 0) {
		snd_soc_add_codec_controls(tfa98xx->codec,
				   tfa98xx_snd_control_spk_l_reboot,
				   ARRAY_SIZE(tfa98xx_snd_control_spk_l_reboot));
	} else {
		snd_soc_add_codec_controls(tfa98xx->codec,
				   tfa98xx_snd_control_spk_r_reboot,
				   ARRAY_SIZE(tfa98xx_snd_control_spk_r_reboot));
	}
	#endif
	init_times++;
	#endif //VENDOR_EDIT
