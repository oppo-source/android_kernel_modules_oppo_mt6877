...
#ifdef CONFIG_OPLUS_POGOPIN_FUNCTION
struct pogo_keyboard_operations pogo_keyboard_ops = {
 ...
 */
static void pogo_uart_xchar_out(struct uart_port *uport, int offset)
{
...
int pogo_tx_dma(struct uart_8250_port *p){
	...
	int ret;
		...
		if(pogo_keyboard_ops.check && pogo_keyboard_ops.write){
			if(pogo_keyboard_ops.check(&p->port)) {
				pogo_keyboard_ops.write(&p->port,1);
			}
		}
	#endif
	...
	if (dma->tx_running) {
		if (up->x_char) {
			...
			pogo_uart_xchar_out(up, UART_TX);
...
static void pogo_dma_tx_complete(void *param){
	...
	if (ret || !dma->tx_running) {
		...
		if(pogo_keyboard_ops.check && pogo_keyboard_ops.write){
			if(pogo_keyboard_ops.check(&p->port)) {
				pogo_keyboard_ops.write(&p->port,0);
			}
		}
	#endif
	}
...
static void mtk8250_dma_rx_complete(void *param)
{
	...
	if (pogo_keyboard_ops.check && pogo_keyboard_ops.recv) {
		if (pogo_keyboard_ops.check(&up->port)) {
			#if defined(KERNEL_mtk_uart_set_kb_port_status)
			KERNEL_mtk_uart_set_kb_port_status(dma->rxchan, true);
			#endif
			pogo_keyboard_ops.recv((unsigned char *)ptr, cnt);
		} else {
			#if defined(KERNEL_mtk_uart_set_kb_port_status)
			KERNEL_mtk_uart_set_kb_port_status(dma->rxchan, false);
			#endif
		}
	} else {
		#if defined(KERNEL_mtk_uart_set_kb_port_status)
...
static int mtk8250_startup(struct uart_port *port)
{
	...
	if(pogo_keyboard_ops.check && pogo_keyboard_ops.init){
		if(pogo_keyboard_ops.check(port)) {
			pogo_keyboard_ops.init(port,1);
		}
	}
	return ret;
#else
...
static irqreturn_t wakeup_irq_handler_bottom_half(int irq, void *dev_id)
{
	...
	struct uart_8250_port *up = NULL;
	u64 wakeup_time[7] = {0};
	int rx_state, wakeup_cb_exists = 0;
	unsigned int uart_fcr = 0;
	unsigned int xoff;
	struct mtk8250_uart_info reg_dump = {{0},{0}};
	...
	if (atomic_read(&data->wakeup_state) == 0) {
		...
		wakeup_time[6] = sched_clock();
		pr_info("[%s]: time[%lld %lld %lld %lld %lld]ns rx_state[%d]"
			"remain[0x%x 0x%x 0x%x 0x%x]"
			"db5[0x%x 0x%x 0x%x 0x%x]\n"
			" wakeup_callback_exists:%d\n"
			, __func__, wakeup_time[6] - wakeup_irq_time,
			wakeup_time[2] - wakeup_time[1],
			wakeup_time[4] - wakeup_time[3],
			wakeup_time[5] - wakeup_time[0],
			wakeup_time[6] - wakeup_time[5],
			rx_state, reg_dump.rx_remain[0],
			reg_dump.rx_remain[1],reg_dump.rx_remain[2],
			reg_dump.rx_remain[3], reg_dump.db5[0],
			reg_dump.db5[1], reg_dump.db5[2], reg_dump.db5[3],wakeup_cb_exists);
	}
	mutex_unlock(&data->clk_mutex);
	return IRQ_HANDLED;
}
