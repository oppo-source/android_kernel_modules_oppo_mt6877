...
static int __gpufreq_volt_scale_gpu(unsigned int vgpu_old, unsigned int vgpu_new,
...
static unsigned int __gpufreq_get_aging_table_idx(u32 a_t0_lvt_rt, u32 a_t0_ulvt_rt,
						  u32 a_shift_error, u32 efuse_error,
						  u32 a_tn_lvt_cnt, u32 a_tn_ulvt_cnt,
...
int __gpufreq_power_control(enum gpufreq_power_state power)
{
	...
	GPUFREQ_LOGD("switch power: %s (Power: %d, Buck: %d, MTCMOS: %d, CG: %d)",
		     power ? "On" : "Off",
		     g_gpu.power_count, g_gpu.buck_count, g_gpu.mtcmos_count, g_gpu.cg_count);
...
done_unlock:
	GPUFREQ_LOGD("PWR_STATUS (0x%x): 0x%08x", (0x10006000 + 0x16C), readl(g_sleep + 0x16C));
...
int __gpufreq_generic_commit_gpu(int target_oppidx, enum gpufreq_dvfs_state key)
{
	...
	GPUFREQ_LOGD("begin to commit GPU OPP index: (%d->%d)", cur_oppidx, target_oppidx);

	/* todo: GED log buffer (gpufreq_pr_logbuf) */

	if (target_freq == cur_freq) {
		/* voltage scaling */
		ret = __gpufreq_volt_scale_gpu(cur_volt, target_volt, cur_vsram, target_vsram);
		if (unlikely(ret)) {
				       ...
				       unsigned int target_volt, enum gpufreq_dvfs_state key)
{
	...
	if (target_freq == cur_freq) {
		...
		while (target_volt != cur_volt) {
			...
			ret = __gpufreq_volt_scale_gpu(cur_volt, sb_volt, cur_vsram, sb_vsram);
			if (unlikely(ret)) {
				GPUFREQ_LOGE("fail to scale Vgpu: (%d->%d), Vsram_gpu: (%d->%d)",
				    ...
				    unsigned int vsram_old, unsigned int vsram_new)
{
	...
	if (vgpu_new > vgpu_old) {
		/* scale-up volt */
		t_settle_vgpu = __gpufreq_settle_time_vgpu(true, (vgpu_new - vgpu_old));
		t_settle_vsram = __gpufreq_settle_time_vsram(true, (vsram_new - vsram_old));

		ret = regulator_set_voltage(g_pmic->reg_vsram_gpu,
		...
		/* scale-down volt */
		t_settle_vgpu = __gpufreq_settle_time_vgpu(false, (vgpu_old - vgpu_new));
		t_settle_vsram = __gpufreq_settle_time_vsram(false, (vsram_old - vsram_new));
	...
	GPUFREQ_LOGD("Vgpu: %d, Vsram: %d, udelay: %d", g_gpu.cur_volt, g_gpu.cur_vsram, t_settle);
...
static unsigned int __gpufreq_get_real_fgpu(void)
{
	...
	//GPUFREQ_LOGE("%s: MFGPLL_CON1 = 0x%x", __func__, (unsigned int)(g_apmixed_base + 0x026C));
...
static void __gpufreq_mfg0_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_OFF) {
		...
		writel((readl(MFG0_PWR_CON) & ~(0x1 << 8)), MFG0_PWR_CON);
...
static void __gpufreq_mfg1_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_OFF) {
		/* TINFO="Set way_en=0" */
		writel(MFG1_PROT_STEP0_WAY_EN_OFF_MASK, INFRA_PDN_MFG1_WAY_EN);
		while ((readl(INFRA_PDN_MFG1_WAY_EN) & MFG1_PROT_STEP0_WAY_EN_OFF_MASK) !=
		...
		writel((readl(MFG1_PWR_CON) | PWR_RST_B), MFG1_PWR_CON);
		/* TINFO="Set SRAM_PDN = 0" */
		writel((readl(MFG1_PWR_CON) & ~(0x1 << 8)), MFG1_PWR_CON);
		/* TINFO="Wait until SRAM_PDN_ACK = 0" */
		...
		/* TINFO="Set way_en=1" */
		writel(MFG1_PROT_STEP0_WAY_EN_ON_MASK, INFRA_PDN_MFG1_WAY_EN);
		while ((readl(INFRA_PDN_MFG1_WAY_EN) & MFG1_PROT_STEP0_WAY_EN_ON_MASK)
...
static void __gpufreq_mfg2_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_OFF) {
		...
		writel((readl(MFG2_PWR_CON) | PWR_RST_B), MFG2_PWR_CON);
		/* TINFO="Set SRAM_PDN = 0" */
		writel((readl(MFG2_PWR_CON) & ~(0x1 << 8)), MFG2_PWR_CON);
		/* TINFO="Wait until SRAM_PDN_ACK = 0" */
...
static void __gpufreq_mfg3_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_OFF) {
		...
		writel((readl(MFG3_PWR_CON) | PWR_RST_B), MFG3_PWR_CON);
		/* TINFO="Set SRAM_PDN = 0" */
		writel((readl(MFG3_PWR_CON) & ~(0x1 << 8)), MFG3_PWR_CON);
		/* TINFO="Wait until SRAM_PDN_ACK = 0" */
...
static void __gpufreq_set_springboard(void)
{
	...
	for (src_idx = 0; src_idx < g_gpu.opp_num; src_idx++) {
		...
		GPUFREQ_LOGD("springboard_up[%02d]: %d", src_idx, g_gpu.sb_table[src_idx].up);
	}
	...
	for (src_idx = 0; src_idx < g_gpu.opp_num; src_idx++) {
		...
		GPUFREQ_LOGD("springboard_down[%02d]: %d", src_idx, g_gpu.sb_table[src_idx].down);
	}
}
						  ...
						  unsigned int is_efuse_read_success)
{
	...
	GPUFREQ_LOGD("tj1=%d, tj2=%d, tj=%d, adiff1=%d, adiff2=%d", tj1, tj2, tj, adiff1, adiff2);
...
static void __gpufreq_aging_adjustment(void)
{
	...
	if (__gpufreq_dvfs_enable()) {
		...
		is_efuse_read_success =
		    __gpufreq_asensor_read_efuse(&a_t0_lvt_rt, &a_t0_ulvt_rt, &a_shift_error,
						 &efuse_error);
		...
		aging_table_idx =
		    __gpufreq_get_aging_table_idx(a_t0_lvt_rt, a_t0_ulvt_rt, a_shift_error,
						  efuse_error, a_tn_lvt_cnt, a_tn_ulvt_cnt,
						  is_efuse_read_success);
	}
...
static int __gpufreq_init_opp_table(struct platform_device *pdev)
{
	...
	g_gpu.sb_table = kcalloc(g_gpu.opp_num, sizeof(struct gpufreq_sb_info), GFP_KERNEL);
	if (!g_gpu.sb_table) {
