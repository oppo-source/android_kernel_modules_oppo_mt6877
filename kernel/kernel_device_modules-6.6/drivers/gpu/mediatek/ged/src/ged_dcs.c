...
int dcs_set_fix_num(unsigned int core_num)
{
	...
	if (g_fix_core_num != core_num) {
		...
		if (core_num > 0 && core_num <= g_max_core_num) {
...
void dcs_debug(void)
{
	...
	trace_GPU_DVFS__Policy__DCS(0, 0, 0);
	trace_GPU_DVFS__Policy__DCS(g_avail_mask_num, g_dcs_opp_setting, g_dcs_opp_setting_org);

	mask_table = gpufreq_get_core_mask_table();
	if (mask_table != NULL) {
		trace_GPU_DVFS__Policy__DCS(1, 1, 1);
		for (i = 0; i < gpufreq_get_core_num(); i++)
			trace_GPU_DVFS__Policy__DCS(i, mask_table[i].num, mask_table[i].mask);
	}

	if (g_core_mask_table != NULL) {
		trace_GPU_DVFS__Policy__DCS(2, 2, 2);
		for (i = 0; i < g_max_core_num; i++)
			trace_GPU_DVFS__Policy__DCS(i, g_core_mask_table[i].num, g_core_mask_table[i].mask);
	}

	if(g_avail_mask_table != NULL) {
		trace_GPU_DVFS__Policy__DCS(3, 3, 3);
		for (i = 0; i < g_avail_mask_num; i++)
			trace_GPU_DVFS__Policy__DCS(i, g_avail_mask_table[i].num, g_avail_mask_table[i].mask);
	}
}
