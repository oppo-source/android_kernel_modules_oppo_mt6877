...
#define KTZ8866_I2C_MASTER_NAME "10-0011"
#define KTZ8866_I2C_SLAVE_NAME "9-0011"

int g_lcd_bias_id = KTZ8866_ID;
//struct i2c_client *lcd_bl_i2c0_aw3750x;

struct i2c_client *lcd_bl_i2c_m_client;
struct i2c_client *lcd_bl_i2c_s_client;
...
EXPORT_SYMBOL(lcd_bl_i2c_m_client);
EXPORT_SYMBOL(lcd_bl_i2c_s_client);
 ...
 int lcd_bl_write_byte(unsigned char addr, unsigned char value)
{
	...
	if (NULL == lcd_bl_i2c_m_client) {
	pr_debug("[LCD][BL] lcd_bl_i2c_m_client is null!!\n");
	return -EINVAL;
	}
	ret = i2c_master_send(lcd_bl_i2c_m_client, write_data, 2);
...
static int lcd_bl_read_byte(u8 regnum)
{
	...
	buffer[0] = regnum;
	res = i2c_master_send(lcd_bl_i2c_m_client, buffer, 0x1);
	if (res <= 0)	{
	  mutex_unlock(&read_lock);
	  pr_debug("read reg send res = %d\n", res);
	  return res;
	}
	res = i2c_master_recv(lcd_bl_i2c_m_client, reg_value, 0x1);
	if (res <= 0) {
	  mutex_unlock(&read_lock);
...
int lcd_set_bl_bias_reg(struct device *pdev, int enable)
{
	...
	if (enable) {
		...
		lcd_bl_write_byte(0x0D, 0x26); /* OUTP_CFG，OUTP = 6.0V */
		lcd_bl_write_byte(0x0E, 0x26); /* OUTN_CFG，OUTN = -6.0V */
...
static int lcd_bl_i2c_probe(struct i2c_client *client)
{
	...
	/* id_ktz8866 check */
	if(!strcmp(dev_name(&client->dev), KTZ8866_I2C_MASTER_NAME)) {
		printk("%s:KTZ8866\n", __func__);
			g_lcd_bias_id = KTZ8866_ID;

		lcd_bl_i2c_m_client = client;
		dev = &lcd_bl_i2c_m_client->dev;
		np = dev->of_node;
		if (of_property_read_bool(np, "skip_ktz8866_i2c_m")){
			pr_warn("[lcd_info] skip %s %s\n", __func__, KTZ8866_I2C_MASTER_NAME);
			return 0;
		}
	}

	if(!strcmp(dev_name(&client->dev), KTZ8866_I2C_SLAVE_NAME)) {
		lcd_bl_i2c_s_client = client;
		dev = &lcd_bl_i2c_s_client->dev;
		np = dev->of_node;
		if (of_property_read_bool(np, "skip_ktz8866_i2c_s")){
				pr_warn("[lcd_info] skip %s %s\n", __func__, KTZ8866_I2C_SLAVE_NAME);
				return 0;
		}
	}
	...
	ret = lcd_bl_write_byte(0x0D, 0x26); /* OUTP_CFG，OUTP = 6.0V */
	ret = lcd_bl_write_byte(0x0E, 0x26); /* OUTN_CFG，OUTN = -6.0V */
	...
	//write backlight reg
	ret = lcd_bl_write_byte(0x02, 0x53); /* BL_CFG1；OVP=34V，线性调光，PWM Disabled */
	ret = lcd_bl_write_byte(0x03, 0xCD); /* LED CURRENT RAMP设置为256mS用于平滑调光*/
	ret = lcd_bl_write_byte(0x11, 0x36); /* BL_OPTION2；电感4.7uH，BL_CURRENT_LIMIT 2.5A；*/
	ret = lcd_bl_write_byte(0x15, 0x99); /* Backlight Full-scale LED Current 22.8mA/CH；*/
	ret = lcd_bl_write_byte(0x08, 0x4F); /* BL enabled and Current sink 1/2/3/4 /5 enabled；*/
...
static void lcd_bl_i2c_remove(struct i2c_client *client)
{
	lcd_bl_i2c_m_client = NULL;
	lcd_bl_i2c_s_client = NULL;
