...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx, 0x8F, 0xA5);
	...
	lcm_dcs_write_seq_static(ctx, 0x8C, 0x8E);
	lcm_dcs_write_seq_static(ctx, 0xCD, 0x6C);
	lcm_dcs_write_seq_static(ctx, 0xC0, 0x8B);
	lcm_dcs_write_seq_static(ctx, 0xC8, 0xF0);
	lcm_dcs_write_seq_static(ctx, 0x8B, 0x10);
	lcm_dcs_write_seq_static(ctx, 0x9F, 0x80);
	lcm_dcs_write_seq_static(ctx, 0xA9, 0x00);
	lcm_dcs_write_seq_static(ctx, 0x97, 0x00);
	lcm_dcs_write_seq_static(ctx, 0xFA, 0x0D);
	lcm_dcs_write_seq_static(ctx, 0xFD, 0x13);
	lcm_dcs_write_seq_static(ctx, 0x83, 0xAA);
	lcm_dcs_write_seq_static(ctx, 0x84, 0x11);
	lcm_dcs_write_seq_static(ctx, 0x9C, 0x10);
	lcm_dcs_write_seq_static(ctx, 0xA9, 0x4B);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (IS_ERR(ctx->avee)) {
		dev_info(dev, "%s: cannot get bias-pos 0 %ld\n",
			__func__, PTR_ERR(ctx->avee));
		return PTR_ERR(ctx->avee);
	}
	devm_gpiod_put(dev, ctx->avee);

	ctx->avdd = devm_gpiod_get(dev, "avdd", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->avdd)) {
		dev_info(dev, "%s: cannot get bias-neg 1 %ld\n",
			__func__, PTR_ERR(ctx->avdd));
		return PTR_ERR(ctx->avdd);
	}
	devm_gpiod_put(dev, ctx->avdd);

	ctx->power_gpio = devm_gpiod_get(dev, "pwren", GPIOD_OUT_HIGH);
	...
	pr_info("%s- lcm,boe-nt51021h-vdo\n", __func__);
...
static const struct of_device_id lcm_of_match[] = {
	{
	    .compatible = "nt51021h,wuxga,vdo,boe",
	},
	{}
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-nt51021h-wuxga-vdo-boe",
		.owner = THIS_MODULE,
...
MODULE_AUTHOR("bincai liu <bincai.liu@mediatek.com>");
MODULE_DESCRIPTION("lcm BOE ft8201 wxga VDO Panel Driver");
MODULE_LICENSE("GPL");
