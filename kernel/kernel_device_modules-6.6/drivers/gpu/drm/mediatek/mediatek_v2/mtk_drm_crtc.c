...
#ifdef OPLUS_FEATURE_AP_UIR_DIMMING
	{DRM_MODE_PROP_ATOMIC, "UIR_DS", 0, ULONG_MAX, 0},
#endif
/* #ifdef OPLUS_FEATURE_DISPLAY */
	...
	for (i = 0; i < CRTC_PROP_MAX; i++) {
		if (private->crtc_property[index][i] == property) {
...
#ifdef OPLUS_FEATURE_AP_UIR_DIMMING
			{
				struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
				if (mtk_crtc) {
					oplus_apuir_set_dsprop(crtc, i, val);
				}
			}
#endif
			return ret;
			...
			const struct mtk_addon_path_data *path_data)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6991) {
		...
		if (comp->id == DDP_COMPONENT_WDMA2)
			return mtk_crtc->gce_obj.event[EVENT_WDMA1_EOF];
		if (comp->id == DDP_COMPONENT_OVLSYS_WDMA2)
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	if (!cwb_info->enable || cwb_info->is_sec ||
			priv->need_cwb_path_disconnect || mtk_crtc->crtc_blank)
		return;
...
#ifdef OPLUS_FEATURE_DISPLAY
	if (oplus_dc_is_support()) {
		if ((exit_dc_flag == 1) && (index == 0)) {
			oplus_mtk_dc_backlight_exit(crtc);
			exit_dc_flag = 0;
		}

		if (((oplus_dc_enable != oplus_dc_enable_real) && (index == 0)) || oplus_dc_recovery) {
...
void mtk_crtc_clr_set_dirty(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (mtk_crtc_is_dc_mode(&mtk_crtc->base) || mtk_crtc->is_mml)
		mtk_crtc_pkt_create(&cmdq_handle, &mtk_crtc->base,
...
static int __mtk_check_trigger(struct mtk_drm_crtc *mtk_crtc)
{
	...
	do {
		if (next_tgt_dur > CT_PRETE_DUR) {
			...
			CRTC_MMP_MARK(index, check_trigger, next_tgt_dur, retry);
			...
			mtk_drm_idlemgr_kick(__func__, &mtk_crtc->base, 0);
			/* separate power keep after kick idle */
			mtk_vidle_user_power_keep(DISP_VIDLE_USER_CRTC);
		}
		// Need to release mtk_crtc->lock here to avoid blocking AT cpu config time due to cmdq resources
		DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, next_tgt_dur);

		//To avoid other threads(eg:pq config) occupying cmdq resources at this time
		cmdq_pkt_refinalize(cmdq_handle2);
		...
		//Recalculate the time difference for distance mapping
		base_t = priv->crtc_rel_present_ts[index] / 1000;
		cur_t = ktime_get() / 1000;
		step = (cur_t - base_t) / step_dur;
		step += 1;
		...
		if (next_tgt_dur > step_dur) {
			DDPPR_ERR("cal error next_tgt_dur:%u > step_dur:%u\n",
				next_tgt_dur, step_dur);
			CRTC_MMP_MARK(index, check_trigger, next_tgt_dur, step_dur);
			CRTC_MMP_EVENT_END(index, check_trigger, 0, 7);
			goto out_unlock;
		}
		retry--;
		// If next_tgt_dur distance not between (TE-1ms) ~ (TE-200us)
		// then sleep to the next TE again.
		valid_interval = (next_tgt_dur < CT_PRETE_DUR) && (next_tgt_dur > CT_NEAR_DUR);
	} while (!valid_interval && (retry > 0));
...
do_set_dirty:
	...
	if (!doze_active && !mtk_state->doze_changed ||
		(doze_active &&	atomic_read(&mtk_crtc->already_config))) {
		if (!mtk_crtc_is_ct_use_event(mtk_crtc))
			mtk_crtc_set_dirty(mtk_crtc);
		else {
			cmdq_handle2 = mtk_crtc_set_dirty_wait(mtk_crtc);
			if (cmdq_handle2) {
				DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
				cmdq_pkt_flush(cmdq_handle2);
	...
	CRTC_MMP_EVENT_END(index, check_trigger, 0, 9);

out_unlock:
	drm_trace_tag_end("check_trigger");
	if (cmdq_handle2 != NULL)
		cmdq_pkt_destroy(cmdq_handle2);
	DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
...
void mml_cmdq_pkt_init(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle)
{
	...
	case MML_DIRECT_LINKING:
		if (mtk_crtc->is_mml) { // mml IR mode
			//make sure dpc has been enabled, before wait pre-TE
						...
						bool need_sync_mml)
{
	...
	if (crtc_id == 0) {
		...
		if (fps == 120 || fps == 144 || mtk_crtc->mode_chg)
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
				mtk_crtc->side_config_regs_pa + MMSYS_EMI_REQ_CTL, 0x801, 0x801);
	}
#endif
	/* Record Vblank start timestamp */
				      ...
				      struct drm_atomic_state *atomic_state)
{
	...
	if (index == 0) {
		...
		if (fps == 120 || fps == 144 || mtk_crtc->mode_chg)
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
				mtk_crtc->side_config_regs_pa + MMSYS_EMI_REQ_CTL, 0x0, 0x801);
	}
#endif
#if !IS_ENABLED(CONFIG_DRM_MEDIATEK_AUTO_YCT)
	mtk_vidle_user_power_release_by_gce(DISP_VIDLE_USER_DISP_CMDQ, cmdq_handle);
