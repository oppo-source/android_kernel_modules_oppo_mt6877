...
#ifdef OPLUS_FEATURE_DISPLAY
extern bool oplus_project_silky_support(void);
	...
	int32_t matrix[16], int32_t hint, bool fte_flag, bool linear)
{
	...
	if (!(((ccorr_data->g_prim_ccorr_force_linear && (ccorr_data->is_linear&0x01))) &&
		...
		if ((!ccorr_data->g_prim_ccorr_force_linear)&& ccorr_data->is_linear != linear) {
			DDPINFO("%s: hwc_linear = %d, comp_linear =%d, comp:%s\n", __func__,
...
static int disp_ccorr_copy_backlight_to_user(struct mtk_ddp_comp *comp, int *backlight)
{
	...
	DDPINFO("%s: %d\n", __func__, ret);
#ifdef OPLUS_FEATURE_DISPLAY
	if (oplus_project_silky_support()){
		int enabled = 0;
		disp_ccorr_set_interrupt(comp, (void *)&enabled);
	}
#endif /* OPLUS_FEATURE_DISPLAY */
	return ret;
}
...
static const struct mtk_disp_ccorr_data mt6771_ccorr_driver_data = {
	.support_shadow     = false,
	.need_bypass_shadow = false,
};
