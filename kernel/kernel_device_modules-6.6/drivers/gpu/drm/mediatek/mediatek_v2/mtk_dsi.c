...
int dbg_preurgent = 1;
module_param(dbg_preurgent, int, 0644);
...
extern bool oplus_project_public_brightness_support(void);
...
unsigned int mtk_dsi_default_rate(struct mtk_dsi *dsi)
{
	...
	if (priv && priv->data &&
		...
		DDPINFO("%s, mipi_hopping_sta, data rate=%d\n", __func__, data_rate);
...
static void mtk_dsi_tx_buf_rw(struct mtk_dsi *dsi)
{
	...
	if (!mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base) &&
		...
		(dsi->driver_data->support_pre_urgent & PREURGENT_SUPPORT_CMD) && dbg_preurgent) {
		...
		DDPMSG("datarate: %d\n", dsi->data_rate);
		DDPMSG("0x%04x: 0x%08x\n", DSI_BUF_CON0(dsi->driver_data),
			readl(dsi->regs + DSI_BUF_CON0(dsi->driver_data)));
		DDPMSG("0x%04x: 0x%08x\n", DSI_BUF_PREURGENT_HIGH(dsi->driver_data),
			readl(dsi->regs + DSI_BUF_PREURGENT_HIGH(dsi->driver_data)));
	}
}
...
static void mtk_dsi_calc_vdo_timing(struct mtk_dsi *dsi)
{
	...
	if (dsi->ext->params->is_cphy) {
		...
		if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_BURST) {
			if (priv->data->mmsys_id == MMSYS_MT6991)
				horizontal_frontporch_byte = t_hfp * dsi_tmp_buf_bpp - 18;
			else
				horizontal_frontporch_byte =
					ALIGN_TO((t_hfp * dsi_tmp_buf_bpp - 18), 4);
		} else {
			if (priv->data->mmsys_id == MMSYS_MT6991)
		...
		if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_SYNC_PULSE) {
			...
			if (dsi->ext && dsi->ext->params->vdo_per_frame_lp_enable) {
				...
				pr_notice("[####] hsyn=0x%x chbp=0x%x hfp=0x%x ps_wc=0x%x lanes=%d\n",
						...
						horizontal_frontporch_byte, ps_wc, dsi->lanes);
			...
			if (dsi->ext && dsi->ext->params->vdo_per_frame_lp_enable) {
				if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_BURST) {
					...
					ps_wc = readl(dsi->regs + DSI_PSCTRL(dsi->driver_data)) & 0x7fff;
					bllp_wc = readl(dsi->regs + DSI_BLLP_WC(dsi->driver_data)) & 0xfff;
					pr_notice("[####] hbp=0x%x hfp=0x%x ps_wc=0x%x bllp_wc=0x%x lanes=%d\n",
							horizontal_backporch_byte, horizontal_frontporch_byte,
							ps_wc, bllp_wc, dsi->lanes);
...
static void mtk_output_en_doze_switch(struct mtk_dsi *dsi)
{
	...
	if (doze_enabled && panel_funcs->doze_enable_start)
		...
		if (oplus_dc_is_support() && panel_funcs->set_seed) {
			panel_funcs->set_seed(dsi, mtk_dsi_cmdq_pack_gce, NULL, seed_mode);
		}
		if(oplus_project_public_brightness_support() && (!oplus_ofp_backlight_filter(dsi->encoder.crtc, NULL, oplus_display_brightness))) {
			panel_funcs->oplus_set_backlight_cmdq(dsi, mtk_dsi_cmdq_pack_gce, NULL, oplus_display_brightness);
		}
	...
	int force_lcm_update)
{
	...
	if (!dsi->driver_data) {
		DDPPR_ERR("%s,dsi->driver_data error\n", __func__);
...
#ifdef OPLUS_FEATURE_DISPLAY
			if(dsi->ext->params->oplus_mipi_6382_mmsys_support){
				cmdq_pkt_sleep_by_poll(handle, CMDQ_US_TO_TICK(30));
			} else {
#endif /* OPLUS_FEATURE_DISPLAY */
			...
			struct mtk_ddic_dsi_msg *cmd_msg)
{
	...
	if (dsi_mode == 0) { /* CMD mode HS/LP */
		...
		// for dual port mode, alway send cmd to dual port
		if (dsi->slave_dsi) {
			cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base,
					dsi->slave_dsi->ddp_comp.regs_pa + DSI_CON_CTRL(dsi->driver_data),
					0, DSI_DUAL_EN);
...
#ifdef OPLUS_FEATURE_DISPLAY
	if (params->dyn_fps.dfps_cmd_num) {
		DDPMSG("%s dfps_cmd_num=%d, send cmd by HS pack\n", __func__, params->dyn_fps.dfps_cmd_num);
		for (i = 0; i < params->dyn_fps.dfps_cmd_num; i++) {
			dfps_cmd = &params->dyn_fps.dfps_cmd_table[i];

			send_cmd_to_ddic.mtk_ddic_cmd_table[i].cmd_num = dfps_cmd->cmd_num;
			send_cmd_to_ddic.mtk_ddic_cmd_table[i].para_list =  dfps_cmd->para_list;
		}
		send_cmd_to_ddic.is_hs = 1;
		send_cmd_to_ddic.is_package = 1;
		send_cmd_to_ddic.cmd_count = params->dyn_fps.dfps_cmd_num;
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (fps_chg_index & MODE_DSI_HFP) {
		...
		if (dsi->ext && dsi->ext->params &&
			...
			if (dsi->slave_dsi && dsi->ext->params->lcm_cmd_if == MTK_PANEL_DUAL_PORT)
				cmdq_pkt_write(handle, dsi->slave_dsi->ddp_comp.cmdq_base,
					dsi->slave_dsi->ddp_comp.regs_pa + DSI_TIME_CON1(dsi->driver_data),
					0x0, 0x80000000);
		}
		...
		if (dsi && dsi->ext && dsi->ext->params
			...
			vbp = adjusted_mode.vtotal -
				adjusted_mode.vsync_end;
		dsi->vm.vback_porch = vbp;
		if (dsi->slave_dsi)
			dsi->slave_dsi->vm.vback_porch = vbp;
		...
		if (dsi && dsi->ext && dsi->ext->params
			...
			/* for crtc first enable,dyn fps fail*/
			if (dsi->data_rate == 0) {
				DDPINFO("%s change vfp dsi->data_rate=0\n", __func__);
				dsi->data_rate = mtk_dsi_default_rate(dsi);
				mtk_mipi_tx_pll_rate_set_adpt(dsi->phy, dsi->data_rate);
				if (dsi->ext->params->data_rate_khz)
					mtk_mipi_tx_pll_rate_khz_set_adpt(dsi->phy,
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DC_POST_EXIT:
	{
		...
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (dsi->ext && dsi->ext->funcs
			&& dsi->ext->funcs->lcm_dc_post_exitd_pack) {
			DDPPR_ERR("%s lcm_dc_post_exitd\n", __func__);
			dsi->ext->funcs->lcm_dc_post_exitd_pack(dsi,
...
#ifdef OPLUS_FEATURE_DISPLAY
	dsi->oplus_is_tablet_panel = of_property_read_bool(dev->of_node,
					      "is-tablet-screen-panel");
	pr_err("%s:oplus_is_tablet_panel=%d\n",__func__,dsi->oplus_is_tablet_panel);
	mtk_panel_lock();
#endif /* OPLUS_FEATURE_DISPLAY */
...
dcs_write_gce oplus_dsi_display_get_dcs_write_gce_dyn(void) {
        return mipi_dsi_dcs_write_gce_dyn;
}
#endif
