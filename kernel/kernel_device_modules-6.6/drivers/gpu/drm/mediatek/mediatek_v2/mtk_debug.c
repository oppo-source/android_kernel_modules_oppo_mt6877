...
int oplus_i2c_setbl(unsigned int level)
{
	struct mtk_panel_funcs *panel_ext_funcs = NULL;
	struct drm_crtc *crtc = NULL;
	int ret = 0;
	...
	panel_ext_funcs = mtk_drm_get_lcm_ext_funcs(crtc);

	return panel_ext_funcs->set_backlight_cmdq(NULL, NULL, NULL, level);
}
EXPORT_SYMBOL(oplus_i2c_setbl);
