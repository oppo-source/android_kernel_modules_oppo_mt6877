...
unsigned int oplus_esd_enable = 1;
EXPORT_SYMBOL(oplus_esd_enable);
#endif /* OPLUS_FEATURE_DISPLAY */
...
int _mtk_esd_check_read(struct drm_crtc *crtc)
{
	...
	if (mtk_drm_is_idle(crtc) && mtk_dsi_is_cmd_mode(output_comp))
		...
		pr_err("[esd]%s oplus_is_tablet_panel=%d\n", __func__,dsi->oplus_is_tablet_panel);
...
static int mtk_drm_esd_check_worker_kthread(void *data)
{
	...
	while (1) {
		...
		if (panel_ext->params->oplus_lcd_display_off_esdcheck) {
			if (oplus_esd_enable == 0) {
				DDPINFO("now display_off need disable esd check");
				continue;
			}
		}
		esd_ctx->chk_retry = 0;
