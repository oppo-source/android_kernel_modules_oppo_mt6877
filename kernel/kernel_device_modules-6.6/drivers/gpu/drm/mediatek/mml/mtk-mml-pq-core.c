...
static void release_pq_task(struct kref *ref)
{
	...
	if (pq_task->aal_hist[0]) {
		mml_pq_msg("%s aal_hist[%p]", __func__, pq_task->aal_hist[0]);
		kfree(pq_task->aal_hist[0]);
		pq_task->aal_hist[0] = NULL;
	}
	if (pq_task->aal_hist[1]) {
		mml_pq_msg("%s aal_hist[%p]", __func__, pq_task->aal_hist[1]);
	...
	if (pq_task->hdr_hist[0]) {
		mml_pq_msg("%s hdr_hist[%p]", __func__, pq_task->hdr_hist[0]);
		kfree(pq_task->hdr_hist[0]);
		pq_task->hdr_hist[0] = NULL;
	}
	if (pq_task->hdr_hist[1]) {
		mml_pq_msg("%s hdr_hist[%p]", __func__, pq_task->hdr_hist[1]);
