...
static int ufs_mtk_init(struct ufs_hba *hba)
{
	...
	if (!of_property_read_bool(np, "mediatek,manual_wb_buf_flush_enabled")) {
...
#if IS_ENABLED(CONFIG_UFSFEATURE)
		if (hba->dev_info.wmanufacturerid == UFS_VENDOR_SAMSUNG) {
			err = ufsf_suspend(ufs_host_get_ufsf(hba), pm_op == UFS_SYSTEM_PM);
			if (err)
				return err;
		}
#endif
...
static void ufs_mtk_fixup_dev_quirks(struct ufs_hba *hba)
{
	...
	if (hba->dev_info.wmanufacturerid == UFS_VENDOR_SAMSUNG) {
		...
		ufsf_set_init(hba);
		ufs_host_exception_event_hook(hba);
	}
#endif
}
