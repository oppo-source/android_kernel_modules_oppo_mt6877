...
const struct ufshid_offset ufshid_idn[] = {
	/* Attribute */
	[QUERY_ATTR_IDN_HID_OP_IDX] = { 0x35 },
	[QUERY_ATTR_IDN_HID_AVAIL_SZ_IDX] = { 0x36 },
	[QUERY_ATTR_IDN_HID_SZ_IDX] = { 0x37 },
	[QUERY_ATTR_IDN_HID_PROGRESS_RATIO_IDX] = { 0x38 },
	[QUERY_ATTR_IDN_HID_STATE_IDX] = { 0x39 },
};
...
/* Common functions are now in ufshid-common.h as static inline */
...
static int ufshid_read_attr(struct ufshid_dev *hid, enum ufshid_idn_idx name, u32 *attr_val)
{
...
static int ufshid_write_attr(struct ufshid_dev *hid, enum ufshid_idn_idx name, u32 val)
{
	struct ufs_hba *hba = hid->ufsf->hba;
	int ret = 0;
	u8 idn;
...
void ufshid_jedec_get_dev_info(struct ufsf_feature *ufsf)
{
	...
	if (!(get_unaligned_be32(desc_buf + DEVICE_DESC_PARAM_EXT_UFS_FEATURE_SUP) &
	      UFS_FEATURE_SUPPORT_JEDEC_HID_BIT)) {
		INFO_MSG("bUFSExFeaturesSupport: JEDEC HID not support");
		goto err_out;
	}
...
{
	ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_OP_IDX, HID_OP_DISABLE);
}
...
static void ufshid_trigger_work_fn(struct work_struct *dwork)
{
	...
	HID_DEBUG(hid, "start hid_trigger_work_fn (JEDEC version)");
	...
	op = hid->is_analyze ? HID_OP_ANALYZE : HID_OP_EXECUTE;
	ret = ufshid_write_attr(hid, QUERY_ATTR_IDN_HID_OP_IDX, op);

	if (!ret && ufshid_is_in_progress(hid)) {
		u32 attr_val;
		...
		ERR_MSG("HID is in progress (%u %%), so re-sched (%d ms)",
			  attr_val, hid->hid_trigger_delay);
		goto resched;
	}
...
void ufshid_jedec_init(struct ufsf_feature *ufsf)
{
	...
	INFO_MSG("HID_INIT_START (JEDEC version)");
...
/* ufshid_reset_host is now in ufshid-common.h as static inline */
					  ...
					  const char *buf, size_t count)
{
	...
	ret = ufshid_trigger_control(hid, val);
	if (ret)
		return ret;
