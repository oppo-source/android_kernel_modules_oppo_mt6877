...
/* Version info */
#define UFSFEATURE_DD_VER			0x010103
...
#endif
#ifdef CONFIG_UFSTP
	struct ufstp_dev_info tp_dev_info;
	struct ufstp_dev *tp_dev;
	atomic_t tp_state;
#endif
#ifdef CONFIG_UFSXLC
	atomic_t xlc_state;
	struct ufsxlc_dev *xlc_dev;
#endif
	u32 kernel_ver;
...
/* HID state management inline functions */
#ifdef CONFIG_UFS_JEDEC_HID
static inline int ufshid_get_state(struct ufsf_feature *ufsf)
{
	return atomic_read(&ufsf->hid_state);
}

static inline void ufshid_set_state(struct ufsf_feature *ufsf, int state)
{
	atomic_set(&ufsf->hid_state, state);
}

static inline void ufshid_set_init_state(struct ufsf_feature *ufsf)
{
	ufshid_set_state(ufsf, HID_NEED_INIT);
}

static inline void ufshid_reset_host(struct ufsf_feature *ufsf)
{
