...
static int ufsf_read_dev_desc(struct ufsf_feature *ufsf, u8 selector)
{
	...
	/* Get HID device info - dynamically select based on device type */
	if (ufsf->samsung_sel == NON_SELECTOR) {
		ufshid_jedec_get_dev_info(ufsf);
#ifdef CONFIG_UFSTP
		ufstp_get_dev_info(ufsf, desc_buf);
#endif
#ifdef CONFIG_UFSXLC
		ufsxlc_get_dev_info(ufsf, desc_buf);
#endif
	} else if (ufsf->samsung_sel == SELECTOR) {
		if (STR_PRFX_EQUAL("KLUFG4LHGC-B0E1", ufsf->hba->dev_info.model)) {
			ufshid_v7_get_dev_info(ufsf, desc_buf);
		}
	}
...
#ifdef CONFIG_UFSTP
	if (ufstp_get_state(ufsf) == TP_NEED_INIT && selector == NON_SELECTOR) {
			ufstp_get_geo_info(ufsf, geo_buf);
	}
#endif
...
inline void ufsf_reset_host(struct ufsf_feature *ufsf)
{
	...
	/* Reset HID - ufshid_reset_host is a common function */
...
#ifdef CONFIG_UFSTP
	INFO_MSG("Run reset_host... tp_state(%d) -> TP_RESET",
		 ufstp_get_state(ufsf));
	if (ufstp_get_state(ufsf) == TP_PRESENT)
		ufstp_reset_host(ufsf);
#endif
#ifdef CONFIG_UFSXLC
	INFO_MSG("run reset_host.. xlc_state(%d) -> XLC_RESET",
		 ufsxlc_get_state(ufsf));
	if (ufsxlc_get_state(ufsf) == XLC_PRESENT)
		ufsxlc_reset_host(ufsf);
#endif
...
inline void ufsf_init(struct ufsf_feature *ufsf)
{
	...
	/* Initialize HID - dynamically select based on device type */
	if (ufshid_get_state(ufsf) == HID_NEED_INIT) {
		if (ufsf->samsung_sel == NON_SELECTOR) {
			/* UFS 4.0/4.1: Use JEDEC standard HID */
			ufshid_jedec_init(ufsf);
		} else if (ufsf->samsung_sel == SELECTOR) {
			/* UFS 2.2/3.1: Use V7 HID */
			ufshid_v7_init(ufsf);
		} else {
			ERR_MSG("Unknown samsung_sel value: %d", ufsf->samsung_sel);
		}
	}
#ifdef CONFIG_UFSTP
	if (ufstp_get_state(ufsf) == TP_NEED_INIT)
		ufstp_init(ufsf);
#endif
#ifdef CONFIG_UFSXLC
	if (ufsxlc_get_state(ufsf) == XLC_NEED_INIT)
		ufsxlc_init(ufsf);
#endif
...
{
	/* Reset HID - dynamically select based on device type */
	if (ufshid_get_state(ufsf) == HID_RESET) {
		if (ufsf->samsung_sel == NON_SELECTOR) {
			ufshid_jedec_reset(ufsf);
		} else if (ufsf->samsung_sel == SELECTOR) {
			ufshid_v7_reset(ufsf);
		}
	}
#ifdef CONFIG_UFSTP
	if (ufstp_get_state(ufsf) == TP_RESET)
		ufstp_reset(ufsf);
#endif
#ifdef CONFIG_UFSXLC
	if(ufsxlc_get_state(ufsf) == XLC_RESET)
		ufsxlc_reset(ufsf);
#endif
}
...
{
	/* Remove HID - dynamically select based on device type */
	if (ufshid_get_state(ufsf) == HID_PRESENT) {
		if (ufsf->samsung_sel == NON_SELECTOR) {
			ufshid_jedec_remove(ufsf);
		} else if (ufsf->samsung_sel == SELECTOR) {
			ufshid_v7_remove(ufsf);
		}
	}
#ifdef CONFIG_UFSTP
	if (ufstp_get_state(ufsf) == TP_PRESENT)
...
#ifdef CONFIG_UFSTP
	ufstp_set_state(ufsf, TP_NEED_INIT);
#endif
#ifdef CONFIG_UFSXLC
	ufsxlc_set_state(ufsf, XLC_NEED_INIT);
#endif
}
...
inline int ufsf_suspend(struct ufsf_feature *ufsf, bool is_system_pm)
{
	...
	/* Suspend HID - dynamically select based on device type */
	if (ufshid_get_state(ufsf) == HID_PRESENT) {
		if (ufsf->samsung_sel == NON_SELECTOR) {
			ufshid_jedec_suspend(ufsf, is_system_pm);
		} else if (ufsf->samsung_sel == SELECTOR) {
			ufshid_v7_suspend(ufsf, is_system_pm);
		}
	}
	return 0;
}
...
{
	/* Resume HID - dynamically select based on device type */
	if (ufshid_get_state(ufsf) == HID_SUSPEND) {
		if (ufsf->samsung_sel == NON_SELECTOR) {
			ufshid_jedec_resume(ufsf);
		} else if (ufsf->samsung_sel == SELECTOR) {
			ufshid_v7_resume(ufsf, is_link_off);
		}
	}
}
