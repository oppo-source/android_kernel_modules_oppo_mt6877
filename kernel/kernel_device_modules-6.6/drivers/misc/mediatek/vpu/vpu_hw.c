...
struct vup_service_info {
	...
	struct vpu_shared_memory *sg_reset_vector;
	struct vpu_shared_memory *sg_main_program;
	struct vpu_shared_memory *sg_algo_binary_data;
	struct vpu_shared_memory *sg_iram_data;
	uint64_t vpu_base;
	uint64_t bin_base;
...
static struct vpu_shared_memory *core_shared_data; /* shared data for all cores */
...
/* mtcmos */
#ifndef VPU_MTCMOS_USE_GENPD
static struct clk *mtcmos_dis;//replace with genpd in kernel-6.6
#endif
...
static struct mtk_pm_qos_request vpu_qos_emi_request[MTK_VPU_CORE];
#endif
		...
		if (ret > 0) {
			...
			do {
				...
				if (PWAITMODE & 0x1) {
					...
					jump_out = true;
					LOG_DBG(
						"[vpu_%d] %s PWAITMODE status(%d), ret(%d)\n",
						core, __func__, PWAITMODE, ret);
				} else {
					LOG_WRN(
...
#define ENABLE_VPU_CLK(clk) \
	...
	#else
	pm_runtime_get_sync(vpu_dev->dev[0]);
	#endif
...
#define DISABLE_VPU_MTCMOS(clk) \
	...
	#else
	pm_runtime_put_sync(vpu_dev->dev[0]);
	#endif
...
#if IS_ENABLED(CONFIG_MTK_PMQOS)
	mtk_pm_qos_update_request(&vpu_qos_vcore_request[core], MTK_PM_QOS_VCORE_OPP_DEFAULT_VALUE);
#else
#ifdef MMDVFS
	ret = mmdvfs_set_fine_step(MMDVFS_SCEN_VPU_KERNEL,
...
out:
		...
		for (i = 0 ; i < req->buffer_count ; i++) {
			for (j = 0 ; j < req->buffers[i].plane_count ; j++) {
				vbuf_free_handle(vpu_dev, req->buf_ion_infos[cnt]);
				cnt++;
			}
		}
...
static void vpu_unmap_mva_of_bin(struct vup_service_info *vpu_info)
{
	if (vpu_info->sg_iram_data) {
		vpu_free_shared_memory(vpu_info->sg_iram_data);
		vpu_info->sg_iram_data = NULL;
		vpu_info->iram_data_mva = 0;
	}

	if (vpu_info->sg_main_program) {
		vpu_free_shared_memory(vpu_info->sg_main_program);
		vpu_info->sg_main_program = NULL;
	}

	if (vpu_info->sg_reset_vector) {
		vpu_free_shared_memory(vpu_info->sg_reset_vector);
		vpu_info->sg_reset_vector = NULL;
	}
}
...
static int vpu_map_mva_of_bin(int core, uint64_t bin_pa)
{
	...
	struct vup_service_info *vpu_info = &vpu_service_cores[core];
	const uint32_t size_algos =
	...
	mem_param.kva_addr = vpu_info->bin_base +
				VPU_OFFSET_RESET_VECTOR +
				VPU_DDR_SHIFT_RESET_VECTOR * (uint64_t)core;
	ret = vpu_alloc_shared_memory(&vpu_info->sg_reset_vector, &mem_param);
	LOG_DBG("reset vector va (0x%llx),pa(0x%x)",
			vpu_info->sg_reset_vector->va,
			vpu_info->sg_reset_vector->pa);
	if (ret) {
	...
	mem_param.kva_addr = vpu_info->bin_base +
				VPU_OFFSET_MAIN_PROGRAM +
				VPU_DDR_SHIFT_RESET_VECTOR * (uint64_t)core;
	ret = vpu_alloc_shared_memory(&vpu_info->sg_main_program, &mem_param);
	LOG_DBG("main program va (0x%llx),pa(0x%x)",
			vpu_info->sg_main_program->va,
			vpu_info->sg_main_program->pa);
	if (ret) {
	...
	if (core == 0) {
		...
		mem_param.phy_addr = bin_pa + VPU_OFFSET_ALGO_AREA;
		mem_param.kva_addr = vpu_info->bin_base +
					VPU_OFFSET_ALGO_AREA;

		ret = vpu_alloc_shared_memory(&vpu_info->sg_algo_binary_data,
									&mem_param);
		if (ret) {
		...
		vpu_info->algo_data_mva = vpu_info->sg_algo_binary_data->pa;
		LOG_DBG("a vpu va_algo_data pa: 0x%x\n",
			(unsigned int)(bin_pa + VPU_OFFSET_ALGO_AREA));
		LOG_DBG("a vpu va_algo_data: 0x%llx, size: 0x%x\n",
			vpu_info->algo_data_mva, size_algos);
	} else {
		vpu_info->algo_data_mva =
	...
	ret = vpu_alloc_shared_memory(&vpu_info->sg_iram_data, &mem_param);
	LOG_DBG("iram data va (0x%llx),pa(0x%x)",
			vpu_info->sg_iram_data->va,
			vpu_info->sg_iram_data->pa);
	if (ret) {
		LOG_ERR("fail to allocate iram data!\n");
		goto out;
	}

	vpu_service_cores[core].iram_data_mva = (uint64_t)(vpu_info->sg_iram_data->pa);

out:
	if (ret)
		vpu_unmap_mva_of_bin(vpu_info);
...
int vpu_init_hw(int core, struct vpu_device *device)
{
	...
	if (core == 0) {
		...
		efuse_data = 0x0;//(get_devinfo_with_index(3) & 0xC00) >> 10;
		...
		for (i = 0 ; i < MTK_VPU_CORE ; i++) {
			...
			ret = vpu_alloc_shared_memory(&(vpu_service_cores[i].exec_kernel_lib),
				&mem_param);
...
int vpu_uninit_hw(void)
{
	...
	if (core_shared_data) {
		vpu_free_shared_memory(core_shared_data);
		core_shared_data = NULL;
	}
	...
	for (i = 0 ; i < MTK_VPU_CORE ; i++) {
		...
		if (vpu_service_cores[i].exec_kernel_lib) {
			vpu_free_shared_memory(vpu_service_cores[i].exec_kernel_lib);
		...
		vpu_unmap_mva_of_bin(&vpu_service_cores[i]);
	}
...
#if IS_ENABLED(CONFIG_MTK_GZ_SUPPORT_SDSP)
		mtk_pm_qos_remove_request(&vpu_qos_emi_request[i]);
#endif
	}
	#endif
...
{
	int ret = 0;
#ifdef VPU_EANBLE_JTAG
	int TEMP_CORE = 0;

	vpu_get_power(TEMP_CORE, false);
#ifdef VPU_EANBLE_JTAG_TEST
...
int vpu_hw_set_debug(int core)
{
	...
	vpu_write_field(core, FLD_CTL_INT, 1);
	LOG_DBG(
		"debug timestamp: %.2llu:%.2llu:%.2llu:%.6lu\n",
		(now.tv_sec / 3600) % (24),
		(now.tv_sec / 60) % (60),
		now.tv_sec % 60,
		now.tv_nsec / 1000);
...
int vpu_hw_enque_request(int core, struct vpu_request *request)
{
	...
	mtk_pm_qos_update_request(&vpu_qos_bw_request[core], PM_QOS_DEFAULT_VALUE);
	#endif
	vpu_trace_end();
	#if defined(VPU_MET_READY)
...
static void sync_for_device(struct vpu_kernel_buf *vkbuf)
{
	struct sg_table *sg = NULL;

	sg = &(vkbuf->sg);
	dma_sync_sg_for_device(vpu_dev->dev[0], sg->sgl, sg->nents, DMA_BIDIRECTIONAL);
}

static void sync_for_cpu(struct vpu_kernel_buf *vkbuf)
{
	struct sg_table *sg = NULL;

	sg = &(vkbuf->sg);
	dma_sync_sg_for_cpu(vpu_dev->dev[0], sg->sgl, sg->nents, DMA_BIDIRECTIONAL);
}
...
int vpu_hw_processing_request(int core, struct vpu_request *request)
{
	...
	if (need_reload) {
		...
		if (ret) {
			...
			LOG_DBG("[vpu_%d] FLD_RUN_STALL position 1\n", core);
			/* RUN_STALL pull up to avoid fake cmd */
			vpu_write_field(core, FLD_RUN_STALL, 1);
		}
		vpu_trace_end();
	...
	if (g_vpu_log_level > VpuLogThre_DUMP_BUF_MVA)
		vpu_dump_buffer_mva(request);
	LOG_INF(
		"[vpu_%d] start d2d, id/frm (%d/%d), bw(%d), algo(%d/%d, %d), bf(%d) work_pa(0x%x)WAKETOBACK in\n",
		core,
	...
	sync_for_device(vpu_service_cores[core].work_buf->vkbuf);
	/* 1. write register */
	/* command: d2d */
	vpu_write_field(
	...
	if (ret) {
		...
		LOG_DBG("[vpu_%d] FLD_RUN_STALL position 2\n", core);
		/* RUN_STALL pull up to avoid fake cmd */
		vpu_write_field(core, FLD_RUN_STALL, 1);
	}
...
int vpu_hw_get_algo_info(int core, struct vpu_algo *algo)
{
	...
	/* RUN_STALL pull up to avoid fake cmd */
	vpu_write_field(core, FLD_RUN_STALL, 1);
	sync_for_cpu(vpu_service_cores[core].work_buf->vkbuf);
				...
				struct vpu_shared_memory_param *param)
{
	...
	*shmem = kzalloc(sizeof(struct vpu_shared_memory), GFP_KERNEL);
	...
	vkbuf = vbuf_kmap_phy_iova(vpu_dev, map_case, param->phy_addr,
...
out:
	vbuf_kunmap_phy_iova(vpu_dev, (*shmem)->vkbuf);
out1:
	if (*shmem) {
		kfree(*shmem);
		*shmem = NULL;
...
int vpu_dump_power(struct seq_file *s)
{
	...
	vpu_print_seq(s,
		"dvfs_debug(rw): vore[%d/%d], hw_vcore[%d], dsp[%d], ipu_if[%d], dsp1[%d], dsp2[%d]\n",
		opps.vcore.index,
		vcore_opp,
		hw_vcore,
		opps.dsp.index,
