...
static void *vpu_dc_vaddr(void *buf_priv)
{
	...
	if (!buf->vaddr && buf->db_attach)
		if (dma_buf_vmap(buf->db_attach->dmabuf, &sys_map) == 0) {
			buf->vaddr = sys_map.vaddr;
			buf->sys_map = sys_map;
		}

	LOG_DBG("%s: add:0x%p, sys_map.vaddr:0x%p\n", __func__, buf->vaddr, sys_map.vaddr);
	return buf->vaddr;
}
...
static int vpu_dc_mmap(void *buf_priv, struct vm_area_struct *vma)
{
	...
	LOG_DBG("%s: mapped dma addr 0x%08lx at 0x%08lx, size %ld\n",
...
static int vpu_dc_dmabuf_ops_attach(struct dma_buf *dbuf,
					...
					enum dma_data_direction dma_dir)
{
	...
	dma_unmap_sg(db_attach->dev, sgt->sgl, sgt->orig_nents, DMA_BIDIRECTIONAL);
	attach->dma_dir = DMA_NONE;
}
...
static int vpu_dc_dmabuf_ops_vmap(struct dma_buf *dbuf, struct iosys_map *sys_map)
{
	struct vpu_dc_buf *buf = dbuf->priv;

	sys_map->vaddr = buf->vaddr;
...
static void vpu_dc_unmap_dmabuf(void *mem_priv)
{
	...
	if (buf->vaddr) {
		LOG_DBG("dma_buf_vunmap:%p\n", buf->vaddr);
		dma_buf_vunmap(buf->db_attach->dmabuf, &buf->sys_map);
		buf->vaddr = NULL;
	}
