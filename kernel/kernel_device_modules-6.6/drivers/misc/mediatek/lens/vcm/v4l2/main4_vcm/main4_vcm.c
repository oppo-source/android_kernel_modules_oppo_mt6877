...
static DEFINE_MUTEX(main4_vcm_mutex);
...
static void stop_background_works(struct main4_vcm_device *main4_vcm)
{
	LOG_INF("+\n");
	main4_vcm->is_stop_flag = true;

	wake_up_interruptible(&main4_vcm->wait_queue_head);

	/* main4_vcm Workqueue */
	if (main4_vcm_init_wq) {
		LOG_INF("flush work queue\n");

		/* flush work queue */
		flush_work(&main4_vcm->poweroff_work);

		flush_workqueue(main4_vcm_init_wq);
		destroy_workqueue(main4_vcm_init_wq);
		main4_vcm_init_wq = NULL;
	}

	main4_vcm->is_stop_flag = false;
...
static int main4_vcm_release(struct main4_vcm_device *main4_vcm)
{
	...
	for (i = 0; i < nStep_count_long; ++i) {
		...
		ret = wait_event_interruptible_timeout(main4_vcm->wait_queue_head,
				main4_vcm->is_stop_flag,
...
LAST_STEP_TO_ORIGIN:
	...
	ret = main4_vcm_set_position(main4_vcm, g_vcmconfig.vcm_config.origin_focus_pos);
	if (ret < 0) {
		LOG_INF("%s I2C failure: %d\n",
			__func__, ret);
		return ret;
...
static void main4_vcm_poweroff_work(struct work_struct *work)
{
	struct main4_vcm_device *main4_vcm = NULL;
	int ret = 0;

	LOG_INF("+\n");

	main4_vcm = container_of(work, struct main4_vcm_device, poweroff_work);

	ret = main4_vcm_power_off(main4_vcm);

	LOG_INF("power off main4_vcm, return value:%d\n", ret);
	LOG_INF("-\n");
}
...
static int main4_vcm_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	mutex_lock(&main4_vcm_mutex);

	stop_background_works(main4_vcm);

	if (main4_vcm->is_powered_off) {
		ret = main4_vcm_power_on(main4_vcm);
		if (ret < 0) {
			LOG_INF("power on fail, ret = %d\n", ret);
			mutex_unlock(&main4_vcm_mutex);
			return ret;
		}
	} else {
...
static int main4_vcm_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	/* main4_vcm Workqueue */
	if (main4_vcm_init_wq == NULL) {
		LOG_INF("create_singlethread_workqueue\n");
		main4_vcm_init_wq =
			create_singlethread_workqueue("main4_vcm_init_work");
		if (!main4_vcm_init_wq) {
	...
	queue_work(main4_vcm_init_wq, &main4_vcm->poweroff_work);
	mutex_unlock(&main4_vcm_mutex);
	LOG_INF("-\n");
...
static long main4_vcm_ops_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MTK_S_LENS_INFO:
	{
		...
		if (main4_vcm->is_powered_off == false) {
		...
		main4_vcm->is_powered_off = false;
...
static int main4_vcm_probe(struct i2c_client *client)
{
	...
	init_waitqueue_head(&main4_vcm->wait_queue_head);
	...
	/* init work queue */
	INIT_WORK(&main4_vcm->poweroff_work, main4_vcm_poweroff_work);
