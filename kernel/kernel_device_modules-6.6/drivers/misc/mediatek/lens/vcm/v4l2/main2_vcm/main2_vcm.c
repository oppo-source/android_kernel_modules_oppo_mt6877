...
static void stop_background_works(struct main2_vcm_device *main2_vcm)
{
	LOG_INF("+\n");
	main2_vcm->is_stop_flag = true;

	wake_up_interruptible(&main2_vcm->wait_queue_head);
	...
	if (main2_vcm_init_wq) {
		...
		/* flush work queue */
		flush_work(&main2_vcm->poweroff_work);

		flush_workqueue(main2_vcm_init_wq);
		destroy_workqueue(main2_vcm_init_wq);
		main2_vcm_init_wq = NULL;
	}

	main2_vcm->is_stop_flag = false;
...
static int main2_vcm_release(struct main2_vcm_device *main2_vcm)
{
	...
	for (i = 0; i < nStep_count_long; ++i) {
		...
		ret = wait_event_interruptible_timeout(main2_vcm->wait_queue_head,
				main2_vcm->is_stop_flag,
...
LAST_STEP_TO_ORIGIN:
	...
	ret = main2_vcm_set_position(main2_vcm, g_vcmconfig.vcm_config.origin_focus_pos);
	if (ret < 0) {
		LOG_INF("%s I2C failure: %d\n",
			__func__, ret);
		return ret;
...
static void main2_vcm_poweroff_work(struct work_struct *work)
{
	struct main2_vcm_device *main2_vcm = NULL;
	int ret = 0;

	LOG_INF("+\n");

	main2_vcm = container_of(work, struct main2_vcm_device, poweroff_work);

	ret = main2_vcm_power_off(main2_vcm);

	LOG_INF("power off main2_vcm, return value:%d\n", ret);
	LOG_INF("-\n");
}
...
static long main2_vcm_ops_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MTK_S_LENS_INFO:
	{
		...
		if (main2_vcm->is_powered_off == false) {
		...
		main2_vcm->is_powered_off = false;
...
static int main2_vcm_probe(struct i2c_client *client)
{
	...
	init_waitqueue_head(&main2_vcm->wait_queue_head);
	...
	/* init work queue */
	INIT_WORK(&main2_vcm->poweroff_work, main2_vcm_poweroff_work);
