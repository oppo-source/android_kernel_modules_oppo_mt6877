...
static DEFINE_MUTEX(main3_vcm_mutex);
...
static int main3_vcm_release(struct main3_vcm_device *main3_vcm)
{
	...
	diff_dac = g_vcmconfig.vcm_config.origin_focus_pos - main3_vcm->cur_pos;
	val = main3_vcm->cur_pos;
...
static long main3_vcm_ops_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case VCM_IOC_POWER_ON:
	{
		...
		ret = main3_vcm_set_position(main3_vcm, main3_vcm->cur_pos);
		if (ret < 0) {
			LOG_INF("%s I2C failure: %d\n",
				__func__, ret);
			return ret;
