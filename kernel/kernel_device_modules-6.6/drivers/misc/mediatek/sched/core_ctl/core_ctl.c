...
static int cluster_init(const struct cpumask *mask)
{
	...
	for_each_cpu(cpu, mask) {
		...
		state->cpu = cpu;
		state->cpu_loading = kcalloc(WINDOW_SIZE, sizeof(unsigned int), GFP_KERNEL);
		for (i=0; i<WINDOW_SIZE; i++)
			state->cpu_loading[i] = 0;
