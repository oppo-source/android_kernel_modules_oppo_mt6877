...
#define idle_prof_emerg(fmt, args...)   \
	pr_info(IDLE_PROF_TAG fmt, ##args)
...
#define latency_prof_crit(fmt, args...) \
	pr_info(LATENCY_PROF_TAG fmt, ##args)
...
__weak unsigned int mt_cpufreq_get_cur_freq(unsigned int id)
{
	return 0;
}
...
#if !IS_ENABLED(CONFIG_FPGA_EARLY_PORTING) && IS_ENABLED(CONFIG_MEDIATEK_CPU_DVFS)
#if IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6763) || IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT8788)
