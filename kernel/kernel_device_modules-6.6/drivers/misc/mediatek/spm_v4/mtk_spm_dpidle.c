...
extern int _exec_ccci_kern_func(unsigned int id, char *buf, unsigned int len);
#endif
...
static void spm_trigger_wfi_for_dpidle(struct pwr_ctrl *pwrctrl)
{
	...
	if (spm_dormant_sta < 0)
		pr_info("[name:spm&]dpidle spm_dormant_sta(%d) < 0\n",
				spm_dormant_sta);
}
					   ...
					   u32 operation_cond)
{
	...
	unsigned long dpidle_log_print_curr_time = 0;
...
static struct timespec64 pre_dpidle_time;
...
int dpidle_active_status(void)
{
	...
	if ((current_time.tv_sec - pre_dpidle_time.tv_sec) > DPIDLE_ACTIVE_TIME)
		...
		DPIDLE_ACTIVE_TIME) &&
		(current_time.tv_nsec > pre_dpidle_time.tv_nsec))
		return 0;
	else
		return 1;
}
...
//RESTORE_IRQ:
	...
	ktime_get_real_ts64(&pre_dpidle_time);
