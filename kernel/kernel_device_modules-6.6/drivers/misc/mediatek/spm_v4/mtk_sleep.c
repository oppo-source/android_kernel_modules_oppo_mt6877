...
#define slp_notice(fmt, args...)    \
	pr_notice("[name:spm&][SLP] " fmt, ##args)
#define slp_info(fmt, args...)      \
	pr_info("[name:spm&][SLP] " fmt, ##args)
#define slp_debug(fmt, args...)     \
	pr_debug("[name:spm&][SLP] " fmt, ##args)
...
#endif /* CONFIG_MTK_PLAT_POWER_MT8788 */
...
static int slp_suspend_ops_enter(suspend_state_t state)
{
...
#if !IS_ENABLED(CONFIG_FPGA_EARLY_PORTING) && IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT8788)
	pll_if_on();
	subsys_if_on();
#endif /* CONFIG_FPGA_EARLY_PORTING */
	...
	if (mcdi_task_pause_fun)
		mcdi_task_pause_fun(true); //mcdi_task_pause(true);
	else
...
#if !defined(SPM_K414_EARLY_PORTING)
	if (mcdi_task_pause_fun)
		mcdi_task_pause_fun(false); //mcdi_task_pause(false);
	else
		pr_info("mcdi task revert pause fail");
#endif
