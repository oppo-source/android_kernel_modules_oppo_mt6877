...
__weak void idle_refcnt_inc(void) {}
__weak void idle_refcnt_dec(void) {}
...
//__weak unsigned long localtimer_get_counter(void)
//{
//	return 0;
//}

//__weak int localtimer_set_next_event(unsigned long evt)
//{
//	return 0;
//}
...
unsigned int is_tee_real_drv;

/* idle_notifier */
static RAW_NOTIFIER_HEAD(mtk_idle_notifier);

#if IS_ENABLED(CONFIG_MTK_LEGACY_THERMAL)
void (*mtkTTimer_start_timer_fn)(void);
void (*mtkTTimer_cancel_timer_fn)(void);
int mtkTTimer_start_cancel_timer_register(void *start, void *cancel)
{
	mtkTTimer_start_timer_fn = start;
	mtkTTimer_cancel_timer_fn = cancel;
	return 0;
}
EXPORT_SYMBOL(mtkTTimer_start_cancel_timer_register);

static void mtkTTimer_start_timer(void)
{
	if (!mtkTTimer_start_timer_fn) {
		pr_info("NO %s !!!\n", __func__);
		return ;
	}
	return mtkTTimer_start_timer_fn();
}

static void mtkTTimer_cancel_timer(void)
{
	if (!mtkTTimer_cancel_timer_fn) {
		pr_info("NO %s !!!\n", __func__);
		return ;
	}
	return mtkTTimer_cancel_timer_fn();
}
#endif

static u32 clk_buf_bblpm_enter_cond(void)
{
	u32 bblpm_cond = 0;
...
{
	pr_debug("[name:spm&]SPM_POWER_ON_VAL0     0x%08x\n",
			spm_read(SPM_POWER_ON_VAL0));
	pr_debug("[name:spm&]SPM_POWER_ON_VAL1     0x%08x\n",
			spm_read(SPM_POWER_ON_VAL1));
	pr_debug("[name:spm&]PCM_PWR_IO_EN         0x%08x\n",
			spm_read(PCM_PWR_IO_EN));
	pr_debug("[name:spm&]PCM_REG0_DATA         0x%08x\n",
			spm_read(PCM_REG0_DATA));
	pr_debug("[name:spm&]PCM_REG7_DATA         0x%08x\n",
			spm_read(PCM_REG7_DATA));
	pr_debug("[name:spm&]PCM_REG12_DATA        0x%08x\n",
			spm_read(PCM_REG12_DATA));
	pr_debug("[name:spm&]PCM_REG13_DATA        0x%08x\n",
			spm_read(PCM_REG13_DATA));
	pr_debug("[name:spm&]PCM_REG15_DATA        0x%08x\n",
			spm_read(PCM_REG15_DATA));
	pr_debug("[name:spm&]SPM_MAS_PAUSE_MASK_B  0x%08x\n",
		spm_read(SPM_MAS_PAUSE_MASK_B));
	pr_debug("[name:spm&]SPM_MAS_PAUSE2_MASK_B 0x%08x\n",
		spm_read(SPM_MAS_PAUSE2_MASK_B));
	pr_debug("[name:spm&]SPM_SW_FLAG           0x%08x\n",
			spm_read(SPM_SW_FLAG));
	pr_debug("[name:spm&]SPM_DEBUG_FLAG        0x%08x\n",
			spm_read(SPM_SW_DEBUG));
	pr_debug("[name:spm&]SPM_PC_TRACE_G0       0x%08x\n",
			spm_read(SPM_PC_TRACE_G0));
	pr_debug("[name:spm&]SPM_PC_TRACE_G1       0x%08x\n",
			spm_read(SPM_PC_TRACE_G1));
	pr_debug("[name:spm&]SPM_PC_TRACE_G2       0x%08x\n",
			spm_read(SPM_PC_TRACE_G2));
	pr_debug("[name:spm&]SPM_PC_TRACE_G3       0x%08x\n",
			spm_read(SPM_PC_TRACE_G3));
	pr_debug("[name:spm&]SPM_PC_TRACE_G4       0x%08x\n",
			spm_read(SPM_PC_TRACE_G4));
	pr_debug("[name:spm&]SPM_PC_TRACE_G5       0x%08x\n",
			spm_read(SPM_PC_TRACE_G5));
	pr_debug("[name:spm&]SPM_PC_TRACE_G6       0x%08x\n",
			spm_read(SPM_PC_TRACE_G6));
	pr_debug("[name:spm&]SPM_PC_TRACE_G7       0x%08x\n",
			spm_read(SPM_PC_TRACE_G7));
	pr_debug("[name:spm&]DCHA_GATING_LATCH_0   0x%08x\n",
		spm_read(DCHA_GATING_LATCH_0));
	pr_debug("[name:spm&]DCHA_GATING_LATCH_5   0x%08x\n",
		spm_read(DCHA_GATING_LATCH_5));
	pr_debug("[name:spm&]DCHB_GATING_LATCH_0   0x%08x\n",
		spm_read(DCHB_GATING_LATCH_0));
	pr_debug("[name:spm&]DCHB_GATING_LATCH_5   0x%08x\n",
...
static void is_tee_real_drv_init(void)
{
	...
	if (!np) {
		pr_err("find utos node failed\n");
		is_tee_real_drv = 0;
		goto OUT;
	}

	ret = of_property_read_u32(np, "microtrust,real-drv", &is_tee_real_drv);
	if (ret) {
		pr_err("MICROTRUST device is NOT enable.\n");
		is_tee_real_drv = 0;
		goto OUT;
	}

OUT:
	pr_info("%s is_tee_real_drv:%u\n", __func__, is_tee_real_drv);
}

void mtk_cpuidle_framework_init(void)
{
#if !IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6739)
	pr_info("mtk cpuidle framework init entry!!\n");
#endif
