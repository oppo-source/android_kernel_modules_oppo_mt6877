	...
	if (wakesta->assert_pc != 0) {
		...
		if (!(wakesta->debug_flag1 & SPM_DBG1_DRAM_SREF_ACK_TO))
			aee_kernel_warning("SPM Warning",
					...
					wakesta->debug_flag,
					wakesta->debug_flag1);
#endif /* CONFIG_MTK_PLAT_POWER_MT6763 */
...
__weak int get_dynamic_period(int first_use,
					      int first_wakeup_time,
					      int battery_capacity_level)
{
	/* spm_crit2("[name:spm&]NO %s !!!\n", __func__); */
	return 5401;
}
...
__weak bool mcdi_is_buck_off(int cluster_idx)
{
//	spm_crit2("NO %s !!!\n", __func__);
	return false;
}
__weak bool cpuhp_is_buck_off(int cluster_idx)
{
//	spm_crit2("NO %s !!!\n", __func__);
	return false;
}
