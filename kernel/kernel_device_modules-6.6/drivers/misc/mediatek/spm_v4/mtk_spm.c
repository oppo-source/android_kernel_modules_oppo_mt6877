...
/* TODO: fix */
	...
	!IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6739) && \
...
/* TODO: fix */
#if !defined(SPM_K414_EARLY_PORTING) && IS_ENABLED(CONFIG_MTK_SYS_CIRQ)
...
__weak ssize_t get_spm_last_wakeup_src(char *ToUserBuf, size_t sz, void *priv)
{
	return 0;
}

__weak ssize_t get_spm_sleep_count(char *ToUserBuf, size_t sz, void *priv)
{
	return 0;
}
...
#endif /* CONFIG_MTK_PLAT_POWER_MT6739 */
}
			...
			void *unused)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		spin_unlock_irqrestore(&__spm_lock, flags);
		if (ret < 0) {
			pr_info("[name:spm&]#@# %s(%d) PM_SUSPEND_PREPARE return %d!!!\n",
	...
	case PM_POST_SUSPEND:
		...
		spin_unlock_irqrestore(&__spm_lock, flags);
		if (ret < 0) {
			pr_info("[name:spm&]#@# %s(%d) PM_POST_SUSPEND return %d!!!\n",
			       __func__, __LINE__, ret);
			return NOTIFY_BAD;
		}
		return NOTIFY_DONE;
...
static void __spm_check_dram_type(void)
{
	...
	pr_info("[name:spm&]#@# %s(%d) __spmfw_idx 0x%x\n",
...
static void __spm_check_dram_type(void)
{
	...
	pr_info("[name:spm&]#@# %s(%d) __spmfw_idx 0x%x (type:%d freq:%d)\n",
...
#endif /* CONFIG_MTK_DRAMC */
#elif IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT8788) && IS_ENABLED(CONFIG_MTK_DRAMC) //DRAMC NEW ARCHITECTURE on MT8788
static void __spm_check_dram_type(void)
{
	/* ddr api changed on mt8788 */
...
/* TODO: fix */
	...
	if (ret) {
		pr_info("[name:spm&]fail to register platform driver\n");
		return ret;
	}

	pspmdev = platform_device_register_simple("spm", -1, NULL, 0);
	if (IS_ERR(pspmdev)) {
		pr_info("[name:spm&]Failed to register platform device.\n");
		return -EINVAL;
	}
	...
	if (ret) {
		pr_info("[name:spm&]Failed to register PM notifier.\n");
		return ret;
	}
#endif /* CONFIG_PM */
#endif /* CONFIG_FPGA_EARLY_PORTING */
...
#endif
#endif
	pr_info("[name:spm&]#@# %s(%d) is_ext_buck_exist() 0x%x\n",
...
void spm_pmic_power_mode(int mode, int force, int lock)
{
	...
	if (mode < PMIC_PWR_NORMAL || mode >= PMIC_PWR_NUM) {
		pr_info("[name:spm&]wrong spm pmic power mode");
		return;
	}
...
int spm_to_sspm_command_async(u32 cmd, struct spm_data *spm_d)
{
	...
	case SPM_LEAVE_SODI3:
		...
		if (ret != 0)
			pr_info("[name:spm&]#@# %s(%d) sspm_ipi_send_async(cmd:0x%x) ret %d\n",
			       __func__, __LINE__, cmd, ret);
		break;
	default:
		pr_info("[name:spm&]#@# %s(%d) cmd(%d) wrong!!!\n",
...
int spm_to_sspm_command_async_wait(u32 cmd)
{
	...
	case SPM_LEAVE_SODI3:
		...
		if (ret != 0) {
			pr_info("[name:spm&]#@# %s(%d) sspm_ipi_send_async_wait(cmd:0x%x) ret %d\n",
			       __func__, __LINE__, cmd, ret);
		} else if (ack_data < 0) {
			ret = ack_data;
			pr_info("[name:spm&]#@# %s(%d) cmd(%d) return %d\n",
			       __func__, __LINE__, cmd, ret);
		}
		break;
	default:
...
int spm_to_sspm_command(u32 cmd, struct spm_data *spm_d)
{
	...
	case SPM_LEAVE_SODI3:
		...
		ret = sspm_ipi_send_sync(IPI_ID_SPM_SUSPEND,
					 ...
					 &ack_data, 1);
		if (ret != 0) {
			pr_info("[name:spm&]#@# %s(%d) sspm_ipi_send_sync(cmd:0x%x) ret %d\n",
			       __func__, __LINE__, cmd, ret);
		} else if (ack_data < 0) {
			ret = ack_data;
			pr_info("[name:spm&]#@# %s(%d) cmd(%d) return %d\n",
