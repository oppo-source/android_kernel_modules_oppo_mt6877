...
#if IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6775) || IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT8788)
...
{
#if IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6758)
	int value;
	...
	return value;
#elif IS_ENABLED(CONFIG_MTK_PLAT_POWER_MT6775)
	return 0;
#else
...
EXPORT_SYMBOL(is_vcorefs_feature_enable);
...
EXPORT_SYMBOL(vcorefs_vcore_dvs_en);
...
EXPORT_SYMBOL(vcorefs_dram_dfs_en);
...
EXPORT_SYMBOL(vcorefs_i_hwpath_en);
...
int vcorefs_get_curr_vcore(void)
{
	...
	regmap_read(pmic_regmap, MT6358_BUCK_VCORE_ELR0, &vcore);
...
#endif
#if IS_ENABLED(CONFIG_MTK_DRAMC)
	ddr_khz = mtk_dramc_get_data_rate() * 1000;
#endif
...
EXPORT_SYMBOL(vcorefs_get_vcore_by_steps);

//refer: build_vcore_opp_table
int vcorefs_get_ddr_by_steps(u32 opp)
{
	int ddr_khz;
	int ddr_type = __spm_get_dram_type();
#if IS_ENABLED(CONFIG_MTK_DRAMC_LEGACY)
	...
	switch (opp) {
	case 0:
		ddr_khz = mtk_dramc_get_steps_freq(opp) * 1000;
		break;
	case 1:
		if (ddr_type == SPMFW_LP4X_2CH_3200)
			ddr_khz = mtk_dramc_get_steps_freq(opp-1) * 1000;
		else if (ddr_type == SPMFW_LP4X_2CH_3733 || ddr_type == SPMFW_LP4_2CH_2400)
			ddr_khz = mtk_dramc_get_steps_freq(opp) * 1000;
...
EXPORT_SYMBOL(vcorefs_get_ddr_by_steps);
...
EXPORT_SYMBOL(dvfsrc_update_sspm_vcore_opp_table);
...
EXPORT_SYMBOL(dvfsrc_update_sspm_ddr_opp_table);
...
EXPORT_SYMBOL(vcorefs_init_opp_table);
...
EXPORT_SYMBOL(vcorefs_module_init);
