...
static void apu_setup_apummu(struct mtk_apu *apu, int boundary, int ns, int domain)
{
	...
	if (apu->platdata->flags & F_SECURE_BOOT) {
		...
		if (apu->platdata->flags & F_FPGA_EP)
			rv_boot(0, 0, hw_logger_addr, eAPUMMU_PAGE_LEN_1MB,
...
static int mt6991_rproc_start(struct mtk_apu *apu)
{
	int ns = 1; /* Non Secure */
	int domain = 0;
...
static void mt6991_apu_pwr_wake_unlock(struct mtk_apu *apu, uint32_t id)
{
#if IS_ENABLED(CONFIG_PM_SLEEP)
	unsigned long flags;
		...
		uint8_t usr_bit, uint8_t ctl, int32_t timeout)
{
	...
	/* dev_info(dev, "%s ++ usr_bit:%d ctl:%d (APU_MBOX_SEMA0_CTRL = 0x%08x)\n",
	 *		__func__, usr_bit, ctl,
	 *		ioread32(apu->apu_mbox + APU_MBOX_SEMA0_CTRL));
	 */

	iowrite32(ctl_bit, apu->apu_mbox + APU_MBOX_OFFSET(usr_bit) + APU_MBOX_SEMA0_CTRL);
	...
	while ((ioread32(apu->apu_mbox + APU_MBOX_OFFSET(usr_bit) + APU_MBOX_SEMA0_STA) & BIT(usr_bit))
			!= BIT(usr_bit)) {

		if (timeout >= 0 && timeout_cnt++ >= timeout) {
			dev_info(dev,
			"%s timeout usr_bit:%d ctl:%d rnd:%d (APU_MBOX_SEMA0_STA = 0x%08x)\n",
				__func__, usr_bit, ctl, timeout_cnt,
				ioread32(apu->apu_mbox + APU_MBOX_OFFSET(usr_bit) + APU_MBOX_SEMA0_STA));
			return -EBUSY;
		}
	...
	dev_info(dev, "%s -- usr_bit:%d ctl:%d (APU_MBOX_SEMA0_STA = 0x%08x)(%u)\n",
			__func__, usr_bit, ctl,
			ioread32(apu->apu_mbox + APU_MBOX_OFFSET(usr_bit) + APU_MBOX_SEMA0_STA), timeout_cnt);
...
int apu_polling_rpc_pwr_off(struct mtk_apu *apu)
{
	int timeout_cnt = 0;
	int timeout = 1200000;
	struct device *dev = apu->dev;
	...
	while (ioread32(apu->apu_rpc + 0x44) != 0) {
		if (timeout >= 0 && timeout_cnt++ >= timeout) {
			dev_info(dev, "%s timeout rnd:%d (APU RPC 0x44 = 0x%08x)\n",
					__func__, timeout_cnt, ioread32(apu->apu_rpc + 0x44));
			return -EBUSY;
		}
		udelay(1);
	}
...
int apu_infra_check_last_pwr_vote(struct mtk_apu *apu, enum apu_infra_bit_id id)
{
	struct device *dev = apu->dev;
	int pwr_total_cnt = ioread32(apu->apu_infra + 0xc10);//bit 1~16 for APU use

	pwr_total_cnt = pwr_total_cnt & 0x1fffe;//and bit 1~16
	dev_info(dev, "%s , id = %d, infra status = 0x%08x\n", __func__, id, pwr_total_cnt);

	if ((BIT(id) | pwr_total_cnt) == BIT(id)) {
		dev_info(dev, "%s , last one use APU\n", __func__);
		return 1;
	}
	dev_info(dev, "%s , not last one use APU\n", __func__);
	return 0;
}
...
static void apu_infra_ctrl(struct mtk_apu *apu, uint32_t op, enum apu_infra_bit_id id)
{
	struct device *dev = apu->dev;
	...
	if (op == 1) {
		dev_info(dev, "%s: release infra, power on, infra bit = %d\n", __func__, id);
		iowrite32(BIT(id), apu->apu_infra + 0xc18);//apu power on, bit :1 ~ 16
	} else if (op == 0) {
		dev_info(dev, "%s: release infra, power off, infra bit = %d\n", __func__, id);
		iowrite32(BIT(id), apu->apu_infra + 0xc14);//apu power off, bit :1 ~ 16
	}

	dev_info(dev, "%s ,  apu infra status bit =  0x%08x\n", __func__, ioread32(apu->apu_infra + 0xc10));
}
...
static int apu_power_ctrl(struct mtk_apu *apu, uint32_t op)
{
	...
	if (apu->platdata->flags & F_SECURE_BOOT) {
		...
		if (apu->platdata->flags & F_INFRA_WA) {
			//lock infra hw sem
			ret = apu_infra_lock(apu, 1, APU_INFRA_SYS_APMCU);

			if (ret) {
				dev_info(dev, "%s(%d): infra sem lock timeout\n", __func__, op);
				return ret;
			}
		}
		...
		global_ref_cnt = ioread32(apu->apu_mbox +  APU_MBOX_OFFSET(11) + APU_MBOX_DUMMY);
		...
		if (op == 0) {
			global_ref_cnt--;
			iowrite32(global_ref_cnt, apu->apu_mbox + APU_MBOX_OFFSET(11) + APU_MBOX_DUMMY);
			/* global_ref_cnt is from 1 to 0, need to power off */
			if (global_ref_cnt == 0) {
				/*polling APU uP power off*/
				if (apu->platdata->flags & F_INFRA_WA) {

					if (apu_infra_check_last_pwr_vote(apu, APU_INFRA_SYS_APMCU) != 1) {
						apu_infra_ctrl(apu, 0, APU_INFRA_SYS_APMCU);
					} else if (apu_polling_rpc_pwr_off(apu) != 0) {
						dev_info(dev, "%s: apu polling rpc pwr off timeout\n", __func__);
					} else {
						dev_info(dev, "%s: apu polling rpc pwr off\n", __func__);
							apu_infra_ctrl(apu, 0, APU_INFRA_SYS_APMCU);
					}

				}
				/* set wkup bit to 0(use mbox11 for linux power ctrl) */
				iowrite32(0, apu->apu_mbox + MBOX_WKUP_CFG + 0xB0000);
			}
		} else if (op == 1) {
			global_ref_cnt++;
			iowrite32(global_ref_cnt, apu->apu_mbox + APU_MBOX_OFFSET(11) + APU_MBOX_DUMMY);
			...
			if (global_ref_cnt == 1) {
				if (apu->platdata->flags & F_INFRA_WA)
					apu_infra_ctrl(apu, 1, APU_INFRA_SYS_APMCU);

				/* set wkup bit to 1(use mbox11 for linux power ctrl) */
				iowrite32(1, apu->apu_mbox + MBOX_WKUP_CFG + 0xB0000);
			}
		}
		...
		if (apu->platdata->flags & F_INFRA_WA) {
			//unlock infra hw sem
			ret = apu_infra_lock(apu, 0, APU_INFRA_SYS_APMCU);

			if (ret) {
				dev_info(dev, "%s(%d): infra sem release timeout\n", __func__, op);
				return ret;
			}
		}
...
static int mt6991_cold_boot_power_on(struct mtk_apu *apu)
{
	int ret;
	struct device *dev = apu->dev;
...
static int mt6991_polling_rpc_status(struct mtk_apu *apu, u32 pwr_stat, u32 timeout)
{
	int ret = 0;
	void *addr = 0;
	uint32_t val = 0;
 ...
 */
static int mt6991_power_on_off_locked(struct mtk_apu *apu, u32 id, u32 on, u32 off)
{
	int ret = 0;
	struct device *dev = apu->dev;
	...
	if (on == 1 && off == 0) {
		...
		if (apu->ipi_pwr_ref_cnt[id] == U32_MAX) {
			...
			apu->local_pwr_ref_cnt++;
			mt6991_apu_pwr_wake_lock(apu, id);
...
static int mt6991_power_on_off(struct mtk_apu *apu, u32 id, u32 on, u32 off)
{
	int ret = 0;
	struct device *dev = apu->dev;
	/* struct timespec64 ts, te; */
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		profile_start(&ts);
		ret = mt6991_power_on_off_locked(apu, id, on, off);
...
static int mt6991_ipi_send_pre(struct mtk_apu *apu, uint32_t id, bool is_host_initiated)
{
	if (apu->platdata->flags & F_BRINGUP)
		return 0;
...
static void mt6991_debug_info_dump(struct mtk_apu *apu, struct seq_file *s)
{
	int i;
...
static int mt6991_irq_affin_init(struct mtk_apu *apu)
{
	int i;
...
static int mt6991_irq_affin_set(struct mtk_apu *apu)
{
...
static int mt6991_irq_affin_unset(struct mtk_apu *apu)
{
	irq_set_affinity_hint(apu->mbox0_irq_number, &normal_cpus);
...
static int mt6991_irq_affin_clear(struct mtk_apu *apu)
{
...
static int mt6991_apu_memmap_init(struct mtk_apu *apu)
{
	...
	apu->apu_infra = ioremap(0x1002c000, 0xfff);

	if (IS_ERR((void const *)apu->apu_infra)) {
		dev_info(dev, "%s: apu_infra remap base fail\n", __func__);
		return -ENOMEM;
	}

	apu->apu_infra_hwsem = ioremap(0x190b0e00, 0xff);
	if (IS_ERR((void const *)apu->apu_infra_hwsem)) {
		dev_info(dev, "%s: apu_infra_hwsem remap base fail\n", __func__);
		return -ENOMEM;
	}
...
static void mt6991_apu_memmap_remove(struct mtk_apu *apu)
{
}

static void mt6991_rv_cg_gating(struct mtk_apu *apu)
{
	iowrite32(0x0, apu->md32_sysctrl + MD32_CLK_EN);
}

static void mt6991_rv_cg_ungating(struct mtk_apu *apu)
{
	iowrite32(0x1, apu->md32_sysctrl + MD32_CLK_EN);
}

static void mt6991_rv_cachedump(struct mtk_apu *apu)
{
	int offset;
	unsigned long flags;
...
static void apu_polling_on_work_func(struct work_struct *p_work)
{
	...
	ret = mt6991_polling_rpc_status(apu, 1, 1);
	if (ret) {
		apu_regdump();
		apu->bypass_pwr_off_chk = true;
...
static int mt6991_rproc_init(struct mtk_apu *apu)
{
	int ret;
#if IS_ENABLED(CONFIG_PM_SLEEP)
	apu->wake_lock_ref_cnt = 0;
	spin_lock_init(&apu->wakelock_spinlock);
	ws = wakeup_source_register(NULL, "mt6991_apusys_rv");
	if (!ws) {
	...
	ret = mt6991_cold_boot_power_on(apu);
	if (ret)
		dev_info(apu->dev, "%s: call mt6991_cold_boot_power_on fail(%d)\n",
			__func__, ret);

	return ret;
}

static int mt6991_rproc_exit(struct mtk_apu *apu)
{
#if IS_ENABLED(CONFIG_PM_SLEEP)
...
const struct mtk_apu_platdata mt6991_platdata = {
	...
	.ops		= {
		...
		.polling_rpc_status = mt6991_polling_rpc_status,
		.wake_lock = mt6991_apu_pwr_wake_lock,
		.wake_unlock = mt6991_apu_pwr_wake_unlock,
		.debug_info_dump = mt6991_debug_info_dump,
		.cg_gating = mt6991_rv_cg_gating,
		.cg_ungating = mt6991_rv_cg_ungating,
		.rv_cachedump = mt6991_rv_cachedump,
		.power_init = mt6991_apu_power_init,
		.ipi_send_pre = mt6991_ipi_send_pre,
		.irq_affin_init = mt6991_irq_affin_init,
