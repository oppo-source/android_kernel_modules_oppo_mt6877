...
static int mha100kn_enable(struct hf_device *hfdev, int sensor_type, int en)
{
	...
	MHALL_ERR("%s mha100kn_enable! value %d\n", __func__, value);
	...
	int64_t delay, int64_t latency)
{
	...
	MHALL_ERR("%s mha100kn_batch! value %d\n", __func__, value);
	hall_handle_report(hall_data, DATA_ACTION, value);
...
{
	MHALL_ERR("hall_optional_handle\n");
...
{
	MHALL_ERR("hall_interrupt_handler_func\n");
	int val = 0;
	...
	__pm_wakeup_event(hall_data->suspend_lock, 500);
...
static int hall_int_gpio_init(struct device *dev, struct hall_mha100kn_data *hall_data)
{
	...
	hall_data->hall_pinctrl = (struct pinctrl *)devm_pinctrl_get(dev);
	...
	hall_data->hall_int_active =
				(struct pinctrl_state *)pinctrl_lookup_state(hall_data->hall_pinctrl, pinctrl_status);
...
#else
	hall_data->irq_gpio = of_get_named_gpio(np, "irq-gpio", 0);
	//hall_data->irq_flags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_SHARED;
	if (of_property_read_u32_index(np, "irq-gpio", 2, &(hall_data->irq_flags))) {
		MHALL_ERR("Failed to get GPIO flags\n");
		return -EINVAL;
	}
#endif
	if (hall_data->irq_gpio < 0) {
		MHALL_ERR("Failed to get gpio number\n");
		return hall_data->irq_gpio;
	}
...
static int mha100kn_probe(struct platform_device *dev)
{
	...
	hall_data->suspend_lock = wakeup_source_register(NULL, "hall wakelock");
	if (!hall_data->suspend_lock) {
		pr_notice("wakeup source init failed.\n");
		goto distory_hf_dev ;
	}
	err = request_threaded_irq(hall_data->irq_number,
                                NULL,
                                hall_interrupt_handler_func,
                                hall_data->irq_flags,
                                hall_data->DEV_NAME,
                                hall_data);
...
static int mha100kn_remove(struct platform_device *dev)
{
	...
	wakeup_source_unregister(hall_data->suspend_lock);
