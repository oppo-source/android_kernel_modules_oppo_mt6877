	...
	char *aed_str, int db_opt)
{
	...
	if( aed_str_for_hash != NULL ) {
		...
		CCCI_ERROR_LOG(0, FSM, "aed_str_for_hash is null!!");
	}
	...
	if ((logBuf != NULL)&&(aed_str_for_hash != NULL)) {
		for (temp_i = 0 ; (temp_i < BUF_LOG_LENGTH) && (temp_i < strlen(aed_str_for_hash)) ; temp_i++) {
			if(aed_str_for_hash[temp_i] == '\n') {
				checkID++;
				CCCI_ERROR_LOG(0, FSM, "checkID = %d",checkID);
				if(2 == checkID) {
					logBuf[temp_i] = '\0';
					break;
				}
		...
		CCCI_NORMAL_LOG(0, FSM, "modem crash wirte to critical log. hashid = %u, cause = %s.", hashId, logBuf);
