...
static void mss_norm_sirq(struct cmdq_cb_data data)
{
	...
	if (data.err < 0) {
		...
		/* 不 goto EXIT，繼續更新軟體狀態機以防資源與 WakeLock 洩漏 */
	}
	...
	if (!mfb_request_running(&mss_reqs) && !mfb_request_running(&vmss_reqs))
		__pm_relax(MSS_wake_lock);
...
static void msf_norm_sirq(struct cmdq_cb_data data)
{
	...
	if (!mfb_request_running(&msf_reqs) && !mfb_request_running(&vmsf_reqs))
		__pm_relax(MSF_wake_lock);
	cmdq_pkt_destroy(pkt);
#endif
}
	...
	struct MFB_REG_IO_STRUCT *data)
{
	...
	if (copy_to_user(compat_ptr(arg), &data32,
			sizeof(struct compat_MFB_REG_IO_STRUCT)) != 0) {
	...
	struct MFB_MSSRequest *data)
{
	...
	data->m_pMssConfig = compat_ptr(data32.m_pMssConfig);
	...
	struct MFB_MSFRequest *data)
{
	...
	data->m_pMsfConfig = compat_ptr(data32.m_pMsfConfig);
	data->m_ReqNum = data32.m_ReqNum;
	...
	struct MFB_MSFRequest *data)
{
	...
	if (copy_to_user(compat_ptr(arg), &data32,
			sizeof(struct compat_MFB_MSFRequest)) != 0) {
		LOG_NOTICE("copy_to_user failed");
		ret = -EFAULT;
	}
	return ret;
	...
	case COMPAT_MFB_MSS_READ_REGISTER:
		{
			...
			err = compat_put_MFB_read_register_data(arg, &data);
			if (err) {
				LOG_INF(
...
static signed int MFB_release(struct inode *pInode, struct file *pFile)
{
	...
	for (i = 0; i < GKI_BUF_NUM_MAX; i++) {
		if (pUserInfo->gkiBuf[i].isUse == 1) {
			LOG_WRN("Force release unmapped gkiBuf[%d] fd:%d", i, pUserInfo->gkiBuf[i].bfd);
			mfb_put_dma_buf(&pUserInfo->gkiBuf[i]);
			pUserInfo->gkiBuf[i].isUse = 0;
		}
	}
...
PM_OK:
		...
		MSS_wake_lock = wakeup_source_register(MFB_dev->dev, "MFB_MSS_WS");
		MSF_wake_lock = wakeup_source_register(MFB_dev->dev, "MFB_MSF_WS");
...
#endif
	if (MSS_wake_lock)
		wakeup_source_unregister(MSS_wake_lock);
	if (MSF_wake_lock)
		wakeup_source_unregister(MSF_wake_lock);
			...
			unsigned long pm_event, void *unused)
{
	...
	case PM_SUSPEND_PREPARE: /*enter suspend*/
		/* Prevent suspend if hardware is still executing */
		if (mfb_request_running(&mss_reqs) || mfb_request_running(&vmss_reqs) ||
		    mfb_request_running(&msf_reqs) || mfb_request_running(&vmsf_reqs)) {
			LOG_ERR("MFB/CMDQ is busy, reject suspend to avoid KE!\n");
			return NOTIFY_BAD;
		}
