...
#define IRQ_LOG_KEEPER_LOG_INF(irq_in, ppb_in, logT_in, fmt, ...) do {\
	char *ptr; \
	char *pDes;\
	signed int avaLen;\
	unsigned int irq = irq_in;\
...
#define IRQ_LOG_KEEPER_LOG_INF(irq, ppb, logT, fmt, args...) \
		LOG_INF(IRQTag fmt,  ##args)
#endif
...
#define IRQ_LOG_PRINTER_LOG_INF(irq, ppb_in, logT_in) do {\
	struct SV_LOG_STR *pSrc = &gSvLog[irq];\
	char *ptr;\
	unsigned int i;\
	unsigned int ppb = 0;\
...
#define IRQ_LOG_PRINTER_LOG_INF(irq, ppb, logT)
#endif
...
static inline void ISP_Reset(signed int module)
{
	...
	default:
		LOG_INF("Not support reset module:%d\n", module);
		break;
	}
...
static signed int ISP_DumpBuffer(struct ISP_DUMP_BUFFER_STRUCT *pDumpBufStruct)
{
	...
	if (pDumpBufStruct->BytesofBufferSize > 0xFFFFFFFF) {
		LOG_INF("pDumpTuningBufStruct->BytesofBufferSize error");
		Ret = -EFAULT;
		goto EXIT;
	}
	/*  */
	if ((void __user *)(pDumpBufStruct->pBuffer) == NULL) {
		LOG_INF("NULL pDumpBufStruct->pBuffer");
		Ret = -EFAULT;
		goto EXIT;
	}
    /* Native Exception */
	...
	case ISP_DUMP_TPIPEBUF_CMD:
		...
		if (g_bIonBufferAllocated == MFALSE) {
			if (g_pTpipeBuffer == NULL)
				...
				LOG_INF("g_pTpipeBuffer:0x%pK is not NULL!!",
					g_pTpipeBuffer);
		}
		if (g_pTpipeBuffer != NULL) {
		...
		if (copy_from_user(g_TpipeBuffer,
		    (void __user *)(pDumpBufStruct->pBuffer),
		    pDumpBufStruct->BytesofBufferSize) != 0) {
			LOG_INF("copy_from_user g_TpipeBuffer failed\n");
			Ret = -EFAULT;
			goto EXIT;
		}
#endif
	...
	case ISP_DUMP_TUNINGBUF_CMD:
		if (pDumpBufStruct->BytesofBufferSize > ISP_DIP_REG_SIZE) {
			LOG_INF("tuning buf size error, size:0x%x",
		...
		if (g_bIonBufferAllocated == MFALSE) {
			if (g_pTuningBuffer == NULL)
				...
				LOG_INF("g_TuningBuffer:0x%pK is not NULL!!",
					g_pTuningBuffer);
		}
		if (g_pTuningBuffer != NULL) {
			if (copy_from_user(g_pTuningBuffer,
			    (void __user *)(pDumpBufStruct->pBuffer),
			    pDumpBufStruct->BytesofBufferSize) != 0) {
				LOG_INF("copy_from_user g_pTuningBuffer failed\n");
				Ret = -EFAULT;
				goto EXIT;
			}
		} else {
			LOG_INF("ERROR: g_TuningBuffer kmalloc failed\n");
		}
#else
		if (copy_from_user(g_TuningBuffer,
		    (void __user *)(pDumpBufStruct->pBuffer),
		    pDumpBufStruct->BytesofBufferSize) != 0) {
			LOG_INF("copy_from_user g_TuningBuffer failed\n");
			Ret = -EFAULT;
			goto EXIT;
		}
#endif
	...
	case ISP_DUMP_ISPVIRBUF_CMD:
		if (pDumpBufStruct->BytesofBufferSize > ISP_DIP_REG_SIZE) {
			LOG_INF("vir isp buffer size error, size:0x%x",
				pDumpBufStruct->BytesofBufferSize);
			Ret = -EFAULT;
			goto EXIT;
		}
		...
		if (g_bIonBufferAllocated == MFALSE) {
			if (g_pVirISPBuffer == NULL)
				...
				LOG_INF("g_pVirISPBuffer:0x%pK is not NULL!!",
					g_pVirISPBuffer);
		}
		if (g_pVirISPBuffer != NULL) {
			if (copy_from_user(g_pVirISPBuffer,
			    (void __user *)(pDumpBufStruct->pBuffer),
			    pDumpBufStruct->BytesofBufferSize) != 0) {
				LOG_INF("copy_from_user g_pVirISPBuffer failed\n");
				Ret = -EFAULT;
				goto EXIT;
			}
		} else {
			LOG_INF("ERROR: g_pVirISPBuffer kmalloc failed\n");
		}
#else
		if (copy_from_user(g_VirISPBuffer,
		    (void __user *)(pDumpBufStruct->pBuffer),
		    pDumpBufStruct->BytesofBufferSize) != 0) {
			LOG_INF("copy_from_user g_VirISPBuffer failed\n");
			Ret = -EFAULT;
			goto EXIT;
		}
#endif
	...
	case ISP_DUMP_CMDQVIRBUF_CMD:
		if (pDumpBufStruct->BytesofBufferSize >
			...
			LOG_INF("cmdq buffer size error, size:0x%x",
				pDumpBufStruct->BytesofBufferSize);
			Ret = -EFAULT;
			goto EXIT;
		}
		...
		if (g_bIonBufferAllocated == MFALSE) {
			if (g_pCmdqBuffer == NULL)
				...
				LOG_INF("g_pCmdqBuffer:0x%pK is not NULL!!",
					g_pCmdqBuffer);
		}
		if (g_pCmdqBuffer != NULL) {
			if (copy_from_user(g_pCmdqBuffer,
			    (void __user *)(pDumpBufStruct->pBuffer),
			    pDumpBufStruct->BytesofBufferSize) != 0) {
				LOG_INF("copy_from_user g_pCmdqBuffer failed\n");
				Ret = -EFAULT;
				goto EXIT;
			}
		} else {
			LOG_INF("ERROR: g_pCmdqBuffer kmalloc failed\n");
		}
#else
		...
		break;
	default:
		LOG_INF("error dump buffer cmd:%d", pDumpBufStruct->DumpCmd);
		break;
	}
...
static signed int ISP_SetMemInfo(struct ISP_MEM_INFO_STRUCT *pMemInfoStruct)
{
	...
	if ((void __user *)(pMemInfoStruct->MemVa) == NULL) {
		LOG_INF("NULL pMemInfoStruct->MemVa");
		Ret = -EFAULT;
		goto EXIT;
	}
	...
	case ISP_MEMORY_INFO_CMDQ_CMD:
		...
		break;
	default:
		LOG_INF("error set memory info cmd:%d",
	...
	if (copy_from_user(&ref_cnt_ctrl, (void __user *)Param,
		...
		if (ref_cnt_ctrl.id < ISP_REF_CNT_ID_MAX) {
			...
			if ((void __user *)(ref_cnt_ctrl.data_ptr) == NULL) {
				LOG_INF("NULL data_ptr");
				return -EFAULT;
			}
			if (put_user(imem_ref_cnt,
			    (signed int *)ref_cnt_ctrl.data_ptr) != 0) {
				LOG_INF("[rc][GET]:copy_to_user failed");
				Ret = -EFAULT;
			}
		} else {
			LOG_INF("[rc]:id(%d) exceed", ref_cnt_ctrl.id);
			Ret = -EFAULT;
		}


	} else {
		LOG_INF("[rc]copy_from_user failed");
		Ret = -EFAULT;
	}
...
static signed int ISP_P2_BufQue_CTRL_FUNC(struct ISP_P2_BUFQUE_STRUCT param)
{
		...
		(P2_FramePack_List_Idx[property].end !=  -1)) {
			...
			"pty(%d), F/L(%d_%d,%d), dCQ(%d,%d), RF/C/L(%d,%d,%d), sts(%d,%d,%d)",
				...
				P2_FrameUnit_List[property]
					...
					bufSts);
			spin_unlock(&(SpinLock_P2FrameList));
			LOG_INF("p2 frame package list is full, enque Fail.");
			ret =  -EFAULT;
			return ret;
		}
		{
			...
			if (param.burstQIdx == 0) {
				...
				P2_FramePackage_List[property]
					...
					param.frameNum;
				P2_FramePackage_List[property]
					[P2_FramePack_List_Idx[property].end].dequedNum = 0;
			}
		}
	...
	case ISP_P2_BUFQUE_CTRL_WAIT_DEQUE:
		...
		}
		{
			...
			if (restTime == 0) {
				...
				LOG_INF("be stopped, restime(%d)", restTime);
				ret =  -EFAULT;
				break;
			}
		}
	...
	case ISP_P2_BUFQUE_CTRL_DEQUE_SUCCESS:
		...
		if (idx2 ==  -1) {
			spin_unlock(&(SpinLock_P2FrameList));
			LOG_INF("ERRRRRRRRRRR findmatch index 2 fail (%d_0x%x_0x%x_%d, %d_%d)",
				param.property, param.processID, param.callerID,
	...
	case ISP_P2_BUFQUE_CTRL_CLAER_ALL:
		...
		break;
	default:
		LOG_INF("do not support this ctrl cmd(%d)", param.ctrl);
		break;
	}
	return ret;
}
...
static signed int ISP_MARK_IRQ(struct ISP_WAIT_IRQ_STRUCT *irqinfo)
{
	...
	if (irqinfo->Type >= ISP_IRQ_TYPE_AMOUNT) {
		LOG_INF("MARK_IRQ: type error(%d)", irqinfo->Type);
		return -EFAULT;
	}

	if (irqinfo->EventInfo.St_type >= ISP_IRQ_ST_AMOUNT) {
		LOG_INF("MARK_IRQ: st_type error(%d)",
	...
	if (irqinfo->EventInfo.UserKey >= IRQ_USER_NUM_MAX ||
		...
		LOG_INF("MARK_IRQ: userkey error(%d)",
	...
	if (irqinfo->EventInfo.St_type == SIGNAL_INT) {
		...
		LOG_INF("Not support DMA interrupt type(%d), Only support signal interrupt!!!",
			irqinfo->EventInfo.St_type);
		return -EFAULT;
	}
...
static signed int ISP_FLUSH_IRQ(struct ISP_WAIT_IRQ_STRUCT *irqinfo)
{
	...
	if (irqinfo->Type >= ISP_IRQ_TYPE_AMOUNT ||
	    irqinfo->Type < 0) {
		LOG_INF("FLUSH_IRQ: type error(%d)", irqinfo->Type);
		return -EFAULT;
	}

	if (irqinfo->EventInfo.St_type >= ISP_IRQ_ST_AMOUNT ||
	    irqinfo->EventInfo.St_type < 0) {
		LOG_INF("FLUSH_IRQ: st_type error(%d)",
			irqinfo->EventInfo.St_type);
		return -EFAULT;
	}
	...
	if (irqinfo->EventInfo.UserKey >= IRQ_USER_NUM_MAX ||
	    irqinfo->EventInfo.UserKey < 0) {
		LOG_INF("FLUSH_IRQ: userkey error(%d)",
			irqinfo->EventInfo.UserKey);
		return -EFAULT;
	}
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_WAKELOCK_CTRL:
		if (copy_from_user(&wakelock_ctrl, (void *)Param,
			...
			LOG_INF("get ISP_WAKELOCK_CTRL from user fail\n");
			Ret = -EFAULT;
		} else {
	...
	case ISP_GET_DROP_FRAME:
		if (copy_from_user(&DebugFlag[0], (void *)Param,
		    sizeof(unsigned int)) != 0) {
			LOG_INF("get irq from user fail\n");
			Ret = -EFAULT;
		} else {
			switch (DebugFlag[0]) {
			case ISP_IRQ_TYPE_INT_CAM_A_ST:
	...
	case ISP_RESET_BY_HWMODULE: {
		if (copy_from_user(&module, (void *)Param,
		    sizeof(unsigned int)) != 0) {
			LOG_INF("get hwmodule from user fail\n");
			Ret = -EFAULT;
		} else {
			ISP_Reset(module);
		}
	...
	case ISP_REGISTER_IRQ_USER_KEY: {
		...
		if (copy_from_user(&RegUserKey, (void *)Param,
			...
			if (RegUserKey.userKey < 0) {
				LOG_INF("query irq user key fail\n");
				Ret = -1;
			}
		} else {
			LOG_INF("copy from user fail\n");
		}
	...
	case ISP_MARK_IRQ_REQUEST:
		if (copy_from_user(&IrqInfo, (void *)Param,
			...
			LOG_INF("ISP_MARK_IRQ, copy_from_user failed\n");
			Ret = -EFAULT;
		}
		break;
	/*  */
	...
	case ISP_DUMP_ISR_LOG:
		if (copy_from_user(DebugFlag, (void *)Param,
			...
			IRQ_LOG_PRINTER_LOG_INF(DebugFlag[0], currentPPB,
						_LOG_ERR);
		...
		if (copy_from_user(DebugFlag, (void *)Param,
			...
			case ISP_IRQ_TYPE_INT_CAM_B_ST:
				...
				if (ISP_PopBufTimestamp(DebugFlag[0], dma_id,
				    pTstp) != 0)
					LOG_INF("Get Buf sof timestamp fail");
	...
	case ISP_LARB_MMU_CTL: {
		...
		if (copy_from_user(&larbInfo, (void *)Param,
			...
			LOG_INF("copy_from_user LARB_MMU_CTL failed\n");
			Ret = -EFAULT;
			goto EXIT;
		}
	...
	case ISP_GET_DUMP_INFO: {
		if (copy_to_user((void *)Param, &g_dumpInfo,
			...
			LOG_INF("ISP_GET_DUMP_INFO copy to user fail");
			Ret = -EFAULT;
		}
		break;
	}
	...
	case ISP_SET_MEM_INFO: {
		if (copy_from_user(&MemInfoStruct, (void *)Param,
			...
			LOG_INF("ISP_SET_MEM_INFO copy_from_user failed\n");
			Ret = -EFAULT;
		}
		break;
	}
	case  ISP_SET_SEC_DAPC_REG:
		if (copy_from_user(
			...
			LOG_INF("get DAPC_REG from user fail\n");
			Ret = -EFAULT;
		} else {
			if (Dapc_Reg[0] < ISP_CAMSYS_CONFIG_IDX ||
				Dapc_Reg[0] >= ISP_DEV_NODE_NUM) {
				LOG_INF("module index(0x%x) error\n",
					Dapc_Reg[0]);
				Ret = -EFAULT;
				break;
			}
			...
			if (Dapc_Reg[1] == MTRUE) {
				...
				LOG_INF("get wrong sec status (0x%x)\n",
					Dapc_Reg[1]);
				Ret = -EFAULT;
			}
		}
...
static inline signed int ISP_RegCharDev(void)
{
	...
	if ((Ret) < 0) {
		LOG_INF("alloc_chrdev_region failed, %d\n", Ret);
		return Ret;
	}
	/* Allocate driver */
	...
	if ((Ret) < 0) {
		LOG_INF("Attach file operation failed, %d\n", Ret);
		goto EXIT;
	}
	/*  */
EXIT:
...
)
{
	...
	case ISP_IRQ_TYPE_AMOUNT:
		LOG_INF("dev name is not found (%s)", moduleName);
		break;
	case ISP_IRQ_TYPE_INT_UNI_A_ST:
	default:
		/*don nothing*/
...
static ssize_t ISP_DumpRegToProc(
	...
	loff_t *Count)
{
	LOG_INF("DumpRegToProc: Not implement");
	return 0;
}
...
static ssize_t ISP_RegDebug(
	...
	loff_t *pData)
{
	LOG_INF("RegDebug: Not implement");
	return 0;
}
...
static signed int __init ISP_Init(void)
{
	...
	do {
		...
		for (i = 0; i < ARRAY_SIZE(SMI_LARB_BASE); i++) {
			...
			if (!node) {
				LOG_INF("find %s node failed!!!\n", comp_str);
				SMI_LARB_BASE[i] = 0;
				continue;
			}
			SMI_LARB_BASE[i] = of_iomap(node, 0);
			if (!SMI_LARB_BASE[i]) {
				LOG_INF("unable to map ISP_SENINF0_BASE registers!!!\n");
				break;
			}
	...
	if (!node) {
		LOG_INF("find mediatek,seninf1 node failed!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_INF("find mediatek,seninf2 node failed!!!\n");
		return -ENODEV;
	}
	ISP_SENINF1_BASE = of_iomap(node, 0);
	if (!ISP_SENINF1_BASE) {
		LOG_INF("unable to map ISP_SENINF1_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_INF("find mediatek,seninf3 node failed!!!\n");
		return -ENODEV;
	}
	ISP_SENINF2_BASE = of_iomap(node, 0);
	if (!ISP_SENINF2_BASE) {
		LOG_INF("unable to map ISP_SENINF2_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_INF("find mediatek,seninf4 node failed!!!\n");
		return -ENODEV;
	}
	ISP_SENINF3_BASE = of_iomap(node, 0);
	if (!ISP_SENINF3_BASE) {
		LOG_INF("unable to map ISP_SENINF3_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_INF("find mediatek,mt6765-apmixedsys node failed!!!\n");
		return -ENODEV;
	}
	CLOCK_CELL_BASE = of_iomap(node, 0);
	if (!CLOCK_CELL_BASE) {
		LOG_INF("unable to map CLOCK_CELL_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!ISP_MMSYS_CONFIG_BASE) {
		LOG_INF("unable to map ISP_MMSYS_CONFIG_BASE registers!!!\n");
		return -ENODEV;
	}
					...
					unsigned int delayCheck)
{
	...
	case ISP_CAM_B_IDX:
		if (dma_id >= _cam_max_) {
			LOG_INF("LINE_%d ERROR: unsupported module:0x%x dma:%d\n"
...
static int32_t ISP_WaitTimestampReady(unsigned int module, unsigned int dma_id)
{
	...
	if (wait_cnt == 0) {
		LOG_INF("ERROR: cam:%d dma:%d wait timestamp timeout!!!\n",
			module, dma_id);
		return -EFAULT;
	}
	#else
			...
			unsigned int frmPeriod)
{
	...
	if (delta_wcnt > 255) {
		if (dmao_mask)
			LOG_INF("ERROR: Cam:%d dma:%d WRONG WCNT:%d_%d_%d\n",
	...
	time_prev1.sec = IspInfo.TstpQInfo[module].Dmao[dma_id].TimeQue
	[wridx_prev1].sec;
	time_prev1.usec = IspInfo.TstpQInfo[module].Dmao[dma_id].TimeQue
	[wridx_prev1].usec;

	time_prev2.sec = IspInfo.TstpQInfo[module].Dmao[dma_id].TimeQue
	[wridx_prev2].sec;
	time_prev2.usec = IspInfo.TstpQInfo[module].Dmao[dma_id].TimeQue
	[wridx_prev2].usec;
		...
		unsigned long long prevTimestp)
{
	...
	while (target_wridx != curr_wridx) {
		...
		if (i > frmPeriod) {
			LOG_INF("Error: too many intpl in sub-sample period %d_%d\n",
...
static void ISP_BH_Workqueue(struct work_struct *pWork)
{
	...
	IRQ_LOG_PRINTER_LOG_INF(pWorkTable->module, m_CurrentPPB, _LOG_ERR);
