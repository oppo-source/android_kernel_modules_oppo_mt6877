...
static u32 PMQoS_BW_value[ISP_PASS1_PATH_TYPE_AMOUNT][_rt_dma_max_];
...
#ifdef ISP_DEBUG
#define log_dbg(format, args...) log_dbg(MyTag "[%s] " format, __func__, ##args)
#else
...
#define log_wrn(format, args...) pr_notice(MyTag "[%s] " format, __func__, ##args)
#define log_err(format, args...) pr_notice(MyTag "[%s] " format, __func__, ##args)
#define log_ast(format, args...) pr_notice(MyTag "[%s] " format, __func__, ##args)
...
static struct tasklet_struct isp_tasklet_VD;
static struct tasklet_struct isp_tasklet_EXPDONE;
 ...
 *
 *****************************************************************************/
static void ISP_TaskletFunc_VD(unsigned long Param)
{
	if (IspInfo.DebugMask & ISP_DBG_TASKLET)
		log_dbg("- E.");
...
//DECLARE_TASKLET(IspTaskletVD, ISP_Tasklet_VD);

/******************************************************************************
 *
 ******************************************************************************/
static void ISP_TaskletFunc_EXPDONE(unsigned long Param)
{
	if (IspInfo.DebugMask & ISP_DBG_TASKLET)
		log_dbg("- E.");
...
//DECLARE_TASKLET(IspTaskletEXPDONE, ISP_Tasklet_EXPDONE);
...
static bool ISP_PM_QOS_CTRL_FUNC(unsigned int bIsOn, unsigned int module)
{
	int port = 0;
	...
	if (G_PM_QOS[module].upd_flag && G_PM_QOS[module].sof_flag) {
		...
		ISP3_SetPMQOS(E_BW_UPDATE, module,
				(unsigned int *)G_PM_QOS[module].port_bw);
	...
	for (port = 0; port < _rt_dma_max_; port++) {
		if (PMQoS_BW_value[module][port] != G_PM_QOS[module].port_bw[port].avg) {
			log_dbg("PM_QoS: module[%d], bIsOn[%d], bw[%d], bw = %d MB/s\n",
				...
				PMQoS_BW_value[module][port] = G_PM_QOS[module].port_bw[port].avg;
		}
	}
	return MTRUE;
}
...
static signed int ISP_WaitIrq_v3(struct ISP_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	unsigned long long sec = 0;
	unsigned long usec = 0;
	unsigned long long time_getrequest_sec = 0;
	unsigned long time_getrequest_usec = 0;
	unsigned long long time_ready2return_sec = 0;
	unsigned long time_ready2return_usec = 0;

	sec = cpu_clock(0);	  /* ns */
	do_div(sec, 1000);	   /*     usec */
	usec = do_div(sec, 1000000); /* sec and usec */
	time_getrequest_usec = usec;
	time_getrequest_sec = sec;
	...
	do_div(sec, 1000);	   /*     usec */
	usec = do_div(sec, 1000000); /* sec and usec */
	time_ready2return_usec = usec;
	time_ready2return_sec = sec;
	...
	if (WaitIrq->UserInfo.UserKey > 0) {
		log_dbg(" [WAITIRQv3]user(%d) mark sec/usec (%d/%d), last irq sec/usec	(%d/%d),enterwait(%d/%d),getIRQ(%d/%d)\n",
			...
			IspInfo.IrqInfo.LastestSigTime_usec[WaitIrq->UserInfo
								    .Type][idx],
			(int)(time_getrequest_sec),
			(int)(time_getrequest_usec),
			(int)(time_ready2return_sec),
			(int)(time_ready2return_usec));
...
/* IRQ_LOG_PRINTER(_IRQ_D,m_CurrentPPB,_LOG_ERR); */
	...
	if ((IrqStatus[ISP_IRQ_TYPE_INT_P1_ST] &
		...
		tasklet_schedule(&isp_tasklet_VD);
	}
	...
	if ((IrqStatus[ISP_IRQ_TYPE_INT_P1_ST] &
		...
		tasklet_schedule(&isp_tasklet_EXPDONE);
	}
...
static void ISP_TaskletFunc(unsigned long data)
{
	if (bSlowMotion == MFALSE) {
		if (bRawEn == MTRUE) {
			...
			if (dump_smi_debug) {
				mtk_smi_dbg_hang_detect("camera_isp_raw");
		...
		if (bRawDEn == MTRUE) {
			...
			if (dump_smi_debug) {
				mtk_smi_dbg_hang_detect("camera_isp_raw_d");
				dump_smi_debug = MFALSE;
			}
		}
	} else {
	...
	case ISP_SET_PM_QOS_INFO:
		{
			...
			if (copy_from_user(&pm_qos_info, (void *)Param,
				...
				G_PM_QOS[pm_qos_info.module].upd_flag =
					MTRUE;
				for (dma_port = 0; dma_port < _rt_dma_max_; dma_port++) {
					G_PM_QOS[pm_qos_info.module].port_bw[dma_port] =
	...
	case ISP_SET_ISPCLK:
		{
			if (copy_from_user(DebugFlag, (void *)Param,
				...
				ISP3_SetPMQOS(E_CLK_UPDATE, 0, &DebugFlag[0]);
	...
	case ISP_GET_ISPCLK:
		{
			...
			pIspclks->clklevelcnt = ISP3_SetPMQOS(E_CLK_SUPPORTED, 0,\
								(u32 *)&pIspclks->clklevel);

			if (pIspclks->clklevelcnt < 0) {
				log_err(
...
/* Check platform_device parameters     */
	...
	// power resource is all described in "ispsys" in dts.
	if (strncmp(pDev->dev.of_node->name, "ispsys", strlen("ispsys")) == 0)
	...
	tasklet_init(&isp_tasklet_VD, ISP_TaskletFunc_VD, 0);
	tasklet_init(&isp_tasklet_EXPDONE, ISP_TaskletFunc_EXPDONE, 0);
...
static signed int ISP_remove(struct platform_device *pDev)
{
	...
	tasklet_kill(&isp_tasklet_VD);
	tasklet_kill(&isp_tasklet_EXPDONE);
...
static void __exit ISP_Exit(void)
{
	...
	log_dbg("- E.");
	for (i = 0; i < ISP_CAM_TYPE_CAM_AMOUNT; i++) {
		ISP3_SetPMQOS(E_BW_REMOVE, i, 0);
		ISP3_SV_SetPMQOS(E_BW_REMOVE, i, 0);
	}
	/*      */
	...
	log_inf("Un-Register M4U callback dump");
#if IS_ENABLED(CONFIG_MTK_M4U)
