...
xhci_free_interrupter_(struct xhci_hcd *xhci, struct xhci_interrupter *ir)
{
	...
	if (ir->event_ring)
		xhci_ring_free_(xhci, ir->event_ring);
...
void xhci_remove_secondary_interrupter_(struct usb_hcd *hcd, struct xhci_interrupter *ir)
{
...
static void xhci_set_hc_event_deq(struct xhci_hcd *xhci, struct xhci_interrupter *ir)
{
	...
	deq = xhci_trb_virt_to_dma_(ir->event_ring->deq_seg,
...
xhci_alloc_interrupter(struct xhci_hcd *xhci, int segs, gfp_t flags)
{
	...
	ir->event_ring = xhci_ring_alloc_(xhci, num_segs, 1, TYPE_EVENT, 0,
					 flags);
	if (!ir->event_ring) {
	...
	ret = xhci_alloc_erst_(xhci, ir->event_ring, &ir->erst, flags);
	if (ret) {
