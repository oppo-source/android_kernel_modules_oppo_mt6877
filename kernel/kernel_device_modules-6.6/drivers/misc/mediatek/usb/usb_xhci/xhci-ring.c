			       ...
			       union xhci_trb *event)
{
	...
	if (hcd->speed >= HCD_USB3 &&
		...
		slot_id = xhci_find_slot_id_by_port_(hcd, xhci, hcd_portnum + 1);
...
irqreturn_t xhci_msi_irq_(int irq, void *hcd)
{
	return xhci_irq(hcd);
}
EXPORT_SYMBOL_GPL(xhci_msi_irq_);
