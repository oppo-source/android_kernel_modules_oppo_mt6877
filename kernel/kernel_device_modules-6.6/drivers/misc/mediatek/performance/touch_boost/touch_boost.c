...
int touch_boost_get_cpu_opp_info_for_leagcy(int **opp_cnt, unsigned int ***opp_tbl)
{
	int i, j;
...
void touch_boost_get_cpu_opp_info_by_idx(void)
{
...
#if IS_ENABLED(CONFIG_MEDIATEK_CPU_DVFS)
	if (touch_boost_get_cpu_opp_info_for_leagcy(&_opp_cnt, &cpu_opp_tbl) < 0)
		return -EFAULT;
#else
...
#if IS_ENABLED(CONFIG_MEDIATEK_CPU_DVFS)
	touch_boost_get_cpu_opp_info_by_idx();
#else
	cpu_policy_init();
#endif
