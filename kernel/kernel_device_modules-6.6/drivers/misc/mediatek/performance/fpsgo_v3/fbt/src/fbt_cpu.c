...
#endif
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (ret) {
			get_cpu_cluster_core(cpu_dvfs);
			for (int i = 0; i < cluster_num; i++) {
				cpu_dvfs[i].num_opp = NR_MT_CPU_CLUSTER_OPP;
				pr_info("%s: %d cores, starting at CPU %d\n",
						__func__, cpu_dvfs[i].num_cpu, cpu_dvfs[i].first_cpu);
			}
		}
#endif
...
#if IS_ENABLED(CONFIG_MEDIATEK_CPU_DVFS)
	cluster_num = NR_MT_CPU_CLUSTER;
	nr_freq_cpu = NR_MT_CPU_CLUSTER_OPP;
#else
