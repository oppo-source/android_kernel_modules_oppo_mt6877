...
#define ALL_CPUS_ONLINE 0
#define ALL_CPUS_NOT_ONLINE -EINVAL
#define DEBUG_LOG	0
...
static bool fpsgo_start_on_init;
...
int check_all_cpus_online(void)
{
	int cpu;
	for_each_possible_cpu(cpu) {
		if (!cpu_online(cpu)) {
#if DEBUG_LOG
			pr_info("%s, cpu%d is offline\n", __func__, cpu);
#endif
			return -EINVAL;
		}
	}
#if DEBUG_LOG
	pr_info("%s, All cpus are online\n", __func__);
#endif
	return 0;
}

int fpsgo_cpu_startup(unsigned int cpu)
{
#if DEBUG_LOG
    pr_info("%s: CPU %u online\n", __func__, cpu);
#endif
    if (check_all_cpus_online() == 0) {
        fpsgo_switch_enable(1);
		fpsgo_start_on_init = true;
        return 0;
    } else {
#if DEBUG_LOG
        pr_err("%s: Not all CPUs are online.\n", __func__);
#endif
        return -EINVAL;
    }
}

int fpsgo_cpu_teardown(unsigned int cpu)
{
    pr_info("%s: CPU %u closed\n", __func__, cpu);
    return 0;
}

void register_cpuhp_setup_state(void)
{
	int ret;

    /* 注册 CPUHP_AP_ONLINE_DYN 回调 */
    ret = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN_END,
                                    "fpsgo/cpuhp",
                                    fpsgo_cpu_startup,
                                    fpsgo_cpu_teardown);
    if (ret) {
#if DEBUG_LOG
        pr_info("[%s] Failed to register CPU hotplug state: %d\n", __func__, ret);
#endif
        return;
    }
#if DEBUG_LOG
	pr_info("[%s] success to register CPU hotplug state: %d\n", __func__, ret);
#endif
}

static void __exit fpsgo_exit(void)
{
	cpuhp_remove_state_nocalls(CPUHP_AP_ONLINE_DYN_END);
...
static int __init fpsgo_init(void)
{
	...
	if (check_all_cpus_online() == 0) {
#if DEBUG_LOG
		pr_info("[%s] fpsgo: all cpus are online, start fpsgo\n", __func__);
#endif
		fpsgo_start_on_init = true;
		fpsgo_switch_enable(1);
	} else {
		register_cpuhp_setup_state();
	}
