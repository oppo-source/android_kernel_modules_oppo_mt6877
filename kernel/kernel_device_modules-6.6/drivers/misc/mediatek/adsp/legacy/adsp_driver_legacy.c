...
#include "adsp_driver_legacy.h"
#include "mtk-afe-external.h"
#include <linux/of_address.h>

#define REDEFINE_SEMA

#ifdef REDEFINE_SEMA
#undef SEMA_AUDIOREG
#undef SEMA_AUDIO

#define SEMA_AUDIOREG             0
#define SEMA_AUDIO                1
#endif

struct adsp_com adsp_common;

const struct adspsys_description mt6781_adspsys_desc = {
	.platform_name = "mt6781",
	.version = 1,
	.semaphore_ways = 2,
	.semaphore_ctrl = 1,
...
const struct adsp_core_description mt6781_adsp_c0_desc = {
	.id = 0,
	.name = "adsp_0",
	.cirq_num = 2, /* wdt, swint */
	.sharedmems = {
...
static const struct of_device_id adspsys_of_ids[] = {
	{ .compatible = "mediatek,mt6781-adspsys", .data = &mt6781_adspsys_desc},
	{}
};

static const struct of_device_id adsp_core_of_ids[] = {
	{ .compatible = "mediatek,mt6781-adsp_core_0", .data = &mt6781_adsp_c0_desc},
...
static int adspsys_drv_probe(struct platform_device *pdev)
{
	...
	adsp_common.infracfg_ao = of_iomap(dev->of_node, 1);
	if (IS_ERR(adsp_common.infracfg_ao))
		goto ERROR;

	adsp_common.pericfg = of_iomap(dev->of_node, 2);
...
static int adsp_core_drv_probe(struct platform_device *pdev)
{
	...
	return 0;
ERROR:
	pr_info("%s id:%d fail, ret = %d", __func__, pdata->id, ret);
	return ret;
}
...
MODULE_AUTHOR("Maunu Singhal <maunu.singhal@mediatek.com>");
