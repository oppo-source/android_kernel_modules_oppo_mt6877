...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_HFCALL)
#include <linux/io.h>
#endif
...
void adsp_update_c2c_memory_info(struct adsp_priv *pdata)
{
	...
	info[0] = adsp_get_reserve_mem_phys(ADSP_C2C_MEM_ID);
	info[1] = adsp_get_reserve_mem_size(ADSP_C2C_MEM_ID);

	item = pdata->mapping_table + ADSP_SHAREDMEM_C2C_BUFINFO;

	if (item->offset && item->size) {
		dst = pdata->dtcm + pdata->dtcm_size - item->offset;
		memcpy_toio(dst, &info, item->size);
	}
#else /* CONFIG_OPLUS_FEATURE_HFCALL */
...
#endif /* CONFIG_OPLUS_FEATURE_HFCALL */
}
