...
bool oplus_daemon_hifi_ssr(void)
{
	const char cmds[] = "oplus_adsp_test 666,";
	enum adsp_ipi_status ret = ADSP_IPI_DONE + 1;

	if (is_aed_work_busy()) {
		pr_info("[ADSP] aed_work busy skip adsp ssr.\n");
		return false;
	}

	if (_adsp_register_feature(ADSP_A_ID, SYSTEM_FEATURE_ID, 0) == 0) {
		ret = adsp_push_message(ADSP_IPI_ADSP_TIMER, (void *)cmds, strlen(cmds), 0, ADSP_A_ID);
		_adsp_deregister_feature(ADSP_A_ID, SYSTEM_FEATURE_ID, 0);
		pr_info("%s(), oplus set adsp crash, ret=%d\n", __func__, ret);
	}

	pr_info("%s(), exit, ret=%d\n", __func__, ret);
	return (ret == ADSP_IPI_DONE);
}
EXPORT_SYMBOL(oplus_daemon_hifi_ssr);
#endif
