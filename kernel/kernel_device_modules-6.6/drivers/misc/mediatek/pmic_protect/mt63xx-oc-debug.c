...
static struct reg_oc_debug_t mt6366_reg_oc_debug[] = {
...
static int md_reg_oc_notify(struct reg_oc_debug_t *reg_oc_dbg)
{
	...
	if (reg_oc_dbg->is_md_reg)
		...
		return 0;
#if IS_ENABLED(CONFIG_MTK_CCCI_DEVICES)
	ret = exec_ccci_kern_func(ID_PMIC_INTR,(char *)&data_int32, 4);
	if (ret)
...
static int mt63xx_oc_debug_probe(struct platform_device *pdev)
{
	...
	case MT6359_CHIP_ID:
		...
		break;
	case MT6366_CHIP_ID:
		ret = register_all_oc_notifier(pdev, mt6366_reg_oc_debug,
					       ARRAY_SIZE(mt6366_reg_oc_debug));
		break;
