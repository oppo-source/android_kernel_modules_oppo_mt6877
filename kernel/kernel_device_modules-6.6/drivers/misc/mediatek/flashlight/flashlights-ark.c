...
static int flash_notify_sys(struct notifier_block *this, unsigned long code, void *unused)
{
	if (code == SYS_DOWN || code == SYS_HALT || code == SYS_POWER_OFF) {
		if (ark_i2c_client) {
			ark_write_reg(ark_i2c_client, ARK_REG_LED_CTRL1, ARK_DISABLE);
			pr_info("[%s] system is power off or power down and flash led is need to be closed\n",
				__func__);
		} else {
			pr_info("[%s] ark_i2c_client is NULL, skip disabling flash\n",
				__func__);
		}
...
static void ark_i2c_remove(struct i2c_client *client)
{
	...
	unregister_reboot_notifier(&flash_notifier);
