...
static struct md_pt_priv md_pt_info[POWER_THROTTLING_TYPE_MAX] = {
	...
	[SOC_POWER_THROTTLING] = {
		...
		.max_thl_lv = BATTERY_PERCENT_LEVEL_NUM - 1,
		.max_src_lv = BATTERY_PERCENT_LEVEL_NUM - 1,
	}
};
...
static void lbat_thl_with_temp(int source, u32 src_lv, u32 temp_stage)
{
	...
	pr_info("%s: s:%d src_lv=%d temp_s=%d, thl_lv[%d]=%d max_s/t=%d/%d\n", __func__, source, src_lv, temp_stage, thl_lv_idx, thl_lv, priv->max_src_lv, priv->max_temp_stage);
...
static void md_lbat_dedicate_callback(unsigned int thd)
{
	...
	pr_info("%s: thd=%d lv=%d cur_src_lv(prev)=%d\n", __func__, thd, lv, priv->cur_src_lv);
...
static void md_pt_battery_percent_cb(enum BATTERY_PERCENT_LEVEL_TAG level)
{
	...
	if (level > md_pt_info[SOC_POWER_THROTTLING].max_src_lv)
		return;
...
static int mtk_md_power_throttling_probe(struct platform_device *pdev)
{
	...
	if (md_pt_info[SOC_POWER_THROTTLING].max_thl_lv > 0)
