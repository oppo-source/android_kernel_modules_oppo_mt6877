...
#if IS_ENABLED(CONFIG_MTK_GPU_MT6771_SUPPORT)
...
static int get_devinfo(void)
{
	...
	/* FTPGM */
	val[0] = read_efuse_by_offset(DEVINFO_OFF_0);
	val[1] = read_efuse_by_offset(DEVINFO_OFF_1);
	val[2] = read_efuse_by_offset(DEVINFO_OFF_2);
	val[3] = read_efuse_by_offset(DEVINFO_OFF_3);
	val[4] = read_efuse_by_offset(DEVINFO_OFF_4);
	val[5] = read_efuse_by_offset(DEVINFO_OFF_5);
	val[6] = read_efuse_by_offset(DEVINFO_OFF_6);
	val[7] = read_efuse_by_offset(DEVINFO_OFF_7);
	val[8] = read_efuse_by_offset(DEVINFO_OFF_8);
	val[9] = read_efuse_by_offset(DEVINFO_OFF_9);
	val[10] = read_efuse_by_offset(DEVINFO_OFF_16);
	val[11] = read_efuse_by_offset(DEVINFO_OFF_17);
	val[12] = read_efuse_by_offset(DEVINFO_OFF_18);
	...
	turbocode = (read_efuse_by_offset(CPUFREQ_SEG_CODE_OFF_0) >> 3) & 0x1;
	if (turbocode) {
		((int *)&turbo_bininfo)[0] =
			read_efuse_by_offset(TURBO_BIN_CODE_OFF_0);
...
/*
#if IS_ENABLED(CONFIG_MTK_LEGACY_THERMAL)
__weak int tscpu_get_temp_by_bank(enum thermal_bank_name ts_bank)
{
	eem_error("cannot find %s (thermal has not ready yet!\n", __func__);
	return 0;
}

__weak int tscpu_is_temp_valid(void)
{
	eem_error("cannot find %s (thermal has not ready yet!)\n", __func__);
	return 0;
}
...
static void eem_init_ctrl(struct eem_ctrl *ctrl)
{
	...
	if (1) {
		...
		ctrl->thread =
			kthread_run(eem_volt_thread_handler, ctrl, "%s", ctrl->name);
...
static int eem_buck_get(struct platform_device *pdev)
{
	...
	eem_regulator_vproc1 = devm_regulator_get_optional(&pdev->dev, "vproc11");
	if (!eem_regulator_vproc1) {
		eem_error("eem_regulator_vproc1 error\n");
		return -EINVAL;
	}

	eem_regulator_vproc2 = devm_regulator_get_optional(&pdev->dev, "vproc12");
