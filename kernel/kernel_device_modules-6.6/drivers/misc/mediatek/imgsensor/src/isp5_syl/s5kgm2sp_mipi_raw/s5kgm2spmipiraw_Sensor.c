...
#define PFX "S5KGM2SP_camera_sensor"
...
#define DEVICE_VERSION_S5KGM2SP    "s5kgm2sp"
...
#define AUXADC_CAMERA_VOLTAGE_CHANNEL   4
extern int CheckGmsFlag;
...
static struct imgsensor_info_struct imgsensor_info = {
  ...
  .mipi_settle_delay_mode = 1, //0,MIPI_SETTLEDELAY_AUTO; 1,MIPI_SETTLEDELAY_MANNUAL
	...
	.i2c_addr_table = {0x20, 0x5a,0x21,0x5B,0xff},
};
...
{  // Preview mode setting
   {0x02, 0x0A,   0x00,   0x08, 0x40, 0x00,
    ...
    0x01, 0x30, 0x274, 0xBA0, 0x03, 0x00, 0x0000, 0x0000},
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] =
{
	...
	//{ 8000, 6000,	 0,  	0, 8000, 6000, 4000, 3000, 0,	0, 4000, 3000, 0, 0, 4000, 3000},   /* hight speed video */
	{ 8000, 6000,  160,   840, 7680, 4320, 1920, 1080, 320,	180, 1280, 720, 0, 0, 1280, 720},	/* hight speed video */
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	UINT32 i = 0;
	if(pStereodata != NULL) {
		pr_debug("s5kgm2 SET_SENSOR_OTP: 0x%x %d 0x%x %d\n",
		...
		if ((pStereodata->uSensorId == S5KGM2SP_SENSOR_ID)
				&& (data_base == 0x1640)
				&& (data_length == 1561)) {
			pr_debug("s5kgm2 Write: %x %x %x %x %x %x %x %x\n", pData[0], pData[39], pData[40], pData[1556],
			...
			msleep(6);
			pr_debug("s5kgm2write_Module_data Write end\n");
		}else {
			pr_err("Invalid Sensor id:0x%x write_gm2 eeprom\n", pStereodata->uSensorId);
			return IMGSENSOR_RETURN_ERROR;
		}
	} else {
		pr_err("s5kgm2write_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	printk("[s5kgm2] gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
static void sensor_init(void)
{
  ...
  write_cmos_sensor(0x6F12, 0x0000);//0001);
...
static void preview_setting(void)
{
  ...
  write_cmos_sensor(0x6F12, 0x00B0);//10
  write_cmos_sensor(0x602A, 0x11B4);
  write_cmos_sensor(0x6F12, 0x0190);//360
  write_cmos_sensor(0x6028, 0x2000);
  ...
  write_cmos_sensor(0x6F12, 0x0604);//0004
  write_cmos_sensor(0x602A, 0x33FE);
  write_cmos_sensor(0x6F12, 0x0704);//0104
  write_cmos_sensor(0x602A, 0x3462);
  write_cmos_sensor(0x6F12, 0x0501);
  write_cmos_sensor(0x602A, 0x347C);
  write_cmos_sensor(0x6F12, 0x0000);//EEEE
  write_cmos_sensor(0x602A, 0x34A6);
  write_cmos_sensor(0x6F12, 0x5555);//EEEE
  write_cmos_sensor(0x602A, 0x34E4);
  write_cmos_sensor(0x6F12, 0x0101);
  write_cmos_sensor(0x602A, 0x34F2);
  write_cmos_sensor(0x6F12, 0x0000);
  write_cmos_sensor(0x0D00, 0x0101); //20210910
  write_cmos_sensor(0x0D02, 0x0101); //20210910
  write_cmos_sensor(0x0114, 0x0301); //20210910
  ...
  write_cmos_sensor(0x6F12, 0x0100);//0103
...
{
	// static int board_id = 0xff;
	// int bat_id_voltage = 0;
	// int res = 0;
    // int data[4] = {0,0,0,0};
	// int rawdata =0 ;
	...
	// if(board_id == 0xff)
	// {
		// res = IMM_GetOneChannelValue(AUXADC_CAMERA_VOLTAGE_CHANNEL,data,&rawdata);
		// bat_id_voltage = data[0] * 1000 + data[1] * 10;
		// printk("[s5kgm2sp]MIN_VOLTAGE = %d,MAX_VOLTAGE = %d\n",MIN_VOLTAGE,MAX_VOLTAGE);
		// if(bat_id_voltage>=MIN_VOLTAGE && bat_id_voltage <= MAX_VOLTAGE)
		// {
			// CheckGmsFlag = 0;
		// }
		// else
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			...
			if ((*sensor_id == imgsensor_info.sensor_id)||(*sensor_id == 0x08D2))
			{
				get_board_id();
				printk("[s5kgm2sp]CheckGmsFlag = %d\n",CheckGmsFlag);
				if(CheckGmsFlag == 0)
				{
				    #ifdef VENDOR_EDIT
				    ...
				    if(deviceInfo_register_value == 0x00){
				    	register_imgsensor_deviceinfo("Cam_b", DEVICE_VERSION_S5KGM2SP, module_id);
...
static kal_uint32 open(void)
{
	...
	if ((imgsensor_info.sensor_id != sensor_id)&&(0x08D2 != sensor_id)) {
		return ERROR_SENSOR_CONNECT_FAIL;
	}
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_MODULE_SN:
			LOG_INF("s5kgm2 GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			s5kgm2sp_read_otp_pdaf_data((kal_uint16)(*feature_data),
