...
#include "s5kgm2sp_pdaf_cal.h"


#include "s5kgm2spmipiraw_Sensor.h"
/*********************Modify Following Strings for Debug**********************/
#define PFX "s5kgm2sp_camera_pdaf"
...
static bool s5kgm2sp_selective_read_eeprom(kal_uint16 addr, BYTE *data)
{
...
static bool s5kgm2sp_read_eeprom(kal_uint16 addr, BYTE *data, kal_uint32 size)
{
	int i = 0;
	/* int offset = addr; */
	int offset = 0x901;
	...
	for (i = 0; i < 1024; i += 2) {
		if (!s5kgm2sp_selective_read_eeprom(offset, &data[i])) {
...
bool s5kgm2sp_read_otp_pdaf_data(
	kal_uint16 addr,
	BYTE *data,
	kal_uint32 size)
{
	...
	if (!s5kgm2sp_read_eeprom(addr, data, size)) {
		get_done = 0;
		last_size = 0;
		last_offset = 0;
