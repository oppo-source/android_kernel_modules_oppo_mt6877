...
#include "hi846mipiraw_oris_Sensor.h"
...
static int sn_flag_addr_tbl = 0x0201;
...
//static BYTE hi846_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };
unsigned char hi846_sn_valid = 0;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_SENSOR_ID_ORIS,
	...
	.hs_video = {
		...
		.max_framerate = 300,
		.mipi_pixel_rate = 144000000, //(720M*4/10)
	},
	.slim_video = {
		.pclk = 288000000,
		.linelength = 3800,
	...
	.i2c_addr_table = {0x40, 0x44, 0x42, 0x46, 0xff},
	.i2c_speed = 400,
};
...
void dumpEEPROMData1(int startAdd, int u4Length,u8* pu1Params)
{
	...
	for(i = 0; i < u4Length; i += 16){
		if(u4Length - i  >= 16){
			printk("eeprom[0x%04x 0x%04x]:0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x ",
...
static  int read_hi846_sn_info(void)
{
	...
	struct CAMERA_DEVICE_INFO *pCamDeviceObj = &gImgEepromInfoOrisC;
	int i = 0;

	/* sn group */
	/* read flag */
	write_cmos_sensor_8(OTP_SET_ADDR_H, (sn_flag_addr_tbl>>8)&0xff);
	write_cmos_sensor_8(OTP_SET_ADDR_L, (sn_flag_addr_tbl)&0xff);
	...
	if (otp_grp_flag == 0x01) {
		...
		i = -1;
		printk("invalid block module flag 0x%x", i);
		return -1;
	}
	...
	if(sn_start_addr != 0){
		...
		#endif
		printk("=== hi846 SN check_sum_sn=0x%x, check_sum_sn_cal=0x%x ===\n", check_sum_sn, check_sum_sn_cal);
		if(check_sum_sn == check_sum_sn_cal){
			printk("check_sum_sn success!\n");
			for (i = 0; i < SN_DATA_SIZE; i++) {
			...
			pCamDeviceObj->i4CurSensorId = HI846_SENSOR_ID_ORIS;
			return 1;
		}else{
			return 0;
			printk("check_sum_sn fail!\n");
		}
	} else {
		return 0;
	}
...
static kal_uint32 return_sensor_id(void)
{
	int i = 0;
	int j = 0;
	i = read_cmos_sensor(0x0F17);
	j = read_cmos_sensor(0x0F16);
	printk("sensor_id: i = %d,j = %d",i,j);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x095A, 0x0240);
...
static void hi846_otp_read_disable(void)
{
	write_cmos_sensor_8(0x0a00, 0x00);
	mdelay(10);
	write_cmos_sensor_8(0x003e, 0x00);
	write_cmos_sensor_8(0x004a, 0x01);
}
...
static int hi846_sensor_otp_info(void)
{
	...
	printk("read_hi846_sn_info return %d",ret);
...
static kal_uint32 open(void)
{
	...
	printk("[open]: PLATFORM:MT6762,MIPI 4LANE\n");
	printk("preview 1632*122430fps,360Mbps/lane;"
