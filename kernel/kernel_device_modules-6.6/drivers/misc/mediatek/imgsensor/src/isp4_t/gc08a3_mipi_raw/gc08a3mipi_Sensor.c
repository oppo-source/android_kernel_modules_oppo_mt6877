...
struct gc08a3_otp_struct gc08a3_otp;
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{ 3264, 2448,	 0,    0, 3264, 2448, 1632,  1224,
		0,    0, 1632,  1224,    0,    0, 1632,  1224}, // hight speed video
	{ 3264, 2448,	 0,    0, 3264, 2448, 1632,  1224,
		0,    0, 1632,  1224,    0,    0, 1632,  1224}  // slim video
};
...
#if VERSION_CHECK
static void gc08a3_otp_read_group(kal_uint16 addr, kal_uint8 *data, kal_uint16 length)
{
	kal_uint16 i = 0;
...
static kal_uint16 gc08a3_otp_init_addr_data[] = {
...
static kal_uint16 gc08a3_otp_close_addr_data[] = {
	0x0316, 0x01,
	0x0a67, 0x00,
};


kal_uint8 gc08a3_version_check(void)
{
	kal_uint8 legacy_id[2][7] = {
	{0x43, 0x30, 0x30, 0x32, 0x39, 0x34, 0x30},
	...
	table_write_cmos_sensor(gc08a3_otp_init_addr_data,
		sizeof(gc08a3_otp_init_addr_data) / sizeof(kal_uint16));

	mdelay(10);
	gc08a3_otp_read_group(0x0038, &current_id[0], 7);

	table_write_cmos_sensor(gc08a3_otp_close_addr_data,
		sizeof(gc08a3_otp_close_addr_data) / sizeof(kal_uint16));
...
kal_uint16 gc08a3_1632x1224_addr_data[] = {
...
kal_uint16 gc08a3_3264x2448_addr_data[] = {
...
static kal_uint8 gc08a3_read_otp_single(kal_uint16 addr)
{
	kal_uint8 data;
...
static void gc08a3_read_otp_continuous(kal_uint16 addr, kal_uint16 length, kal_uint8 *data)
{
	int i;
...
static void gc08a3_read_sensor_otp(void)
{
	kal_uint8 awb_flag, lsc_flag;

	otp_init_setting();

	awb_flag = gc08a3_read_otp_single(AWBINFO_FLAG_ADDR);
	LOG_INF("awb_flag = %d", awb_flag);
	if(awb_flag == 0x07) {
		gc08a3_otp.awb_group_flag = 2;
		gc08a3_read_otp_continuous(AWB_GROUP2_START_ADDR, 12, gc08a3_otp.awb_info);
		gc08a3_otp.awb_checksum = gc08a3_read_otp_single(AWB_GROUP2_CHECKSUM_ADDR);
	} else {
		gc08a3_otp.awb_group_flag = 1;
		gc08a3_read_otp_continuous(AWB_GROUP1_START_ADDR, 12, gc08a3_otp.awb_info);
		gc08a3_otp.awb_checksum = gc08a3_read_otp_single(AWB_GROUP1_CHECKSUM_ADDR);
	}

	lsc_flag = gc08a3_read_otp_single(LSCINFO_FLAG_ADDR);
	LOG_INF("lsc_flag = %d", lsc_flag);
	if(lsc_flag == 0x07) {
		gc08a3_otp.lsc_group_flag = 2;
		gc08a3_read_otp_continuous(LSC_GROUP2_START_ADDR, 1868, gc08a3_otp.lsc_info);
		gc08a3_otp.lsc_checksum = gc08a3_read_otp_single(LSC_GROUP2_CHECKSUM_ADDR);
	} else {
		gc08a3_otp.lsc_group_flag = 1;
		gc08a3_read_otp_continuous(LSC_GROUP1_START_ADDR, 1868, gc08a3_otp.lsc_info);
		gc08a3_otp.lsc_checksum = gc08a3_read_otp_single(LSC_GROUP1_CHECKSUM_ADDR);
	}
			...
			unsigned char *data, unsigned int size)
{
	...
	if(addr == 0 && size == 0x1500) //Avoiding preloading, hal print error log "Preload data failed"
		...
		memcpy(data, gc08a3_otp.lsc_info, size * sizeof(kal_uint8));
	else if(addr == LSCINFO_FLAG_ADDR && size == 1)
		memcpy(data, &(gc08a3_otp.lsc_checksum), size * sizeof(kal_uint8));
	else if(addr == AWBINFO_FLAG_ADDR && size == 12)
		memcpy(data, gc08a3_otp.awb_info, size * sizeof(kal_uint8));
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info("[gc08a3_camera_sensor] %s :i2c write id: 0x%x, sensor id: 0x%x\n",
					__func__, imgsensor.i2c_write_id, *sensor_id);
				gc08a3_read_sensor_otp();
				return ERROR_NONE;
			}
			pr_info("[gc08a3_camera_sensor] %s :Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				__func__, imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
#if VERSION_CHECK
	if(gc08a3_version_check()){
		write_cmos_sensor_8bit(0x008c, 0x01);
		pr_debug("[gc08a3_camera_sensor] %s : Sensor Version Error! Please Contact FAE!!!\n",
			__func__);
	}
#endif
