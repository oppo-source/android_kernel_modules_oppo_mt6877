...
{
	kal_uint32 ker = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));

	LOG_INF("ker read id : %d\n",ker);
	return  ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));//;//0x5a
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_8(0x0100, 0x00);
	LOG_INF("  debug : on init setting E\n");
...
static kal_uint32 open(void)
{
	...
	//UINT32 *sensor_id2  = kmalloc(FeatureParaLen, GFP_KERNEL);
	kal_uint32 cnt = 0;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		//spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];//0x5a;
		//spin_unlock(&imgsensor_drv_lock);
		//LOG_INF("no importance sensor id: 0x%x\n",return_sensor_id());
		...
		do {
			...
			LOG_INF("Readi2c id: 0x%x sensor id : %x cnt %d\n",imgsensor.i2c_write_id,sensor_id,cnt);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				LOG_INF("Read id fail_id: 0x%x\n",
				imgsensor.i2c_write_id);
				pr_debug("Read id fail id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
				mdelay(100);
			retry--;
		} while (retry > 0);
