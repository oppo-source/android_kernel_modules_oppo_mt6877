...
static struct slbc_config p_config[] = {
	...
	SLBC_ENTRY(UID_BIF,        8,   0,   0,   0, 0x0,   0x00f, 0),
};
...
static int slbc_check_uid_gid(enum slc_ach_uid uid, int gid)
{
	int ret = 0;
...
static void slbc_get_gid_by_req(enum slc_ach_uid uid, int *gid)
{
	if (*gid != GID_REQ)
		return;
...
#ifdef SLBC_AOV_MAE_WA
	if (uid == ID_MAE && gid_ref[GID_AOV]) {
		SLBC_TRACE_REC(LVL_ERR, TYPE_C, uid, -EINVAL, "MAE conflicts with AOV");
		return -EINVAL;
	} else if (uid == ID_AOV && gid_ref[GID_MAE]) {
		SLBC_TRACE_REC(LVL_ERR, TYPE_C, uid, -EINVAL, "AOV conflicts with MAE");
		return -EINVAL;
	}
#endif /* SLBC_AOV_MAE_WA */
...
void slbc_get_gid_for_dma(struct dma_buf *dmabuf_2)
{
	...
	if (ret) {
		SLBC_TRACE_REC(LVL_ERR, TYPE_C, 0, ret, "dma_buf_vmap_unlocked failed");
		goto err1;
	}
