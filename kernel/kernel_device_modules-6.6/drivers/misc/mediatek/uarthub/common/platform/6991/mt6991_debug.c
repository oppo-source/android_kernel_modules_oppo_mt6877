...
struct uarthub_debug_ops_struct mt6991_plat_debug_data = {
	.uarthub_plat_get_intfhub_base_addr = uarthub_get_intfhub_base_addr_mt6991,
...
int uarthub_get_adsp_uart_mux_info_mt6991(void)
{
	if (!topckgen_base_remap_addr_mt6991) {
		pr_notice("[%s] topckgen_base_remap_addr_mt6991 is NULL\n", __func__);
		return -1;
	}

	return (UARTHUB_REG_READ_BIT(topckgen_base_remap_addr_mt6991 + CLK_CFG_13,
		CLK_CFG_13_ADSP_UARTHUB_BCLK_SEL_MASK) >> CLK_CFG_13_ADSP_UARTHUB_BCLK_SEL_SHIFT);
}
...
int uarthub_get_uart_mux_info_mt6991(void)
{
	...
	return (UARTHUB_REG_READ_BIT(topckgen_base_remap_addr_mt6991 + CLK_CFG_6,
...
int uarthub_get_uarthub_mux_info_mt6991(void)
{
	...
	return (UARTHUB_REG_READ_BIT(topckgen_base_remap_addr_mt6991 + CLK_CFG_16,
		CLK_CFG_16_UARTHUB_BCLK_SEL_MASK) >> CLK_CFG_16_UARTHUB_BCLK_SEL_SHIFT);
}
...
int uarthub_get_uarthub_cg_info_mt6991(int *p_topckgen_cg, int *p_peri_cg)
{
	...
	topckgen_cg = (UARTHUB_REG_READ_BIT(topckgen_base_remap_addr_mt6991 + CLK_CFG_16,
		CLK_CFG_16_PDN_UARTHUB_BCLK_MASK) >> CLK_CFG_16_PDN_UARTHUB_BCLK_SHIFT);

	peri_cg = (UARTHUB_REG_READ_BIT(pericfg_ao_remap_addr_mt6991 + PERI_CG_2,
...
#if !(UARTHUB_SUPPORT_FPGA)
int uarthub_get_peri_uart_pad_mode_mt6991(void)
{
	if (!pericfg_ao_remap_addr_mt6991) {
		pr_notice("[%s] pericfg_ao_remap_addr_mt6991 is NULL\n", __func__);
		return -1;
	...
	/* 1: UART_PAD mode */
	/* 0: UARTHUB mode */
	return (UARTHUB_REG_READ_BIT(pericfg_ao_remap_addr_mt6991 + PERI_UART_WAKEUP,
...
int uarthub_get_uart_src_clk_info_mt6991(void)
{
	...
	return (UARTHUB_REG_READ_BIT(pericfg_ao_remap_addr_mt6991 + PERI_CLOCK_CON,
...
{
	int spm_res_uarthub_1 = 0, spm_res_uarthub_2 = 0;
	int spm_res_internal = 0, spm_res_26m_off = 0;
	...
	/* SPM_REQ_STA_15[3] = UART_HUB_VRF18_REQ */
	/* SPM_REQ_STA_15[2] = UART_HUB_VCORE_REQ */
	/* SPM_REQ_STA_15[1] = UART_HUB_SRCCLKENA */
	/* SPM_REQ_STA_15[0] = UART_HUB_PMIC_REQ */
	spm_res_uarthub_1 = (UARTHUB_REG_READ_BIT(
		spm_remap_addr_mt6991 + SPM_REQ_STA_15,
		SPM_REQ_STA_15_UARTHUB_REQ_MASK) >>
		SPM_REQ_STA_15_UARTHUB_REQ_SHIFT);

	/* SPM_REQ_STA_14[31] = UART_HUB_INFRA_REQ */
	spm_res_uarthub_2 = (UARTHUB_REG_READ_BIT(
		spm_remap_addr_mt6991 + SPM_REQ_STA_14,
		SPM_REQ_STA_14_UARTHUB_REQ_MASK) >>
		SPM_REQ_STA_14_UARTHUB_REQ_SHIFT);

	if (p_spm_res_uarthub)
		*p_spm_res_uarthub = ((spm_res_uarthub_1 << 1) | (spm_res_uarthub_2));

#if SPM_RES_CHK_EN
	if (spm_res_uarthub_1 != 0xF || spm_res_uarthub_2 != 0x1)
		return 0;
#endif

	spm_res_internal = UARTHUB_REG_READ(spm_remap_addr_mt6991 + MD32PCM_SCU_CTRL1);
	spm_res_26m_off = UARTHUB_REG_READ(spm_remap_addr_mt6991 + MD32PCM_SCU_CTRL0);
...
#if !(UARTHUB_SUPPORT_FPGA)
int uarthub_get_gpio_trx_info_mt6991(struct uarthub_gpio_trx_info *info)
{
	if (!info) {
	...
	if (!gpio_base_remap_addr_mt6991) {
		pr_notice("[%s] gpio_base_remap_addr_mt6991 is NULL\n", __func__);
		return -1;
	}

	if (!iocfg_tm3_remap_addr_mt6991) {
		pr_notice("[%s] iocfg_tm3_remap_addr_mt6991 is NULL\n", __func__);
		return -1;
	}

	UARTHUB_READ_GPIO_BIT(info->tx_mode, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_HUB_MODE_TX, GPIO_HUB_MODE_TX_MASK, GPIO_HUB_MODE_TX_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->tx_dir, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_HUB_MODE_TX_DIR, GPIO_HUB_MODE_TX_DIR_MASK, GPIO_HUB_MODE_TX_DIR_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->tx_dataout, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_HUB_MODE_TX_DATAOUT, GPIO_HUB_MODE_TX_DATAOUT_MASK, GPIO_HUB_MODE_TX_DATAOUT_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->tx_pu, IOCFG_TM3_BASE_ADDR, iocfg_tm3_remap_addr_mt6991,
		GPIO_HUB_MODE_TX_PU, GPIO_HUB_MODE_TX_PU_MASK, GPIO_HUB_MODE_TX_PU_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->tx_pd, IOCFG_TM3_BASE_ADDR, iocfg_tm3_remap_addr_mt6991,
		GPIO_HUB_MODE_TX_PD, GPIO_HUB_MODE_TX_PD_MASK, GPIO_HUB_MODE_TX_PD_SHIFT);

	UARTHUB_READ_GPIO_BIT(info->rx_mode, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_HUB_MODE_RX, GPIO_HUB_MODE_RX_MASK, GPIO_HUB_MODE_RX_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->rx_dir, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_HUB_MODE_RX_DIR, GPIO_HUB_MODE_RX_DIR_MASK, GPIO_HUB_MODE_RX_DIR_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->rx_dataout, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_HUB_MODE_RX_DATAOUT, GPIO_HUB_MODE_RX_DATAOUT_MASK, GPIO_HUB_MODE_RX_DATAOUT_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->rx_pu, IOCFG_TM3_BASE_ADDR, iocfg_tm3_remap_addr_mt6991,
		GPIO_HUB_MODE_RX_PU, GPIO_HUB_MODE_RX_PU_MASK, GPIO_HUB_MODE_RX_PU_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->rx_pd, IOCFG_TM3_BASE_ADDR, iocfg_tm3_remap_addr_mt6991,
		GPIO_HUB_MODE_RX_PD, GPIO_HUB_MODE_RX_PD_MASK, GPIO_HUB_MODE_RX_PD_SHIFT);

	UARTHUB_READ_GPIO_BIT(info->bt_rst_mode, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_BT_RST_PIN, GPIO_BT_RST_PIN_MASK, GPIO_BT_RST_PIN_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->bt_rst_dir, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_BT_RST_PIN_DIR, GPIO_BT_RST_PIN_DIR_MASK, GPIO_BT_RST_PIN_DIR_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->bt_rst_dataout, GPIO_BASE_ADDR, gpio_base_remap_addr_mt6991,
		GPIO_BT_RST_PIN_DATAOUT, GPIO_BT_RST_PIN_DATAOUT_MASK, GPIO_BT_RST_PIN_DATAOUT_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->bt_rst_pu, IOCFG_TM3_BASE_ADDR, iocfg_tm3_remap_addr_mt6991,
		GPIO_BT_RST_PIN_PU, GPIO_BT_RST_PIN_PU_MASK, GPIO_BT_RST_PIN_PU_SHIFT);
	UARTHUB_READ_GPIO_BIT(info->bt_rst_pd, IOCFG_TM3_BASE_ADDR, iocfg_tm3_remap_addr_mt6991,
...
int uarthub_dump_intfhub_debug_info_mt6991(const char *tag)
{
	...
	val = uarthub_is_apb_bus_clk_enable_mt6991();
	ret = snprintf(dmp_info_buf + len, DBG_LOG_LEN - len,
		",APB_BUS=[%d]", val);
	if (ret > 0)
		len += ret;
	...
	val = uarthub_get_uarthub_cg_info_mt6991(&topckgen_cg, &peri_cg);
	if (val >= 0) {
	...
	val = uarthub_get_uart_src_clk_info_mt6991();
	if (val >= 0) {
	...
	val = uarthub_get_peri_uart_pad_mode_mt6991();
	if (val >= 0) {
		/* the expect value is 0x0 */
		ret = snprintf(dmp_info_buf + len, DBG_LOG_LEN - len,
	...
	val = uarthub_get_spm_res_info_mt6991(
	...
	val = uarthub_get_hwccf_univpll_on_info_mt6991();
	if (val >= 0) {
		/* the expect value is 0x1 */
		ret = snprintf(dmp_info_buf + len, DBG_LOG_LEN - len,
			",UNIVPLL=[%d]", val);
	...
	val = uarthub_get_uarthub_mux_info_mt6991();
	val1 = uarthub_get_uart_mux_info_mt6991();
	val2 = uarthub_get_adsp_uart_mux_info_mt6991();
	if (val >= 0) {
	...
	ret = snprintf(dmp_info_buf + len, DBG_LOG_LEN - len, ",HUB/UART/ADSP_CLK=[%d-%d-%d]",
		...
		mt_get_fmeter_freq(FM_ADSP_UARTHUB_B_CK, CKGEN));
	if (ret > 0)
		len += ret;
	...
	len = 0;
	val = uarthub_get_gpio_trx_info_mt6991(&gpio_base_addr);
	if (val == 0) {
		ret = snprintf(dmp_info_buf + len, DBG_LOG_LEN - len,
	...
	ret = snprintf(dmp_info_buf, DBG_LOG_LEN,
		...
		def_tag, ((tag == NULL) ? "null" : tag), MT6991_UARTHUB_DUMP_VERSION,
		dev0_sta, dev1_sta, dev2_sta);
	if (ret > 0)
		len += ret;
#endif
...
int uarthub_dump_debug_monitor_packet_info_mode_mt6991(const char *tag)
{
...
int uarthub_dump_debug_monitor_check_data_mode_mt6991(const char *tag)
{
	int debug_monitor_sel = 0;
...
int uarthub_dump_debug_monitor_crc_result_mode_mt6991(const char *tag)
{
	int debug_monitor_sel = 0;
...
int uarthub_dump_debug_tx_rx_count_mt6991(const char *tag, int trigger_point)
{
	...
	if (uarthub_is_apb_bus_clk_enable_mt6991() == 0) {
		pr_notice("[%s] apb bus clk disable\n", __func__);
		return UARTHUB_ERR_APB_BUS_CLK_DISABLE;
	}

#if !(UARTHUB_SUPPORT_FPGA)
	uarthub_get_gpio_trx_info_mt6991(&gpio_base_addr);
#endif
...
int uarthub_dump_debug_apdma_uart_info_mt6991(const char *tag)
{
	...
	pr_info("[%s][%s] 0=[0x%x],4=[0x%x],8=[0x%x],c=[0x%x],10=[0x%x],14=[0x%x],18=[0x%x]\n",
		def_tag, ((tag == NULL) ? "null" : tag),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x00),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x04),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x08),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x0c),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x10),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x14),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x18));

	pr_info("[%s][%s] 1c=[0x%x],20=[0x%x],24=[0x%x],28=[0x%x],2c=[0x%x],30=[0x%x],34=[0x%x]\n",
		def_tag, ((tag == NULL) ? "null" : tag),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x1c),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x20),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x24),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x28),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x2c),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x30),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x34));

	pr_info("[%s][%s] 38=[0x%x],3c=[0x%x],40=[0x%x],44=[0x%x],48=[0x%x],4c=[0x%x],50=[0x%x]\n",
		def_tag, ((tag == NULL) ? "null" : tag),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x38),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x3c),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x40),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x44),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x48),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x4c),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x50));

	pr_info("[%s][%s] 54=[0x%x],58=[0x%x],5c=[0x%x],60=[0x%x],64=[0x%x]\n",
		def_tag, ((tag == NULL) ? "null" : tag),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x54),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x58),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x5c),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x60),
		UARTHUB_REG_READ(apdma_uart_tx_int_remap_addr_mt6991 + 0x64));
...
int uarthub_dump_sspm_log_mt6991(const char *tag)
{
	...
	g_buf_mt6991[0] = '\0';
	log_addr = UARTHUB_LOG_IRQ_IDX_ADDR(sys_sram_remap_addr_mt6991);
	...
	for (i = 0; i < UARTHUB_IRQ_OP_LOG_SIZE; i++) {
		...
		n = snprintf(tmp + used, DBG_LOG_LEN - used, "[%llu:%X] ",
							t,
	...
	pr_info("[%s][%s] [%x] %s",
		def_tag, ((tag == NULL) ? "null" : tag), irq_idx, g_buf_mt6991);

	log_addr = UARTHUB_LOG_TSK_IDX_ADDR(sys_sram_remap_addr_mt6991);
	...
	for (i = 0; i < UARTHUB_TSK_OP_LOG_SIZE; i++) {
		t = UARTHUB_REG_READ(log_addr);
		t = t << 32 | UARTHUB_REG_READ(log_addr + 4);
		n = snprintf(tmp + used, DBG_LOG_LEN - used, "[%llu:%x-%x-%x]",
							t,
							UARTHUB_REG_READ(log_addr + 8),
	...
	log_addr = UARTHUB_CK_CNT_ADDR(sys_sram_remap_addr_mt6991);
	val = UARTHUB_REG_READ(log_addr);

	log_addr = UARTHUB_LAST_CK_ON(sys_sram_remap_addr_mt6991);
	v1 = UARTHUB_REG_READ(log_addr);
	v2 = UARTHUB_REG_READ(log_addr + 4);

	log_addr = UARTHUB_LAST_CK_ON_CNT(sys_sram_remap_addr_mt6991);
...
int uarthub_stop_sample_baud_rate_mt6991(void)
{
	int cg_en = 0;
