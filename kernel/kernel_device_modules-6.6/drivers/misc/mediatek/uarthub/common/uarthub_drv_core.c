...
static void trigger_uarthub_error_worker_handler(struct work_struct *work)
{
	...
	/* Skip assert when ONLY in dev0 tx timeout */
	if (err_total == 1 && (err_type & (0x1 << dev0_tx_timeout_err)))
		skipAssert = true;
	...
	if (g_core_irq_callback && !skipAssert)
