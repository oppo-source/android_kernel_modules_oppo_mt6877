...
static void mt6360_enable_irq(struct mt6360_chip *chip, const char *name, bool en);
...
static int mt6360_enable_usbid_polling(struct mt6360_chip *chip, bool en)
{
	...
	if (en) {
		if (chip->usbid_as_wd0 == true) {
			...
			if (ret < 0) {
				dev_err(chip->dev, "%s usbid src on 40us fail\n", __func__);
				return ret;
			}

			ret = charger_dev_set_usbid_rup(chip->chgdev, 5000);
			if (ret < 0) {
				dev_err(chip->dev, "%s usbid rup5k fail\n", __func__);
				return ret;
			}
			/* set usbid waveform period 100 : 100 * IS_TDET */
			ret = charger_dev_set_usbid_period(chip->chgdev, 100);
			if (ret < 0) {
...
static irqreturn_t mt6360_pmu_usbid_evt_handler(int irq, void *data)
{
	...
	tcpci_unlock_typec(chip->tcpc);
	queue_delayed_work(system_freezable_wq, &chip->usbid_evt_dwork, msecs_to_jiffies(0));
#else
	tcpci_lock_typec(chip->tcpc);
	mt6360_enable_irq(chip, "usbid_evt", false);
	tcpci_unlock_typec(chip->tcpc);
