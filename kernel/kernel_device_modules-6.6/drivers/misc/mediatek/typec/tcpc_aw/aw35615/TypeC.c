...
void StateMachineTypeC(Port_t *port)
{
	do {
		...
		if (port->ConnState != port->old_ConnState) {
			AW_LOG("TC_ST=%s\n", TYPEC_STATE_TBL[port->ConnState]);
...
void StateMachineUnattached(Port_t *port)
{
	...
	port->src_cap_header.NumDataObjects = port->src_pdo_size + port->src_pps_size;
	...
	port->snk_cap_header.NumDataObjects = port->snk_pdo_size + port->snk_pps_size;
...
void SetStateUnattached(Port_t *port)
{
	...
	port->Registers.Control5.BD_DTCT_MSB = 4;
	DeviceWrite(port, regControl5, 2, &port->Registers.Control5.byte[0]);
	...
	port->Registers.UfpDfpcnt.UFPCNT = port->snk_tog_time;
	port->Registers.UfpDfpcnt.DFPCNT = port->src_tog_time;
	DeviceWrite(port, regControl7, 2, &port->Registers.UfpDfpcnt.byte[0]);
...
#ifdef AW_HAVE_LPD
	port->Registers.Control4.CC2_OV_TH = port->cc2_ov_th;
	port->Registers.Control5.CC1_OV_TH = port->cc1_ov_th;
	port->Registers.Control9.LPD_TIM_SET = port->lpd_tim_set;
	DeviceWrite(port, regControl4, 1, &port->Registers.Control4.byte);
	DeviceWrite(port, regControl9, 1, &port->Registers.Control9.byte);
#endif /* AW_HAVE_LPD */
...
void clearState(Port_t *port)
{
	...
	DeviceWrite(port, regMask, 1, &port->Registers.Mask.byte);
	port->Registers.MaskAdv.byte[0] = 0xFF;
	port->Registers.MaskAdv.byte[1] = 0xFF;
