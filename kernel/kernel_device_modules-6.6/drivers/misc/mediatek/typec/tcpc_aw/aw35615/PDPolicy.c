...
void USBPDDisable(Port_t *port, AW_BOOL DeviceUpdate)
{
	...
	port->Registers.MaskAdv.M_BAD_CRC = 1;
...
//	}
	...
	case peGetManufacturerInfo:
		PolicyGetManufacturerInfot(port);
		break;
	default:
#ifdef AW_HAVE_VDM
...
void PolicySourceStartup(Port_t *port)
{
	...
	case 0:
		...
		port->Registers.MaskAdv.M_BAD_CRC = 0;
		DeviceWrite(port, regMaska, 2, &port->Registers.MaskAdv.byte[0]);
	...
	if (port->ProtocolMsgRx) {
		...
		if (port->PolicyRxHeader.NumDataObjects == 0) {
			...
			case EXTGetBatteryStatus:
				...
				break;
			case EXTGetManufacturerInfo:
				if (Get_Manufacturer_Info)
					SetPEState(port, peGetManufacturerInfo);
				else
					SetPEState(port, peNotSupported);
				break;
			default:
...
#ifdef AW_HAVE_DRP
	if (port->PolicyRxHeader.SpecRevision == USBPDSPECREV2p0)
		NumDataObjects = DPM_GetSinkCapHeader(port->dpm, port)->NumDataObjects - port->snk_pps_size;
	else
		NumDataObjects = DPM_GetSinkCapHeader(port->dpm, port)->NumDataObjects;
...
void PolicySinkStartup(Port_t *port)
{
	...
	port->bak_sdac = port->Registers.Slice.SDAC;
	DeviceWrite(port, regSlice, 1, &port->Registers.Slice.byte);
	//port->Registers.Switches.AUTO_CRC = 1;
	DeviceWrite(port, regSwitches1, 1, &port->Registers.Switches.byte[1]);
...
void PolicySinkWaitCaps(Port_t *port)
{
	if (port->ProtocolMsgRx) {
		...
		DeviceWrite(port, regPower, 1, &port->Registers.Power.byte);
		port->PEIdle = AW_TRUE;
		AW_LOG("pd connect typec only snk\n");
	}
}
...
void PolicySinkEvaluateCaps(Port_t *port)
{
	...
	for (i = DPM_GetSinkCapHeader(port->dpm, port)->NumDataObjects; i > 0; i--) {
		if (DPM_GetSinkCap(port->dpm, port)[i - 1].PDO.SupplyType == pdoTypeFixed) {
...
{
	memset(port->SrcCapExt.byte, 0, sizeof(port->SrcCapExt.byte));
...
void PolicyGetManufacturerInfot(Port_t *port)
{
	AW_U32 len;
	AW_U8 i;
	char *manufacturer = "Not Supported";
	port->SrcCapExt.VID = 0x29CF;
	port->SrcCapExt.PID = 0x5081;
	port->SrcCapExt.byte[5] = 0;
	port->GetExtFlag = AW_TRUE;
	len = 5;
	if (port->ProtocolRxBuffer[2]) {
		port->SrcCapExt.VID = 0xFFFF;
		port->SrcCapExt.PID = 0x0;
		len = 18;
		for (i = 0; i < 13; i++)
			port->SrcCapExt.byte[4 + i] = (char)manufacturer[i];
	}
	PolicySendData(port, EXTManufacturerInfo, port->SrcCapExt.byte, len,
			port->PolicyIsSource ? peSourceReady : peSinkReady, 0,
			SOP_TYPE_SOP, AW_TRUE);
	port->ExtChunkOffset = 0;
}
...
void policyBISTCarrierMode2(Port_t *port)
{
	...
	switch (port->PolicySubIndex) {
	case 0:
		if (port->chip_id == AW35615P_CHIP_ID) {
			port->Registers.Control5.EN_EYE = 0x1;
			DeviceWrite(port, regControl5, 1, &port->Registers.Control5.byte[0]);
		}
