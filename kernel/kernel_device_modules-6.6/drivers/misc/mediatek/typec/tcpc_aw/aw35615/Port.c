...
void InitializeRegisters(Port_t *port)
{
	...
	DeviceRead(port, regControl5, 2, &port->Registers.Control5.byte[0]);
...
void InitializeEfuseRetry(Port_t *port)
{
	AW_U8 data_buf;

	data_buf = 0xc2;
	DeviceWrite(port, 0xf9, 1, &data_buf);
	data_buf = 0x01;
	DeviceWrite(port, 0xe2, 1, &data_buf);
	data_buf = 0x0F;
	DeviceWrite(port, 0x0b, 1, &data_buf);
	data_buf = 0x00;
	DeviceWrite(port, 0xf3, 1, &data_buf);
	data_buf = 0x52;
	DeviceWrite(port, 0xf4, 1, &data_buf);
