...
#define biolog_fmt2 "wl:%lld%%,%lld%%,%lld,%lld,%lld,%lld"
	...
	struct seq_file *seq, struct mtk_blocktag *btag)
{
	...
	for (i = end;;) {
		...
		if (tr->workload.period != 0){
			...
			w_speed_curr = tr->throughput[BTAG_IO_WRITE].size;
			if (period_ms != 0)
				do_div(w_speed_curr, period_ms);
		}

		if (tr->throughput[BTAG_IO_READ].size != 0 && tr->throughput[BTAG_IO_WRITE].size != 0){
			write_percent = tr->throughput[BTAG_IO_WRITE].size*100;
			do_div(write_percent, (tr->throughput[BTAG_IO_WRITE].size + tr->throughput[BTAG_IO_READ].size));
		}

		if (tr->workload.usage != 0){
			usage_ms = tr->workload.usage;
			do_div(usage_ms, 1000000);  /* convert ns to ms */
			w_speed_full = tr->throughput[BTAG_IO_WRITE].size;
			if (usage_ms != 0)
		...
		throughput_w_all += tr->throughput[BTAG_IO_WRITE].size;
		throughput_r_all += tr->throughput[BTAG_IO_READ].size;
	...
	BTAG_PRINTF(buff, size, seq, biolog_fmt2,
		(__u64)percent_all,
		write_percent_all,
		usage_ms_all,
		period_ms_all,
	...
	struct seq_file *seq)
{
	...
	BTAG_PRINTF(buff, size, seq, "earaio control unit: %zu bytes\n",
