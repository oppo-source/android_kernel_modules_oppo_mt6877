...
DEFINE_SPINLOCK(swpm_sub_data_spinlock);
//struct mem_swpm_data mem_idx_snap;
struct share_subsys_data sub_idx_snap;
...
static int32_t swpm_res_group_info_get(uint32_t grp, uint32_t *out1, uint32_t *out2, uint32_t *out3)
{
	unsigned int ret = 0;
...
static int32_t swpm_res_group_id_get(uint32_t ip1, uint32_t ip2, uint32_t ip3,
...
static void subsys_idx_snapshot(void)
{
	unsigned long flags;
...
static int swpm_sub_data_event(struct notifier_block *nb,
				unsigned long event, void *v)
{
	switch (event) {
	case SWPM_LOG_DATA_NOTIFY:
		// pr_notice("%s SWPM_LOG_DATA_NOTIFY\n", __func__);
		subsys_idx_snapshot();
		break;
	default:
		break;
...
void swpm_v6991_ext_init(void)
{
	...
	for (i = 0; i < NR_XPU_IP; i++) {
		strscpy(xpu_ip_stats[i].ip_name,
