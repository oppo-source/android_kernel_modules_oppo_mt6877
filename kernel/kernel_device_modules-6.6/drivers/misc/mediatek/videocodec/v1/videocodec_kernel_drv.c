...
static int vcodec_open(struct inode *inode, struct file *file)
{
	...
	pr_info("%s pid = %d, gDrvInitParams->drvOpenCount %d\n",
			__func__, current->pid, atomic_read(&gDrvInitParams->drvOpenCount));
	mutex_unlock(&gDrvInitParams->driverOpenCountLock);
					...
					unsigned long arg)
{
	...
	{
		struct VAL_POWER_T power_data;
		unsigned char *user_data_addr = NULL;

		user_data_addr = (unsigned char *)arg;

		ret = (long)copy_from_user(&power_data, user_data_addr,
...
static int vcodec_probe(struct platform_device *pdev)
{
	...
	gDrvInitParams->vcodec_class = class_create(VCODEC_DEVNAME);
