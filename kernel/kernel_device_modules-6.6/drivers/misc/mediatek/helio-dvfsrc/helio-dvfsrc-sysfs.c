...
static struct mtk_pm_qos_request dvfsrc_emi_request;
static struct mtk_pm_qos_request dvfsrc_vcore_request;
				 ...
				 char *buf)
{
	...
	p += snprintf(p, buff_end - p, "%-24s: %d\n",
			"QOS_EMI_OPP",
			mtk_pm_qos_request(MTK_PM_QOS_EMI_OPP));
	p += snprintf(p, buff_end - p, "%-24s: %d\n",
			"QOS_VCORE_OPP",
				  ...
				  const char *buf, size_t count)
{
	...
	if (!strcmp(cmd, "kir_emi"))
		mtk_pm_qos_update_request(&dvfsrc_emi_request, val);
	else if (!strcmp(cmd, "kir_vcore"))
		mtk_pm_qos_update_request(&dvfsrc_vcore_request, val);
...
{
	mtk_pm_qos_add_request(&dvfsrc_emi_request,
		MTK_PM_QOS_EMI_OPP, MTK_PM_QOS_EMI_OPP_DEFAULT_VALUE);
	mtk_pm_qos_add_request(&dvfsrc_vcore_request, MTK_PM_QOS_VCORE_OPP,
