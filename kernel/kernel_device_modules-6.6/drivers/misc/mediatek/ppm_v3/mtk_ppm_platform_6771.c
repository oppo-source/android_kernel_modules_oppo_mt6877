...
static unsigned int (*mt_cpufreq_get_cpu_level_cb)(void);
void ppm_cpufreq_get_cpu_level_register(unsigned int (*cb)(void))
{
	mt_cpufreq_get_cpu_level_cb = cb;
}
EXPORT_SYMBOL(ppm_cpufreq_get_cpu_level_register);
...
static void ppm_get_cluster_status(struct ppm_cluster_status *cl_status)
{
	...
	for_each_ppm_clusters(i) {
		...
		ppm_cpufreq_get_cur_phy_freq_no_lock_get(i), CPUFREQ_RELATION_L);
	}
}
...
int ppm_platform_init(void)
{
	...
	online_core = ioremap(PPM_ONLINE_CORE_SRAM_ADDR, 4 * NR_PPM_CLUSTERS);
	if (!online_core) {
...
EXPORT_SYMBOL(mt_ppm_thermal_get_power_big_max_opp);
