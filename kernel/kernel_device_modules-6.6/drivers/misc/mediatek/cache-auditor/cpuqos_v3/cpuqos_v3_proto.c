...
static ssize_t policy_proc_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	len = sprintf(s, "%d %d %d %d %d %s %d %d %d %d %d\n",
			policy, init_pd, init_rank, target_pd, target_rank, mpam_sub_target,
			uah_cgrp, uah_flt_grp, cpuqos_perf_mode, uah_pcgrp, uah_bitmap);
