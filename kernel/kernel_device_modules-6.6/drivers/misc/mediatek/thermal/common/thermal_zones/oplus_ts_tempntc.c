...
void oplus_tempntc_read_ntcswitch1_high(struct temp_data *info) {
	int ret;
        int iio_chan1_volt;
        int iio_chan3_volt;
	if (is_project(23703) || is_project(23704) || is_project(24777)) {
...
void oplus_tempntc_read_ntcswitch2_low(struct temp_data *info) {
	...
	int iio_chan0_volt;
	int iio_chan3_volt;
	if (is_project(23703) || is_project(23704) || is_project(24777) || is_project(24700) || is_project(24701) || is_project(24702) || is_project(24709)) {
		ret = iio_read_channel_processed(info->iio_channel_btc, &iio_chan3_volt);
		if (ret < 0) {
			pr_err("CHARGE_NTC read error!\n");
		} else {
				...
				struct device *dev)
{
	...
	if (is_project(23703) || is_project(23704) || is_project(24777)) {
		...
		info->iio_channel_btc = iio_channel_get(dev, "auxadc3-pa_or_charge_v");
		pr_err("PA CHARGE_NTC CHANNEL \n");
		if (IS_ERR(info->iio_channel_btc)){
			pr_err("PA CHARGE_NTC CHANNEL ERR \n");
		}
