...
/* ************************************ */
static bool evt2_ntc_solution = false;
...
static int get_hw_PA_4G_temp(void)
{
	...
	} else if (is_project(25706) || is_project(25707) || is_project(25708) || is_project(25727) || is_project(25728) || is_project(25729)) {
			if (evt2_ntc_solution == 1) {
				ret = iio_read_channel_processed(thermistor_ch1_1, &val);    //EVT2
			} else {
				val = get_pa_ntc_volt();    //EVT1
			}
	} else {
		val = get_pa_ntc_volt();
	}
...
static int mtkts_PA_probe(struct platform_device *pdev)
{
	...
	evt2_ntc_solution = of_property_read_bool(np, "oplus,EVT2NTCSolution");
	mtkts_PA_4G_dprintk("[%s] evt2_ntc_solution: %d\n",
		__func__, evt2_ntc_solution);
