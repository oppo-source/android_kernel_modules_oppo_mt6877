...
static int mt6379_enable_otg(struct charger_device *chgdev, bool en)
{
	...
	if (en) {
		is_enabled = regulator_is_enabled(cdata->otg_regu);
		if (!is_enabled) {
			ret = regulator_enable(cdata->otg_regu);
			if (ret < 0) {
				dev_err(cdata->dev, "Failed to enable otg regulator: %d\n", ret);
				return ret;
			}
			dev_dbg(cdata->dev, "otg regulator enabled\n");
		} else {
			dev_dbg(cdata->dev, "otg regulator already enabled\n");
		}
	} else {
		is_enabled = regulator_is_enabled(cdata->otg_regu);
		if (is_enabled) {
			...
			if (ret < 0) {
				dev_err(cdata->dev, "Failed to disable otg regulator: %d\n", ret);
				return ret;
			}
			dev_dbg(cdata->dev, "otg regulator disabled\n");
		} else {
			dev_dbg(cdata->dev, "otg regulator already disabled, skipping to avoid unbalanced disables\n");
		}
	}
