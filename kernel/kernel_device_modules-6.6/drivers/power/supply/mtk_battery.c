...
extern bool oplus_set_ntc_switch_lock(enum ntc_switch_type type, bool lock);
#endif
...
#if IS_ENABLED(CONFIG_HORAE_FLASH_LED_THERMAL)
	if (ntc_switch_batt_id) {
		ret = oplus_set_ntc_switch_lock(BATT_ID, true);
		msleep(200);
		ret = iio_read_channel_processed(batt_id, &ret_value);
		oplus_set_ntc_switch_lock(BATT_ID, false);
	} else {
		ret = iio_read_channel_processed(batt_id, &ret_value);
	}
#else
...
static int meter_fg_30_get_dec_fg_type(void)
{
	int type = MTK_FG;
...
static int meter_set_sili_ic_deep_term_volt(int volt)
{
	struct mtk_battery *gm;
	gm = get_mtk_battery();
	...
	exec_BAT_EC(gm, 809, volt);

	return 0;
}

static bool meter_set_fg_vct(int vct)
{
	struct mtk_battery *gm;
	gm = get_mtk_battery();
...
#define DOD_MAX_VALUE 10000

static int meter_get_battery_dod(int index)
{
	int dod = 0;
	...
	case GAUGE_INDEX_SUB:
		dod = DOD_MAX_VALUE - oplus_gm->fg_cust_data.ui_old_soc;
		if (dod < 0 || dod > DOD_MAX_VALUE) {
			bm_err(oplus_gm, "[%s]: battery_dod out of range.\n", __func__);
			return -EINVAL;
		}
		break;
	default:
...
static struct oplus_gauge_operations oplus_battery_gauge = {
	...
	.set_sili_ic_deep_term_volt	= meter_set_sili_ic_deep_term_volt,
	.set_fg_vct					= meter_set_fg_vct,
	.get_battery_dod			= meter_get_battery_dod,
};
#endif /* CONFIG_OPLUS_MTK_BATTERY_V2 */
#endif  /* OPLUS_FEATURE_CHG_BASIC */
...
/*add for distinguish fuelgague and outlay-gague*/
...
#if IS_ENABLED(CONFIG_HORAE_FLASH_LED_THERMAL)
	fg_read_dts_val(gm, pdev->dev.of_node, "NTC_SWITCH_BATT_ID", &(ntc_switch_batt_id), 1);
	bm_err(gm, "%s, ntc_switch_batt_id:%d\n", __func__, ntc_switch_batt_id);
#endif
	...
	if (node) {
#if IS_ENABLED(CONFIG_HORAE_FLASH_LED_THERMAL)
		gm->ntc_switch_batt_id = of_property_read_bool(node, "oplus,ntc_switch_batt_id");
#endif
