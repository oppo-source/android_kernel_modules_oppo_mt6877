...
static void msdc_request_timeout(struct work_struct *work)
{
	...
	if (host->mrq) {
		...
		if (host->cmd) {
			...
			dev_err(host->dev, "%s: abort data: cmd%d; arg=%d, %d blocks\n",
					__func__, host->mrq->cmd->opcode, host->mrq->cmd->arg,
					host->data->blocks);
 ...
 */
#define BAD_SD_DETECTER_COUNT 2

/* we take it as bad sd when the bad sd condition occurs
 * out of tolerance
 */
u32 bad_sd_tolerance[BAD_SD_DETECTER_COUNT] = {10, 4};
 ...
 */
u32 bad_sd_forget[BAD_SD_DETECTER_COUNT] = {3, 20};
...
static int msdc_detect_bad_sd(struct msdc_host *host, u32 condition)
{
	...
	pr_notice("%s:bad_sd_detecter:%d, condition=%d\n", __func__, bad_sd_detecter[condition], condition);
