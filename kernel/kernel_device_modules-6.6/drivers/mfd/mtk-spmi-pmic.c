...
#define PMIC_RG_VSYS_UVLO_VTHL_ADDR        0xa8a
#define MT6373_PMIC_RG_VSYS_UVLO_VTHL_ADDR 0xa8a

extern unsigned int get_project(void);
extern unsigned int get_PCB_Version(void);

#define PMIC_MT6363_ID      0x63
#define CLEAR_RCS_INT_DONE_TIME	15

static struct hrtimer spmi_timer;
static ktime_t kt_period;
static struct work_struct spmi_work;
...
struct mtk_spmi_pmic_data {
	...
	unsigned int vsys_uvlo_vthl_addr;
	unsigned int vsys_uvlo_vthl_val;
};
...
static const struct mtk_spmi_pmic_data mt6363_data = {
	...
	.vsys_uvlo_vthl_addr = PMIC_RG_VSYS_UVLO_VTHL_ADDR,
	.vsys_uvlo_vthl_val = 0x2,
};
...
static const struct mtk_spmi_pmic_data mt6368_data = {
	...
	.vsys_uvlo_vthl_addr = MT6373_PMIC_RG_VSYS_UVLO_VTHL_ADDR,
	.vsys_uvlo_vthl_val = 0x1,
};
...
static void spmi_work_callback(struct work_struct *data)
{
	int i, ret;
	int  rcs_retry = 10;
	unsigned int rcs_int_done;

	if (!map) {
		printk("Error: map is NULL\n");
		return;
	}
	ret = regmap_read(map, RCS_INT_DONE, &rcs_int_done);

	if (ret) {
		printk("Failed to read ret = %d, RCS_INT_DONE: %d\n", ret, rcs_int_done);
		return;
	}

	if (rcs_int_done == 1){
		for (i = 0; i < rcs_retry; i++) {
			ret = regmap_write(map, RCS_INT_DONE, 1);
			if (ret) {
				printk("Failed write 1 to clear, ret = %d, RCS_INT_DONE: %d retry %d/%d\n",
				ret, rcs_int_done, i + 1, rcs_retry);
			} else {
				printk("Success write 1 to clear ret = %d, i = %d\n", ret, i + 1);
				break;
			}
		}
	}
	printk("Cleared RCS_INT_DONE: %d\n", rcs_int_done);
}

static enum hrtimer_restart spmi_timer_func(struct hrtimer *timer)
{
	schedule_work(&spmi_work);
	//hrtimer_forward_now(timer, kt_period);

	return HRTIMER_NORESTART;
}

/* start: Emira VS1 BYP solution */
static void sdn28_uvlo22_setting(struct regmap *map, const struct mtk_spmi_pmic_data *chip_data)
{
	if (chip_data->vsys_uvlo_vthl_addr && chip_data->vsys_uvlo_vthl_val) {
		pr_info("[%s] no uvlo item.\n", __func__);
		return;
	}
	/* 6363 UVLO 2.2V, 6373 UVLO 2.1V */
	regmap_update_bits(map, chip_data->vsys_uvlo_vthl_addr, 0xF, chip_data->vsys_uvlo_vthl_val);
	pr_info("[%s] RG_UVLO_VTHL = 0x%x\n", __func__, chip_data->vsys_uvlo_vthl_val);
}
...
static int mtk_spmi_pmic_probe(struct spmi_device *sdev)
{
	...
	if (core->chip_id == PMIC_MT6363_ID){
		map = core->regmap;

		kt_period = ktime_set(CLEAR_RCS_INT_DONE_TIME, 0);

		/* init work queue */
		INIT_WORK(&spmi_work, spmi_work_callback);

		/* init timer */
		hrtimer_init(&spmi_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
		spmi_timer.function = spmi_timer_func;
		hrtimer_start(&spmi_timer, kt_period, HRTIMER_MODE_REL);
		dev_err(&sdev->dev, "hrtimer_start\n");
	}
	...
	/* start: VS1 BYP solution */
	project_id = get_project();
	board_result = check_board_info();
	dev_err(&sdev->dev, "project = %d)\n", project_id);
	if ((project_id == 24813) && (board_result == 1)) {
		sdn28_uvlo22_setting(core->regmap, chip_data);
	}
	/* end: VS1 BYP solution */
