...
static struct mm_freq_config disp_freq = {
	...
	.prop_name = "disp_freq",
	.pm_qos_class = MTK_PM_QOS_DISP_FREQ,
...
static struct mm_freq_config mdp_freq = {
	.nb.notifier_call = mm_freq_notify,
	.prop_name = "mdp_freq",
	.pm_qos_class = MTK_PM_QOS_MDP_FREQ,
	.current_step = STEP_UNREQUEST,
};
...
static struct mm_freq_config vdec_freq = {
	.nb.notifier_call = mm_freq_notify,
	.prop_name = "vdec_freq",
	.pm_qos_class = MTK_PM_QOS_VDEC_FREQ,
	.current_step = STEP_UNREQUEST,
};
...
static struct mm_freq_config venc_freq = {
	.nb.notifier_call = mm_freq_notify,
	.prop_name = "venc_freq",
	.pm_qos_class = MTK_PM_QOS_VENC_FREQ,
	.current_step = STEP_UNREQUEST,
};
...
static struct mm_freq_config cam_freq = {
	.nb.notifier_call = mm_freq_notify,
	.prop_name = "cam_freq",
	.pm_qos_class = MTK_PM_QOS_CAM_FREQ,
	.current_step = STEP_UNREQUEST,
};
...
static struct mm_freq_config img_freq = {
	.nb.notifier_call = mm_freq_notify,
	.prop_name = "img_freq",
	.pm_qos_class = MTK_PM_QOS_IMG_FREQ,
	.current_step = STEP_UNREQUEST,
};
...
static struct mm_freq_config dpe_freq = {
	.nb.notifier_call = mm_freq_notify,
	.prop_name = "dpe_freq",
	.pm_qos_class = MTK_PM_QOS_DPE_FREQ,
	.current_step = STEP_UNREQUEST,
};

static struct mm_freq_config ipe_freq = {
	.prop_name = "ipe_freq",
	.pm_qos_class = MTK_PM_QOS_RESERVED,
	.current_step = STEP_UNREQUEST,
};
...
{
	u32 i = pm_qos_class - MTK_PM_QOS_DISP_FREQ;
		...
		unsigned long freq_value, void *v)
{
	...
	step = step_size - 1;
	if (freq_value == MTK_PM_QOS_MM_FREQ_DEFAULT_VALUE) {
	...
	const char *freq_name, struct mm_freq_step_config step_configs[])
{
	...
	of_property_for_each_string(dev->of_node, freq_name, prop, name) {
		...
		strscpy(ext_name, name, sizeof(ext_name)-1);
	...
	struct mm_freq_limit_config *limit_config)
{
	...
	strscpy(ext_name, freq_name, sizeof(ext_name)-1);
...
static int mmdvfs_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(all_freqs); i++) {
		...
		if (likely(mm_freq->pm_qos_class >= MTK_PM_QOS_DISP_FREQ)) {
	...
	mmdvfs_qos_get_freq_steps(MTK_PM_QOS_DISP_FREQ, freq_steps, &value);
...
int dump_setting(char *buf, const struct kernel_param *kp)
{
	...
	for (i = 0; i < ARRAY_SIZE(all_freqs); i++) {
		...
		length += snprintf(buf + length, MAX_DUMP  - length,
			...
			mmdvfs_qos_get_freq(MTK_PM_QOS_DISP_FREQ + i));
...
static const struct kernel_param_ops dump_param_ops = {.get = dump_setting};
...
int mmdvfs_qos_force_step(int step)
{
	...
	force_step = step;
	update_step(MTK_PM_QOS_NUM_CLASSES, -1);
	return 0;
}
...
static const struct kernel_param_ops force_step_ops = {
	.set = set_force_step,
	.get = param_get_int,
};
...
static const struct kernel_param_ops mmdvfs_enable_ops = {
	.set = set_enable,
	.get = param_get_bool,
};
module_param_cb(
...
int set_vote_freq(const char *val, const struct kernel_param *kp)
{
	...
	if (!vote_req_init) {
		mtk_pm_qos_add_request(
			&vote_req, MTK_PM_QOS_DISP_FREQ,
			MTK_PM_QOS_MM_FREQ_DEFAULT_VALUE);
		vote_req_init = true;
	}
...
static const struct kernel_param_ops vote_freq_ops = {
	.set = set_vote_freq,
	.get = param_get_int,
};
...
int mmdvfs_ut_set(const char *val, const struct kernel_param *kp)
{
	...
	log_level = 1 << log_freq |
		1 << log_limit;
	mtk_pm_qos_add_request(&disp_req, MTK_PM_QOS_DISP_FREQ,
		MTK_PM_QOS_MM_FREQ_DEFAULT_VALUE);
...
static const struct kernel_param_ops mmdvfs_ut_ops = {
	.set = mmdvfs_ut_set,
	.get = param_get_int,
};
