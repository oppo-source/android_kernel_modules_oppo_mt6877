...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_OSVELTE)
	if (len >= SZ_256M) {
		mm_logi("allocate %lu >= sz_256M size\n", len);
	}
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_OSVELTE)
		if (len >= SZ_256M) {
			current_tm = sched_clock();
			if ((current_tm - alloc_page_tm) > (1000000000ULL)) {
				mm_logi("elapse:%llu, buffer->len:%lu,size_remaining:%lu \n",
					current_tm - tm1, buffer->len, size_remaining);
				alloc_page_tm = current_tm;
			}
		}
#endif
