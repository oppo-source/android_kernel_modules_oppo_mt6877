...
#define DEFAULT_NICE (19)
#define TEMPORARY_NICE (0)
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_BOOSTPOOL)
	/*
	 *  actually no need task_nice here because set_user_nice,
	 *  add here to print log.
	 */
	if (list_nr_pages > (SZ_1G >> PAGE_SHIFT) &&
	    task_nice(mtk_freelist_task) != TEMPORARY_NICE) {
		pr_info("[defer_free] set mtk_freelist_task %d\n", TEMPORARY_NICE);
		set_user_nice(mtk_freelist_task, TEMPORARY_NICE);
	}
#endif /* CONFIG_OPLUS_FEATURE_MM_BOOSTPOOL */
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_BOOSTPOOL)
		if (list_nr_pages < (SZ_16M >> PAGE_SHIFT) &&
		    task_nice(current) != DEFAULT_NICE) {
			pr_info("[defer_free] set mtk_freelist_task %d\n",
				DEFAULT_NICE);
			set_user_nice(mtk_freelist_task, DEFAULT_NICE);
		}
#endif /* CONFIG_OPLUS_FEATURE_MM_BOOSTPOOL */
	}
