// SPDX-License-Identifier: GPL-2.0
/*
 * Copyright (c) 2019 MediaTek Inc.
 */


#include <linux/clk-provider.h>
#include <linux/syscore_ops.h>
#include <linux/version.h>
#include <linux/io.h>
#include <linux/module.h>
#include <linux/platform_device.h>
#include <linux/seq_file.h>

#define WARN_ON_CHECK_PLL_FAIL		0
#define CLKDBG_CCF_API_4_4	1
#define BUG_ON_CHK_ENABLE		0
#define CHECK_VCORE_FREQ		0

#include "clk-mt6781-pg.h"

#include "clkchk.h"
#include "clkchk-mt6781.h"
#if IS_ENABLED(CONFIG_MTK_DEVAPC)
#include <devapc_public.h>
#endif

#define TAG	"[clkchk] "

#if !CLKDBG_CCF_API_4_4

/* backward compatible */

static const char *clk_hw_get_name(const struct clk_hw *hw)
{
	return __clk_get_name(hw->clk);
}

static bool clk_hw_is_prepared(const struct clk_hw *hw)
{
	return __clk_is_prepared(hw->clk);
}

static bool clk_hw_is_enabled(const struct clk_hw *hw)
{
	return __clk_is_enabled(hw->clk);
}

#endif /* !CLKDBG_CCF_API_4_4 */

static unsigned int suspend_cnt;

/*
 * clkchk dump_regs
 */

#define REGBASE_V(_phys, _id_name, _pg, _pn) { .phys = _phys,	\
		.name = #_id_name, .pg = _pg, .pn = _pn}

static struct regbase rb[] = {
	[topckgen] = REGBASE_V(0x10000000, topckgen, PD_NULL, CLK_NULL),
	[infracfg] = REGBASE_V(0x10001000, infracfg, PD_NULL, CLK_NULL),
	[scpsys]   = REGBASE_V(0x10006000, scpsys, PD_NULL, CLK_NULL),
	[apmixedsys]  = REGBASE_V(0x1000c000, apmixedsys, PD_NULL, CLK_NULL),
	[audiosys]    = REGBASE_V(0x11220000, audiosys, PD_NULL, CLK_NULL),
	[mfgsys]   = REGBASE_V(0x13000000, mfgsys, PD_NULL, CLK_NULL),
	[mmsys]    = REGBASE_V(0x14000000, mmsys, PD_NULL, CLK_NULL),
	[imgsys]   = REGBASE_V(0x15020000, imgsys, PD_NULL, CLK_NULL),
	[camsys]   = REGBASE_V(0x1a000000, camsys, PD_NULL, CLK_NULL),
	[vencsys]  = REGBASE_V(0x17000000, vencsys, PD_NULL, CLK_NULL),
	[vdecsys]  = REGBASE_V(0x16000000, vdecsys, PD_NULL, CLK_NULL),
	[ipu_vcore]  = REGBASE_V(0x19020000, ipu_vcore, PD_NULL, CLK_NULL),
	[ipu_conn]  = REGBASE_V(0x19000000, ipu_conn, PD_NULL, CLK_NULL),
	[ipu0]  = REGBASE_V(0x19180000, ipu0, PD_NULL, CLK_NULL),
	[ipu1]  = REGBASE_V(0x19280000, ipu1, PD_NULL, CLK_NULL),
	[ipu2]  = REGBASE_V(0x19380000, ipu2, PD_NULL, CLK_NULL),
};

#define REGNAME(_base, _ofs, _name)	\
	{ .base = &rb[_base], .ofs = _ofs, .name = #_name }

static struct regname rn[] = {
	REGNAME(topckgen,  0x020, CLK_CFG_0),
	REGNAME(topckgen,  0x030, CLK_CFG_1),
	REGNAME(topckgen,  0x040, CLK_CFG_2),
	REGNAME(topckgen,  0x050, CLK_CFG_3),
	REGNAME(topckgen,  0x060, CLK_CFG_4),
	REGNAME(topckgen,  0x070, CLK_CFG_5),
	REGNAME(topckgen,  0x080, CLK_CFG_6),
	REGNAME(topckgen,  0x090, CLK_CFG_7),
	REGNAME(topckgen,  0x0a0, CLK_CFG_8),
	REGNAME(topckgen,  0x0b0, CLK_CFG_9),
	REGNAME(topckgen,  0x0c0, CLK_CFG_10),
	REGNAME(topckgen,  0x0d0, CLK_CFG_11),
	REGNAME(topckgen,  0x0e0, CLK_CFG_12),
	REGNAME(topckgen,  0x0f0, CLK_CFG_13),
	REGNAME(apmixedsys,  0x200, ARMPLL_LL_CON0),
	REGNAME(apmixedsys,  0x204, ARMPLL_LL_CON1),
	REGNAME(apmixedsys,  0x20C, ARMPLL_LL_PWR_CON0),
	REGNAME(apmixedsys,  0x210, ARMPLL_BL_CON0),
	REGNAME(apmixedsys,  0x214, ARMPLL_BL_CON1),
	REGNAME(apmixedsys,  0x21C, ARMPLL_BL_PWR_CON0),
	REGNAME(apmixedsys,  0x220, ARMPLL_BB_CON0),
	REGNAME(apmixedsys,  0x224, ARMPLL_BB_CON1),
	REGNAME(apmixedsys,  0x22C, ARMPLL_BB_PWR_CON0),
	REGNAME(apmixedsys,  0x230, MAINPLL_CON0),
	REGNAME(apmixedsys,  0x234, MAINPLL_CON1),
	REGNAME(apmixedsys,  0x23C, MAINPLL_PWR_CON0),
	REGNAME(apmixedsys,  0x240, UNIVPLL_CON0),
	REGNAME(apmixedsys,  0x244, UNIVPLL_CON1),
	REGNAME(apmixedsys,  0x24C, UNIVPLL_PWR_CON0),
	REGNAME(apmixedsys,  0x250, MFGPLL_CON0),
	REGNAME(apmixedsys,  0x254, MFGPLL_CON1),
	REGNAME(apmixedsys,  0x25C, MFGPLL_PWR_CON0),
	REGNAME(apmixedsys,  0x260, MSDCPLL_CON0),
	REGNAME(apmixedsys,  0x264, MSDCPLL_CON1),
	REGNAME(apmixedsys,  0x26C, MSDCPLL_PWR_CON0),
	REGNAME(apmixedsys,  0x270, TVDPLL_CON0),
	REGNAME(apmixedsys,  0x274, TVDPLL_CON1),
	REGNAME(apmixedsys,  0x27C, TVDPLL_PWR_CON0),
	REGNAME(apmixedsys,  0x280, MMPLL_CON0),
	REGNAME(apmixedsys,  0x284, MMPLL_CON1),
	REGNAME(apmixedsys,  0x28C, MMPLL_PWR_CON0),
	REGNAME(apmixedsys,  0x2A0, CCIPLL_CON0),
	REGNAME(apmixedsys,  0x2A4, CCIPLL_CON1),
	REGNAME(apmixedsys,  0x2AC, CCIPLL_PWR_CON0),
	REGNAME(apmixedsys,  0x2B0, ADSPPLL_CON0),
	REGNAME(apmixedsys,  0x2B4, ADSPPLL_CON1),
	REGNAME(apmixedsys,  0x2BC, ADSPPLL_PWR_CON0),
	REGNAME(apmixedsys,  0x2C0, APLL1_CON0),
	REGNAME(apmixedsys,  0x2C4, APLL1_CON1),
	REGNAME(apmixedsys,  0x2D0, APLL1_PWR_CON0),
	REGNAME(apmixedsys,  0x2D4, APLL2_CON0),
	REGNAME(apmixedsys,  0x2D8, APLL2_CON1),
	REGNAME(apmixedsys,  0x2E4, APLL2_PWR_CON0),
	REGNAME(scpsys,  0x160, PWR_STATUS),
	REGNAME(scpsys,  0x164, PWR_STATUS_2ND),
	REGNAME(scpsys,  0x308, ISP_PWR_CON),
	REGNAME(scpsys,  0x350, IPE_PWR_CON),
	REGNAME(scpsys,  0x304, VEN_PWR_CON),
	REGNAME(scpsys,  0x300, VDE_PWR_CON),
	REGNAME(scpsys,  0x30C, DIS_PWR_CON),
	REGNAME(scpsys,  0x31C, AUD_PWR_CON),
	REGNAME(scpsys,  0x324, CAM_PWR_CON),
	REGNAME(scpsys,  0x318, MD1_PWR_CON),
	REGNAME(scpsys,  0x320, CONN_PWR_CON),
	REGNAME(scpsys,  0x33C, VPU_VCORE_PWR_CON),
	REGNAME(scpsys,  0x340, VPU_CONN_PWR_CON),
	REGNAME(scpsys,  0x344, VPU_CORE0_PWR_CON),
	REGNAME(scpsys,  0x348, VPU_CORE1_PWR_CON),
	REGNAME(scpsys,  0x34C, VPU_CORE2_PWR_CON),
	REGNAME(camsys,  0x000, CAMSYS_CG),
	REGNAME(imgsys,  0x000, IMG_CG),
	REGNAME(infracfg,  0x090, MODULE_SW_CG_0),
	REGNAME(infracfg,  0x094, MODULE_SW_CG_1),
	REGNAME(infracfg,  0x0ac, MODULE_SW_CG_2),
	REGNAME(infracfg,  0x0c8, MODULE_SW_CG_3),
	REGNAME(mmsys,	0x100, MMSYS_CG_CON0),
	REGNAME(mmsys,	0x110, MMSYS_CG_CON1),
	REGNAME(vdecsys,  0x000, VDEC_CKEN),
	REGNAME(vdecsys,  0x008, VDEC_LARB1_CKEN),
	REGNAME(vencsys,  0x000, VENCSYS_CG),
	{}
};

static const struct regname *get_all_regnames(void)
{
	return rn;
}

u32 get_mt6781_reg_value(u32 id, u32 ofs)
{
	if (id >= chk_sys_num)
		return 0;

	return clk_readl(rb[id].virt + ofs);
}
EXPORT_SYMBOL_GPL(get_mt6781_reg_value);

static void  init_regbase(void)
{
	size_t i;

	for (i = 0; i < ARRAY_SIZE(rb); i++)
		rb[i].virt = ioremap(rb[i].phys, PAGE_SIZE);
}


static const char * const off_pll_names[] = {
		"univ2pll",
		"mfgpll",
		"apll1",
		"mmpll",
		"msdcpll",
		NULL
};

static const char * const notice_pll_names[] = {

	NULL
};

static const char * const *get_off_pll_names(void)
{
	return off_pll_names;
}

static const char * const *get_notice_pll_names(void)
{
	return notice_pll_names;
}


/*
 * clkdbg pwr_status
 */


static u32 pwr_ofs[STA_NUM] = {
	[PWR_STA] = 0x180,
	[PWR_STA2] = 0x184,
};

static u32 pwr_sta[STA_NUM];

u32 *get_spm_pwr_status_array(void)
{
	static void __iomem *pwr_addr[STA_NUM];
	int i;

	for (i = 0; i < STA_NUM; i++) {
		if (pwr_ofs[i]) {
			pwr_addr[i] = rb[scpsys].virt + pwr_ofs[i];
			pwr_sta[i] = clk_readl(pwr_addr[i]);
		}
	}

	return pwr_sta;
}


static bool is_pll_chk_bug_on(void)
{
#if BUG_ON_CHK_ENABLE
	return true;
#endif
	return false;
}

void print_subsys_reg_mt6781(enum chk_sys_id id)
{
	struct regbase *rb_dump;
	const struct regname *rns = &rn[0];
	int i;

	if (rns == NULL)
		return;

	if (id >= chk_sys_num || id < 0) {
		pr_info("wrong id:%d\n", id);
		return;
	}

	rb_dump = &rb[id];

	for (i = 0; i < ARRAY_SIZE(rn) - 1; i++, rns++) {
		if (!is_valid_reg(ADDR(rns)))
			return;

		/* filter out the subsys that we don't want */
		if (rns->base != rb_dump)
			continue;

		pr_info("%-18s: [0x%08x] = 0x%08x\n",
			rns->name, PHYSADDR(rns), clk_readl(ADDR(rns)));
	}
}
EXPORT_SYMBOL(print_subsys_reg_mt6781);

#if IS_ENABLED(CONFIG_MTK_DEVAPC)
static void devapc_dump(void)
{
	print_subsys_reg_mt6781(scpsys);
	print_subsys_reg_mt6781(topckgen);
	print_subsys_reg_mt6781(infracfg);
	print_subsys_reg_mt6781(apmixedsys);
}

static struct devapc_vio_callbacks devapc_vio_handle = {
	.id = DEVAPC_SUBSYS_CLKMGR,
	.debug_dump = devapc_dump,
};

#endif

static int get_vcore_opp(void)
{
#if IS_ENABLED(CONFIG_MTK_DVFSRC_HELPER) && CHECK_VCORE_FREQ
	return mtk_dvfsrc_query_opp_info(MTK_DVFSRC_SW_REQ_VCORE_OPP);
#else
	return VCORE_NULL;
#endif
}


/*
 * clkchk pwr_msk  need to review
 */
static struct pvd_msk pvd_pwr_mask[] = {
	{"topckgen", PWR_STA, 0x00000000},
	{"infracfg_ao", PWR_STA, 0x00000000},
	{"apmixed", PWR_STA, 0x00000000},
	{"mfg_cfg", PWR_STA, BIT(11) | BIT(12) | BIT(13) | BIT(14)},
	{"mmsys_config", PWR_STA, BIT(5)},
	{"imgsys", PWR_STA, BIT(6)},
	{"camsys", PWR_STA, BIT(7)},
	{"venc_gcon", PWR_STA, BIT(9)},
	{"vdec_gcon", PWR_STA, BIT(8)},
	{},
};

static struct pvd_msk *get_pvd_pwr_mask(void)
{
	return pvd_pwr_mask;
}

/*
 * init functions
 */

static bool is_suspend_retry_stop(bool reset_cnt)
{
	if (reset_cnt == true) {
		suspend_cnt = 0;
		return true;
	}

	suspend_cnt++;
	pr_notice("%s: suspend cnt: %d\n", __func__, suspend_cnt);

	if (suspend_cnt < 2)
		return false;

	return true;
}

static struct clkchk_ops clkchk_mt6781_ops = {
	.get_all_regnames = get_all_regnames,
	.get_spm_pwr_status_array = get_spm_pwr_status_array,
	.get_pvd_pwr_mask = get_pvd_pwr_mask,  // get_pvd_pwr_mask need to recheck
	.get_off_pll_names = get_off_pll_names,
	.get_notice_pll_names = get_notice_pll_names,
	.is_pll_chk_bug_on = is_pll_chk_bug_on,
	.get_vcore_opp = get_vcore_opp,
#if IS_ENABLED(CONFIG_MTK_DEVAPC)
	.devapc_dump = devapc_dump,
#endif
	.is_suspend_retry_stop = is_suspend_retry_stop,
};

static int clk_chk_mt6781_probe(struct platform_device *pdev)
{
	suspend_cnt = 0;

	init_regbase();

	set_clkchk_notify();

	set_clkchk_ops(&clkchk_mt6781_ops);

#if IS_ENABLED(CONFIG_MTK_DEVAPC)
	register_devapc_vio_callback(&devapc_vio_handle);
#endif

	return 0;
}

static const struct of_device_id of_match_clkchk_mt6781[] = {
	{
		.compatible = "mediatek,mt6781-clkchk",
	}, {
		/* sentinel */
	}
};

static struct platform_driver clk_chk_mt6781_drv = {
	.probe = clk_chk_mt6781_probe,
	.driver = {
		.name = "clk-chk-mt6781",
		.owner = THIS_MODULE,
		.pm = &clk_chk_dev_pm_ops,
		.of_match_table = of_match_clkchk_mt6781,
	},
};

/*
 * init functions
 */

static int __init clkchk_mt6781_init(void)
{
	return platform_driver_register(&clk_chk_mt6781_drv);
}

static void __exit clkchk_mt6781_exit(void)
{
	platform_driver_unregister(&clk_chk_mt6781_drv);
}

late_initcall(clkchk_mt6781_init);
module_exit(clkchk_mt6781_exit);
MODULE_LICENSE("GPL");

