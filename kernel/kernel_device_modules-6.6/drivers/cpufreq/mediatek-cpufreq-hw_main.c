...
/* Make sure this section is consistent with oplus_freq_tune_policy.c*/
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FREQ_TUNE)
		if (policy->efficiencies_available && (policy->freq_table[policy->cached_resolved_idx].flags & CPUFREQ_OPLUS_INEFFICIENT_FREQ))
			index = cpufreq_table_find_index_dh(policy, target_freq, true);
		else
#endif
					...
					const u16 *offsets)
{
	...
	if (cpu == perf_domain_master[index])
		...
		if (master < 0 || master >= num_possible_cpus()) {
			dev_info(dev, "master %d invalid for control group %d, index %d\n",
					master, control_group, index);
			return -EINVAL;
		}
