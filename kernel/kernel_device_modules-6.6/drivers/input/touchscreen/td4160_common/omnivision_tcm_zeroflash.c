...
#define FW_IMAGE_NAME "hdl_firmware"
	...
	while(retry_cnt--) {
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					...
					td4160_firmware_name, retry_cnt);
			if (retry_cnt == 0) {
				return retval;
			}
			msleep(1000);
...
static void zeroflash_download_config(void)
{
	...
	if (!fw_status->need_app_config && !fw_status->need_disp_config
			...
			&& (atomic_read(&tcm_hcd->host_downloading))) {
		atomic_set(&tcm_hcd->host_downloading, 0);
		LOGN(tcm_hcd->pdev->dev.parent,"host_downloading done...\n");
...
static int zeroflash_init(struct ovt_tcm_hcd *tcm_hcd)
{
	...
	if (IS_ERR_OR_NULL(zeroflash_hcd)) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	zeroflash_hcd->sysfs_dir = kobject_create_and_add(SYSFS_DIR_NAME,
			tcm_hcd->sysfs_dir);
	if (IS_ERR_OR_NULL(zeroflash_hcd->sysfs_dir)) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static int zeroflash_reinit(struct ovt_tcm_hcd *tcm_hcd)
{
	...
	if (IS_ERR_OR_NULL(zeroflash_hcd) && tcm_hcd->in_hdl_mode) {
