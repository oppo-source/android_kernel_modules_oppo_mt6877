...
#define FW_IMAGE_NAME_01 "hdl_firmware_01.img"
...
extern char td4160_firmware_name[256];
static int reflash_get_fw_image(void)
{
    int retval;
    struct ovt_tcm_hcd *tcm_hcd = reflash_hcd->tcm_hcd;
    ...
    if (reflash_hcd->image == NULL) {
        if (reflash_hcd->reflash_by_manual == false) {
            ...
            retval = request_firmware(&reflash_hcd->fw_entry,
                                      td4160_firmware_name, tcm_hcd->pdev->dev.parent);
            if (retval < 0) {
                LOGE(tcm_hcd->pdev->dev.parent,
                     "Failed to request %s\n",
                     td4160_firmware_name);
                return retval;
            }
...
static int reflash_init(struct ovt_tcm_hcd *tcm_hcd)
{
    ...
    if (IS_ERR_OR_NULL(reflash_hcd->image_buf)) {
        LOGE(tcm_hcd->pdev->dev.parent,
    ...
    reflash_hcd->sysfs_dir = kobject_create_and_add(SYSFS_DIR_NAME,
                             tcm_hcd->sysfs_dir);
    if (IS_ERR_OR_NULL(reflash_hcd->sysfs_dir)) {
        LOGE(tcm_hcd->pdev->dev.parent,
...
static int reflash_reinit(struct ovt_tcm_hcd *tcm_hcd)
{
    ...
    if (IS_ERR_OR_NULL(reflash_hcd) && !(tcm_hcd->in_hdl_mode)) {
