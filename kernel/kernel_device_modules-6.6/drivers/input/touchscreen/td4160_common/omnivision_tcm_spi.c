...
#include <linux/platform_data/spi-mt65xx.h>  //具体哪一个平台文件请修改
...
char td4160_firmware_name[256];

#ifdef CONFIG_OF
static int mtk_drm_lcm_info_get(struct device *dev, struct ovt_tcm_board_data *bdata)
{
	...
	if (chosen_node) {
		...
		if (videolfb_tag) {
			bdata->lcm_name = videolfb_tag->lcmname;
			LOGI(dev, "find lcm name:%s", bdata->lcm_name);
			if (strcmp(bdata->lcm_name, "td4160_hdp_dsi_vdo_boe_120hz") == 0) {
				strscpy(td4160_firmware_name, "hdl_firmware_01.img", sizeof(td4160_firmware_name)); 
			} else if (strcmp(bdata->lcm_name, "td4160_hdp_dsi_vdo_hkc_120hz") == 0) {
				strscpy(td4160_firmware_name, "hdl_firmware_00.img", sizeof(td4160_firmware_name));
			} else {
				strscpy(td4160_firmware_name, "hdl_firmware_00.img", sizeof(td4160_firmware_name));
				LOGE(dev, "not find right lcm name, use default: %s", td4160_firmware_name);
			}
			LOGI(dev, "firmware is :%s", td4160_firmware_name);
		} else {
			LOGE(dev, "[touch] videolfb_tag not found");
		}
	} else {
		LOGE(dev, "touch of_chosen not found");
	}
		...
		unsigned int length)
{
	...
	if (bdata->byte_delay_us == 0)
		...
		retval = ovt_tcm_spi_alloc_mem(tcm_hcd, length, 1);
	if (retval < 0) {
		LOGE(&spi->dev, "Failed to allocate memory\n");
		goto exit;
	}
...
static struct mtk_chip_config spi_ctrdata = {
	.cs_setuptime = 625, //25*9.6ns  6us == 625*9.6 ns
	//.rx_mlsb = 1,
	//.tx_mlsb = 1,
};
#endif
...
static int ovt_tcm_spi_probe(struct spi_device *spi)
{
    ...
    if (IS_ERR_OR_NULL(ovt_tcm_spi_device)) {
        LOGE(&spi->dev,
    ...
    if (IS_ERR_OR_NULL(hw_if.bdata)) {
        LOGE(&spi->dev,
             "Failed to allocate memory for board data\n");
        return -ENOMEM;
    }
    parse_dt(&spi->dev, hw_if.bdata);
    mtk_drm_lcm_info_get(&spi->dev, hw_if.bdata);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	spi->cs_setup.value = 5; //delay 5
	spi->cs_setup.unit = 0;  //delay us
#endif
...
static void ovt_tcm_spi_remove(struct spi_device *spi)
{
...
static struct of_device_id ovt_tcm_of_match_table[] = {
    ...
    /*bsp:add for bringup by guoxin.zhu 20230625 start*/
    {   .compatible = "adaptive-touchscreen",},
    /*bsp:add for bringup by guoxin.zhu 20230625 end*/
    {},
};
