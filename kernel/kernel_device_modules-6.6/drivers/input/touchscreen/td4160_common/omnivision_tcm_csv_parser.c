				...
				char *ptr, int32_t* data, int rows)
{
	...
	for (i = 0; i < rows; i++) {
        ...
        if(IS_ERR_OR_NULL(ptr) || (0 == strlen(ptr)) || (ptr >= (buf_start + buf_size))) {
...
int ovt_tcm_parse_csvfile(char *file_path, char *target_name, int32_t  *data, int rows, int columns)
{
    ...
    if (read_ret > 0) {
        ...
        // walk thru this line
        goto_next_line(&ptr);
        if ((IS_ERR_OR_NULL(ptr)) || (0 == strlen(ptr))) {
...
void ovt_tcm_store_to_file(struct file *fp, char* format, ...)
{
    ...
    pos = fp->f_pos;
    //zzzzvfs_write(fp, buf, strlen(buf), &pos);
    fp->f_pos = pos;
