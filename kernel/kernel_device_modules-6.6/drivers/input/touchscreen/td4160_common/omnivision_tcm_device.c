...
static int device_init(struct ovt_tcm_hcd *tcm_hcd)
{
    int retval;
    dev_t dev_num;
    //USD:removed for don't support export gpio by yun.luo 20240528 start
    //const struct ovt_tcm_board_data *bdata = tcm_hcd->hw_if->bdata;
    //USD:removed for don't support export gpio by yun.luo 20240528 end

    device_hcd = kzalloc(sizeof(*device_hcd), GFP_KERNEL);
    if (IS_ERR_OR_NULL(device_hcd)) {
        LOGE(tcm_hcd->pdev->dev.parent,
    ...
    LOGI(tcm_hcd->pdev->dev.parent,"success..\n");
    //USD:removed for don't support export gpio by yun.luo 20240528 end
...
/*bsp: modify for tp core by guoxin.zhu 20230919 start*/
//EXPORT_SYMBOL(device_module_init);
//EXPORT_SYMBOL(device_module_exit);
/*bsp: modify for tp core by guoxin.zhu 20230919 end*/
