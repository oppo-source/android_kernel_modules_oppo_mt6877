		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (input == 0) {
		...
		printk("ovt tcm_hcd->enable_irq false %d ", __LINE__);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
		...
		printk("ovt tcm_hcd->enable_irq TRUE %d ", __LINE__);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
	...
	if (tcm_hcd->report.id == REPORT_TOUCH) {
		...
		if (!list_empty(&mod_pool.list)) {
			list_for_each_entry(mod_handler, &mod_pool.list, link) {
				if (!mod_handler->insert &&
                        !mod_handler->detach &&
                        (!IS_ERR_OR_NULL(mod_handler->mod_cb->syncbox)))
...
static irqreturn_t ovt_tcm_isr(int irq, void *data)
{
    ...
    if(tcm_hcd->in_suspend) {
        pm_wakeup_event(&tcm_hcd->pdev->dev, 5000);
    }
    retval = tcm_hcd->read_message(tcm_hcd,
                                   NULL,
                                   0);
...
static int ovt_tcm_enable_irq(struct ovt_tcm_hcd *tcm_hcd, bool en, bool ns)
{
    ...
    printk("[omnivision] ovt_tcm_enable_irq en=%d ns=%d\n",en,ns);
		...
		bool config, int dir, int state)
{
	...
	if (config) {
        ...
        LOGI(tcm_hcd->pdev->dev.parent,
                 "Sucess to set GPIO %d direction(%d) to %d \n",
                 gpio, dir, state);
    } else {
        gpio_free(gpio);
    }
...
dispatch_reinit:
	...
	if (!list_empty(&mod_pool.list)) {
		list_for_each_entry(mod_handler, &mod_pool.list, link) {
			if (!mod_handler->insert &&
                    !mod_handler->detach &&
                    (!IS_ERR_OR_NULL(mod_handler->mod_cb->reinit)))
...
static int td4160_tcm_disp_notifier_callback(struct notifier_block *nb,
		unsigned long value, void *v)
{
	int retval = 0;
	struct ovt_tcm_hcd *tcm_hcd =
			container_of(nb, struct ovt_tcm_hcd, disp_notifier);
	int *data = (int *)v;

	LOGI(tcm_hcd->pdev->dev.parent, "%s: value:0x%lx, *data:0x%x in\n", __func__, value, *data);
	if (tcm_hcd && v) {
		if (value == MTK_DISP_EVENT_BLANK) {
			//resume: touch power on is after display to avoid display disturb
			if (*data == MTK_DISP_BLANK_UNBLANK) {
				LOGI(tcm_hcd->pdev->dev.parent, "%s: resume in\n", __func__);
				retval = ovt_tcm_resume(&tcm_hcd->pdev->dev);
				LOGI(tcm_hcd->pdev->dev.parent, "%s: resume out\n", __func__);
			} else if (*data == MTK_DISP_BLANK_POWERDOWN) {
					LOGI(tcm_hcd->pdev->dev.parent, "%s: suspend in\n", __func__);
					retval = ovt_tcm_suspend(&tcm_hcd->pdev->dev);
					LOGI(tcm_hcd->pdev->dev.parent, "%s: suspend out\n", __func__);
			}
		} else if (value == MTK_DISP_EARLY_EVENT_BLANK) {
			//suspend: touch power off is before display to avoid touch report event
			//after screen is off
				if (*data == MTK_DISP_BLANK_POWERDOWN) {
					LOGI(tcm_hcd->pdev->dev.parent, "%s: ealy suspend in\n", __func__);
					retval = ovt_tcm_early_suspend(&tcm_hcd->pdev->dev);
					LOGI(tcm_hcd->pdev->dev.parent, "%s: ealy suspend out\n", __func__);
			}
		} else {
			LOGI(tcm_hcd->pdev->dev.parent, "%s: touch IC can not suspend or resume\n", __func__);
				return -1;
		}
	}
	return 0;
...
static int ovt_tcm_probe(struct platform_device *pdev)
{
    ...
    /*BSP: remove useless module for USICPRO-52845 by yun.luo 20240109 start*/
    //diag_module_init();
    /*BSP: remove useless module for USICPRO-52845 by yun.luo 20240109 end*/
    //recovery_module_init(); no need

    sysfs_dir = kobject_create_and_add(PLATFORM_DRIVER_NAME,
                                       NULL); //&pdev->dev.kobj);  move to /sys
    if (IS_ERR_OR_NULL(sysfs_dir)) {
        LOGE(tcm_hcd->pdev->dev.parent,
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
	tcm_hcd->disp_notifier.notifier_call = td4160_tcm_disp_notifier_callback;
	if (mtk_disp_notifier_register("Touch", &tcm_hcd->disp_notifier))
		LOGI(tcm_hcd->pdev->dev.parent, "Failed to register disp notifier\n");
	else
...
static int ovt_tcm_remove(struct platform_device *pdev)
{
	...
	if (!list_empty(&mod_pool.list)) {
		list_for_each_entry_safe(mod_handler,
				...
				&mod_pool.list,
				link) {
            if (!IS_ERR_OR_NULL(mod_handler->mod_cb->remove))
				mod_handler->mod_cb->remove(tcm_hcd);
			list_del(&mod_handler->link);
			kfree(mod_handler);
		}
	...
	if (disp_notify_reg_flag) {
		if (mtk_disp_notifier_unregister(&tcm_hcd->disp_notifier))
			LOGI(tcm_hcd->pdev->dev.parent, "error occurred when unregister disp_notifier\n");
		else
			disp_notify_reg_flag = false;
	}
#endif
...
/*bsp:modify for bring up by guoxin.zhu 20230625 start*/
//static int __init ovt_tcm_module_init(void)
int ovt_tcm_module_init(void)
{
    int retval;

	pr_info("ovt tcm driver %s: entry\n", __func__);
