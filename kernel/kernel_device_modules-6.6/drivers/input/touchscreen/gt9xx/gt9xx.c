...
int gtp_i2c_read(struct i2c_client *client, u8 *buf, int len)
{
	...
	while (pos != len) {
		...
		for (retry = 0; retry < RETRY_MAX_TIMES; retry++) {
			...
			dev_dbg(&client->dev, "[gt9xx]I2c read retry[%d]:0x%x\n",
				retry + 1, address);
			usleep_range(2000, 2100);
		}
		if (unlikely(retry == RETRY_MAX_TIMES)) {
			dev_dbg(&client->dev,
				"[gt9xx]I2c read failed,dev:%02x,reg:%04x,size:%u\n",
...
static int gtp_gesture_handler(struct goodix_ts_data *ts)
{
	...
	if (ret < 0) {
		dev_dbg(&ts->client->dev, "Failed read doze buf");
		return -EINVAL;
	}
			 ...
			 u8 *key_value)
{
	...
	if (touch_num > 1) {
		...
		if (ret < 0) {
			dev_dbg(&ts->client->dev, "I2C error. %d\n", ret);
			finger_state = 0;
			goto exit_get_point;
		}
...
void gtp_int_output(struct goodix_ts_data *ts, int level)
{
	...
	if (level == 0) {
		if (ts->pinctrl.pinctrl)
			...
			dev_dbg(&ts->client->dev,
				"[gt9xx]Failed set int pin output low\n");
	} else {
		if (ts->pinctrl.pinctrl){
			pinctrl_select_state(ts->pinctrl.pinctrl,
			...
			dev_dbg(&ts->client->dev,
				"[gt9xx]Failed set int pin output high\n");
	}
}
...
void gtp_int_sync(struct goodix_ts_data *ts, s32 ms)
{
	...
	if (ts->pinctrl.pinctrl) {
		...
		dev_dbg(&ts->client->dev, "[gt9xx] Failed sync int pin\n");
	}
}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	dev_info(&client->dev, "[gt9xx]---Guitar reset");
	set_bit(PANEL_RESETTING, &ts->flags);
	if (!gpio_is_valid(ts->pdata->rst_gpio)) {
		dev_dbg(&client->dev, "reset failed no valid reset gpio");
		return;
	}
...
static int gtp_enter_doze(struct goodix_ts_data *ts)
{
	...
	dev_dbg(&ts->client->dev, "Failed enter doze mode\n");
...
static s8 gtp_enter_sleep(struct goodix_ts_data *ts)
{
	...
	dev_dbg(&ts->client->dev, "Failed send sleep cmd\n");
...
static int gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	dev_dbg(&ts->client->dev, "Failed wakeup from sleep mode\n");
	return -EINVAL;
}
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data *ts)
{
	...
	memcpy(&cfg->data[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id],
	       cfg->length);
#endif
	pr_info("[goodix-ts]%s: cfg->length: %d , sensor_id: %d \n", __func__,cfg->length,sensor_id);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	if (!ts->pdata->driver_send_cfg) {
		...
		ret = gtp_i2c_read(ts->client,
				   ...
				   GTP_ADDR_LENGTH);
		if (ret < 0)
			dev_dbg(&ts->client->dev, "Read origin Config Failed\n");
	...
	if (ret < 0)
		dev_dbg(&ts->client->dev, "Send config error\n");
	else
		usleep_range(10000, 11000); /* 10 ms */

	/* restore config version */
	cfg->data[GTP_ADDR_LENGTH] = drv_cfg_version;
	pr_info("[goodix-ts]%s : Send config succeful from dts or header \n", __func__);
	return 0;
}
				    ...
				    size_t size, loff_t *ppos)
{
	...
	if (!ptr) {
		dev_dbg(&ts->client->dev, "Failed alloc memory for config\n");
		return -ENOMEM;
	}
				     ...
				     size_t count, loff_t *off)
{
	...
	if (count > PAGE_SIZE) {
		dev_dbg(&ts->client->dev, "config to long %zu\n", count);
		return -EFAULT;
	}

	temp_buf = kzalloc(count, GFP_KERNEL);
	if (!temp_buf) {
		dev_dbg(&ts->client->dev, "failed alloc temp memory");
		return -ENOMEM;
	}
	...
	file_config = kzalloc(GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH,
			      GFP_KERNEL);
	if (!file_config) {
		dev_dbg(&ts->client->dev, "failed alloc config memory");
		kfree(temp_buf);
		return -ENOMEM;
	}
	...
	if (copy_from_user(temp_buf, buffer, count)) {
		dev_dbg(&ts->client->dev, "Failed copy from user\n");
		ret = -EFAULT;
		goto send_cfg_err;
	}
	...
	if (file_cfg_len < 0) {
		dev_dbg(&ts->client->dev, "failed covert ascii to hex");
		ret = -EFAULT;
		goto send_cfg_err;
	}
	...
	while (i++ < 5) {
		...
		dev_dbg(&ts->client->dev, "Send config i2c error.");
		ret = -EFAULT;
		goto send_cfg_err;
	}
				    ...
				    const char *buf, size_t count)
{
	...
	if (retval == FAIL)
		dev_dbg(&ts->client->dev, "Fail to update GTP firmware.\n");
	else
				 ...
				 const char *buf, size_t count)
{
	...
	if (err < 0) {
		dev_dbg(dev, "Failed to convert value\n");
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ('1' != buf[0]) {
		dev_dbg(dev, "Invalid argument for reset\n");
		return -EINVAL;
	}
...
static int gtp_create_file(struct goodix_ts_data *ts)
{
	...
	if (ret) {
		dev_dbg(&client->dev, "Failure create sysfs group %d\n", ret);
		/*TODO: debug change */
...
exit_free_config_proc:
	remove_proc_entry(ts->pdata->touch_proc, gtp_config_proc);
	return -ENODEV;
}
...
s32 gtp_get_fw_info(struct i2c_client *client, struct goodix_fw_info *fw_info)
{
	...
	if (ret < 0) {
		dev_dbg(&client->dev, "Failed read fw_info\n");
		return ret;
	}
	/* product id */
...
static int gtp_i2c_test(struct i2c_client *client)
{
	...
	while (retry++ < 3) {
		...
		dev_dbg(&client->dev, "[gt9xx] GTP i2c test failed time %d, ret:%d\n", retry, ret);
		usleep_range(10000, 11000); /* 10 ms */
	}
	return -EAGAIN;
}
...
static int gtp_pinctrl_init(struct goodix_ts_data *ts)
{
	...
	if (IS_ERR_OR_NULL(pinctrl->pinctrl)) {
		dev_info(&ts->client->dev, "[gt9xx] No pinctrl found\n");
	...
	if (IS_ERR_OR_NULL(pinctrl->default_sta)) {
		dev_info(&ts->client->dev,
			 "[gt9xx] Failed get pinctrl state:default state\n");
	...
	if (IS_ERR_OR_NULL(pinctrl->int_out_high)) {
		dev_info(&ts->client->dev,
			 "[gt9xx] Failed get pinctrl state:output_high\n");
		goto exit_pinctrl_init;
	}
	...
	if (IS_ERR_OR_NULL(pinctrl->int_out_low)) {
		dev_info(&ts->client->dev,
			 "[gt9xx] Failed get pinctrl state:output_low\n");
		goto exit_pinctrl_init;
	}
	...
	if (IS_ERR_OR_NULL(pinctrl->int_input)) {
		dev_info(&ts->client->dev,
			 "[gt9xx] Failed get pinctrl state:int-input\n");
		goto exit_pinctrl_init;
	}
	dev_info(&ts->client->dev, "[gt9xx] Success init pinctrl\n");
	return 0;
exit_pinctrl_init:
...
static int gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	if (gpio_is_valid(ts->pdata->irq_gpio)) {
		...
		if (ret < 0) {
			dev_dbg(&ts->client->dev,
				"[gt9xx] Failed to request GPIO:%d, ERRNO:%d\n",
				(s32)ts->pdata->irq_gpio, ret);
			return -ENODEV;
		}

		gpio_direction_input(ts->pdata->irq_gpio);
		dev_info(&ts->client->dev, "[gt9xx]Success request irq-gpio\n");
	}
	...
	if (gpio_is_valid(ts->pdata->rst_gpio)) {
		...
		gpio_direction_input(ts->pdata->rst_gpio);
		dev_info(&ts->client->dev,  "[gt9xx] Success request rst-gpio\n");
	}
	return 0;
}
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	ts->input_dev->name = ts->pdata->touch_name;
	ts->input_dev->phys = ts->pdata->touch_phys;
	...
	if (ret) {
		dev_dbg(&ts->client->dev, "Register %s input device failed\n",
			ts->input_dev->name);
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	if (prop) {
		...
		ret = of_property_read_u32_array(np,
				...
				key_nums);
		if (ret) {
			dev_dbg(dev, "Unable to read key codes\n");
			pdata->key_nums = 0;
	...
	if (!gpio_is_valid(pdata->irq_gpio))
		dev_dbg(dev, "No valid irq gpio");

	pdata->rst_gpio = of_get_named_gpio(np, "reset-gpios", 0);
	if (!gpio_is_valid(pdata->rst_gpio))
		dev_dbg(dev, "No valid rst gpio");

	ret = of_property_read_string(np, "touch-identity", &touch_identity);
	if (ret < 0) {
		dev_info(dev, "Unable to read 'touch_identity 01' property\n");
		touch_identity = "default";
	} else {
		dev_info(dev, "touch_identity: %s\n", touch_identity);
	}
	snprintf(pdata->touch_name, sizeof(pdata->touch_name), "%s", touch_identity);
	snprintf(pdata->touch_phys, sizeof(pdata->touch_phys), "input/%s", touch_identity);
	snprintf(pdata->touch_proc, sizeof(pdata->touch_proc), "gt9xx_config_%s", touch_identity);
	return 0;
}
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if (!prop || !prop->value || *cfg_len == 0 ||
		...
		*cfg_len = 0;
		pr_info("[goodix-ts]%s gtp_parse_dt_cfg error  \n", __func__);
		ret = -EPERM;/* failed */
	} else {
...
static int gtp_power_init(struct goodix_ts_data *ts)
{
	...
	if (IS_ERR(ts->vdd_ana)) {
		...
		dev_info(&ts->client->dev,
			 "[gt9xx]Regulator get failed vdd ret=%d\n", ret);
	}
	...
	if (IS_ERR(ts->vcc_i2c)) {
		...
		dev_info(&ts->client->dev,
			 "[gt9xx]Regulator get failed vcc_i2c ret=%d\n", ret);
	}
	ret = regulator_set_voltage(ts->vdd_ana, 3300000, 3300000);
	if (ret) {
		pr_info("[gt9xx] regulator_set_voltage failed %d\n", ret);
	}
	return 0;
}
#endif
...
static int gtp_probe(struct i2c_client *client)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dev_dbg(&client->dev, "Failed check I2C functionality");
		return -ENODEV;
	}
	ts = devm_kzalloc(&client->dev, sizeof(*ts), GFP_KERNEL);
	if (!ts) {
		dev_dbg(&client->dev, "Failed alloc ts memory");
		return -ENOMEM;
	}
	pdata = devm_kzalloc(&client->dev, sizeof(*pdata), GFP_KERNEL);
	if (!pdata) {
		dev_dbg(&client->dev, "Failed alloc pdata memory\n");
	...
	if (client->dev.of_node) {
		...
		if (ret) {
			dev_dbg(&client->dev, "Failed parse dts\n");
	...
	if (ret) {
		dev_dbg(&client->dev, " [gt9xx] Failed get regulator\n");
		ret = -EINVAL;
		goto exit_free_client_data;
	}
	ret = gtp_power_on(ts);
	if (ret) {
		dev_dbg(&client->dev, " [gt9xx] Failed power on device\n");
		ret = -EINVAL;
	...
	if (ret < 0) {
		 ...
		 */
		dev_dbg(&client->dev, " [gt9xx] Failed get wanted pinctrl state\n");
#if GTP_USE_POWER_REGULATOR
		goto exit_deinit_power;
#endif
	}
	...
	if (ret < 0) {
		dev_dbg(&client->dev, " [gt9xx] Failed request IO port\n");
#if GTP_USE_POWER_REGULATOR
		goto exit_power_off;
#endif
	}
	gpio_direction_output(ts->pdata->rst_gpio, 1);
	///usleep_range(2000, 3000);//hc add 2-19
	///gtp_reset_guitar(ts->client, 20);//hc add 2-19
	ret = gtp_i2c_test(client);
	if (ret) {
		dev_info(&client->dev, "[gt9xx]Failed communicate with IC use I2C\n");
		//goto exit_free_io_port;
	}
	dev_info(&client->dev, "[gt9xx] I2C Addr is %x\n", client->addr);
	ret = gtp_get_fw_info(client, &ts->fw_info);
	if (ret < 0) {
		dev_info(&client->dev, "[gt9xx]Failed read FW version\n");
		//goto exit_free_io_port;
	}
	...
	if (ret < 0) {
		dev_info(&client->dev, "[gt9xx]Failed request input device\n");
		//goto exit_free_io_port;
	}
	mutex_init(&ts->lock);
	...
	if (ret < 0) {
		dev_dbg(&client->dev, "[gt9xx]Failed create work thread");
	...
	if (ts->pdata->slide_wakeup) {
		...
		if (ret < 0)
			dev_dbg(&client->dev, "[gt9xx]Failed set irq wake\n");
	}
	gtp_register_powermanager(ts);

	ret = gtp_create_file(ts);
	if (ret) {
		dev_info(&client->dev, "[gt9xx]Failed create attributes file");
...
void gtp_drv_remove(struct i2c_client *client)
{
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
...
static void gtp_suspend(struct goodix_ts_data *ts)
{
	...
	if (ret < 0)
		dev_dbg(&ts->client->dev, "Failed enter suspend\n");
...
static void gtp_resume(struct goodix_ts_data *ts)
{
	...
	if (ts->pdata->slide_wakeup && test_bit(DOZE_MODE, &ts->flags)) {
		...
		if (ret)
			dev_dbg(&ts->client->dev, "Failed wake up from gesture mode\n");
	} else if (ts->pdata->power_off_sleep) {
#if GTP_USE_POWER_REGULATOR
		ret = gtp_power_on(ts);
#endif
	...
	if (ret)
		dev_dbg(&ts->client->dev, "Later resume failed\n");
	else
		gtp_esd_on(ts);
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	pr_info("The esd is not needed for tmp\n");
/*
	s32 i;
	s32 ret = -1;
	...
	if (i >= 3) {
		dev_info(&ts->client->dev, "IC working abnormally! Reset IC\n");
		esd_buf[0] = 0x42;
		esd_buf[1] = 0x26;
		esd_buf[2] = 0x01;
		esd_buf[3] = 0x01;
