...
static int semi_touch_tcm_disp_notifier_callback(struct notifier_block *nb,
...
static ssize_t gesenable_store(const struct class *class, const struct class_attribute *attr, const char *buf, size_t count)
{
	unsigned long state;
	ssize_t ret = -EINVAL;
...
/*----------------------------------------------------------------------------*/
static ssize_t gesenable_show(const struct class *dev, const struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s:%d\n", "gesenable",tp_gesture_switch);    
}
//static CLASS_ATTR(gesenable, 0664, tp_gestrue_function_show, tp_gestrue_function_store);
static CLASS_ATTR_RW(gesenable);
/***********************************add for gesture end**************************************/
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	*/
	...
	disp_notifier.notifier_call = semi_touch_tcm_disp_notifier_callback;
	...
	tp_gesture_class = class_create("tp_gesture");
...
int  fix_tp_proc_info(void  *tp_data, u32 data_len)
{
	int ret = 0;
...
static const struct proc_ops wtk_tp_info_proc_fops = {	
	.proc_open       = wtk_tp_info_open,
	.proc_read       = seq_read,
};

int wtk_creat_proc_tp_info(void)
{
	struct proc_dir_entry *tp_info_entry = proc_create("wtk_tpInfo", 0440, NULL, &wtk_tp_info_proc_fops);
