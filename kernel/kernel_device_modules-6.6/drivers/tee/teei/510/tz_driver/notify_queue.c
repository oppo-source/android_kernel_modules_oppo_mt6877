...
static unsigned long create_notify_queue(unsigned long size)
{
	...
	retVal = add_work_entry(SMC_CALL_TYPE, N_INIT_T_FC_BUF,
				soter_sec_world_id[SOTER_SEC_WORLD_ID_NQ],
				soter_sec_world_id[SOTER_SEC_WORLD_ID_NQ]>>32);
	if (retVal != 0) {
...
static int teei_trigger_callback_irq(void)
{
	int cpu_id = 0;
	int retVal = 0;

	cpu_id = get_current_teei_cpuid();

	retVal = smp_call_function_single(cpu_id, teei_callback_fun,
						NULL, false);
	if (retVal != 0) {
		IMSG_ERROR("[%s][%d] Failed to smp_call_function_single %d\n",
