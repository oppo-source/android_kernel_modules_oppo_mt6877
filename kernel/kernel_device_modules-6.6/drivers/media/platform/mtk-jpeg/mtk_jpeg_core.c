...
static void mtk_jpeg_enc_device_run(void *priv)
{
	...
	if (ctx->enable_q_table) {
		...
		ret = mtk_jpeg_alloc_mem(ctx, ctx->jpeg->smmu_dev,
			&ctx->qtable, &ctx->qtable.buf_att, &ctx->qtable.sgt);
...
static int mtk_jpeg_release(struct file *file)
{
	...
	// #ifdef OPLUS_FEATURE_CAMERA_COMMON MTK patch ALPS10003765
	int ret;
	// #endif OPLUS_FEATURE_CAMERA_COMMON MTK patch ALPS10003765
