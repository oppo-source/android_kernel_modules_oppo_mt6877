...
static enum led_brightness get_brightness2(struct led_classdev *led_cdev);
...
{
	return m_brightness2_enable;
}
EXPORT_SYMBOL(oplus_brightness2_supported);
...
bool oplus_project_public_brightness_support(void) {
	return oplus_public_brightness_support;
}
EXPORT_SYMBOL(oplus_project_public_brightness_support);
#endif /* OPLUS_FEATURE_DISPLAY */
...
static enum led_brightness get_brightness2(struct led_classdev *led_cdev)
{
	return m_brightness2_value;
}
void set_brightness2_for_show(unsigned int level)
{
	m_brightness2_value = level;
}
EXPORT_SYMBOL(set_brightness2_for_show);
#endif /* OPLUS_FEATURE_DISPLAY */
/****************************************************************************
...
int mt_leds_parse_dt(struct mt_led_data *mdev, struct fwnode_handle *fwnode)
{
...
#ifdef OPLUS_FEATURE_DISPLAY
	ret = fwnode_property_read_string(fwnode, "brightness2-state", &state);
	if (!ret) {
		if (!strncmp(state, "enable", strlen("enable")))
			m_brightness2_enable = true;
		else
			m_brightness2_enable = false;
	} else {
		m_brightness2_enable = false;
	}
	pr_info("parse m_brightness2_enable: %d\n", m_brightness2_enable);
#endif /* OPLUS_FEATURE_DISPLAY */
	...
	ret = fwnode_property_read_string(fwnode, "public_brightness_support", &state);
	if (!ret) {
		if (!strncmp(state, "true", strlen("true")))
			oplus_public_brightness_support = true;
		else
			oplus_public_brightness_support = false;
	} else {
		oplus_public_brightness_support = false;
	}
#endif /* OPLUS_FEATURE_DISPLAY */
...
#ifdef OPLUS_FEATURE_DISPLAY
    if (oplus_brightness2_supported()) {
        led_dat->conf.cdev.brightness_get = get_brightness2;
	}
#endif /* OPLUS_FEATURE_DISPLAY */
