...
#if IS_ENABLED(CONFIG_AUDIO_DAEMON_KERNEL_MTK_HIFI) || IS_ENABLED(CONFIG_AUDIO_DAEMON_KERNEL_MTK_RV)
#include "../../codecs/audio/oplus_audio_daemon/oplus_audio_daemon_kernel.h"
#endif

#define AUDIO_KERNEL_FB_VERSION                "1.0.0"

#define OPLUS_AUDIO_EVENTID_AUDIO_KERNEL_ERR   (10047)
#define OPLUS_AUDIO_EVENTID_AUDIO_KERNEL_TEMP   (30000)
...
#else /* CONFIG_AUDIO_DAEMON_KERNEL_MTK_HIFI || CONFIG_AUDIO_DAEMON_KERNEL_MTK_RV */
#define audio_kernel_fb(level, delay, fmt, ...) \
	do { \
		static DEFINE_RATELIMIT_STATE(_rs, \
						FEEDBACK_RATELIMIT_INTERVAL, \
...
#endif /* CONFIG_AUDIO_DAEMON_KERNEL_MTK_HIFI || CONFIG_AUDIO_DAEMON_KERNEL_MTK_RV */

#define audio_temp_fb(level, delay, fmt, ...) \
	do { \
		static DEFINE_RATELIMIT_STATE(_rs, \
						FEEDBACK_RATELIMIT_INTERVAL, \
						FEEDBACK_RATELIMIT_BURST); \
		if (__ratelimit(&_rs)) \
			mm_fb_audio(OPLUS_AUDIO_EVENTID_AUDIO_KERNEL_TEMP, \
					MM_FB_KEY_RATELIMIT_30MIN, delay, level, fmt, ##__VA_ARGS__); \
	} while (0)
...
#define ratelimited_fb_fatal(fmt, ...) \
	audio_kernel_fb(FB_FATAL, 0, fmt, ##__VA_ARGS__)
...
#define pr_temp_base(level, delay, fmt, ...) \
	do { \
		printk(KERN_ERR pr_fmt(fmt), ##__VA_ARGS__); \
		audio_temp_fb(level, delay, "payload@@"fmt, ##__VA_ARGS__); \
	} while (0)

#define pr_err_fb(fmt, ...) \
	pr_err_base(FB_ERROR, 0, fmt, ##__VA_ARGS__)

#define pr_temp_fb(fmt, ...) \
	pr_temp_base(FB_ERROR, 0, fmt, ##__VA_ARGS__)
