...
static struct mt6358_priv *mCodec_priv;
...
void audckbufEnable(bool enable)
{
	...
	if (enable) {
		...
		if (audck_buf_Count < 0) {
			pr_info("audck_buf_Count count < 0\n");
			audck_buf_Count = 0;
		}
	}
...
static void headset_volume_ramp(int from, int to)
{
	...
	if (!is_valid_hp_pga_idx(from) || !is_valid_hp_pga_idx(to))
		pr_info("%s(), volume index is not valid, from %d, to %d\n",
			__func__, from, to);
...
static int detect_impedance(void)
{
	...
	if (enable_dc_compensation &&
		...
		pr_info("%s(), dc compensation ops not ready\n", __func__);
		return 0;
	}
...
static int SetDcCompenSation(bool enable)
{
	...
	if (enable_dc_compensation == NULL ||
		...
		pr_info("%s(), function not ready, enable %p, lch %p, rch %p\n",
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	if (!set_hp_impedance_ctl) {
		pr_info("%s(), set_hp_impedance_ctl == NULL\n", __func__);
		return 0;
	}
	...
	if (OpenHeadPhoneImpedanceSetting(true)) {
		...
		pr_info("%s(), pmic dl busy, do nothing\n", __func__);
...
static bool AudioPreAmp1_Sel(int Mul_Sel)
{
	if (Mul_Sel >= 0 && Mul_Sel < NUM_PREAMP_INPUT_SELECT)
		...
		pr_info("%s(), error, Mul_Sel = %d", __func__, Mul_Sel);
...
static int mtk_mt6358_codec_dev_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		if (ret != -EPROBE_DEFER)
			dev_info(&pdev->dev, "Error: Get iio ch failed (%d)\n",
				ret);
		return ret;
	}
	...
	mt63xx_set_local_regmap(mCodec_priv->regmap);
	...
	if (pdev->dev.of_node) {
		...
		pr_debug("%s(), use-hp-depop-flow = %d\n",
