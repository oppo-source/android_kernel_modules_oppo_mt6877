 ...
 *                         D A T A   T Y P E S
 *****************************************************************************/
struct regmap *mRegmap;

void mt63xx_set_local_regmap(struct regmap *regmap)
{
	mRegmap = regmap;
}
EXPORT_SYMBOL(mt63xx_set_local_regmap);

unsigned int Ana_Get_Reg(unsigned int addr)
{
	unsigned int val = 0;

	regmap_read(mRegmap, addr, &val);
	return val;
}
...
{
	regmap_update_bits(mRegmap, addr, mask, value);
}
