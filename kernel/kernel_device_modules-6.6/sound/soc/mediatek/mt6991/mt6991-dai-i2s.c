...
//#ifdef OPLUS_ARCH_EXTENDS
		if (i2s_priv->use_tdm && i2s_priv->pad_top_value > 0 && id == DAI_I2SIN4) {
			pad_top = i2s_priv->pad_top_value;
			dev_info(afe->dev, "%s() change pad_top to %d\n", __func__, pad_top);
		}
//#endif
...
//#ifdef OPLUS_ARCH_EXTENDS
		if (i2s_priv->use_tdm && id == DAI_I2SIN4) {
			dev_info(afe->dev, "%s(), set ETDM_IN4_CON3!!\n", __func__);
			mtk_regmap_update_bits(afe->regmap, ETDM_IN4_CON3,
							       REG_DISABLE_OUT_MASK,
							       0x8,
							       REG_DISABLE_OUT_SFT);
		}
//#endif
...
//#ifdef OPLUS_ARCH_EXTENDS
	/* get if use tdm protocol */
	ret = of_property_read_u32(afe->dev->of_node, "use-tdm-protocol",
				   &i2s_priv->use_tdm);
	if (ret) {
		dev_info(afe->dev, "%s() failed to read use-tdm-protocol, default is 0\n",
			 __func__);
		i2s_priv->use_tdm = 0;
	} else if (i2s_priv->use_tdm != 0){
		for (i = 0; i < I2S_IN_NUM; i++) {
			i2s_priv = afe_priv->dai_priv[MT6991_DAI_I2S_IN0 + i];
			i2s_priv->use_tdm = 1;
		}
	...
	/* get pad top value if it is config */
	int pad_top = 0;
	ret = of_property_read_u32(afe->dev->of_node, "pad-top-value",
				&pad_top);
	if (ret) {
		dev_info(afe->dev, "%s() failed to read pad-top-value, default is 0\n",
			 __func__);
		i2s_priv->pad_top_value = 0;
	} else if (pad_top > 0){
		dev_info(afe->dev, "%s() pad_top is config to %d\n", __func__, pad_top);
		for (i = 0; i < I2S_IN_NUM; i++) {
			i2s_priv = afe_priv->dai_priv[MT6991_DAI_I2S_IN0 + i];
			i2s_priv->pad_top_value = pad_top;
		}
	}
//#endif
