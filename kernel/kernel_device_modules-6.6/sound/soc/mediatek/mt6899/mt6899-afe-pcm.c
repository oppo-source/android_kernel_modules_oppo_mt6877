...
/*
 *  Mediatek ALSA SoC AFE platform driver for 6899
 *
...
#include "mt6899-afe-cm.h"
#include "mt6899-afe-clk.h"
#include "mt6899-afe-gpio.h"
#include "mt6899-interconnection.h"
...
static ssize_t mt6899_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe);

static const struct snd_pcm_hardware mt6899_afe_hardware = {
...
static int mt6899_fe_startup(struct snd_pcm_substream *substream,
...
void mt6899_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6899_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	if (!in_interrupt()) {
		...
		//mt6899_aud_update_power_scenario();
	}
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		if (id == MT6899_MEMIF_DL23)
			mtk_memif_set_pbuf_size(afe, id, MT6899_MEMIF_PBUF_SIZE_32_BYTES);
		...
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8") || !strcmp(memif->data->name, "VUL_CM0")) {
				if (mt6899_is_need_enable_cm(afe, CM0))
					mt6899_enable_cm(afe, CM0, 1);
			}
			if (!strcmp(memif->data->name, "VUL9") || !strcmp(memif->data->name, "VUL_CM1")) {
				if (mt6899_is_need_enable_cm(afe, CM1))
					mt6899_enable_cm(afe, CM1, 1);
			}
		}
		// VUL24~VUL25 need to set on use to avoid out con being CM2
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8") || !strcmp(memif->data->name, "VUL_CM0"))
				mt6899_enable_cm(afe, CM0, 0);
			if (!strcmp(memif->data->name, "VUL9") || !strcmp(memif->data->name, "VUL_CM1"))
				mt6899_enable_cm(afe, CM1, 0);
		}
...
static int mt6899_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	rate_reg = mt6899_rate_transform(afe->dev, rate, id);

	switch (id) {
	case MT6899_MEMIF_VUL8:
	case MT6899_MEMIF_VUL_CM0:
		cm = 0x0;
		break;
	case MT6899_MEMIF_VUL9:
	case MT6899_MEMIF_VUL_CM1:
		cm = 0x1;
		break;
	default:
		cm = 0x0;
	...
	mt6899_set_cm_rate(cm, rate_reg);

	return rate_reg;
}

static int mt6899_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6899_rate_transform(afe->dev, rate, dai_id);
}

static int mt6899_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	if (!component)
		return -EINVAL;
	afe = snd_soc_component_get_drvdata(component);
	return mt6899_general_rate_transform(afe->dev, rate);
}

int mt6899_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6899_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6899_MEMIF_PBUF_SIZE_32_BYTES;
}

/* FE DAIs */
static const struct snd_soc_dai_ops mt6899_memif_dai_ops = {
	.startup        = mt6899_fe_startup,
	.shutdown       = mt6899_fe_shutdown,
	.hw_params      = mtk_afe_fe_hw_params,
	.hw_free        = mtk_afe_fe_hw_free,
	.prepare        = mtk_afe_fe_prepare,
	.trigger        = mt6899_fe_trigger,
};
...
static struct snd_soc_dai_driver mt6899_memif_dai_driver[] = {
...
/* kcontrol */
static int mt6899_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6899_PRIMARY_MEMIF];
	return 0;
}

static int mt6899_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6899_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6899_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6899_RECORD_MEMIF];
	return 0;
}

static int mt6899_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6899_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6899_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6899_DEEP_MEMIF];
	return 0;
}

static int mt6899_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6899_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6899_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6899_VOIP_MEMIF];
	return 0;
}

static int mt6899_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6899_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6899_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6899_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6899_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6899_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6899_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6899_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6899_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6899_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6899_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6899_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

static int mt6899_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_vow_barge_in_irq_id_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6899_BARGE_IN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6899_adsp_ref_mem_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_mmap_dl_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6899_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6899_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6899_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6899_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6899_mmap_ion_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_dl_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6899_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6899_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
// ALPS08709395: AudioQOS, set VIP
static int mt6899_audio_vip_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int mt6899_audio_vip_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mt6899_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6899_adsp_ref_mem_get,
		       mt6899_adsp_ref_mem_set),
	...
	SOC_SINGLE_EXT("mmap_play_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6899_mmap_dl_scene_get, mt6899_mmap_dl_scene_set),
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6899_mmap_ul_scene_get, mt6899_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6899_dl_mmap_fd_get,
		       mt6899_dl_mmap_fd_set),
	SOC_SINGLE_EXT("aaudio_ul_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6899_ul_mmap_fd_get,
		       mt6899_ul_mmap_fd_set),
	SOC_ENUM_EXT("MTK_RECORD_MISO1", mt6899_pcm_type_enum[0],
		     record_miso1_en_get, record_miso1_en_set),
	SOC_SINGLE_EXT("audio_vip", SND_SOC_NOPM, 0, 0x3fffff, 0,
		       mt6899_audio_vip_get, mt6899_audio_vip_set),
};
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		mt6899_enable_cm_bypass(afe, CM0, 0x0);
		mt6899_set_cm(afe, CM0, 0x1, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt6899_enable_cm_bypass(afe, CM0, 0x1);
		break;
	default:
		break;
	}
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		channels = mtk_get_channel_value();
		mt6899_enable_cm_bypass(afe, CM1, 0x0);
		mt6899_set_cm(afe, CM1, 0x1, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* remove so we fix in normal mode */
		// mt6899_enable_cm_bypass(afe, CM1, 0x1);
		break;
	default:
		break;
	}
...
static const struct snd_kcontrol_new memif_ul_cm1_ch1_mix[] = {
	...
	// Add for 4ch AEC
	SOC_DAPM_SINGLE_AUTODISABLE("DL_24CH_CH1", AFE_CONN048_1,
					I_DL_24CH_CH1, 1, 0),
#endif
};
...
#ifdef OPLUS_ARCH_EXTENDS
	// Add for 4ch AEC
	SOC_DAPM_SINGLE_AUTODISABLE("DL_24CH_CH2", AFE_CONN049_1,
					I_DL_24CH_CH2, 1, 0),
#endif
};
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(widget->name, "CM1_UL_MUX")) {
		ucontrol->value.integer.value[0] = mt6899_get_cm_mux(CM1);
	} else if (!strcmp(widget->name, "CM0_UL_MUX")) {
		ucontrol->value.integer.value[0] = mt6899_get_cm_mux(CM0);
	} else {
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(widget->name, "CM1_UL_MUX"))
		mt6899_set_cm_mux(CM1, ucontrol->value.integer.value[0]);
	else if (!strcmp(widget->name, "CM0_UL_MUX"))
		mt6899_set_cm_mux(CM0, ucontrol->value.integer.value[0]);
	else
		dev_info(afe->dev, "%s(), No match: %s\n", __func__, widget->name);
...
static const struct snd_soc_dapm_widget mt6899_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6899_memif_routes[] = {
	{"UL0", NULL, "UL0_CH1"},
	{"UL0", NULL, "UL0_CH2"},
	/* Normal record */
...
#ifdef OPLUS_ARCH_EXTENDS
	// Add for 4ch AEC
	{"UL_CM1_CH1", "DL_24CH_CH1", "Hostless_UL1 UL"},
	{"UL_CM1_CH2", "DL_24CH_CH2", "Hostless_UL1 UL"},
...
static const struct mtk_base_memif_data memif_data[MT6899_MEMIF_NUM] = {
	[MT6899_MEMIF_DL0] = {
		.name = "DL0",
		.id = MT6899_MEMIF_DL0,
...
static const struct mtk_base_irq_data irq_data[MT6899_IRQ_NUM] = {
	[MT6899_IRQ_0] = {
		.id = MT6899_IRQ_0,
...
static const int memif_irq_usage[MT6899_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6899_MEMIF_DL0] = MT6899_IRQ_0,
	[MT6899_MEMIF_DL1] = MT6899_IRQ_1,
	[MT6899_MEMIF_DL2] = MT6899_IRQ_2,
	[MT6899_MEMIF_DL3] = MT6899_IRQ_3,
	[MT6899_MEMIF_DL4] = MT6899_IRQ_4,
	[MT6899_MEMIF_DL5] = MT6899_IRQ_5,
	[MT6899_MEMIF_DL6] = MT6899_IRQ_6,
	[MT6899_MEMIF_DL7] = MT6899_IRQ_7,
	[MT6899_MEMIF_DL8] = MT6899_IRQ_8,
	[MT6899_MEMIF_DL23] = MT6899_IRQ_9,
	[MT6899_MEMIF_DL24] = MT6899_IRQ_10,
	[MT6899_MEMIF_DL25] = MT6899_IRQ_11,
	[MT6899_MEMIF_DL_24CH] = MT6899_IRQ_12,
	[MT6899_MEMIF_VUL0] = MT6899_IRQ_13,
	[MT6899_MEMIF_VUL1] = MT6899_IRQ_14,
	[MT6899_MEMIF_VUL2] = MT6899_IRQ_15,
	[MT6899_MEMIF_VUL3] = MT6899_IRQ_16,
	[MT6899_MEMIF_VUL4] = MT6899_IRQ_17,
	[MT6899_MEMIF_VUL5] = MT6899_IRQ_18,
	[MT6899_MEMIF_VUL6] = MT6899_IRQ_19,
	[MT6899_MEMIF_VUL7] = MT6899_IRQ_20,
	[MT6899_MEMIF_VUL8] = MT6899_IRQ_21,
	[MT6899_MEMIF_VUL9] = MT6899_IRQ_22,
	[MT6899_MEMIF_VUL10] = MT6899_IRQ_23,
	[MT6899_MEMIF_VUL24] = MT6899_IRQ_24,
	[MT6899_MEMIF_VUL25] = MT6899_IRQ_25,
	[MT6899_MEMIF_VUL_CM0] = MT6899_IRQ_26,
	[MT6899_MEMIF_VUL_CM1] = MT6899_IRQ_0,
	[MT6899_MEMIF_ETDM_IN0] = MT6899_IRQ_0,
	[MT6899_MEMIF_ETDM_IN1] = MT6899_IRQ_0,
	[MT6899_MEMIF_ETDM_IN2] = MT6899_IRQ_0,
	[MT6899_MEMIF_ETDM_IN4] = MT6899_IRQ_0,
	[MT6899_MEMIF_ETDM_IN6] = MT6899_IRQ_0,
	[MT6899_MEMIF_HDMI] = MT6899_IRQ_31
};

static bool mt6899_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6899_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6899_is_volatile_reg,
...
static irqreturn_t mt6899_afe_irq_handler(int irq_id, void *dev)
{
	...
	/* clear irq */
	for (i = 0; i < MT6899_IRQ_NUM; ++i) {
		if (((cus_status_mcu & (0x1 << irq_data[i].id)) && i == MT6899_IRQ_31) ||
		    ((status_mcu & (0x1 << irq_data[i].id)) && i != MT6899_IRQ_31)) {
			regmap_read(afe->regmap, irq_data[i].irq_clr_reg, &tmp_reg);
			regmap_update_bits(afe->regmap, irq_data[i].irq_clr_reg,
					0xc0000000,
					tmp_reg^0xc0000000);
...
static int mt6899_afe_runtime_suspend(struct device *dev)
{
...
skip_regmap:
	mt6899_afe_sram_release(afe);
	mt6899_afe_disable_clock(afe);
	return 0;
}

static int mt6899_afe_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	int ret = 0;
...
static int mt6899_afe_pcm_copy(struct snd_pcm_substream *substream,
...
static int mt6899_set_memif_sram_mode(struct device *dev,
...
static int mt6899_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	/* set memif sram mode */
	mt6899_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6899_sram_ops = {
	.set_sram_mode = mt6899_set_sram_mode,
};
				    ...
				    char *buf, loff_t offset, size_t size)
{
	...
	read_size = mt6899_debug_read_reg(buffer, AFE_SYS_DEBUG_SIZE, afe);
...
static int mt6899_afe_component_probe(struct snd_soc_component *component)
{
	struct mtk_base_afe *afe = NULL;
	...
	if (component) {
		...
		mt6899_add_misc_control(component);
...
static const struct snd_soc_component_driver mt6899_afe_component = {
	.name = AFE_PCM_NAME,
	.probe = mt6899_afe_component_probe,
...
static ssize_t mt6899_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe)
{
	...
	n += scnprintf(buffer + n, size - n,
		"AUDIO_TOP_CON4 = 0x%x\n", value);
	regmap_read(afe->regmap, CKSYS_AUD_TOP_CFG, &value);
	n += scnprintf(buffer + n, size - n,
		"AUD_TOP_CFG = 0x%x\n", value);
...
static ssize_t mt6899_debugfs_read(struct file *file, char __user *buf,
				   size_t count, loff_t *pos)
{
	...
	n = mt6899_debug_read_reg(buffer, size, afe);
...
static const struct mtk_afe_debug_cmd mt6899_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6899_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6899_debugfs_read,
};
#endif

static int mt6899_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6899_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6899_memif_dai_driver);

	dai->controls = mt6899_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6899_pcm_kcontrols);
	dai->dapm_widgets = mt6899_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6899_memif_widgets);
	dai->dapm_routes = mt6899_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6899_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6899_dai_adda_register,
	mt6899_dai_i2s_register,
	mt6899_dai_hw_gain_register,
	mt6899_dai_src_register,
	mt6899_dai_pcm_register,
	mt6899_dai_tdm_register,
	mt6899_dai_hostless_register,
...
static int mt6899_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	struct mt6899_afe_private *afe_priv;
	struct resource *res;
	struct device *dev;
	...
	if (!ret)
		dev_info(dev, "%s(), pm_runtime_get_sync error", __func__);

	afe->regmap = devm_regmap_init_mmio(&pdev->dev, afe->base_addr,
					    &mt6899_afe_regmap_config);
	...
	if (!ret)
		dev_info(dev, "%s(), pm_runtime_put_sync error", __func__);

	regcache_cache_only(afe->regmap, true);
	regcache_mark_dirty(afe->regmap);

	/* init gpio */
	ret = mt6899_afe_gpio_init(afe);
	if (ret)
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
	...
	ret = mtk_audio_sram_init(dev, afe->sram, &mt6899_sram_ops);
	if (ret)
		return ret;
	...
	afe->memif_size = MT6899_MEMIF_NUM;
	...
	afe->memif[MT6899_DEEP_MEMIF].ack = mtk_sp_clean_written_buffer_ack;
	afe->memif[MT6899_FAST_MEMIF].fast_palyback = 1;

	mutex_init(&afe->irq_alloc_lock);       /* needed when dynamic irq */

	/* init irq */
	afe->irqs_size = MT6899_IRQ_NUM;
	...
	ret = devm_request_irq(dev, irq_id, mt6899_afe_irq_handler,
			       IRQF_TRIGGER_NONE,
	...
	/* others */
	afe->mtk_afe_hardware = &mt6899_afe_hardware;
	afe->memif_fs = mt6899_memif_fs;
	afe->irq_fs = mt6899_irq_fs;
	afe->get_dai_fs = mt6899_get_dai_fs;
	afe->get_memif_pbuf_size = mt6899_get_memif_pbuf_size;

	afe->runtime_resume = mt6899_afe_runtime_resume;
	afe->runtime_suspend = mt6899_afe_runtime_suspend;

	afe->request_dram_resource = mt6899_afe_dram_request;
	afe->release_dram_resource = mt6899_afe_dram_release;

	afe->copy = mt6899_afe_pcm_copy;
...
#if IS_ENABLED(CONFIG_DEBUG_FS)
	/* debugfs */
	afe->debug_cmds = mt6899_debug_cmds;
	afe->debugfs = debugfs_create_file("mtksocaudio",
					   S_IFREG | 0444, NULL,
					   afe, &mt6899_debugfs_ops);
#endif
	...
	mt6899_set_cm_mux(CM0, 0);
	mt6899_set_cm_mux(CM1, 0);
err_find_pmic:
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUDIO_DSP)
...
static int mt6899_afe_pcm_dev_remove(struct platform_device *pdev)
{
	...
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6899_afe_runtime_suspend(&pdev->dev);
...
MODULE_DEVICE_TABLE(of, mt6899_afe_pcm_dt_match);

static const struct dev_pm_ops mt6899_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6899_afe_runtime_suspend,
			   mt6899_afe_runtime_resume, NULL)
};

static struct platform_driver mt6899_afe_pcm_driver = {
	.driver = {
		.name = "mt6899-audio",
		.of_match_table = mt6899_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		.pm = &mt6899_afe_pm_ops,
#endif
	},
