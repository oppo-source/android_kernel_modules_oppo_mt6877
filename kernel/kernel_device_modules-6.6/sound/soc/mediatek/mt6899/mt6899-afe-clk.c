...
/*
 *  mt6899-afe-clk.c  --  Mediatek 6899 afe clock ctrl
 *
...
int mt6899_init_clock(struct mtk_base_afe *afe) { return 0; }
int mt6899_afe_apll_init(struct mtk_base_afe *afe) { return 0; }
int mt6899_afe_enable_clock(struct mtk_base_afe *afe) { return 0; }
void mt6899_afe_disable_clock(struct mtk_base_afe *afe) {}
int mt6899_afe_sram_request(struct mtk_base_afe *afe) { return 0; }
void mt6899_afe_sram_release(struct mtk_base_afe *afe) {}
int mt6899_afe_dram_request(struct device *dev) { return 0; }
int mt6899_afe_dram_release(struct device *dev) { return 0; }
int mt6899_apll1_enable(struct mtk_base_afe *afe) { return 0; }
void mt6899_apll1_disable(struct mtk_base_afe *afe) {}
int mt6899_apll2_enable(struct mtk_base_afe *afe) { return 0; }
void mt6899_apll2_disable(struct mtk_base_afe *afe) {}
int mt6899_get_apll_rate(struct mtk_base_afe *afe, int apll) { return 0; }
int mt6899_get_apll_by_rate(struct mtk_base_afe *afe, int rate) { return 0; }
int mt6899_get_apll_by_name(struct mtk_base_afe *afe, const char *name) { return 0; }
int mt6899_mck_enable(struct mtk_base_afe *afe, int mck_id, int rate) { return 0; }
int mt6899_mck_disable(struct mtk_base_afe *afe, int mck_id) { return 0; }
int mt6899_set_audio_int_bus_parent(struct mtk_base_afe *afe, int clk_id) { return 0; }
#else
...
static const char *aud_clks[CLK_NUM] = {
	...
	[CLK_CK_MUX_AUD_ENG1] = "top_mux_aud_eng1",
	[CLK_CK_APLL1_D4] = "ck_apll1_d4",
	[CLK_CK_MUX_AUD_ENG2] = "top_mux_aud_eng2",
	[CLK_CK_APLL2_D4] = "ck_apll2_d4",
	[CLK_CK_MUX_AUDIO_H] = "top_mux_audio_h",
...
int mt6899_set_audio_int_bus_parent(struct mtk_base_afe *afe,
				    int clk_id)
{
...
int mt6899_set_audio_h_parent(struct mtk_base_afe *afe,
				    int clk_id)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int ret = 0;
	...
	ret = clk_set_parent(afe_priv->clk[CLK_CK_MUX_AUDIO_H],
			     afe_priv->clk[clk_id]);
	if (ret) {
		dev_info(afe->dev, "%s() clk_set_parent %s-%s fail %d\n",
			__func__, aud_clks[CLK_CK_MUX_AUDIO_H],
			aud_clks[clk_id], ret);
	}
	return ret;
}
static int apll1_mux_setting(struct mtk_base_afe *afe, bool enable)
{
	...
	if (enable) {
		...
		ret = clk_prepare_enable(afe_priv->clk[CLK_CK_MUX_AUD_ENG1]);
		if (ret) {
			dev_info(afe->dev, "%s clk_prepare_enable %s fail %d\n",
				__func__, aud_clks[CLK_CK_MUX_AUD_ENG1], ret);
			goto EXIT;
		}
		ret = clk_set_parent(afe_priv->clk[CLK_CK_MUX_AUD_ENG1],
				     afe_priv->clk[CLK_CK_APLL1_D4]);
		if (ret) {
			dev_info(afe->dev, "%s clk_set_parent %s-%s fail %d\n",
				__func__, aud_clks[CLK_CK_MUX_AUD_ENG1],
				aud_clks[CLK_CK_APLL1_D4], ret);
			goto EXIT;
		}
		ret = clk_prepare_enable(afe_priv->clk[CLK_CK_MUX_AUDIO_H]);
		if (ret) {
			dev_info(afe->dev, "%s clk_prepare_enable %s fail %d\n",
				 __func__, aud_clks[CLK_CK_MUX_AUDIO_H], ret);
			goto EXIT;
		}

		mt6899_set_audio_h_parent(afe, CLK_CK_APLL1_CK);
	} else {
		ret = clk_set_parent(afe_priv->clk[CLK_CK_MUX_AUD_ENG1],
		...
		clk_disable_unprepare(afe_priv->clk[CLK_CK_MUX_AUD_ENG1]);
		...
		mt6899_set_audio_h_parent(afe, CLK_CLK26M);
		clk_disable_unprepare(afe_priv->clk[CLK_CK_MUX_AUDIO_H]);
	}
...
static int apll2_mux_setting(struct mtk_base_afe *afe, bool enable)
{
	...
	if (enable) {
		...
		/* 196.608 / 4 = 49.152MHz */
		ret = clk_prepare_enable(afe_priv->clk[CLK_CK_MUX_AUD_ENG2]);
		if (ret) {
			dev_info(afe->dev, "%s clk_prepare_enable %s fail %d\n",
				__func__, aud_clks[CLK_CK_MUX_AUD_ENG2], ret);
			goto EXIT;
		}
		ret = clk_set_parent(afe_priv->clk[CLK_CK_MUX_AUD_ENG2],
				     afe_priv->clk[CLK_CK_APLL2_D4]);
		if (ret) {
			dev_info(afe->dev, "%s clk_set_parent %s-%s fail %d\n",
				__func__, aud_clks[CLK_CK_MUX_AUD_ENG2],
		...
		mt6899_set_audio_h_parent(afe, CLK_CK_APLL2_CK);
	} else {
		ret = clk_set_parent(afe_priv->clk[CLK_CK_MUX_AUD_ENG2],
				     afe_priv->clk[CLK_CLK26M]);
		if (ret) {
		...
		clk_disable_unprepare(afe_priv->clk[CLK_CK_MUX_AUD_ENG2]);
...
int mt6899_afe_disable_apll(struct mtk_base_afe *afe)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int ret = 0;
...
int mt6899_afe_apll_init(struct mtk_base_afe *afe)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	 ...
	 */
	if (afe_priv->apmixed) {
		regmap_write(afe_priv->apmixed, APLL1_CON2, 0x6f28bd4c);
		regmap_write(afe_priv->apmixed, APLL1_TUNER_CON0, 0x6f28bd4d);
		regmap_write(afe_priv->apmixed, APLL2_CON2, 0x78fd5264);
		regmap_write(afe_priv->apmixed, APLL2_TUNER_CON0, 0x78fd5265);
	} else {
		dev_info(afe->dev, "%s vlp_ck regmap is null ptr\n", __func__);
	}
	return 0;
}

int mt6899_afe_enable_ao_clock(struct mtk_base_afe *afe)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int ret = 0;
...
int mt6899_afe_enable_clock(struct mtk_base_afe *afe)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int ret = 0;
	...
	ret = mt6899_set_audio_int_bus_parent(afe, CLK_CLK26M);
...
void mt6899_afe_disable_clock(struct mtk_base_afe *afe)
{
	...
	//mt6899_aud_swpm_power_off();
}

int mt6899_afe_dram_request(struct device *dev)
{
...
int mt6899_afe_dram_release(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
#if !defined(SKIP_SMCC_SB)
...
int mt6899_afe_sram_request(struct mtk_base_afe *afe)
{
	struct arm_smccc_res res;
...
int mt6899_apll1_enable(struct mtk_base_afe *afe)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
int mt6899_apll2_enable(struct mtk_base_afe *afe)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
int mt6899_get_apll_rate(struct mtk_base_afe *afe, int apll)
{
	return (apll == MT6899_APLL1) ? 180633600 : 196608000;
}

int mt6899_get_apll_by_rate(struct mtk_base_afe *afe, int rate)
{
	return ((rate % 8000) == 0) ? MT6899_APLL2 : MT6899_APLL1;
}

int mt6899_get_apll_by_name(struct mtk_base_afe *afe, const char *name)
{
...
/* mck */
struct mt6899_mck_div {
	int m_sel_id;
	int div_clk_id;
};


static const struct mt6899_mck_div mck_div[MT6899_MCK_NUM] = {
	[MT6899_I2SIN0_MCK] = {
...
int mt6899_mck_enable(struct mtk_base_afe *afe, int mck_id, int rate)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int apll = mt6899_get_apll_by_rate(afe, rate);
	int apll_clk_id = apll == MT6899_APLL1 ?
...
int mt6899_mck_disable(struct mtk_base_afe *afe, int mck_id)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int m_sel_id = 0;
	int div_clk_id = 0;
...
int mt6899_init_clock(struct mtk_base_afe *afe)
{
	struct mt6899_afe_private *afe_priv = afe->platform_priv;
	int i = 0;
	...
	mt6899_afe_apll_init(afe);
	mt6899_afe_disable_apll(afe);
	mt6899_afe_enable_ao_clock(afe);
	return 0;
}
#endif
