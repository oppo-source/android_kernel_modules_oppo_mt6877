...
/*
 *  Mediatek ALSA SoC AFE platform driver for 6781
 *
...
static const struct snd_pcm_hardware mt6781_afe_hardware = {
...
static int mt6781_fe_startup(struct snd_pcm_substream *substream,
...
void mt6781_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6781_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
...
static int mt6781_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	return mt6781_rate_transform(afe->dev, rate, id);
}

static int mt6781_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6781_rate_transform(afe->dev, rate, dai_id);
}

static int mt6781_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	return mt6781_general_rate_transform(afe->dev, rate);
}

int mt6781_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6781_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6781_MEMIF_PBUF_SIZE_32_BYTES;
}

/* FE DAIs */
static const struct snd_soc_dai_ops mt6781_memif_dai_ops = {
	.startup	= mt6781_fe_startup,
	.shutdown	= mt6781_fe_shutdown,
	.hw_params	= mtk_afe_fe_hw_params,
	.hw_free	= mtk_afe_fe_hw_free,
	.prepare	= mtk_afe_fe_prepare,
	.trigger	= mt6781_fe_trigger,
};
...
static struct snd_soc_dai_driver mt6781_memif_dai_driver[] = {
	/* FE DAIs: memory intefaces to CPU */
	{
		...
		.ops = &mt6781_memif_dai_ops,
	},
	{
		.name = "DL12",
		.id = MT6781_MEMIF_DL12,
		.playback = {
			.stream_name = "DL12",
			.channels_min = 1,
			.channels_max = 2,
...
/* kcontrol */
static int mt6781_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6781_PRIMARY_MEMIF];
	return 0;
}

static int mt6781_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6781_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6781_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6781_RECORD_MEMIF];
	return 0;
}

static int mt6781_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6781_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6781_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6781_DEEP_MEMIF];
	return 0;
}

static int mt6781_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6781_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6781_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6781_VOIP_MEMIF];
	return 0;
}

static int mt6781_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6781_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6781_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6781_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6781_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6781_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6781_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
					 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6781_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6781_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_vow_barge_in_irq_id_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6781_BARGEIN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUDIO_DSP)
static int mt6781_adsp_ref_mem_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6781_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6781_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6781_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6781_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6781_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6781_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6781_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static const struct snd_kcontrol_new mt6781_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6781_adsp_ref_mem_get,
		       mt6781_adsp_ref_mem_set),
	...
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6781_mmap_ul_scene_get, mt6781_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6781_dl_mmap_fd_get,
		       mt6781_dl_mmap_fd_set),
...
static const struct snd_soc_dapm_widget mt6781_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6781_memif_routes[] = {
	{"UL1", NULL, "UL1_CH1"},
	{"UL1", NULL, "UL1_CH2"},
	{"UL1", NULL, "UL1_CH3"},
	{"UL1", NULL, "UL1_CH4"},
...
static const struct mtk_base_memif_data memif_data[MT6781_MEMIF_NUM] = {
	[MT6781_MEMIF_DL1] = {
		.name = "DL1",
		.id = MT6781_MEMIF_DL1,
...
static const struct mtk_base_irq_data irq_data[MT6781_IRQ_NUM] = {
	[MT6781_IRQ_0] = {
		.id = MT6781_IRQ_0,
...
static const int memif_irq_usage[MT6781_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6781_MEMIF_DL1] = MT6781_IRQ_0,
	[MT6781_MEMIF_DL2] = MT6781_IRQ_1,
	[MT6781_MEMIF_DL3] = MT6781_IRQ_2,
	[MT6781_MEMIF_DL4] = MT6781_IRQ_3,
	[MT6781_MEMIF_DL5] = MT6781_IRQ_4,
	[MT6781_MEMIF_DL6] = MT6781_IRQ_5,
	[MT6781_MEMIF_DL7] = MT6781_IRQ_6,
	[MT6781_MEMIF_DL8] = MT6781_IRQ_7,
	[MT6781_MEMIF_DL12] = MT6781_IRQ_9,
	[MT6781_MEMIF_DAI] = MT6781_IRQ_10,
	[MT6781_MEMIF_MOD_DAI] = MT6781_IRQ_11,
	[MT6781_MEMIF_DAI2] = MT6781_IRQ_12,
	[MT6781_MEMIF_VUL12] = MT6781_IRQ_13,
	[MT6781_MEMIF_VUL2] = MT6781_IRQ_14,
	[MT6781_MEMIF_AWB] = MT6781_IRQ_15,
	[MT6781_MEMIF_AWB2] = MT6781_IRQ_16,
	[MT6781_MEMIF_VUL3] = MT6781_IRQ_17,
	[MT6781_MEMIF_VUL4] = MT6781_IRQ_18,
	[MT6781_MEMIF_VUL5] = MT6781_IRQ_19,
	[MT6781_MEMIF_VUL6] = MT6781_IRQ_20,
};

static bool mt6781_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6781_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6781_is_volatile_reg,
...
static irqreturn_t mt6781_afe_irq_handler(int irq_id, void *dev)
{
...
static int mt6781_afe_runtime_suspend(struct device *dev)
{
...
static int mt6781_afe_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	struct mt6781_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
static int mt6781_afe_pcm_copy(struct snd_pcm_substream *substream,
...
static int mt6781_set_memif_sram_mode(struct device *dev,
...
static int mt6781_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);

	/* set memif sram mode */
	mt6781_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6781_sram_ops = {
	.set_sram_mode = mt6781_set_sram_mode,
};

static int mt6781_afe_pcm_platform_probe(struct snd_soc_component *platform)
{
	mtk_afe_add_sub_dai_control(platform);
	mt6781_add_misc_control(platform);
	return 0;
}

const struct snd_soc_component_driver mt6781_afe_component = {
	.name     = AFE_PCM_NAME,
	.probe    = mt6781_afe_pcm_platform_probe,
...
static ssize_t mt6781_debugfs_read(struct file *file, char __user *buf,
...
static const struct mtk_afe_debug_cmd mt6781_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6781_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6781_debugfs_read,
};

static int mt6781_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6781_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6781_memif_dai_driver);

	dai->controls = mt6781_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6781_pcm_kcontrols);
	dai->dapm_widgets = mt6781_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6781_memif_widgets);
	dai->dapm_routes = mt6781_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6781_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6781_dai_adda_register,
	mt6781_dai_i2s_register,
	mt6781_dai_hw_gain_register,
	mt6781_dai_src_register,
	mt6781_dai_pcm_register,
	mt6781_dai_hostless_register,
	mt6781_dai_memif_register,
};

static int mt6781_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i;
#if !defined(CONFIG_FPGA_EARLY_PORTING) || defined(FORCE_FPGA_ENABLE_IRQ)
	int irq_id;
	...
	struct mt6781_afe_private *afe_priv;
	struct resource *res;
	struct device *dev;
	...
	afe->memif_size = MT6781_MEMIF_NUM;
	...
	afe->memif[MT6781_DEEP_MEMIF].ack = mtk_sp_clean_written_buffer_ack;
	...
	ret = devm_request_irq(dev, irq_id, mt6781_afe_irq_handler,
			       IRQF_TRIGGER_NONE,
	...
	/* others */
	afe->mtk_afe_hardware = &mt6781_afe_hardware;
	afe->memif_fs = mt6781_memif_fs;
	afe->irq_fs = mt6781_irq_fs;
	afe->get_dai_fs = mt6781_get_dai_fs;
	afe->get_memif_pbuf_size = mt6781_get_memif_pbuf_size;

	afe->runtime_resume = mt6781_afe_runtime_resume;
	afe->runtime_suspend = mt6781_afe_runtime_suspend;

	afe->request_dram_resource = mt6781_afe_dram_request;
	afe->release_dram_resource = mt6781_afe_dram_release;

	afe->copy = mt6781_afe_pcm_copy;

	/* debugfs */
	afe->debug_cmds = mt6781_debug_cmds;
...
static int mt6781_afe_pcm_dev_remove(struct platform_device *pdev)
{
	...
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6781_afe_runtime_suspend(&pdev->dev);
...
MODULE_DEVICE_TABLE(of, mt6781_afe_pcm_dt_match);

static const struct dev_pm_ops mt6781_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6781_afe_runtime_suspend,
			   mt6781_afe_runtime_resume, NULL)
};

static struct platform_driver mt6781_afe_pcm_driver = {
	.driver = {
		   .name = "mt6781-audio",
		   .of_match_table = mt6781_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		   .pm = &mt6781_afe_pm_ops,
#endif
	},
	.probe = mt6781_afe_pcm_dev_probe,
	.remove = mt6781_afe_pcm_dev_remove,
};

module_platform_driver(mt6781_afe_pcm_driver);

MODULE_DESCRIPTION("Mediatek ALSA SoC AFE platform driver for 6781");
