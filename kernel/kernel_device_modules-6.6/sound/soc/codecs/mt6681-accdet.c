...
/* 2024/12/10, add for supporting mic and ground switch to fix headset detect bug */
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH)
#include "oplus_typec_switch.h"
#define DEFAULT_MIC_GND_SWAP_DELAY_TIME 200
#endif
...
#endif
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH)
	struct delayed_work accdet_delaywork;
#endif
	u32 water_r;
	u32 moisture_ext_r;
	u32 moisture_int_r;
...
/* 2024/12/10, add for supporting mic and ground switch to fix headset detect bug */
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH)
		if (accdet_dts.mic_gnd_swap_enable) {
			if (status && !b_mic_ground_switch) {
				accdet->accdet_status = PLUG_OUT;
				accdet->cable_type = NO_DEVICE;
				typec_switch_event(NULL, TYPEC_SWITCH_MIC_GND_SWAP);
				b_mic_ground_switch = true;
				pr_info("typec_switch_event  mic and ground switch\n");
				int delay_jiffies = msecs_to_jiffies(accdet_dts.mic_gnd_swap_delay);
				queue_delayed_work(accdet->accdet_workqueue, &accdet->accdet_delaywork, delay_jiffies);
				return;
			} else if (!status) {
...
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH)
		if (accdet_dts.mic_gnd_swap_enable && !status) {
			b_mic_ground_switch = false;
		}
#endif
...
#else /*CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH*/
    if (accdet_dts.headset_eint0_disable != 1) {
	    eintID = pmic_read_mbit(MT6681_ACCDET_EINT0_MEM_IN_ADDR,
				    MT6681_ACCDET_EINT0_MEM_IN_SHIFT,
				    MT6681_ACCDET_EINT0_MEM_IN_MASK);
	    if (eintID == M_PLUG_OUT) {
...
#endif /*CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH*/
...
/* 2024/12/10, add for supporting mic and ground switch to fix headset detect bug */
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH)
		if (accdet_dts.mic_gnd_swap_enable && b_mic_ground_switch) {
			typec_switch_event(NULL, TYPEC_SWITCH_MIC_GND_SWAP);
			b_mic_ground_switch = false;
...
static void accdet_irq_handle(void)
{
	...
	if ((irq_status & ACCDET_IRQ_B0) && (eintID == 0)) {
		...
		clear_accdet_int();
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH)
		if (accdet_dts.mic_gnd_swap_enable && accdet->accdet_workqueue) {
			cancel_delayed_work_sync(&accdet->accdet_delaywork);
		}
#endif
		accdet_queue_work();
		clear_accdet_int_check();
...
static inline int ext_eint_setup(struct platform_device *pdev)
{
	...
	//ALPS09832125 add for wake up the system
...
static int accdet_get_dts_data(void)
{
	...
	/* 2025/2/17, add for mic and gnd switch*/
	ret = of_property_read_u32(node, "mic-gnd-swap-enable", &accdet_dts.mic_gnd_swap_enable);
	if (ret) {
		pr_info("%s: read prop mic-gnd-swap-enable fail\n", __func__);
		accdet_dts.mic_gnd_swap_enable = 0;
	}
	if (accdet_dts.mic_gnd_swap_enable) {
		pr_info("%s: mic-gnd-swap enabled", __func__);
		ret = of_property_read_u32(node, "mic-gnd-swap-delay", &accdet_dts.mic_gnd_swap_delay);
		if (ret) {
			/* use default time*/
			pr_debug("%s: read prop mic-gnd-swap-delay failed, use default: %dms", __func__, DEFAULT_MIC_GND_SWAP_DELAY_TIME);
			accdet_dts.mic_gnd_swap_delay = DEFAULT_MIC_GND_SWAP_DELAY_TIME;
		} else {
			pr_info("%s: read prop mic-gnd-swap-delay=%dms", __func__, accdet_dts.mic_gnd_swap_delay);
		}
	}
#endif
...
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_TYPEC_SWITCH)
	if (accdet_dts.mic_gnd_swap_enable) {
		INIT_DELAYED_WORK(&accdet->accdet_delaywork, accdet_work_callback);
	}
#endif
