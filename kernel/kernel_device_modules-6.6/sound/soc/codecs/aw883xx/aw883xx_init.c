...
int aw883xx_init_check_chipid(struct aw883xx *aw883xx, uint16_t chipid)
{
	...
	case AW883XX_PID_2183:
		...
		break;
	default:
		aw_dev_info(aw883xx->dev, "unsupported chip_id 0x%04x", chipid);
		return -EINVAL;
	}
	return 0;
}
...
int aw883xx_init(struct aw883xx *aw883xx)
{
	...
	if (aw_pa == NULL) {
		aw_dev_info(aw883xx->dev, "dev kalloc failed");
		return -ENOMEM;
	}
	...
	case AW883XX_PID_2183:
		...
		break;
	default:
		aw_dev_info(aw883xx->dev, "unsupported chip_id 0x%04x", aw883xx->chip_id);
		return -EINVAL;
	}

	ret = aw883xx_device_probe(aw_pa);
	if (ret < 0) {
		aw_dev_info(aw883xx->dev, "device_probe failed");
		return -EINVAL;
	}
