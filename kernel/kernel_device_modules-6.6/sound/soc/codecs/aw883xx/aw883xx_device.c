...
static int aw_dev_set_volume(struct aw_device *aw_dev, uint32_t set_vol)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "set volume failed");
		return ret;
	}
...
static int aw_dev_get_volume(struct aw_device *aw_dev, uint32_t *get_vol)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read volume failed");
		return ret;
	}
...
static int aw_dev_get_fade_time(unsigned int *time, fade_time_t fade_type)
{
	if (fade_type == AW_FADE_IN_TIME) {
		...
		*time = g_fade_out_time;
		...
		aw_pr_info("unsupported fade type:%d", fade_type);
		return -EINVAL;
	}
...
static int aw_dev_set_hmute(struct aw_device *aw_dev, uint32_t mute)
{
	...
	if (mute == AW_DEV_HMUTE_ENABLE) {
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "write mute enable failed");
			return ret;
		}
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "write mute disable failed");
			return ret;
		}
		aw_dev_fade_in(aw_dev);
	} else {
		aw_dev_info(aw_dev->dev, "unsupported mute state: %d", mute);
		return -EINVAL;
	}
...
static int aw_dev_get_icalk(struct aw_device *aw_dev, int16_t *icalk)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read icalkh_reg failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read icalkl_reg failed");
		return ret;
	}
	...
	if (efuse_check_desc->check_val == AW_EF_AND_CHECK) {
		...
		aw_dev_info(aw_dev->dev, "unsupported check type:%d", efuse_check_desc->check_val);
		return -EINVAL;
	}
...
static int aw_dev_get_vcalk(struct aw_device *aw_dev, int16_t *vcalk)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read vcalkh_reg failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read vcalkl_reg failed");
		return ret;
	}
...
static int aw_dev_get_internal_vcalk(struct aw_device *aw_dev, int16_t *vcalk)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read vcalkh_dac_reg failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read vcalkl_dac_reg failed");
		return ret;
	}
...
static int aw_dev_vsense_select(struct aw_device *aw_dev, int *vsense_select)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read vsense_reg_val failed");
		return ret;
	}
...
static int aw_dev_set_vcalb_to_hw(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "get icalk failed");
		return ret;
	}
	...
	if (vsense_select == AW_DEV_VDSEL_VSENSE) {
		...
		aw_dev_info(aw_dev->dev, "unsupport vsense status");
		return -EINVAL;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "write vcalb_reg failed");
		return ret;
	}
	return 0;
}
...
static int aw_dev_set_vcalb(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "set_vcalb failed");
		return ret;
	}
...
static int aw_dev_get_cali_f0_delay(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read cali delay failed, ret=%d", ret);
		return ret;
	}
...
static int aw_dev_get_int_status(struct aw_device *aw_dev, uint32_t *int_val)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read interrupt reg fail, ret=%d", ret);
		return ret;
	}
...
static int aw_dev_get_sysst_value(struct aw_device *aw_dev, uint32_t *value)
{
	...
	ret = aw_dev->ops.aw_reg_read(aw_dev, desc->reg, &reg_val);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read sysst failed");
		return ret;
	}
...
static bool aw_dev_get_iis_status(struct aw_device *aw_dev)
{
	...
	ret = aw_dev->ops.aw_reg_read(aw_dev, desc->reg, &reg_val);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read pll failed");
		return false;
	}
	...
	if ((reg_val & desc->pll_check) == desc->pll_check) {
		...
		status = false;
		aw_dev_info(aw_dev->dev, "check pll lock fail,reg_val:0x%04x", reg_val);
	}
...
static int aw_dev_mode1_pll_check(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
		if (!(aw_dev_get_iis_status(aw_dev))) {
			aw_dev_info(aw_dev->dev, "mode1 iis signal check error, ret:%d", ret);
...
static int aw_dev_mode2_pll_check(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
		if (!(aw_dev_get_iis_status(aw_dev))) {
			aw_dev_info(aw_dev->dev, "mode2 iis signal check error");
			usleep_range(AW_2000_US, AW_2000_US + 10);
			ret = -EINVAL;
		} else {
			ret = 0;
	...
	if (ret == 0) {
		...
		for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
			...
			if (ret < 0) {
				aw_dev_info(aw_dev->dev, "mode2 switch to mode1, iis signal check error");
				usleep_range(AW_2000_US, AW_2000_US + 10);
			} else {
				break;
			}
...
static int aw_dev_syspll_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "mode2 check iis failed");
			return ret;
		}
	}
...
static int aw_dev_sysst_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0)
		aw_dev_info(aw_dev->dev, "check fail");
...
static int aw_dev_sysint_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "get int status fail ret:%d", ret);
		return ret;
	}

	if (reg_val & (desc->intst_mask)) {
		aw_dev_info(aw_dev->dev, "int check fail:0x%04x", reg_val);
		ret = -EINVAL;
	}
...
static int aw_dev_set_intmask(struct aw_device *aw_dev, uint32_t flag)
{
	...
	if (flag == AW_DEV_UNMASK_INT_VAL) {
		...
		aw_dev_info(aw_dev->dev, "unsupported flag : %d", flag);
		return -EINVAL;
	}
...
static int aw_dev_dsp_enable(struct aw_device *aw_dev, uint32_t dsp)
{
	...
	if (dsp == AW_DEV_DSP_WORK) {
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "enable dsp failed");
			return ret;
		}
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "disable dsp failed");
			return ret;
		}
	} else {
		aw_dev_info(aw_dev->dev, "unsupported flag : %d", dsp);
		return -EINVAL;
	}
 ...
 static void aw_dev_memclk_select(struct aw_device *aw_dev, unsigned char flag)
{
	...
	if (flag == AW_DEV_MEMCLK_PLL) {
		...
		if (ret < 0)
			aw_dev_info(aw_dev->dev, "memclk select pll failed");

	} else if (flag == AW_DEV_MEMCLK_OSC) {
		ret = aw_dev->ops.aw_reg_write_bits(aw_dev, desc->reg,
					desc->mask, desc->osc_clk);
		if (ret < 0)
			aw_dev_info(aw_dev->dev, "memclk select OSC failed");
	} else {
		aw_dev_info(aw_dev->dev, "unknown memclk config, flag=0x%x", flag);
	}
...
static int aw_dev_cfg_crc_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "write cfg_len_val failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "write crc_check_enable failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read crc_check_reg failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "write crc_check_disable failed");
		return ret;
	}

	/*compare crc check result and pass value*/
	if (check_val != crc_check_desc->crc_check_pass) {
		aw_dev_info(aw_dev->dev, "crc_check failed,check val %x != %x", check_val,
...
static int aw_dev_fw_crc_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "write fw_len_val failed");
		return ret;
	}
	/*enable fw crc check*/
	ret = aw_dev->ops.aw_reg_write_bits(aw_dev, crc_check_desc->crc_ctrl_reg,
...
static int aw_dev_hw_crc_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "write ram_clk_on failed");
		return ret;
	}

	ret = aw_dev_fw_crc_check(aw_dev);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, " fw_crc_check failed");
		goto ram_clk_off;
	}

	ret = aw_dev_cfg_crc_check(aw_dev);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, " cfg_crc_check failed");
		goto ram_clk_off;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "write crc_init_val failed");
		goto ram_clk_off;
	}
...
static int aw_dev_crc_check(struct aw_device *aw_dev)
{
	...
	if (aw_dev->crc_type == AW_SW_CRC_CHECK) {
		if (aw_dev->ops.aw_sw_crc_check) {
			...
			if (ret < 0) {
				aw_dev_info(aw_dev->dev, "dsp crc check failed");
				return ret;
			}
		} else {
			aw_dev_info(aw_dev->dev, "sw_crc_check ops is NULL");
			return ret;
		}
	} else if (aw_dev->crc_type == AW_HW_CRC_CHECK) {
		ret = aw_dev_hw_crc_check(aw_dev);
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "hw_crc_check failed");
			return ret;
		}
	}
	return 0;
...
static int aw_dev_get_firmware_ver(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read firmware version failed, ret=%d", ret);
		return ret;
	}
				...
				uint8_t *data, uint32_t len)
{
	...
	if (data_len % 2 != 0) {
		aw_dev_info(aw_dev->dev, "data len:%d unsupported",
				data_len);
		return -EINVAL;
	}
					...
					uint8_t *data, uint32_t len)
{
	...
	if (len && (data != NULL)) {
		...
		aw_dev_info(aw_dev->dev, "reg data is null or len is 0");
		return -EPERM;
	}
			...
			uint8_t *data, uint32_t len)
{
	...
	if (len && (data != NULL)) {
		...
		aw_dev_info(aw_dev->dev, "dsp firmware data is null or len is 0");
		return -EPERM;
	}
			...
			uint8_t *data, uint32_t len)
{
	...
	if (len && (data != NULL)) {
		...
		aw_dev_info(aw_dev->dev, "dsp config data is null or len is 0");
		return -EPERM;
	}
...
static int aw_dev_sram_check(struct aw_device *aw_dev)
{
	...
	if (dsp_mem_desc->dsp_rom_check_data != reg_val) {
		aw_dev_info(aw_dev->dev, "check reg 0x40 failed, read[0x%x] does not match [0x%x]",
	...
	if (AW_DSP_ODD_NUM_BIT_TEST != reg_val) {
		aw_dev_info(aw_dev->dev, "check dsp cfg failed, read[0x%x] does not match write[0x%x]",
...
int aw883xx_device_fw_update(struct aw_device *aw_dev, bool up_dsp_fw_en, bool force_up_en)
{
	...
	if (aw_dev->fw_status == AW_DEV_FW_FAILED) {
		aw_dev_info(aw_dev->dev, "fw status[%d] error", aw_dev->fw_status);
		return -EPERM;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "update reg failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "check sram failed");
		goto error;
	}
	...
	if (up_dsp_fw_en) {
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "update dsp fw failed");
			goto error;
		}
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "update dsp cfg failed");
		goto error;
	}
...
static int aw_dev_dsp_check(struct aw_device *aw_dev)
{
	...
	if (aw_dev->dsp_cfg == AW_DEV_DSP_BYPASS) {
		...
		for (i = 0; i < AW_DEV_DSP_CHECK_MAX; i++) {
			...
			if (ret < 0) {
				aw_dev_info(aw_dev->dev, "dsp wdt status error=%d", ret);
				usleep_range(AW_2000_US, AW_2000_US + 10);
			} else {
				return 0;
			}
		...
		aw_dev_info(aw_dev->dev, "unknown dsp cfg=%d", aw_dev->dsp_cfg);
		return -EINVAL;
	}
...
static void aw_dev_cali_re_update(struct aw_cali_desc *cali_desc)
{
	...
	if (aw_dev->cali_desc.cali_re < aw_dev->re_range.re_max &&
		...
		aw_dev_info(aw_dev->dev, "cali_re:%d out of range, no set",
...
static bool aw_dev_get_pll_wdt_status(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "pll abnormal");
		return false;
	}

	ret = aw_dev_get_dsp_status(aw_dev);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "dsp not work");
		return false;
	}
					...
					uint32_t spin_angle)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_pa->dev, "set rx failed");
		return ret;
	}
...
static int aw_dev_set_params(struct aw_device *aw_dev, dev_params_t type, uint32_t *params, uint32_t len)
{
	...
	if ((params == NULL) || (len == 0)) {
		aw_dev_info(aw_dev->dev, "input params error");
		return -EINVAL;
	}
...
int aw883xx_device_params(struct aw_device *aw_dev, dev_params_t params_type, void *data, uint32_t size, params_option_t params_ops)
{
	if (params_ops == AW_SET_DEV_PARAMS) {
		...
		aw_dev_info(aw_dev->dev, "unsupported params_ops:%d", params_ops);
		return -EINVAL;
	}
}
...
bool aw883xx_device_status(struct aw_device *aw_dev, dev_status_t type, status_option_t status_ops)
{
	if (status_ops == AW_SET_DEV_STATUS) {
		...
		aw_dev_info(aw_dev->dev, "unsupported status_ops:%d", status_ops);
		return false;
	}
}
...
int aw883xx_device_start(struct aw_device *aw_dev)
{
	...
	if (aw_dev->dsp_cfg == AW_DEV_DSP_WORK) {
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "crc check failed");
			aw_dev_reg_dump(aw_dev);
			goto crc_check_fail;
		}
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "check dsp status failed");
			aw_dev_reg_dump(aw_dev);
			goto dsp_check_fail;
		}
	} else {
...
int aw883xx_device_init(struct aw_device *aw_dev, struct aw_container *aw_cfg)
{
	...
	if (aw_dev == NULL || aw_cfg == NULL) {
		aw_pr_info("aw_dev is NULL or aw_cfg is NULL");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		...
		aw_dev_info(aw_dev->dev, "aw_dev acf parse failed");
		return -EINVAL;
	}
	...
	ret = aw883xx_device_fw_update(aw_dev, AW_FORCE_UPDATE_ON,
			AW_DSP_FW_UPDATE_ON);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "fw update failed");
		return ret;
	}
	...
	if (aw_dev->ops.aw_get_dev_num() > 0) {
		...
		list_for_each(pos, dev_list) {
			...
			if (local_dev->channel == channel_value) {
				aw_dev_info(local_dev->dev, "sound-channel:%d already exists",
					channel_value);
				return -EINVAL;
			}
		}
...
static int aw_device_parse_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "parse channel failed");
		return ret;
	}
	return 0;
}
...
int aw883xx_device_probe(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "device parse failed");
		return ret;
	}
