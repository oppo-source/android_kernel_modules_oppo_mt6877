			...
			uint8_t *data, uint32_t size)
{
	...
	if (crc_dsp_cfg->data == NULL) {
		...
		if (!crc_dsp_cfg->data) {
			aw_dev_info(aw_dev->dev, "error allocating memory");
			return -ENOMEM;
		}
...
static int aw_pid_2049_set_dsp_crc32(struct aw_device *aw_dev)
{
	...
	if (crc_data_len > crc_dsp_cfg->len) {
		aw_dev_info(aw_dev->dev, "crc data len :%d > cfg_data len:%d",
			crc_data_len, crc_dsp_cfg->len);
		return -EINVAL;
	}

	if (crc_data_len % 4 != 0) {
		aw_dev_info(aw_dev->dev, "The crc data len :%d unsupport", crc_data_len);
		return -EINVAL;
	}
	...
	ret = aw_dev->ops.aw_dsp_write(aw_dev, desc->dsp_reg, crc_value,
						desc->data_type);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "set dsp crc value failed");
		return ret;
	}
...
static int aw_pid_2049_dsp_crc_check_enable(struct aw_device *aw_dev, bool flag)
{
	...
	if (flag) {
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "enable dsp crc failed");
			return ret;
		}
	} else {
		ret = aw_dev->ops.aw_reg_write_bits(aw_dev, dsp_crc_desc->ctl_reg,
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "close dsp crc failed");
			return ret;
		}
	}
...
static int aw_pid_2049_dsp_st_check(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_DSP_ST_CHECK_MAX; i++) {
		...
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "read reg0x%x failed", desc->reg);
			continue;
		}

		if ((reg_val & (~desc->dsp_mask)) != desc->dsp_check) {
			aw_dev_info(aw_dev->dev, "check dsp st fail,reg_val:0x%04x", reg_val);
			ret = -EINVAL;
			aw_dev->ops.aw_dsp_read(aw_dev, crc_desc->check_reg, &crc_val,
				crc_desc->check_reg_data_type);
			aw_dev_info(aw_dev->dev, "read crc value:0x%04x", crc_val);
...
static int aw_pid_2049_dsp_crc32_check(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "check crc32 fail");
		return ret;
	}
					...
					uint32_t *re)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "dsp read ra error");
		return ret;
	}
...
static int aw_pid_2049_set_cali_re_to_dsp(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "set cali re error");
		return ret;
	}
...
static int aw_pid_2049_get_icalk(struct aw_device *aw_dev, int16_t *icalk)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "reg read failed");
		return ret;
	}
...
static int aw_pid_2049_init_vcalb_update(struct aw_device *aw_dev, backup_sec_t flag)
{
	...
	if (flag == AW_RECOVERY_SEC_DATA) {
		ret = aw_dev->ops.aw_dsp_write(aw_dev, AW_PID_2049_DSP_REG_VCALB,
						(uint32_t)desc->init_value, AW_DSP_16_DATA);
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "write vcalb failed");
			return ret;
		}
	} else if (flag == AW_RECORD_SEC_DATA) {
		ret = aw_dev->ops.aw_dsp_read(aw_dev, AW_PID_2049_DSP_REG_VCALB,
						(uint32_t *)&desc->init_value, AW_DSP_16_DATA);
		if (ret < 0) {
			aw_dev_info(aw_dev->dev, "read vcalb failed");
			return ret;
		}
	} else {
		aw_dev_info(aw_dev->dev, "unsupported type:%d", flag);
...
static int aw_pid_2049_set_vcalb(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read vcalb_adj failed");
		return ret;
	}
	...
	if ((vcalk == 0) || (AW_PID_2049_ISCAL_FACTOR == 0)) {
		aw_dev_info(aw_dev->dev, "vcalk:%d or desc->iscal_factor:%d unsupported",
