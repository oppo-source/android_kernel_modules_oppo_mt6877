...
static int aw_pid_2066_set_volume(struct aw_device *aw_pa, uint16_t volume)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_pa->dev, "read vol reg failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_pa->dev, "write vol reg failed");
		return ret;
	}
...
static int aw_pid_2066_get_icalk(struct aw_device *aw_dev, int16_t *icalk)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read efrm_reg failed");
		return ret;
	}
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "read efrl_reg failed");
		return ret;
	}
...
static int aw_pid_2066_set_vcalb(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_info(aw_dev->dev, "get vcalk failed");
		return ret;
	}
