...
#ifdef OPLUS_ARCH_EXTENDS
/* change micbias to 2.8v */
			regmap_write(priv->regmap,
				     MT6368_AUDENC_ANA_CON33, 0x0);
			regmap_write(priv->regmap,
				     MT6368_AUDENC_ANA_CON34, 0x31);
#else
			regmap_write(priv->regmap,
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
static ssize_t mt6368_codec_read(struct mt6368_priv *priv, char *buffer,
				 size_t size);
				...
				size_t count, loff_t *ppos)
{
	...
	struct mt6368_priv *priv = (struct mt6368_priv *)filp->private_data;
	ssize_t ret;
