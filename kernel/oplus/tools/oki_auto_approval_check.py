...
def git_show_filtered(commit_id, output_filename):
    ...
    filtered_output = '\n'.join(
        [line for line in git_output.splitlines() if line.startswith('+') or line.startswith('-')])
...
def main():
    ...
    git_log_commit = get_commit_id(latest_commit_message)
    print("Google commit id is:")
    print(git_log_commit)
    git_show_filtered(git_log_commit, output_gki)
